...
商務用 Skype Online 通話方案現在支援封鎖從公用交換電話網絡 (PSTN) 的入站通話。 此功能可讓您針對清單中的每個撥入 PSTN 呼叫，針對其進行檢查，以進行數位模式的租使用者識別碼。 如果進行了相符，就會拒絕來電。
...
數位模式是透過 ```CsInboundBlockedNumberPattern``` 命令 ```New``` 、 ```Get``` 、和來管理 ```Set``` ```Remove``` 。 您可以使用這些 Cmdlet 來管理指定的模式，包括切換指定模式啟用的功能。
...
查看並啟用整個呼叫封鎖功能是透過 ```CsTenantBlockingCallingNumbers``` 命令和來管理 ```Get``` ```Set``` 。
...
您可以透過 ```CsTenantBlockNumberExceptionPattern``` 命令、、、及等方式，將例外狀況新增至封鎖的編號模式 ```New``` ```Get``` ```Set``` ```Remove``` 。
...
在這個範例中，會建立新的數位例外模式，而且預設會將模式新增為 [啟用]。 ```-Enabled```And ```-Description``` 參數是選擇性的。
...
在這個範例中，-身分識別參數是選擇性的。 如果 ```-Identity``` 未指定參數，此 Cmdlet 會傳回為租使用者輸入的所有數位例外模式清單。
...
在這個範例中， ```-Phonenumber``` ```-Tenant``` 需要和參數。 ```-PhoneNumber```參數應該是不含任何其他字元（例如 + 或-）的數值字串。 在 TRPS 中， ```-Tenant parameter``` 是選擇性的。 如果該 ```isNumberBlocked``` 數位在租使用者中遭到封鎖，則結果參數會傳回 True 值，如果未封鎖，則傳回 False。
