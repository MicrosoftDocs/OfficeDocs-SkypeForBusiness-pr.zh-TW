...
8. 執行下列動作，以確認您目前的 Skype 會議廣播設定：
   ...
   ```
    ...
    確認 [參數_EnableBroadcastMeeting_ ] 已設定為`False`。
...
9. 執行下列動作，為您的組織啟用 Skype 會議廣播：
   ...
   ```
    ...
    您可以`Get-CsBroadcastMeetingConfiguration`再次執行，確認已啟用該設定。
...
如果您有防火牆，而您想要在企業外部的人員（不是同盟企業）中保留廣播，您必須使用這些指示來設定您的網路：[為您的 Skype 會議廣播設定您的網路](set-up-your-network-for-skype-meeting-broadcast.md)。 
