...
在將 商務用 Skype部署給使用者之前，請確定您已完成設定線上版一文中的步驟 1-3 商務用 Skype[步驟](set-up-skype-for-business-online.md)。 如此一來商務用 Skype網域設定，每個人都會擁有他們的授權，而且您將為貴組織設定 IM 和在 商務用 Skype [Online 中](configure-presence-in-skype-for-business-online.md)設定目前狀態。
...
- 若要控制公司中的特定人員是否取得軟體：請登錄 Microsoft 365 系統管理中心、前往 [使用者使用中使用者」、選取要授予軟體存取權限的人，然後按一下 [產品授權旁的編輯>，然後開啟或  >  關閉授權。
...
有了 Office 365，如果您想要控制應用程式安裝位置，商務用 Skype數種部署應用程式的選項。 其中一些選項包括下列專案：
  
- 從 商務用 Skype將應用程式下載到您的Microsoft 365 系統管理中心網路，如手動將 商務用 Skype[部署給使用者中所述](deploy-the-skype-for-business-client-in-office-365.md#bkmk_manual_1)。
    
- 使用 Office **[部署工具](https://go.microsoft.com/fwlink/p/?LinkID=626065)**，將 Microsoft 365 Apps 企業版應用程式商務用 Skype應用程式下載到您的本地網路。 然後，使用 Office部署工具將應用程式部署到您的使用者。 Office部署工具讓您能夠控制部署的某些層面，例如語言和版本 (32 位或 64 位) 。
    
- 使用現有的軟體部署工具和程式 ，例如 Microsoft Endpoint Configuration Manager，將 Microsoft 365 Apps 企業版或 商務用 Skype應用程式部署到您的使用者。 您可以使用現有的工具和程式Office[部署](https://go.microsoft.com/fwlink/p/?LinkID=626065)工具，或使用從 Microsoft 365 系統管理中心 下載的軟體。
...
2. 建立 configuration.xml 檔案，用於具有您想要的用戶端應用程式設定之 Office 部署工具，例如設定版本 (32 位或 64 位) 、安裝語言等。
    
3. 使用 Office部署工具及configuration.xml檔案，將設定檔案從 Office 內容傳遞網路 (CDN) 下載至您的本地或內部Office 內容傳遞網路 (CDN) 。
    
4. 使用 Office部署工具及configuration.xml來安裝Office應用程式，包括 商務用 Skype 應用程式。
...
您可以使用現有的軟體部署工具和程式 ，例如 Microsoft Endpoint Configuration Manager，來部署商務用 Skype應用程式。 您可以使用這些工具和程式與從應用程式下載的軟體Microsoft 365 系統管理中心或Office工具。
...
根據預設，安裝 商務用 Skype應用程式之後，當 Microsoft 提供更新時，系統會自動從網際網路下載更新。 如果您想要進一控制更新發生的時間或更新的安裝位置，您可以使用 Office部署工具或群組原則來設定。
...
您可以使用部署工具或群組原則Office設定更新通道，控制取得功能更新的頻次。 每月通道會提供每月功能更新 (大約) ，而 Semi-Annual通道則每六個月提供一次功能更新。 有關頻道詳細資訊，請參閱更新通道概觀[的 Microsoft 365 Apps 企業版。](https://support.office.com/article/9ccf0f13-28ff-4975-9bd2-7e4ea2fefef4)
