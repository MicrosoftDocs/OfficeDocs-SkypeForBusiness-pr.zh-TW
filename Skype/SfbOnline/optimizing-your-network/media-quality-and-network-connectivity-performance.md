...
> [!TIP]
> 雖然本主題提供您整體網路績效指引，但網路評定的完整指南並超出本檔的範圍。 若要尋找可協助Microsoft Teams網路績效評估之合作夥伴清單，以做為完整網路評定的一部分，請流覽合作夥伴解決方案商務用 Skype[清單](http://partnersolutions.skypeforbusiness.com/)。 
  
## <a name="network-connectivity-requirements-to-microsoft-teams"></a>網內網路Microsoft Teams

### <a name="factors-that-impact-microsoft-teams-media-quality"></a>影響媒體質量Microsoft Teams因素

有許多不同因素Microsoft Teams Real-Time媒體 (音訊、視) 應用程式共用品質，包括所使用的裝置、環境和網路連接。 
...
- **延遲** 這是從 A 點到網路 B 點取得 IP 封包所花的時間。 此網路傳播延遲與兩點之間的實際距離和光速有關，包括介於兩者之間的各種路由器所收取的額外負荷。 延遲是以 RTT 或 RTT 的單向或往返時間 (測量) 。
...
## <a name="network-performance-requirements-to-connect-to-microsoft-teams"></a>要連接到網路的網路Microsoft Teams
...
下圖說明會議中的單向音訊流程，從一個參與者Microsoft Teams到另一個參與者。
...
2. **Microsoft 網路內的連接** 這是在Microsoft Edge Microsoft Teams之間，使用 A/V 會議伺服器。
    
3. **Microsoft Network 內的連接** 這是在資料中心Microsoft Teams Microsoft 網路 Edge 之間。
    
4. **從 Microsoft 網路邊緣連接到使用者 2** 這包括從網路 Edge 到 Microsoft 網路 Edge 的網際網路連接、從使用者 2 到網際網路出口點的 WAN (您的網路 Edge) ，以及網路連接 ，例如 WiFi 或乙太網路。
    
下圖顯示 PSTN 通話中元件和網路Microsoft Teams區段：
...
2. **Microsoft 網路內的連接** 這是使用中繼伺服器Microsoft Edge Microsoft Teams資料中心之間的位置。
    
3. **Microsoft Network 內的連接** 這是在資料中心Microsoft Teams Microsoft 網路 Edge 之間。
    
4. **Microsoft Network 與 PSTN 服務提供者合作夥伴之間的連接** 這是從 Microsoft 網路外部的用戶端商務用 Skype PSTN 通話的建立連接。
...
為了商務用 Skype媒體質量，從公司網路到 Microsoft 網路 Edge 的連接需要下列網路績效度量目標或臨界值。 此網路區段包含您的內部網路，這包括所有 WiFi 和乙太網路連接、任何公司網站對網站流量的 WAN 連接，例如 Multiprotocol 標籤切換 (MPLS) ，以及網際網路或 ExpressRoute 合作夥伴與 Microsoft 網路 Edge 的連絡。
...
- 網路績效目標會假設適當的頻寬和/或服務品質規劃。 換句話說，當網路連接在最大負載商務用 Skype Real-Time，這直接適用于媒體流量。
...
- 網路績效目標會假設適當的頻寬和/或服務品質規劃。 當網路連接負載商務用 Skype Real-Time時，這也適用于媒體流量。 若要正確規劃頻寬和 QoS，請參閱在 Microsoft Teams 中[ExpressRoute 和 QoS。](https://support.office.com/article/ExpressRoute-and-QoS-in-Skype-for-Business-Online-20c654da-30ee-4e4f-a764-8b7d8844431d)
...
除了針對 Microsoft 網路 Edge 網站進行測試，還有來自 商務用 Skype 與合作夥伴的網路評估解決方案，這些解決方案會針對雲端中的服務Microsoft Azure設定。 在這些解決方案中，網路評估工具會針對在 Azure 雲端中設定為服務的自訂端點測試延遲、封包遺失和抖動。 因此，測試網路流量會經過另一個網路區段，即網路邊緣與託管網路評定服務的 Azure 資料中心之間的 Microsoft 網路內連接。
...
Azure ExpressRoute Microsoft 365或 Office 365是一種專用網路連接，可Microsoft 365或Office 365。 它讓客戶能夠控制網路流量的路徑。 他們不再需要擔心網際網路上發生無法預測的路由，因為不明的電信公司、提供者和 ISP 會傳送資料。 透過 ExpressRoute 所送出的網路流量會直接透過 ExpressRoute 合作夥伴的網路送往 Microsoft 的網路。 這可讓客戶將Microsoft 365或Office 365視為位於自己的非現場資料中心，並擁有專用連線。
  
Azure ExpressRoute 適用于所有授權Microsoft 365 Office 365方案。 不過，若要啟用全域路由進階版，Microsoft 365 Azure ExpressRoute Office 365附加元件。 擁有至少 500 個使用 ExpressRoute 的客戶可以取得所需的 *ExpressRoute* 進階版附加元件，而不需要額外付費。
  
### <a name="is-expressroute-required-for-good-media-quality"></a>需要 ExpressRoute 才能確保良好的媒體質量嗎？

Azure ExpressRoute 不是獲得最佳媒體質量Microsoft Teams需求。 不過，這是其中一個部署選項，可協助確保雲端連接子合商務用 Skype網路績效目標或臨界值。
  
Microsoft 365 Office 365是使用網際網路的高品質及安全服務。 我們會持續投資新的安全性功能和地區邊緣節點，以持續改善安全性與績效。 Azure ExpressRoute 不要求使用 Microsoft 365 或 Office 365 服務 ，Microsoft Teams。 Azure ExpressRoute 是可用的部署選項之一，可協助確保 Microsoft 365 或 Office 365 的連接子合 商務用 Skype 網路性能需求，並確保最佳的 Microsoft Teams 媒體質量體驗。
  
對於 Microsoft Teams 媒體質量，公司網站與 Microsoft 網路 Edge 之間的連接必須符合[從 商務用 Skype](media-quality-and-network-connectivity-performance.md#bkSfBClienttoEdge)用戶端到 Microsoft 網路 Edge 之網路績效需求中的績效目標，而且網路 Edge 與 Microsoft 網路邊緣之間的連接必須符合網路 Edge 到[Microsoft](media-quality-and-network-connectivity-performance.md#bkYourNetworkEdge)網路 Edge 之網路績效需求中的績效目標。  
  
您公司實體網路連接性 ，包括您的內部網路和雲端連接容量，也一樣，必須能容納媒體流量的高峰。 Azure ExpressRoute 是協助客戶確保其雲端Microsoft Teams符合所有這些性能需求之一。
  
### <a name="is-expressroute-required-for-voice-quality-sla"></a>Voice 品質 SLA 需要 ExpressRoute 嗎？

否，ExpressRoute 不一Microsoft Teams語音品質 SLA。 語音[Microsoft Teams SLA](http://www.microsoftvolumelicensing.com/DocumentSearch.aspx?Mode=3&amp;DocumentTypeId=37)適用于任何 Microsoft Teams 語音服務使用者在正確的授權和訂閱內撥打的任何合格通話，讓使用者可以撥打任何類型的 VoIP 或 PSTN 通話。 語音品質 SLA 應包含解決下列所有條件：
...
客戶必須先根據網路性能需求所述之網路Microsoft Teams，評估其網路和目前的網際網路連接能力，才能決定要連接到[Microsoft Teams。](media-quality-and-network-connectivity-performance.md#bkNetworkPerf)
  
如果目前網際網路連接上的網路績效設定為在尖峰時段內有足夠的容量，而且符合從網站到 Microsoft 網路 Edges，以及從網路 Edges 到 Microsoft 網路 Edges 的網路績效需求，您可以繼續使用現有的網際網路連接來連接到 Microsoft Teams。
  
針對網路績效需求未達的公司網站，我們強烈建議您先與現有的網路服務提供者合作，以改善整體網路績效。 不過，如果仍無法符合這些需求，使用 Azure ExpressRoute 可協助確保您的雲端Microsoft Teams可協助您符合網路性能需求。
...
如需 ExpressRoute QoS 和容量規劃詳細資訊，請參閱在 Microsoft Teams 中[ExpressRoute 和 QoS。](https://support.office.com/article/ExpressRoute-and-QoS-in-Skype-for-Business-Online-20c654da-30ee-4e4f-a764-8b7d8844431d)
  
### <a name="can-i-set-up-azure-expressroute-for-microsoft-teams-only"></a>我可以設定 Azure ExpressRoute Microsoft Teams嗎？

可以，您可以設定 Azure ExpressRoute，以確保從公司網路到只有您公司的網路Microsoft Teams。 這會為使用者提供最佳Real-Time媒體質量，但之後您可以繼續Microsoft 365或Office 365網際網路服務。
  
BGP (BGP) 的 Border Gateway 通訊協定是網際網路上的路由通訊協定，用來路由網際網路上的網路流量。 它的設計目的是在跨網際網路找到的自動 (AS) 之間交換路由資訊。 BGP 社群值是可適用于傳入或外發路由的屬性標記。 BGP 社群通常用來向接收 AS 發出訊號，接收 AS 會依據地理位置、服務類型或其他準則，使用輸出連結來到達指定目的地。
  
Microsoft 在 BGP 社群支援下，會根據 BGP 社群所屬的服務，以適當的 BGP 社群值標記首碼和路由。 Microsoft 會以適當的 BGP 社群值標記透過公用對等和 Microsoft 對等互連所宣告的首碼，指出首碼所託管的區域。 您可以仰賴社群值來做出適當的路由決策，以提供最佳路由。 您可以使用 BGP Microsoft Teams值來設定 ExpressRoute 連接，Microsoft Teams。 您可以瞭解[ExpressRoute 路由需求。](/azure/expressroute/expressroute-routing)
  
## <a name="expressroute-connectivity-scenarios-for-microsoft-teams"></a>ExpressRoute Microsoft Teams
<a name="bkNetworkPerf"> </a>
