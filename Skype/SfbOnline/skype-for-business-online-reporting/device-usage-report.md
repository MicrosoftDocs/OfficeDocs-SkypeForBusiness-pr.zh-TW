...
2. 在 [**使用方式商務用 Skype** 左側的 [選取報表清單>上，按一下 [裝置使用量狀況說明> 。 或者，按一下 [商務用 Skype **活動** 小工具，然後按一下 [商務用 Skype **活動清單** 上的商務用 Skype **使用方式**。
    
     ![商務用 Skype裝置使用方式報告小工具](../images/13e7a51c-f680-4164-9a18-67de10fceb7f.png)
  
    視您Microsoft 365訂閱Office 365，您可能看不到此處顯示的所有產品和活動報告。
...
***
![數位 4](../images/sfbcallout4.png)<br/>使用通訊組橫條圖上的互動式圖表資料以瞭解使用方式，以查看組織中使用唯一裝置的使用者數目。 它會顯示使用裝置的使用者Windows、Windows Phone、Android 手機、iPhone和iPad使用者總數。     
***
![數位 5](../images/sfbcallout5.png)<br/>您可以按一下圖例中的專案，來篩選圖表上看到的數列。 例如，在 **[** 使用者」 圖表上，按一下或點Windows **、Windows Phone、Android****手機**、iPhone 或 **iPad，** 只查看與每個專案相關的資訊。  變更此選取範圍不會變更格線表格中的資訊。 
***
![數位 6](../images/sfbcallout6.png)<br/>每個圖表都有一個水準軸 (X，) 垂直軸 (Y) 。
*    在使用者 **圖表** 上，Y 軸是已安裝應用程式之裝置商務用 Skype使用者總數。
...
*    **Windows** 已將用戶端應用程式安裝在 商務用 Skype 型作業系統上且正在使用Windows，系統會選取該選項。 
*    **Windows Phone** 裝置上安裝用戶端應用程式商務用 Skype且正在使用Windows Phone，系統即會選取此選項。
*    **如果使用者已安裝** Android 手機上的用戶端應用程式商務用 Skype且正在使用 Android 手機，系統即會選取 Android 手機。
*    **iPhone** 使用者已安裝用戶端應用程式，商務用 Skype已安裝用戶端應用程式，iPhone應用程式。 
*    **iPad** 使用者已安裝用戶端應用程式，商務用 Skype已安裝用戶端應用程式iPad且正在使用該應用程式。 
