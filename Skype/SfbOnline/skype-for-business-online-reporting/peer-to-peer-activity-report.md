...
ms.locfileid: "58592937"
---
# <a name="peer-to-peer-activity-report"></a>對等活動報告

[!INCLUDE [sfbo-retirement](../../Hub/includes/sfbo-retirement.md)]

報表 **儀表板** 會顯示您組織中Microsoft 365 Office 365活動概觀。 它可讓您深入查看個別的產品層級報告，以進一步深入瞭解每個產品內的活動。 例如，您可以使用 商務用 Skype對等活動報告來查看使用者使用 IM、音訊、視像、應用程式共用和傳輸檔案的量。 

請查看報告 [概觀](https://support.office.com/article/0d6dfb17-8582-4172-a9a9-aed798150263)。
...
2. On the **Usage** page, select **Skype for Business** > **Peer-to-peer activity** on the **Select a report list** on the left. 或者，按一下 [商務用 Skype **活動** 小工具，然後按一下 商務用 Skype活動清單上的 [對等 **商務用 Skype活動。**
...
![Skype圖說說來建立對等報表。](../images/82dec398-ca05-46c7-b0fe-affcbfc0ddd5.PNG)
  
***
![數位 1](../images/sfbcallout1.png)<br/>您可以 **商務用 Skype** 對等活動報告，查看過去 7 天、30 天、90 天或 180 天的趨勢。 不過，如果您按一下報告中的特定日期，表格 (請參閱數位 7) 會顯示 30 天內的資料，最多 30 天 (請參閱產生報表時的第 2) 個數字。
...
***
![數位 3](../images/sfbcallout3.png)<br/>使用活動圖表上的互動式圖表資料，瞭解使用趨勢，並查看貴組織中每個會話類型的會話總數。 它會顯示整個組織的 **IM、** 音訊、視像、**應用程式** 共用和 **檔案** 傳輸會話總數和類型。 
***
...
***
![數位 5](../images/sfbcallout5.png)<br/>使用分鐘數圖表上的互動式圖表資料來瞭解使用趨勢，以及查看使用音訊和視像進行對等活動的使用者所使用的分鐘數。 它會顯示點對點工作階段中使用的音訊和視音訊的總分鐘數。
***
![數位 6](../images/sfbcallout6.png)<br/>每個圖表都有一個水準 ("X"，) 垂直軸 ("Y") 。 
...
***
![數位 7](../images/sfbcallout7.png)<br/>您可以按一下圖例中的專案，來篩選圖表上看到的數列。 例如，在 [活動圖上，按一下或點一下 **IM、****音訊**、**視** 像、應用程式共用和檔案傳輸，以查看每個傳輸專案的資訊。  變更此選取範圍不會變更格線表格中的資訊。 
***
![數位 8](../images/sfbcallout8.png)<br/>下表顯示每個使用者的對等活動明細。 這會顯示已指派商務用 Skype的所有使用者，以及他們的對等活動。 您可以新增其他欄至資料表。
*    **使用者名稱** 是使用者的名稱。
*    **已刪除** 表示使用者授權已移除。 <br/> <br/> **注意：**  只要在所選時段內獲得授權，已刪除使用者的活動仍然會顯示在報告中。 刪除 **欄** 可協助您注意使用者可能不再作用中，但已對報告中的資料有影響。  <br/><br/>
*    **刪除日期** 是移除使用者授權的日期。 
*    **UTC (的最後**) 日期是該使用者 (UTC) 活動日期。
*    **IM** 顯示使用者使用的點對點工作階段總數。
*    **音訊** 會顯示使用音訊的點對點工作階段總數。
*    **影片** 顯示使用視像的點對點工作階段總數。
*    **應用程式共用** 會顯示對等應用程式共用會話總數。
*    **檔案傳輸** 會顯示對等檔案傳輸會話總數。
*    **音訊分鐘** 數會顯示整個組織使用的音訊分鐘總數。 
*    **影片分鐘** 數顯示整個組織使用的視音訊分鐘總數。 
...
***
![數位 10](../images/sfbcallout10.png)<br/>![商務用 Skype線上報告管理按鈕。](../images/4c8f5387-cebb-4d6c-b7d3-05c954a2c234.png)<br/>按一下或點一 **下** 任一欄的 [欄」 圖示，即可從報表新增或移除欄。         
