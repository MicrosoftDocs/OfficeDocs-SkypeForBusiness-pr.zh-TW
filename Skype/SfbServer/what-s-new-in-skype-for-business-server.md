...
影片 Interop 伺服器（VIS）充當商務用 Skype Server 和 Cisco Video 電話會議（VTC）系統之間的仲介。 加入會議時，使用者現在可以選取 Cisco VTC 系統。 影片 Interop 伺服器（VIS）是以獨立伺服器角色的形式來執行內部部署。 如需詳細資訊，請參閱[在商務用 Skype server 中規劃影片 Interop 伺服器 2015](plan-your-deployment/video-interop-server.md)。
...
SQL Server AlwaysOn 可用性群組是 SQL Server 2012 和 SQL Server 2014 中的高可用性和嚴重損壞修復解決方案，可提供資料庫鏡像的替代方法。 可用性群組支援容錯移轉環境一組分開的資料庫（稱為可用性資料庫）。 可用性群組支援一組可讀寫的主資料庫，以及一到四組對應次要資料庫。 另外，次要資料庫可供唯讀存取和某些備份作業使用。
  
SQL Server 容錯移轉叢集實例利用 Windows Server 容錯移轉叢集（WSFC）功能，透過伺服器實例層級的冗余（容錯移轉叢集實例（FCI））提供本機高可用性。 FCI 是安裝在 Windows Server 容錯移轉叢集（WSFC）節點（可能是跨多個子網上）的單一 SQL Server 實例。
...
集中式記錄服務是商務用 Skype Server 2015 的慣用記錄環境。 這種版本沒有任何新功能，但已改進服務和集中式記錄服務代理程式（ClsAgent.exe）的可靠性和效能，可與控制器進行通訊，並接收系統管理員所發出的命令。
  
商務用 Skype Server 2015 使用 Windows PowerShell Cmdlet 來記錄管理服務代理程式、啟動追蹤並產生報告。 （Lync Server 2013 使用 ClsController.exe 來執行這些工作。）
  
集中式記錄服務可以在任何商務用 Skype Server 2015 上執行。 內建案例（預先定義的追蹤）保持不變，也就是建立自訂案例的功能。 有一個稱為「AlwaysOn 永遠執行」的特殊案例，可讓系統管理員在近即時找到常見問題。
...
- 控制台的功能可識別及篩選已移至內部部署使用者之商務用 Skype Online （即混合使用者）的內部部署使用者物件。
...
- 用戶端/裝置類型-使用哪一種商務用 Skype 用戶端，以及用戶端所執行的裝置（PC、mobile、iPad 等等）
