...
瞭解如何準備 cloud Connector 裝置以進行部署，並搭配電話系統 (Cloud PBX) 使用。

本節說明如何取得商務用 Skype Cloud Connector Edition 安裝檔案、安裝雲端連接器軟體，以及準備用於部署的雲端連接器裝置。 在您完成本節中的所有步驟之後，您就可以為單一網站或多個網站部署雲端連接器。 如果您有現有的雲端連接器部署，但尚未升級為雲端連接器版本2.1，請參閱 [Upgrade to An Cloud connector 的新版本](upgrade-to-a-new-version-of-cloud-connector.md)。
...
> [!NOTE]
> 若為雲端連接器版本2.1 和更新版本，主機裝置必須安裝 .NET Framework 4.6.1 或更新版本。 

> [!IMPORTANT]
> 若要順利進行部署，當您執行 Cmdlet 以設定商務用 Skype Cloud Connector Edition 時，請務必使用與您在中開始的相同主控台會話。 避免在部署及設定期間切換至不同的會話。 

> [!NOTE]
> 您只需針對部署中的第一個裝置執行一些步驟：建立網站目錄的共用、下載 bits，以及準備虛擬硬碟 (VHDX) 檔案從 Windows 伺服器 ISO 映像。 

## <a name="download-the-skype-for-business-cloud-connector-edition-installer"></a>下載商務用 Skype Cloud Connector Edition 安裝程式
...
1. 以系統管理員身分開啟 PowerShell 主控台，並確認商務用 Skype Cloud Connector Edition Cmdlet 可使用下列 Cmdlet：

   ```powershell
   Get-Command *-Cc*
   ```

    命令應該會傳回商務用 Skype Cloud Connector Edition 的 Cmdlet 清單。
...
3. **裝置目錄** 是商務用 Skype Cloud Connector Edition 的本機工作根目錄，以及儲存外部憑證、實例及記錄檔的位置。 預設位置為：%USERPROFILE%\CloudConnector\ApplianceRoot。
   ...
   ```
    ...
    裝置目錄應設定為裝置上的本機資料夾。 您應只在開始商務用 Skype Cloud Connector Edition 部署之前，先設定 **裝置目錄**。 如果您在部署後變更它，您將需要重新部署主伺服器。
...
- 執行下列 Cmdlet，將路徑（包括檔案名）設定為外部 Edge 憑證。 例如： C:\certs\cce\ap.contoso.com.pfx。 憑證必須包含私密金鑰。
  ...
  ```
    ...
    > [!IMPORTANT]
    > **如果您已更新 Cloud Connector 版本1.4.2 或更新版本**，請確定準備好的外部憑證包含私密金鑰和完整的憑證鏈，包括根 ca 憑證和中間 ca 憑證。 若尚未 **更新為雲端連接器版本 1.4.2**，請確定準備好的外部憑證包含私密金鑰。 此外部憑證必須由預設所信任的憑證授權單位單位所發行 Windows。
...
使用您在 [[規劃商務用 Skype Cloud Connector Edition](plan-skype-for-business-cloud-connector-edition.md)中的「[部署參數](plan-skype-for-business-cloud-connector-edition.md#BKMK_SiteParams)」主題中所收集的資訊準備 CloudConnector.ini 檔案。
...
> [!NOTE]
> 本節不會討論 .ini 檔案的所有值，只會在此提及那些具有特殊考慮的值。 如需完整清單，請參閱[Plan for 商務用 Skype Cloud Connector Edition](plan-skype-for-business-cloud-connector-edition.md)主題中的 [[決定部署參數](plan-skype-for-business-cloud-connector-edition.md#BKMK_SiteParams)] 區段。 如需更多有關其他裝置或新網站之值的相關資訊，請參閱 [具有高可用性的單一網站 (HA) 與](deploy-multiple-sites-in-cloud-connector.md#BKMK_SingleSitecomparedtomulti-site) 主題在 [雲端連接器中部署多個網站](deploy-multiple-sites-in-cloud-connector.md)」中的多網站部署。 

- **SiteName：** 預設值為 **Site1**。 您必須先將它更新，然後再部署雲端連接器，因為當您執行 **CcAppliance** 將裝置登錄至現有或新的網站時，Cmdlet 會使用 **SiteName** 來判斷要註冊的網站。

     如果您想要將裝置登錄至新的網站， **SiteName** 的值必須是唯一的，且與現有的網站不同。 若要將裝置登錄至現有的網站，.ini 檔案中 **SiteName** 的值必須符合 Microsoft 365 或 Office 365 組織設定中所定義的名稱。 若要將設定檔從一個網站複製到另一個網站，請確定為每個網站更新 **SiteName** 的值。

- **ServerName：** 伺服器名稱不應該包含功能變數名稱，且應該限制為15個字元。

- **HardwareType：** 如果您未設定或保留值為 null，則會使用 **標準** 的預設值。 如果您打算部署較大版本的雲端連接器，以支援500的 [商務用 Skype Cloud Connector Edition 計畫](plan-skype-for-business-cloud-connector-edition.md)同時通話每一主機，請使用 **Normal** 。 使用 **最小值** 進行同時支援50同時通話的部署。
...
- **基本 VM 特定設定：** 必須為 **CcIsoToVhdx 指令程式** 設定本節中設定。
...
此步驟會從 Windows Server 2012 ISO 映像準備虛擬硬碟 (VHDX) 檔案。 VHDX 將用來在部署期間建立虛擬機器。 將會建立 (基底) 的臨時虛擬機器，且會從 ISO 檔案安裝 Windows Server 2012。 在建立 VM 之後，將會安裝一些必要的元件，並套用最新的 Windows 更新。 最後，基本 VM 將會泛化 (sysprep) 並進行清理，只留下所產生的虛擬磁片檔案。
...
> [!IMPORTANT]
> 根據預設，不會在基本 VM 上設定 proxy 設定。 如果您的網路環境中需要有 proxy，以透過 Windows 更新更新 VM，您應該在執行 "Convert CcIsoToVhdx" 時，新增-PauseBeforeUpdate 參數。 Windows 更新之前，腳本會暫停，您將有機會在 VM 上手動設定 proxy。 在設定 proxy 並且 VM 可以存取網際網路之後，您可以繼續執行腳本，以完成其餘的步驟。 
...
在商務用 Skype Cloud Connector Edition 部署期間建立 CceService 帳戶。 它會執行雲端連接器管理服務，並且需要卸載 cloudconnector.msi 的許可權。 您必須變更雲端連接器主機機上的「群組原則」設定，以指定使用者登出時不應該卸載使用者登錄。 請遵循下列步驟：
...
## <a name="set-up-your-microsoft-365-or-office-365-organization"></a>設定 Microsoft 365 或 Office 365 組織

需要商務用 Skype 線上及電話系統的 Microsoft 365 或 Office 365 組織。 在嘗試使用雲端連接器之前，請確定您的租使用者已設定並設定。

有些 Microsoft 365 和 Office 365 安裝步驟需要您使用承租人 Remote PowerShell (TRPS) 來設定 Microsoft 365 或 Office 365 組織。 **這應該安裝在主伺服器上。** 您可以從：[商務用 Skype 線上，Windows PowerShell 模組](https://www.microsoft.com/download/details.aspx?id=39366)下載 PowerShell 的商務用 Skype 線上模組。
