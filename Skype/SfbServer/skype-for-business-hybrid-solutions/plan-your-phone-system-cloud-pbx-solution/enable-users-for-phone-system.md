...
在將您的使用者移至商務用 Skype Online 之前，建議您在商務用 Skype Server 或 Lync Server 2013 中啟用您的使用者，然後在線上移動。 如需詳細資訊，請參閱[在商務用 Skype Server 和商務用 Skype Online 之間規劃混合式連接](../../skype-for-business-hybrid-solutions/plan-hybrid-connectivity.md)，以及[啟用企業內部部署使用者](enable-the-users-for-enterprise-voice-on-premises.md)的特殊考慮區段（在使用者託管內部部署時執行）。 
...
- [將使用者同步處理到雲端並指派授權](synchronize-users-to-the-cloud-and-assign-licenses.md)（使用 Office 365 執行）。
    
- [將內部部署使用者移至商務用 Skype Online](https://docs.microsoft.com/en-us/SkypeForBusiness/hybrid/move-users-from-on-premises-to-skype-for-business-online) （使用 Windows PowerShell 內部部署，但使用您的 Office 365 系統管理員認證）。
    
- [在 Office 365 語音信箱中，讓使用者使用企業語音線上和電話系統](enable-users-for-enterprise-voice-online-and-phone-system-voicemail.md)（使用遠端 PowerShell 執行）。
