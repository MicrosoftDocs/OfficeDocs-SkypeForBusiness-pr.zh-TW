...
將使用者移至商務用 Skype Online 之前，建議您先在商務用 Skype Server 或 Lync Server 2013 中，啟用您的使用者，然後再將其移動到線上。 如需詳細資訊，請參閱[規劃在商務用 Skype Server 與商務用 Skype Online 之間的混合式連線](../../skype-for-business-hybrid-solutions/plan-hybrid-connectivity.md)和[啟用企業語音內部部署的使用者](enable-the-users-for-enterprise-voice-on-premises.md)的特殊考慮一節（當使用者在內部部署時執行）。 
...
- [為使用者啟用企業語音的內部部署](enable-the-users-for-enterprise-voice-on-premises.md)（使用者位於內部部署時執行）。
    
- [指派語音路由原則](assign-a-voice-routing-policy.md)（在使用者裝載內部部署時執行）。
    
- [將使用者同步處理至雲端並指派授權](synchronize-users-to-the-cloud-and-assign-licenses.md)（使用 Office 365 執行）。
    
- [將內部部署使用者移至商務用 Skype Online](https://docs.microsoft.com/SkypeForBusiness/hybrid/move-users-from-on-premises-to-skype-for-business-online) （使用 Windows PowerShell 內部部署，但使用您的 Office 365 系統管理員認證）。
    
- [為使用者啟用企業語音線上和電話語音信箱](enable-users-for-enterprise-voice-online-and-phone-system-voicemail.md)（使用遠端 PowerShell 執行。
