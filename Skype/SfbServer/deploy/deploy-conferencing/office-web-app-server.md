...
您必須先確定已部署並設定 Office Web Apps server，才能設定商務用 Skype 伺服器以使用 Office Web Apps Server。 如需 Office Web Apps Server 的資訊，請參閱 [部署基礎結構： Office Online 伺服器](/webappsserver/deploy-the-infrastructure-office-web-apps-server)一文。 
