...
[管理工具] 包括 [拓撲建立器] 和 [控制台]。 系統管理工具必須安裝在拓撲中的至少一個伺服器, 或是執行商務用 Skype Server 支援的 Windows OS 版本的64位管理工作站。 您可以依照任何循序執行步驟1到5。 不過, 您必須在順序中執行步驟6、7和 8, 並在步驟1到5之後, 如圖表中所述。 安裝 [管理工具] 是8的步驟3。
