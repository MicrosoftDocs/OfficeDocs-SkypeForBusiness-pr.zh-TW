...
當您匯入語音路由設定檔 (。 vcfg) ，但目前在伺服器上對語音路由設定進行變更時，商務用 Skype Server [控制台] 中的 [ **語音路由** ] 群組中的頁面會指出有未認可的變更可供語音路由使用。 這些未認可的變更會使兩個設定出現差異且需要重新調整。
...
6. 依序按一下 **[認可]** 和 **[全部認可]**。
    ...
    > [!NOTE]
    > 每當您匯入語音設定檔時，您必須執行 **[全部認可]** 命令來發佈設定變更。 如需詳細資訊，請參閱 Operations 檔中的在 [商務用 Skype 中發佈待定的變更至語音路由](voice-route-config-changes.md) 設定。
