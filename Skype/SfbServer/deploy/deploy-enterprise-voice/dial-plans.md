...
5. [**簡稱] 欄位會**預先填入與 [**名稱**] 欄位中相同的名稱。 您可以選擇性地編輯此欄位，以指定更具描述性的名稱，以反映要套用撥號方案的網站、服務或使用者。
...
8. 可選在 [**外部存取前置**詞] 欄位中，只有在使用者需要撥一或多個額外的前導數位（例如9）來取得外部線路時，才指定值。 您可以輸入最多四個字元（#、* 及0-9）的前置詞值。
...
8. 可選在 [**外部存取前置**詞] 欄位中，只有在使用者需要撥一或多個額外的前導數位來取得外部線路時，才指定值（例如9）。 您可以輸入最多四個字元（也就是 #、* 及0-9）的前置詞值。
