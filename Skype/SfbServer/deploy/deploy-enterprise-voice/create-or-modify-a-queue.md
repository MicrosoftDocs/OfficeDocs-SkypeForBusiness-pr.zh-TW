...
8. 若要指定呼叫者在工程師接聽通話之前等待的時間上限, 請選取 [**啟用佇列超時設定**] 核取方塊, 然後執行下列動作:
   ...
   - 若要將來電轉寄到語音信箱, 請按一下 [**轉寄給語音信箱**], 然後在 [ **sip 位址**] 欄位中, 輸入 [sip: * \< \>username*@ *\<功能變數名稱\> * ] 的語音信箱位址 (適用于範例、sip:bob@contoso.com)。
    
   - 若要將來電轉接到另一個電話號碼, 請按一下 [**轉寄電話號碼**], 然後在 [ **sip 位址**] 欄位中, 輸入 [sip: * \<號碼\>*@ *\<功能變數名稱\>* (例如, sip:+14255550121@contoso.com)。
