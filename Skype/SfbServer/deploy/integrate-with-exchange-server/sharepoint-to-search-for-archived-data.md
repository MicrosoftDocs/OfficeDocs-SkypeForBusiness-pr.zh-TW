...
**摘要：** 設定 SharePoint Server 以搜尋 Exchange Server 2016 或 Exchange Server 2013 及商務用 Skype Server 所歸檔的資料。
  
在 Exchange 伺服器（而不是商務用 Skype 伺服器）中儲存立即訊息和網路會議記錄的主要優點之一，是將資料儲存在同一個位置，讓系統管理員可以使用單一工具搜尋已歸檔的 Exchange 資料和/或已歸檔的商務用 Skype 伺服器資料。 由於所有資料都儲存在同一個位置（Exchange）中，任何可以搜尋已歸檔 Exchange 資料的工具也都可以搜尋已歸檔的商務用 Skype 伺服器資料。
  
可讓您輕鬆搜尋已歸檔資料的一個工具是 Microsoft SharePoint Server 2013。 如果您想要使用 SharePoint 來搜尋商務用 Skype Server 資料，您必須先完成在商務用 Skype Server 中設定 Exchange 封存所需的所有步驟。 成功整合 Exchange Server 和商務用 Skype Server 之後，您必須接著在 SharePoint 伺服器上安裝 Exchange [Web Services MANAGED API](https://go.microsoft.com/fwlink/p/?LinkId=258305) 。 下載的檔案（EWSManagedAPI）可儲存至 SharePoint 伺服器上的任何資料夾。
  
檔案下載之後，請在 SharePoint 伺服器上完成下列程式：
  
1. 按一下 [**開始**]，按一下 [**所有程式**]，按一下 [**附件**]，以滑鼠右鍵按一下 [**命令提示**字元]，然後按一下 [**以系統管理員身分執行**] 來開啟命令視窗。
    
2. 在命令視窗中，使用 [cd] 命令，將目前的目錄變更為已儲存檔案 EWSManagedAPI 的資料夾。 例如，如果您已將檔案儲存至 C:\Downloads，請在命令視窗中輸入下列命令，然後按 Enter：
...
在已安裝 Exchange Web 服務之後，您必須在 SharePoint Server 與 Exchange Server 之間設定伺服器對伺服器的驗證。 若要這樣做，請先開啟 SharePoint 管理命令介面，並執行下列命令組：
...
若要設定 Exchange Server 的伺服器對伺服器驗證，請開啟 Exchange 管理命令介面，並執行類似這個的命令（假設 Exchange 已安裝在磁碟機 C：，且它使用預設的資料夾路徑）：
...
在 Exchange 與 SharePoint 之間建立伺服器間驗證之後，下一步是在 SharePoint 中建立 eDiscovery 網站。 若要這樣做，可以從 SharePoint 管理命令介面執行類似下列的命令：
...
新網站準備就緒後，下一步就是將 Exchange Server 設定為充當 SharePoint 的結果來源。 您可以從 [SharePoint 管理中心] 頁面完成下列程式來執行此動作：
...
5. 在 [新的：探索設定] 頁面上，于 [**探索集名稱**] 方塊中輸入使用者的電子郵件別名。 在 [**篩選**] 方塊中輸入**eDiscovery Lync\\***，然後按一下 [ ** &amp;新增管理來源**]。
