...
在 Exchange 伺服器 (而不是商務用 Skype 伺服器) 中儲存立即訊息和網路會議記錄的主要優點之一, 是將資料儲存在同一個位置, 讓系統管理員可以使用單一工具搜尋已歸檔的 Exchange 資料以及/或已歸檔的商務用 Skype 伺服器資料。 由於所有資料都儲存在同一個位置 (Exchange) 中, 任何可以搜尋已歸檔 Exchange 資料的工具也都可以搜尋已歸檔的商務用 Skype 伺服器資料。
