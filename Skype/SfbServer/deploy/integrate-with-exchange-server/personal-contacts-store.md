...
如果您要整合商務用 Skype Server 2015 和 Exchange Server 2016 或 Exchange Server 2013，則應該設定用戶端所使用的個人連絡人存放區。 尤其是，您應該將商務用 Skype 設定為使用 Exchange 做為個人連絡人存放區，並同時確定使用者無法覆寫該決策。 您可以在每一部用戶端電腦上建立及設定登錄值，以完成此動作。
  
> [!NOTE]
> 下列程式只需要使用 Lync 2010 用戶端或更早版本的用戶端。 Lync 2013 用戶端和所有商務用 Skype 用戶端將無法選擇覆寫連絡人存放區設定。
