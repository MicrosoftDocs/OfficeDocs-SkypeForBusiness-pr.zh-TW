...
功能變數名稱服務（DNS）伺服器對應主機名稱（例如 www）。<span> </span>contoso<span></span>.com （大概是 web 伺服器）至 IP 位址（例如10.10.10.10）。 它可協助用戶端和相互獨立的伺服器彼此通訊在網路上。 當您設定商務用 Skype Server 2015 的實施時，您必須確認新伺服器名稱的對應（通常會反映其所做的角色）符合指派給它們的 IP 位址。
...
當您按照在[建立商務用 Skype server 的 dns 記錄](../../deploy/install/create-dns-records.md)及建立商務用 skype server 的拓撲中所述，我們認識到您可以選擇使用 Windows Server 2016 或協力廠商 dns 套件中內置的 dns 功能，因此我們將在本文中繼續進行，而不是特定的討論。 我們會詳細說明所需的專案，以及您符合該需求的方式是您的決定。
