...
|:-----|:-----|:-----|:-----|:-----|
|含私人 IP 位址和 NAT 的單一合併邊緣  <br/> |否  <br/> |否  <br/> |否  <br/> |否  <br/> |
|含公用 IP 位址的單一合併邊緣  <br/> |否  <br/> |否  <br/> |否  <br/> |否  <br/> |
|使用私人 IP 位址和 NAT 調整合並後的邊緣（DNS 負載平衡）  <br/> |是  <br/> |是  <br/> |是  <br/> |是&sup1;  <br/> |
|使用公用 IP 位址調整合並的邊緣（DNS 負載平衡）  <br/> |是  <br/> |是  <br/> |是  <br/> |是&sup1;  <br/> |
|使用硬體負載平衡器調整合並後的邊緣  <br/> |是  <br/> |否（每個 VIP 一個 DNS A 記錄）  <br/> |是  <br/> |是  <br/> |
...
- 內部網路介面卡：172.25.33.10 （沒有指派預設閘道）
    ...
    > [!NOTE]
    > 請確定您的網路有一個路由，該網路包含邊緣內部介面到任何網路，且包含執行商務用 Skype Server 或 Lync Server 2013 用戶端的伺服器（例如從172.25.33.0 到192.168.10.0）。 
...
- 外部網路介面卡：
  ...
  網路會議與 A/V 邊緣公用 IP 位址是在 Windows Server 中的 [網際網路通訊協定版本4（TCP/IPv4）] 和 [Internet 通訊協定版本6（TCP/IPv6）] 屬性的 [高級] 區段中的其他（次要） IP 位址。
...
|外部 DNS  <br/> |SRV 記錄  <br/> |443  <br/> |_sip. _tls. .com  <br/> |sip.contoso.com  <br/> |您存取邊緣服務的外部介面。 商務用 Skype Server、Lync Server 2013 和 Lync Server 2010 用戶端都需要此 SRV 記錄才能在外部作業。 您必須為每一個網域使用商務用 Skype 使用者。  <br/> |
|外部 DNS  <br/> |SRV 記錄  <br/> |5061  <br/> |_sipfederationtls. _tcp. .com  <br/> |sip.contoso.com  <br/> |您存取邊緣服務的外部介面。 如果聯盟夥伴的自動 DNS 探索稱為 [允許的 SIP 網域]，則需要這個 SRV 記錄。 您必須為每一個網域使用商務用 Skype 使用者。  <br/> |
...
- 內部網路介面卡：
  ...
  - 節點1：172.25.33.10 （沒有指派預設閘道）
    
  - 節點2：172.25.33.11 （沒有指派預設閘道）
...
|外部 DNS  <br/> |SRV 記錄  <br/> |443  <br/> |_sip. _tls. .com  <br/> |sip.contoso.com  <br/> |您存取邊緣服務的外部介面。 商務用 Skype Server、Lync Server 2013 和 Lync Server 2010 用戶端都需要此 SRV 記錄才能在外部作業。 您必須針對每一個網域使用商務用 Skype。  <br/> |
|外部 DNS  <br/> |SRV 記錄  <br/> |5061  <br/> |_sipfederationtls. _tcp. .com  <br/> |sip.contoso.com  <br/> |您存取邊緣服務的外部介面。 如果聯盟夥伴的自動 DNS 探索稱為 [允許的 SIP 網域]，則需要這個 SRV 記錄。 您必須針對每一個網域使用商務用 Skype。  <br/> |
...
|:-----|:-----|:-----|:-----|:-----|:-----|
|外部 DNS  <br/> |DNS-SRV  <br/> |5061  <br/> |_sipfederationtls_tcp. .com  <br/> |sip.contoso.com  <br/> |自動 DNS 探索所需的 SIP 存取邊緣外部介面。 由您其他可能的同盟合作夥伴所使用。 它也稱為「允許 SIP 網域」。 您將需要使用商務用 Skype 使用者的每個 SIP 網域的其中一個專案。  <br/><br/> **注意：** 您需要行動與推播通知結算所需的此 SRV 記錄。 <br/> |
   
### <a name="dns-records-for-extensible-messaging-and-presence-protocol"></a>可擴展訊息和目前狀態通訊協定的 DNS 記錄

|**位置**|**類型**|**通道**|**稱**|**IP 位址或 FQDN 主機記錄**|**筆記**|
|:-----|:-----|:-----|:-----|:-----|:-----|
|外部 DNS  <br/> |DNS-SRV  <br/> |5269  <br/> |_xmpp-_tcp. .com  <br/> |xmpp.contoso.com  <br/> |您存取邊緣服務或 Edge 池中的 XMPP proxy 介面。 您必須在已啟用商務用 Skype Server 使用者的情況下，視需要為所有內部 SIP 網域重複此動作，且允許與 XMPP 連絡人的連絡人：  <br/> •全域原則  <br/> •使用者已啟用的網站原則  <br/> •已套用至商務用 Skype Server 的使用者原則  <br/> 您也必須在 XMPP 聯盟使用者原則中設定允許的 XMPP 原則。  <br/> |
