...
> [!TIP]
> 您可以將 Edge Server 設定為使用具有不同埠的單一 IP 位址，以供每項服務使用，也可以針對每個服務使用不同的 IP 位址，但使用相同的預設埠（預設埠為 TCP 443）。 以下是我們的 IP 位址需求區段中的詳細資訊。 
...
您不會在路由器或防火牆背後對單一及調整式合併 Edge Server 拓撲執行對稱 NAT （只要您不是使用硬體負載平衡）的情況下，存取、Web 會議及 A/V Edge 介面的問題。
...
|調整式合併 Edge （透過公用 IP 位址）（DNS 負載平衡）  <br/> |是  <br/> |是  <br/> |是  <br/> |是&sup1;  <br/> |
...
在基礎層級，三項服務需要 IP 位址;Access Edge service、Web 會議 Edge service 及 A/V Edge service。 您可以選擇使用三個 IP 位址，一個用於每個服務，也可以使用一個和選擇將每個服務放在不同的埠上（您可以查看「[埠和防火牆規劃](edge-environmental-requirements.md#PortFirewallPlan)」一節，以取得其中某些部分的詳細資訊）。 針對單一合併 Edge 環境，其非常大。
  
> [!NOTE]
> 如以上所述，您可以選擇讓所有三種服務具有一個 IP 位址，並在不同的埠上執行這些服務。 不過，建議您不要這麼做。 如果您的客戶無法存取您在此案例中所使用的備用埠，則他們也無法存取 Edge 環境的完整功能。 
  
調整式合併拓撲會變得更複雜，讓我們來看看一些配置 IP 位址需求的表格，請記住，拓撲選擇的主要決策點是高可用性和負載平衡。 高可用性需求可能會影響您的負載平衡選擇（我們將在表格之後討論這一點）。
...
|5   <br/> |15   <br/> |3（每個 VIP 各1個） + 15  <br/> |
   
#### <a name="ip-address-requirements-for-scale-consolidated-edge-single-ip-address-for-all-roles"></a>調整式合併 Edge 的 IP 位址需求（所有角色的單一 IP 位址）
...
- **高可用性**：若您的部署需要高可用性，您應該在集區中至少部署兩部 Edge server。 值得注意的一點是，單一 Edge 集區最多可支援12部伺服器（不過，拓撲產生器允許您新增20個，但不是經過測試或支援，因此我們建議您不要這麼做）。 如果您需要超過12部 Edge Server，則應該為它們建立其他 Edge 集區。
...
- 內部網路介面卡：172.25.33.10 （未指派任何預設閘道）
    ...
    > [!NOTE]
    > 確定有從包含 Edge 內部介面的網路，路由傳送至任何包含執行商務用 Skype Server 或 Lync Server 2013 用戶端（例如，從172.25.33.0 到192.168.10.0）之伺服器的網路。 
  
- 外部網路介面卡：
    
  - 公用 Ip：
    
  - Access Edge：131.107.155.10 （這是主要的，且預設閘道已設定為您的公用路由器，ex：131.107.155.1）
    
  - Web 會議 Edge：131.107.155.20 （次要）
    
  - A/V Edge：131.107.155.30 （第二）
    
  Web 會議和 A/V 邊際公開 IP 位址是 Windows Server 中，Internet 通訊協定第4版（TCP/IPv4）及網際網路通訊協定第6版（TCP/IPv6）的 [網際網路通訊協定第4版] 內容中的 [高級] 區段中的其他（次要） IP 位址。
    
  - 私人 Ip：
    
  - Access Edge：10.45.16.10 （這是主要的，且預設閘道已設定為您的路由器，ex：10.45.16.1）
    
  - Web 會議 Edge：10.45.16.20 （次要）
    
  - A/V Edge：10.45.16.30 （第二）
...
- 您可以在外部網路介面卡上使用一個 IP 位址。 建議您不要這麼做，因為接下來，您需要使用不同的埠來區分 thee 服務（可以在商務用 Skype Server 中執行），但是有些防火牆可能會封鎖備用埠。 如需此方面的詳細資訊，請參閱[埠與防火牆計畫](edge-environmental-requirements.md#PortFirewallPlan)一節。
...
|外部 DNS  <br/> |SRV 記錄  <br/> |443  <br/> |_sip _tls .com  <br/> |sip.contoso.com  <br/> |Access Edge service 的外部介面。 商務用 Skype Server、Lync Server 2013 和 Lync Server 2010 用戶端必須使用此 SRV 記錄，才能在外部工作。 您將需要每個網域的一個供商務用 Skype 使用者使用。  <br/> |
|外部 DNS  <br/> |SRV 記錄  <br/> |5061  <br/> |_sipfederationtls _tcp .com  <br/> |sip.contoso.com  <br/> |Access Edge service 的外部介面。 自動 DNS 探索稱為「允許的 SIP 網域」的同盟合作夥伴需要此 SRV 記錄。 您將需要每個網域的一個供商務用 Skype 使用者使用。  <br/> |
...
- 外部網路介面卡：
  ...
  - 節點2
    ...
    - 公用 Ip：
      ...
      - Access Edge：131.107.155.11 （這是主要的，且預設閘道已設定為您的公用路由器，ex：131.107.155.1）
    
      - Web 會議 Edge：131.107.155.21 （次要）
    
      - A/V Edge：131.107.155.31 （第二）
    
      Web 會議和 A/V 邊際公開 IP 位址是 Windows Server 中，Internet 通訊協定第4版（TCP/IPv4）及網際網路通訊協定第6版（TCP/IPv6）的 [網際網路通訊協定第4版] 內容中的 [高級] 區段中的其他（次要） IP 位址。
    
  - 私人 Ip：
    
    - Access Edge：10.45.16.11 （這是主要的，且預設閘道已設定為您的路由器，ex：10.45.16.1）
    
    - Web 會議 Edge：10.45.16.21 （次要）
    
    - A/V Edge：10.45.16.31 （第二）
...
|外部 DNS  <br/> |SRV 記錄  <br/> |443  <br/> |_sip _tls .com  <br/> |sip.contoso.com  <br/> |Access Edge service 的外部介面。 商務用 Skype Server、Lync Server 2013 和 Lync Server 2010 用戶端必須使用此 SRV 記錄，才能在外部工作。 您將需要每個具有商務用 Skype 之網域的一個。  <br/> |
|外部 DNS  <br/> |SRV 記錄  <br/> |5061  <br/> |_sipfederationtls _tcp .com  <br/> |sip.contoso.com  <br/> |Access Edge service 的外部介面。 自動 DNS 探索稱為「允許的 SIP 網域」的同盟合作夥伴需要此 SRV 記錄。 您將需要每個具有商務用 Skype 之網域的一個。  <br/> |
...
|:-----|:-----|:-----|:-----|:-----|:-----|
|外部 DNS  <br/> |SRV  <br/> |5269  <br/> |_xmpp-_tcp .com  <br/> |xmpp.contoso.com  <br/> |您的 Access Edge service 或 Edge 集區上的 XMPP proxy 介面。 您必須針對具有商務用 Skype 伺服器的使用者啟用所有內部 SIP 網域，針對所有內部 SIP 網域重複此動作，以便允許透過下列方式聯繫 XMPP 連絡人：  <br/> •全域原則  <br/> •使用者已啟用的網站原則  <br/> •套用到啟用商務用 Skype Server 之使用者的使用者原則  <br/> 您也需要在 XMPP 同盟使用者原則中設定允許的 XMPP 原則。  <br/> |
|外部 DNS  <br/> |SRV  <br/> |A  <br/> |xmpp.contoso.com  <br/> |主控 XMPP Proxy 服務之 Edge Server 或 Edge 集區上之 Access Edge service 的 IP 位址  <br/> |這會指向主控 XMPP Proxy 服務之 Edge Server 或 Edge 集區上的 Access Edge service。 一般來說，您建立的 SRV 記錄會指向此主機（A 或 AAAA）記錄。  <br/> |
...
商務用 Skype 伺服器會在伺服器之間及從伺服器到用戶端的安全、加密通訊使用憑證。 如您所料，您的憑證必須具有伺服器的 DNS 記錄，與憑證上的任何主體名稱（SN）和主體替代名稱（SAN）相符。 這現在會在規劃階段進行，以確保您在 DNS 中為您的憑證註冊 SN 和 SAN 專案的正確 Fqdn。
  
我們會個別討論外部和內部的憑證需求，然後查看提供兩者需求的表格。
  
### <a name="external-certificates"></a>外部憑證

指定給外部 Edge Server 介面的憑證至少必須由公用憑證授權單位（CA）所提供。 我們無法為您推薦特定 CA，但我們有一個 Ca、[整合通訊憑證合作夥伴](/SkypeForBusiness/certification/services-ssl)清單，您可以查看您的首選 CA 是否已列出。
...
- Audio/Video 驗證服務（請勿將此服務與 A/V Edge service 混淆，因為這不會使用憑證來加密音訊和影片資料流程）
    
> [!IMPORTANT]
> 所有 Edge Server （如果屬於相同的 Edge Server 集區），都必須具有與媒體轉送驗證服務具有相同私密金鑰的完全相同的憑證。 
...
|**元件**|**主體名稱 (SN)**|**主體替代名稱（SAN）/order**|**附註**|
|:-----|:-----|:-----|:-----|
...
|內部 Edge  <br/> |sfbedge.contoso.com  <br/> |NA  <br/> |您可以從公用 CA 或內部 CA 取得此憑證。 它必須包含伺服器 EKU （增強型金鑰使用方式），您會將其指派給內部 Edge 介面。  <br/> |
   
如果您需要可延伸訊息和顯示狀態通訊協定（XMPP）的憑證，它看起來會與上述的外部 Edge 表專案完全相同，但會有下列兩個額外的 SAN 專案：
...
為商務用 Skype Server Edge Server 部署的埠和防火牆，取得您的規劃許可權，可將您的疑難排解與壓力的天數或周數。 因此，我們會列出兩個表格，以指出我們的通訊協定使用方式，以及您對 NAT 和公用 IP 案例必須具有開啟、輸入和輸出的埠。 我們也會有個別的表格可用於硬體負載平衡案例（HLB），以及進一步的指導方針。 為了進一步閱讀，我們也[在商務用 Skype server 中有一些 Edge server 案例](scenarios.md)，您可以查看您的特定部署考慮。
...
|存取/SIP （TLS）  <br/> |TCP  <br/> |443  <br/> |任何  <br/> |**使用 NAT 的私人 IP：** Edge Server Access Edge service <br/> **公用 IP：** Edge Server Access Edge service 公用 IP 位址 <br/> |外部使用者存取的用戶端對伺服器 SIP 流量。  <br/> |
|存取/SIP （MTLS）  <br/> |TCP  <br/> |5061  <br/> |任何  <br/> |**使用 NAT 的私人 IP：** Edge Server Access Edge service <br/> **公用 IP：** Edge Server Access Edge service 公用 IP 位址 <br/> |適用于使用 SIP 的同盟與公用 IM 連線。  <br/> |
|存取/SIP （MTLS）  <br/> |TCP  <br/> |5061  <br/> |**使用 NAT 的私人 IP：** Edge Server Access Edge service <br/> **公用 IP：** Edge Server Access Edge service 公用 IP 位址 <br/> |任何  <br/> |適用于使用 SIP 的同盟與公用 IM 連線。  <br/> |
...
|PSOM/MTLS  <br/> |TCP  <br/> |8057  <br/> |任何：  <br/> •前端伺服器  <br/> •每一部前端伺服器  <br/>  在前端集區中 <br/> |Edge Server 內部介面  <br/> |前端伺服器或每一部前端伺服器（如果有前端集區，則為 Edge Server 內部介面）的 Web 會議流量。  <br/> |
...
|MTLS  <br/> |TCP  <br/> |50001  <br/> |任何  <br/> |Edge Server 內部介面  <br/> |集中式記錄服務控制器使用商務用 Skype Server 管理命令介面和集中式記錄服務 Cmdlet、ClsController 命令列（ClsController.exe）或代理程式（ClsAgent.exe）命令和記錄檔集合。  <br/> |
|MTLS  <br/> |TCP  <br/> |50002  <br/> |任何  <br/> |Edge Server 內部介面  <br/> |集中式記錄服務控制器使用商務用 Skype Server 管理命令介面和集中式記錄服務 Cmdlet、ClsController 命令列（ClsController.exe）或代理程式（ClsAgent.exe）命令和記錄檔集合。  <br/> |
|MTLS  <br/> |TCP  <br/> |50003  <br/> |任何  <br/> |Edge Server 內部介面  <br/> |集中式記錄服務控制器使用商務用 Skype Server 管理命令介面和集中式記錄服務 Cmdlet、ClsController 命令列（ClsController.exe）或代理程式（ClsAgent.exe）命令和記錄檔集合。  <br/> |
   
### <a name="hardware-load-balancers-for-edge-port-tables"></a>Edge 埠表的硬體負載平衡器

我們會為硬體負載平衡器（HLBs）和 Edge 埠提供其專屬的區段，因為其他硬體會變得更複雜。 請參閱下表，以取得此特定案例的指導方針：
...
|HTTPS:  <br/> |TCP  <br/> |4443  <br/> |任何：  <br/> •具有中央管理存放區的前端伺服器  <br/> •具有中央管理存放區的前端集區  <br/> |Edge Server 內部介面  <br/> |從中央管理存放區對 Edge Server 所做的變更複寫。  <br/> |
|PSOM/MTLS  <br/> |TCP  <br/> |8057  <br/> |任何：  <br/> •前端伺服器  <br/> •前端集區中的每一部前端伺服器  <br/> |Edge Server 內部介面  <br/> |前端伺服器或每一部前端伺服器（如果有前端集區，則為 Edge Server 內部介面）的 Web 會議流量。  <br/> |
...
下表提供這些案例的指導方針，但您應該可以依靠中央管理存放區（CMS）將流量路由傳送至其所知道的個別 Edge Server （當然，這會要求 CMS 在 Edge Server 資訊中保持最新狀態）。
  
|**Protocol** (通訊協定)|**TCP 或 UDP**|**Port**|**來源 IP 位址**|**目的地 IP 位址**|**附註**|
|:-----|:-----|:-----|:-----|:-----|:-----|
|存取/SIP （MTLS）  <br/> |TCP  <br/> |5061  <br/> |任何：  <br/> • Director  <br/> • Director 集區 VIP 位址  <br/> •前端伺服器  <br/> •前端集區 VIP 位址  <br/> |Edge Server 內部介面  <br/> |從 Director、Director 集區 VIP 位址、前端伺服器或前端集區 VIP 位址到 Edge Server 內部介面的輸出 SIP 流量。  <br/> |
|存取/SIP （MTLS）  <br/> |TCP  <br/> |5061  <br/> |Edge Server 內部 VIP 介面  <br/> |任何：  <br/> • Director  <br/> • Director 集區 VIP 位址  <br/> •前端伺服器  <br/> •前端集區 VIP 位址  <br/> |從 Edge Server 內部介面到 Director、Director 集區 VIP 位址、前端伺服器或前端集區 VIP 位址的輸入 SIP 流量。  <br/> |
