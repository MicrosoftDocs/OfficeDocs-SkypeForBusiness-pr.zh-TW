...
本文針對該網站上的使用者數目（如 [商務用 Skype Server 中](user-models.md)所述的使用方式），提供在網站上的多少伺服器所需的指導方針。

> [!NOTE]
> 本文中的所有建議都假設您已在伺服器上安裝了商務用 Skype 累計更新、11月2015或更新版本。
...
|**硬體元件**|**建議**|
|:-----|:-----|
|CPU  <br/> |64位雙處理器、六角-核心、2.26 ghz (GHz) 或更高版本。  <br/> 商務用 Skype Server 伺服器角色不支援 Intel Itanium 處理器。  <br/> |
|記憶體  <br/> |32 gb (GB) 。  <br/> |
|磁片  <br/> |8個或更多 10000 RPM 硬碟，至少有 72 GB 的可用磁碟空間。 磁片的兩個應該使用 RAID 1，而六個應該使用 RAID 10。  <br/> -OR- <br/>穩定狀態磁片磁碟機 (Ssd) ，可提供與 8 10000 RPM 機械磁片磁碟機類似的效能。 <br/> |
...
|**伺服器角色**|**支援的使用者人數上限**|
|:-----|:-----|
|前端集區，具有十二部前端伺服器和一部後端伺服器，或一對後端伺服器的鏡像對。  <br/> |80000同時登入的個別使用者，加上50% 的多點狀態 (MPOP) 代表非行動實例，加上40% 的使用者，以行動為總數，總使用152000端點。  <br/> |
|會議 A/V  <br/> |前端集區所提供的 A/V 會議服務，可支援集區的會議，其為最大的會議大小為250的使用者，且一次只執行一個這類大型會議。  <br/> **附注：** 此外，您可以將個別的前端集區與兩部前端伺服器搭配使用，以主控大型會議，以支援250和1000使用者之間的大型會議。 如需詳細資訊，請參閱 [在商務用 Skype Server 中規劃大型會議](../../plan-your-deployment/conferencing/large-meetings.md)。  <br/> |
|一部 Edge Server  <br/> |12000並行遠端使用者。  <br/> |
...
在前端集區中，您的集區中每個6660使用者都應該有一個前端伺服器，假設集區中的所有伺服器都已啟用超執行緒，而且伺服器硬體符合商務用 skype server 2015 的 [伺服器需求](../requirements-for-your-environment/server-requirements.md) 或  [商務用 skype Server 2019 的系統需求](../../../SfBServer2019/plan/system-requirements.md)的建議。 一個前端集區中的使用者數目上限為80000，但前提是在集區中的所有伺服器上啟用超執行緒。 如果網站上的使用者超過80000，您可以部署一個以上的前端集區。
...
針對具有80000使用者的前端集區，在遵循 [商務用 Skype Server 之使用者模型](user-models.md)的一般部署中，十二部前端伺服器的效能會比較好。 設計用來支援嚴重損壞修復容錯移轉的部署假設每兩個成對的前端集區中，最多可裝載40000個使用者，而每個集區都有足夠的前端伺服器以包含兩個集區中的使用者，則必須有一個集區容錯移轉至另一個集區。
...
下表顯示 IM 和目前狀態的平均頻寬（如使用者模型 [在商務用 Skype Server 中](user-models.md)所定義）。
...
一般商務用 Skype 伺服器前端集區所主控的支援會議大小上限，也是250參與者。 當發生 250-使用者會議時，集區仍會同時支援其他會議，例如，總的集區使用者人數為同時召開會議的5%。 例如，在十二部前端伺服器和80000使用者的集區中，當250使用者的會議發生時，商務用 Skype 伺服器支援3750其他參加小型會議的使用者。
...
|:-----|:-----|:-----|:-----|
|雙處理器，hex core，2.26 GHz 超 **執行緒** 處理的超執行緒 CPU，含 32 GB 記憶體和一個雙埠網路介面卡。  <br/> |1100  <br/> |46  <br/> |35  <br/> |
|雙處理器、十六進位核心、2.26 GHz 超執行緒 CPU，含 32 GB 記憶體和一個雙埠網路介面卡。  <br/> |1500  <br/> |63  <br/> |47  <br/> |
...
|**伺服器硬體**|**呼叫數目上限**|
|:-----|:-----|
|雙處理器、十六進位核心、2.26 GHz 超執行緒 CPU，含 32 GB 記憶體和 2 1GB 網路介面卡。  <br/> |150  <br/> |
...
|**硬體元件**|**建議**|
|:-----|:-----|
|CPU  <br/> |64位雙處理器、六角-核心、2.26 ghz (GHz) 或更高版本  <br/> |
|記憶體  <br/> |48 gb (GB)   <br/> |
|磁片  <br/> |每個磁片上都有 300 GB 的 25 10000-RPM 硬碟，具有下表中的設定  <br/> |
|網路  <br/> | 1個雙埠網路介面卡，建議使用 1 Gbps 以上的 (2，需要搭配單一 MAC 位址和單一 IP 位址進行搭配)   <br/> |
