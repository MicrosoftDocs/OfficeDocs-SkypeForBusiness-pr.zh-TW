...
我們的範例包括 SharePoint 和 Exchange online 等線上屬性，但 AuthN 和 AuthZ 的處理常式會以相同的方式，在內部部署和混合式部署中運作。 最後，當您將內部部署帳戶和密碼同步處理至雲端的 AD (（即 Azure AD) 或 intruding 流程中的使用者），以將內部部署帳戶和密碼同步處理至雲端的 AD （例如，在雲端中切換工作負載時，建立單一 Sign-On 案例）時，類似 AAD Connect 和 ADFS 的工具就會加入 AuthN 和 AuthZ 案例。 不過，他們本身不負責 AuthN 或 AuthZ，只是部分機制。
