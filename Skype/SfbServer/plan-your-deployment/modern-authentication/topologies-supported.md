...
因此，它看起來像這樣，EXO 和 SFBO 在雲端中使用 Azure AD，並 Exchange Server (nm-exch-um-2nd) 及商務用 Skype Server (SFB) 部署。
...
- 授權伺服器是以三角形表示，例如，Azure AD 為三角形後的三角形。
...
|:-----|:-----|:-----|:-----|
|僅限雲端  <br/> |![僅限具有 MA 拓撲的支援 SFB （僅限雲端）。](../../media/4d19b47f-8257-4a6f-9dab-0755206f7c52.PNG)使用者駐留/信箱位於：線上  <br/> |MA 同時適用于 EXO 和 SFBO。  <br/> 因此，授權伺服器會 Azure AD。  <br/> |多重要素驗證 (MFA) ，以用戶端憑證為基礎的驗證 (CBA) ，條件式存取 (CA) /Mobile 應用程式管理 (MAM) 使用 Intune。 \*  <br/> |
