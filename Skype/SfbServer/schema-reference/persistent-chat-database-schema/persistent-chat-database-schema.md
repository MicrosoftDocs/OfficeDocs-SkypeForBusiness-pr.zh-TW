...
Persistent 聊天資料庫指的是對應到商務用 Skype Server 後端伺服器角色**PersistentChatStore** (對應于 mgc 資料庫) 和**PersistentChatComplianceStore** (對應至mgccomp 資料庫)。 發佈此架構的目的是讓您建立查詢, 並深入瞭解如何在聊天使用、作用中的聊天室、主要海報等上建立有用的報告。
...
- [常設聊天室伺服器清單表格](list-of-persistent-chat-server-tables.md)
...
- [常設聊天室資料庫查詢範例](sample-persistent-chat-database-queries.md)
