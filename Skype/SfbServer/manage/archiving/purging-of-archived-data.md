...
**總結：** 瞭解如何管理商務用 Skype Server 的封存資料清除。
...
在執行這個命令之後，商務用 Skype Server 只會清除符合兩個準則的封存記錄： 1) 它們比為 KeepArchivingDataForDays 參數所指定的值還舊; 而 2) 使用 **Export-CsArchivingData** 指令程式匯出。
