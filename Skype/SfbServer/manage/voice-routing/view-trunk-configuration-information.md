...
2. 在 [**幹線**設定] 索引標籤上，您會看到所有中繼設定集合的清單;針對每個集合，您會看到**名稱**、**範圍**、**狀態**及**媒體旁路**屬性的值，以及**PSTN 使用量**數、**呼叫編號規則**，以及與集合相關聯的已**呼叫編號規則**。 若要查看主幹設定設定集合的其他詳細資料，請按一下感興趣的集合，按一下 [**編輯**]，然後按一下 [**顯示詳細資料**]。 請注意，您可以一次只查看一個主幹設定設定集合的詳細資訊。
