...
*範圍*指的是支援界限。 在內部部署的商務用 Skype 伺服器中, [*範圍*] 表示我們完全支援, 且已針對所列的產品版本, 已針對 TLS 1.0 和1.1 停用測試。 *目前正在調查*的意思是,我們正在積極調查如何將這些產品帶入 TLS 停用支援的範圍。 [不在*範圍*] 表示這些產品版本不支援停用 TLS 1.0 或 1.1, 且無法運作, 但已注明例外狀況。

### <a name="fully-tested-and-supported-servers"></a>經過充分測試和支援的伺服器

- 商務用 Skype Server 2019
- 商務用 Skype Server 2015 CU6 HF2 6.0.9319.516 ([2018 年3月更新](https://support.microsoft.com/en-us/help/4086059/march-2018-cumulative-update-6-0-9319-516-for-skype-for-business)) 及更高版本: 
    - Windows Server 2012 (含 KB 3140245 或取代更新), 2012 R2 或2016
- 就地升級的商務用 Skype Server 2015 (含 CU6 HF2 及更高版本) 
    - Windows Server 2008 R2、2012 (含 KB [3140245](https://support.microsoft.com/en-us/help/3140245/update-to-enable-tls-1-1-and-tls-1-2-as-a-default-secure-protocols-in)或取代更新), 或 2012 R2
...
- Lync 2013 (商務用 Skype) 桌面用戶端、MSI 與 C2R, 包括基本[15.0.5023.1000 及更高版本](https://support.microsoft.com/en-us/help/4018334/april-3-2018-update-for-skype-for-business-2015-lync-2013-kb4018334)
- 商務用 Skype 2016 桌面用戶端、MSI [16.0.4678.1000 及更新版本](https://support.microsoft.com/en-us/help/4018323/april-3-2018-update-for-skype-for-business-2016-kb4018323), 包括基本
- 商務用 Skype 2016 按一下以執行需要[2018 年4月](https://docs.microsoft.com/officeupdates/release-notes-office365-proplus)的更新: 
    - 每月與半年目標, 16\.0\.9126\.2152 及更新版本
    - 半年和延遲頻道、16\.0\.8431\.2242 及更新版本
- Mac 版商務用 Skype 16.15 及以上版本
- IOS 版和 Android 版商務用 Skype 6.19 及以上版本
- Skype Web App 2015 CU6 HF2 及更新版本 (隨附于伺服器)

### <a name="currently-being-investigated"></a>目前正在調查

#### <a name="devices"></a>台

- Lync 會議室系統 (a.k.a。 SRSv1)
- Microsoft 團隊聊天室
- Surface Hub
- 2015基礎 Survivable 分支裝置 (SBA) 或 Survivable 分支伺服器 (SBS)
...
如果您的組織必須停用 TLS 1.0 和 1.1, 且您目前使用的是 Lync Server 2013, 我們建議您開始進行規劃, 您可能必須就地升級或並排遷移 (新的池、移動使用者) 至 SkypeBusiness Server 2015 或更新版本。  或者, 您可能想要加速遷移到商務用 Skype Online。
...
從較高層面來看, 在商務用 skype server 2019 已準備好進行安裝時, 商務用 Skype Server 2015 將會要求您安裝 CU6 HF2、將必備的更新套用至 .NET 與 SQL、部署必備的登錄機碼, 以及最後一個獨立的一輪的 OS 設定更新 (亦即, 透過登錄檔案匯入停用 TLS 1.0 和 1.1)。 在您的環境中, 在任何伺服器上停用 TLS 1.0 和1.1 之前, 請務必先完成所有先決條件 (包括商務用 Skype Server 2015 CU6 HF2) 的安裝。 每個商務用 Skype 伺服器, 包括 Edge 角色和 SQL Backends, 都需要更新。 另外, 請確定所有支援的 (範圍內) 用戶端都已更新為所需的最低版本。 別忘了更新管理工作站。

我們想要遵循在升級商務用 Skype 伺服器的一般運算順序。 以您正常的方式來對待控制器池、持續聊天和配對的池。 升級的順序與方法如下所[](topology.md)述。 [](https://support.microsoft.com/en-us/help/3061064/updates-for-skype-for-business-server-2015)

### <a name="high-level-process"></a>高層次處理常式

1. 在設定生產伺服器之前, 請先測試實驗室中的所有步驟。
2. 在每個要更新的個別伺服器上, 備份並保留一份匯出的登錄複本。 您無法在伺服器之間共用註冊表;它們包含唯一的電腦機密鑰。
3. 將所有商務用 Skype 2015 伺服器升級為 CU6 HF2 或更新版本。 (適用于商務用 Skype Server 2019, 不需要 CU)
4. 安裝所有伺服器的所有先決條件。
5. 部署必備的登錄機碼。
6. 確定所有的範圍內用戶端都已更新。
...
- 商務用 Skype Server 2015 CU6 HF2 6.0.9319.516 ([2018 年3月更新](https://support.microsoft.com/en-us/help/4086059/march-2018-cumulative-update-6-0-9319-516-for-skype-for-business)) 或更高版本
- 在 registry 中啟用 SchUseStrongCrypto 的[.Net Framework 4.7](https://www.microsoft.com/en-us/download/details.aspx?id=55167)或更新版本 (如下所述)
- 在所有商務用 Skype 2015 伺服器和 backends, 必須更新 SQL。 首先更新企業版池 SQL Backends, 然後再更新其各自的 FEs。 
    - SQL Server 2014 SP1 + CU5 ([連結](https://support.microsoft.com/help/3130926)) 或更高版本/sql SERVER 2012 SP2 + CU16 或更高版本/sql SERVER 2014 RTM + CU12 ([連結](https://support.microsoft.com/en-us/help/3130923/cumulative-update-12-for-sql-server-2014)) 或更高版本/sql server 2014 sp2
    - Sql server Native Client for SQL Server 2012 ([連結](https://www.microsoft.com/en-us/download/details.aspx?id=50402))
    - Microsoft ODBC Driver 11 for SQL Server ([連結](https://www.microsoft.com/en-us/download/details.aspx?id=36434)) 或更高版本
    - SQL Server 2014 SP2 的共用管理物件 ([連結](https://www.microsoft.com/en-in/download/details.aspx?id=42295))
    - SQL server 2014 SP2 的 SQLSysClrTypes ([連結](https://www.microsoft.com/en-in/download/details.aspx?id=42295))

### <a name="basic-steps-to-install-pre-requisites-in-recommended-order-of-operations"></a>安裝必備元件 (建議作業順序) 的基本步驟

1. 將商務用 Skype Server CU6HF2 (6.0.9319.516) 更新安裝到所有伺服器。 
    1. 使用更新程式安裝元件的更新程式。
    2. 根據已記錄的程式更新資料庫。 說明已記錄在[https://support.microsoft.com/en-us/help/3061064/updates-for-skype-for-business-server-2015](https://support.microsoft.com/en-us/help/3061064/updates-for-skype-for-business-server-2015)。
...
3. 在所有伺服器上更新 SQL Express 2014。 
    ...
    4. 確保前端伺服器上的商務用 Skype Server 2015 服務已停止 
        - Ex (標準版): 停止 CsWindowsService
        - Ex (企業版): Invoke-CsComputerFailove
...
4. 更新 SQL 原生用戶端。 
    1. 參照: [https://support.microsoft.com/en-us/help/3135244/tls-1-2-support-for-microsoft-sql-server](https://support.microsoft.com/en-us/help/3135244/tls-1-2-support-for-microsoft-sql-server)。
    2. 下載自[https://www.microsoft.com/en-us/download/details.aspx?id=50402](https://www.microsoft.com/en-us/download/details.aspx?id=50402)
    3. 確保前端伺服器上的商務用 Skype Server 2015 服務已停止。 
        - Ex (標準版): 停止 CsWindowsServices
        - Ex (企業版): Invoke-CsComputerFailove
    4. 停止執行已安裝的 SQL 實例 
        - Ex: 取得服務 ' MSSQL $ RTCLOCAL」 |停止 Servic
        - Ex: 取得服務 ' MSSQL $ LYNCLOCAL」 |停止 Servic
        - Ex (僅限標準版): 取得服務「MSSQL $ RTC」 |停止 Servic
    5. 安裝更新。
5. 更新 SQL Server 的 ODBC Driver 11。 
    1. 參照: [https://support.microsoft.com/en-us/help/3135244/tls-1-2-support-for-microsoft-sql-server](https://support.microsoft.com/en-us/help/3135244/tls-1-2-support-for-microsoft-sql-server)。
    2. 下載自[https://www.microsoft.com/en-us/download/confirmation.aspx?id=36434](https://www.microsoft.com/en-us/download/confirmation.aspx?id=36434)
    3. 確定商務用 Skype Server 2015 服務已在前端伺服器上停止 
        - Ex (標準版): 停止 CsWindowsService
        - Ex (企業版): Invoke-CsComputerFailove
    4. 安裝更新。
6. 部署必備的登錄機碼。
...
**選項 1:** 使用[SmartSetup](../../deploy/install/install-skype-for-business-server.md)。 請注意, 我們正在更新 SmartSetup, 以在未來的 CU 中容納更新的 SQL 二進位檔案, 並將于未來更新此文章。

**選項 2:** 預先安裝本機 SQL 實例 (RTCLOCAL 和 LYNCLOCAL)

1. 將 SQL Express 2014 SP2 (SQLEXPR_x64) 下載並複製到 FE 上的本機資料夾。 假設 [資料夾路徑] <SQL_FOLDER_PATH>]。
2. 啟動 PowerShell 或命令提示字元, 然後流覽至 <SQL_FOLDER_PATH>。
3. 若要建立 RTCLOCAL SQL 實例, 請執行下列命令。 等到 SQLEXPR_x64 完成後再繼續:

    SQLEXPR_x64/Q/IACCEPTSQLSERVERLICENSETERMS/UPDATEENABLED = 0/HIDECONSOLE/ACTION = 安裝/FEATURES = SQLEngine, Tools/INSTANCENAME = RTCLOCAL/TCPENABLED = 1/SQLSVCACCOUNT = "NT AUTHORITY\NetworkService"/SQLSYSADMINACCOUNTS = "內建 \系統管理員 "/BROWSERSVCSTARTUPTYPE =" 自動 "/AGTSVCACCOUNT =" NTAUTHORITY\NetworkService "/SQLSVCSTARTUPTYPE = Automati
1. 若要建立 LYNCLOCAL SQL 實例, 請執行下列命令。 等到 SQLEXPR_x64 完成之後, 再繼續進行下一個步驟:

    SQLEXPR_x64/Q/IACCEPTSQLSERVERLICENSETERMS/UPDATEENABLED = 0/HIDECONSOLE/ACTION = 安裝/FEATURES = SQLEngine, Tools/INSTANCENAME = LYNCLOCAL/TCPENABLED = 1/SQLSVCACCOUNT = "NT AUTHORITY\NetworkService"/SQLSYSADMINACCOUNTS = "內建 \系統管理員 "/BROWSERSVCSTARTUPTYPE =" 自動 "/AGTSVCACCOUNT =" NTAUTHORITY\NetworkService "/SQLSVCSTARTUPTYPE = 自動
1. 執行商務用 Skype Server 2015 RTM 設定。
2. 請依照上述 [先決條件] 區段的其餘步驟進行。

**選項 3:** 您也可以在本機安裝媒體目錄中手動取代二進位檔案, 如下所示:
...
2. 2. 安裝 .NET 4.7: 
      ...
      - 下載: https://www.microsoft.com/en-us/download/details.aspx?id=55167。
...
8. 安裝本機配置存放區 (步驟 1): 
     - 開啟 [部署嚮導], 按一下 [安裝或更新商務用 Skype Server System], 然後按一下 [**執行**] (在步驟 1: 安裝本機配置存儲區)。
...
9. 設定或移除商務用 Skype Server 元件 (步驟 2):
    - 開啟 [部署] 嚮導, 按一下 [**安裝或更新商務用 Skype Server System**], 然後按一下 [**執行**] 步驟 2: 設定或移除商務用 skype server 元件
