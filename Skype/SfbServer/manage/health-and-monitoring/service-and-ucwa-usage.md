...
在大部分的部署中, UCWA 的 CPU 使用量平均應低於 15%。 記憶體使用量應該在監視器中的 [[商務用 Skype 伺服器的伺服器記憶體容量限制] 中](server-memory-capacity-limits.md)所述的限制範圍內。
...
在大部分的部署中, 行動服務 CPU 使用量的平均應低於 15%。 記憶體使用量應該在監視器中的 [[商務用 Skype 伺服器的伺服器記憶體容量限制] 中](server-memory-capacity-limits.md)所述的限制範圍內。
