...
使用者註冊報告提供使用者登入活動的概覽，最值得注意的是在指定期間 (每小時、每日、每週、每月) 時，登入商務用 Skype 伺服器的使用者人數。 請記住，報告只會告訴您已登入的人員人數。 它不會告訴您登入的人員。 監視報告不會提供有關哪些特定使用者使用商務用 Skype Server (，以及哪些使用者未) 的資訊。 不過，您可以使用 [使用者活動報告] 來取得使用者資訊的粗略估計。
...
此外，使用者註冊報告會記錄在指定的時段內，有多少「作用中」使用者存在。 作用中的使用者是在立即訊息 (IM) 會話中參與的使用者，參與商務用 Skype Server 會議、撥打或接聽電話，或在該期間內使用商務用 Skype Server。 這不同於登入但實際上從未使用系統的使用者。
...
在您部署商務用 Skype Server 的一個常見問題是：如何知道我的使用者是否真的使用這項新技術？ 雖然在這方面有些許限制，但是使用者註冊報告可以協助您回答這個問題。 若要判斷使用者是否正在使用商務用 Skype Server，您需要執行兩項操作。 首先，從使用者註冊報告取得唯一登入使用者計量值。 這個值會告訴您已登入商務用 Skype 伺服器的不同個人人數。
  
相比之下，Total 登入度量單位會顯示登入商務用 Skype Server 的任何人的總時間。 例如，假設 Ken Myer 在一天內登入商務用 Skype Server 五個不同的時間。 在此情況下，就總登入計量而言，Ken Myer 會計為五個不同的登入工作階段，但是就唯一登入使用者計量而言，只有一位登入使用者。 同樣地，一位使用者從多個裝置或多個位置登入並無不同。 例如，使用者可以使用她的桌上型電腦（她的膝上型電腦）登入，而她可以擁有可自動登入商務用 Skype 伺服器的 IP 電話。 在此範例中，共有一位唯一使用者登入三次。
...
除了瞭解唯一登入的次數之外，您還需要知道啟用商務用 Skype Server 的使用者總數。 開啟商務用 Skype Server 管理命令介面，並執行下列 Windows PowerShell 命令，即可取得此值：
  
```PowerShell
(Get-CsUser).Count
```

如果上述命令傳回的值為1236，且唯一登入使用者度量會傳回平均值667，則表示啟用商務用 Skype 的一半以上的使用者，實際上就是每日 (，也就是667除以1236，也就是大約 54% ) 。
...
您也應該要比較唯一登入使用者值與唯一作用中使用者計量值。 「作用中的作用中使用者」度量可告訴您，實際使用商務用 Skype 伺服器的使用者數目有多少：他們撥打了電話，他們加入了商務用 Skype Server 會議，或參加了 IM 會話。 這是很有用的資訊，因為商務用 Skype 伺服器可以設定為在使用者每次啟動 Windows 時自動啟動。 因此，您可能會有大量的使用者在每日登入 Windows 時自動登入商務用 Skype，但在該時段內，則永不實際使用商務用 Skype Server。
  
「作用中的「作用中使用者」度量也會在組織中提供更有意義的資料，而使用者通常不會在一天結束時登出 Windows。 相反地，他們只是鎖定其電腦，讓 Windows 和商務用 Skype 繼續運作。 在此情況下，您可能會最後一次登入，因為您的使用者在數天前登入，而且永不登出。 不過，具有唯一性的作用中使用者可告訴您使用者積極使用商務用 Skype 或其他商務用 Skype Server 用戶端。
