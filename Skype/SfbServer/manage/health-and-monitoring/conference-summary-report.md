...
會議摘要報告提供線上會議工作階段的整體檢視。 會議通常會包含超過2個使用者，且需要使用會議服務。 相比之下，點對點工作階段一般只包含2位使用者，不需要使用商務用 Skype Server 會議服務。 對等活動會在 [商務用 Skype Server 的 Peer-to-Peer 活動摘要報告](peer-to-peer-activity-summary-report.md)上報告。
