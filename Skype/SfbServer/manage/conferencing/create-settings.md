...
您可以使用商務用 Skype Server 控制台或使用商務用 Skype Server 管理命令介面來建立會議設定設定。
...
9. 若要自訂傳送給參與者的會議邀請，請執行下列動作。 請注意，URLs 和自訂頁腳文字的最大長度為1KB。 除了 [說明 **URL**] 之外，如果您沒有指定自訂專案的值，這些自訂專案不會包含在會議中。 如果不包含自訂的 [說明] URL，則會在邀請中顯示商務用 Skype 的預設「說明」 URL。 
...
## <a name="create-meeting-configuration-settings-by-using-skype-for-business-server-management-shell"></a>使用商務用 Skype Server 管理命令介面建立會議設定設定
...
如需詳細資訊，包括完整的參數清單，請參閱 [New-CsMeetingConfiguration](https://docs.microsoft.com/powershell/module/skype/new-csmeetingconfiguration?view=skype-ps)。
