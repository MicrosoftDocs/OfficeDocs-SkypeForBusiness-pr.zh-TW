...
3. 如果您要編輯的是 Enterprise Edition 集區的屬性，您可以選取 [覆 **寫 FQDN**] 選項。 只有在您使用網域名稱系統 (DNS) 負載平衡時，才應選取此選項。 如果您使用 DNS 負載平衡，請選取 **[覆寫 FQDN]**，然後在文字方塊中輸入集區的 FQDN，以解析至該集區中所有伺服器的實體 IP 位址。 如果您未使用 DNS 負載平衡，且未選取 **[覆寫 FQDN]**，則無法變更內部 Web 服務 FQDN。 內部 web 服務 FQDN 是內部使用者用來連線至商務用 Skype 伺服器的 URL。
