...
> [!TIP]
> **Office Web apps server 探索 URL** 的預設行為是，以 [格式： `https://<FQDN of the Office Web Apps Server/hosting/discovery` ] 的 Office Web apps server 的 FQDN 建立 URL。 在大多數的情況下，您不需要變更預設的格式。 當 Office web apps server 和 Office Web apps server 探索 URL 必須不同時，您可能需要變更預設的格式。 例如，您的 Office Web Apps Server 位於周邊網路中，而且會根據位置而有不同的 URL。
