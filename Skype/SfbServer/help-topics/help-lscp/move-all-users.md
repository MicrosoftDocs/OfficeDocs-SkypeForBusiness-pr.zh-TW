---
title: 移動所有使用者
ms.reviewer: ''
ms.author: v-cichur
author: cichur
manager: serdars
ms.date: 11/17/2018
audience: ITPro
ms.topic: article
f1.keywords:
- CSH
ms.custom:
- ms.lync.lscp.UserMoveAll
ms.prod: skype-for-business-itpro
localization_priority: Normal
ms.assetid: 48bf9dad-bc51-4996-9417-8f15c1d476ad
description: 在 [移動使用者] 對話方塊中，您可以選取來源和目的地集區，將使用者帳戶從一個集區移到部署中的另一個集區。
ms.openlocfilehash: 65a34b4c96d0a16b48ab422aba2def28361748302cee23e8d1fb8d960009d7d8
ms.sourcegitcommit: a17ad3332ca5d2997f85db7835500d8190c34b2f
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/05/2021
ms.locfileid: "54287102"
---
# <a name="move-all-users"></a>移動所有使用者

在 [移動使用者] 對話方塊中，您可以選取來源和目的地集區，將使用者帳戶從一個集區移到部署中的另一個集區。

如需移動使用者的詳細資訊，請參閱＜[Move Users to Another Pool](/previous-versions/office/lync-server-2013/lync-server-2013-move-users-to-another-pool)＞。