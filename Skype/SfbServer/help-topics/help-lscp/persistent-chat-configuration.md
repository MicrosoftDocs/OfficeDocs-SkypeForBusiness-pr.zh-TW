...
> [!NOTE]
> 雖然聊天室的許多管理功能都可在針對使用者執行持續聊天的電腦中使用, 但持續聊天系統管理員 (在**cspersistentchatadministrator**角色中) 必須使用控制台或管理命令介面建立或管理類別的 Cmdlet。
  
持續性聊天系統管理員使用商務用 Skype 伺服器控制台或 Windows PowerShell Cmdlet 來建立及管理類別, 以及針對組織中的使用者設計聊天室的存取。
  
持久的聊天室管理員 (能管理一或多個聊天室), 可以使用用戶端啟動會議室管理 Web 應用程式來建立及管理會議室 (或客戶可以建立要呼叫的自訂解決方案和工作流程)。 持續聊天
