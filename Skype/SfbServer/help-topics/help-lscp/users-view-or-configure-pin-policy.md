---
title: 使用者查看或設定 PIN 原則
ms.reviewer: ''
ms.author: v-mahoffman
author: HowlinWolf-92
manager: serdars
ms.date: 11/17/2018
audience: ITPro
ms.topic: article
f1.keywords:
- CSH
ms.custom:
- ms.lync.lscp.UserViewUserPin
ms.prod: skype-for-business-itpro
ms.localizationpriority: medium
ms.assetid: 4a68227c-f559-442f-955b-5c4ccc06b7f4
description: 您要執行的工作 按一下您要執行之任務的連結。
ms.openlocfilehash: 5d88e4a266f9ecee99f7deba22916df5a31bcb91
ms.sourcegitcommit: 67324fe43f50c8414bb65c52f5b561ac30b52748
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 11/08/2021
ms.locfileid: "60852297"
---
# <a name="users-view-or-configure-pin-policy"></a>使用者：檢視或設定 PIN 原則
 
您要執行的工作 按一下您要執行之任務的連結。
  
- [在商務用 Skype Server 2015 中設定使用者的電話撥入式會議 PIN](../../manage/authentication/set-a-user-s-dial-in-conferencing-pin.md)
    
如需使用商務用 Skype Server 控制台可執行之不同程式的詳細資訊，請參閱[Manage 商務用 Skype Server 2015](../../manage/manage.md)。

