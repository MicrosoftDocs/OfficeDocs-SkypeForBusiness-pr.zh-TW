...
您可以使用 [常設聊天室] **** 群組的 [常設聊天室原則] **** 頁面來管理全域、集區、網站或使用者層級的原則，包括為部署設定預設全域原則，以及建立一個或多個其他使用者和站台原則。 如果使用者依原則啟用持續聊天伺服器，則持續聊天伺服器環境會出現在其用戶端中。
...
您可以在 [常設聊天室原則] **** 頁面上執行下列工作：啟用常設聊天室伺服器原則；管理常設聊天室伺服器原則。
...
如果使用者已啟用商務用 Skype Server 的功能，您可以將適當的原則套用至特定的使用者，以啟用或停用持久聊天伺服器。
