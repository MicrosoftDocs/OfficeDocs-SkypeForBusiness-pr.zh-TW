...
# <a name="select-translation-rules"></a>選取轉譯規則
 ...
 Enterprise Voice 要求所有的撥號字串都正常化為 e.164 格式，以執行反向號碼查閱 (RNL) 。 主幹對等 (亦即，關聯的閘道、PBX 或 SIP 主幹) 可能要求號碼要為當地撥號格式。 為了將號碼從 E.164 格式轉譯成當地撥號格式，您可以選用定義一或多個轉譯規則，先操作要求 URI 再將它遞送給主幹對等。 例如，您可以撰寫轉譯規則，從撥號字串的開頭移除 +44 並替換成 0144。
