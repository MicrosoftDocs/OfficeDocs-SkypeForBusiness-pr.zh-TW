...
若要開始安裝，或更新現有的商務用 Skype Server 2015 部署，請按一下 [ **安裝或更新商務用 Skype 伺服器**]。 本節中的步驟會根據拓撲產生器檔，安裝指定伺服器角色特有的元件。 在您開始執行這系列步驟之前，必須先在拓撲產生器中定義部署。 您必須以本機系統管理員身分登入您要部署的電腦，並以伺服器所在網域的 Domain Admins 群組成員身分登入。 完成此步驟之前，電腦必須已加入網域。
