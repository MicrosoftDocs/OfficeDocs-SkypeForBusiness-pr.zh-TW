---
title: 商務用 Skype Server：行動計畫工具
ms.reviewer: ''
ms.author: v-cichur
author: cichur
manager: serdars
ms.date: 4/8/2016
audience: ITPro
ms.topic: article
f1.keywords:
- CSH
ms.custom:
- ms.lync.plan.Mobility
- ms.lync.plan.Mobility
ms.prod: skype-for-business-itpro
ms.localizationpriority: medium
ms.assetid: feceed86-0f71-47e4-8e56-e5181c6b5cc9
description: 商務用 Skype Server 2015 中的行動功能可支援行動裝置上商務用 Skype 功能。
ms.openlocfilehash: f185a91897ae6efd5cc007a3f13250c221f543e4
ms.sourcegitcommit: 556fffc96729150efcc04cd5d6069c402012421e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/26/2021
ms.locfileid: "58600308"
---
# <a name="skype-for-business-server-mobility-planning-tool"></a>商務用 Skype Server：行動計畫工具
 
商務用 Skype Server 2015 中的行動功能可支援行動裝置上商務用 Skype 功能。
  
行動功能與 Lync Server 2013 中的功能相同。 如需準備行動環境的詳細資訊，請參閱 [在 Lync Server 2013 中規劃行動](/previous-versions/office/lync-server-2013/lync-server-2013-planning-for-mobility)。
