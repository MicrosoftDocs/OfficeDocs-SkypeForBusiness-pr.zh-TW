...
在理想的世界中，您絕不會遇到商務用 Skype Server 2015 的問題。 不過，商務用 Skype 伺服器可能會受到外部因素的影響（例如，網路損毀和硬體失敗）。 透過使用商務用 Skype Server 2015 管理元件，您可以主動識別並處理潛在的問題。 如此一來，商務用 Skype Server 2015 管理套件可擴充 System Center Operations Manager 的功能。
  
這項資訊是以商務用 Skype Server 2015 通訊軟體之監控套件版本9319.0 為基礎。
  
## <a name="configuration-overview"></a>設定概述

 若要設定商務用 Skype Server 2015 基礎結構以與 System Center Operations Manager 搭配使用，您必須執行下列三項動作：
...
識別及  [設定主要管理伺服器](configure-the-primary.md)。 若要這麼做，您必須安裝 System Center Operations Manager 2012 SP1 或 R2。 
 ...
 識別及 [安裝及設定監視程式節點](watcher-nodes.md)。 監看員節點是定期執行商務用 Skype Server 綜合交易的電腦，Windows PowerShell Cmdlet 會驗證重要的商務用 Skype Server 元件（例如，登入系統或 exchange 立即訊息的功能）會如預期的方式運作。 
...
|無代理監控  <br/> |不支援。  <br/> |
|虛擬環境  <br/> |是。  <br/> |
|加入網域的伺服器角色  <br/> |所有內部商務用 Skype Server 2015 伺服器角色必須已加入網域。  <br/> |
|獨立伺服器角色  <br/> |商務用 Skype Server 2015 Edge Server 不一定要加入網域。  <br/> |
...
下列是商務用 Skype Server 2015 管理套件的新功能。
...
- **影片 Interop 服務綜合交易** 從其他廠商解決方案遷移至商務用 Skype Server 2015 的客戶，通常需要繼續使用電話會議裝置的 [影片] 裝置 (VTCs) 從其他廠商取得。 Video Interop 伺服器是新的商務用 Skype Server 2015 伺服器角色，可讓客戶透過透過視頻 SIP 主幹連線到 Cisco CUCM，繼續使用其會議室中的 Cisco VTCs。 這項功能也會新增綜合交易，以協助確認影片 Interop 伺服器是否已開啟，並可以處理透過視頻 SIP 主幹的傳入連線。
    
- **應用程式共用會議綜合交易** 現在支援應用程式共用會議的端對端案例驗證。
    
## <a name="monitoring-scenarios"></a>監視案例

商務用 Skype Server 2015 管理元件會利用多種功能來協助您偵測和診斷問題。 這些功能可讓您即時看到商務用 Skype Server 2015 環境的健康情況。
...
|通話可靠性警示  <br/> |商務用 Skype Server 2015 server 所撰寫的詳細通話記錄 (Cdr) 會反映使用者是否可以連線至通話或終止通話的原因。 通話可靠性警示會查詢 CDR 資料庫，以產生警示，指出當高數目的使用者遇到對等通話或基本會議功能的連線問題時。  <br/> 案例覆蓋範圍包括音訊通話、對等立即訊息 (IM) 及其他會議功能。  <br/> |
|媒體質量警示  <br/> |查看經驗品質 (QoE) 商務用 Skype Server 2015 用戶端在每次通話結束時所發佈的報表。 這些查詢會產生警示，以找出使用者最可能經歷通話和會議期間受損媒體質量的情況。 資料是以重要的度量單位（例如資料包延遲和遺失）為基礎，其可直接促進使用者經驗的品質。  <br/> |
 ...
 **高優先順序警示：** 這些警示表示的情況會導致大量使用者的服務中斷，而且需要立即採取行動。 綜合交易和離線服務 (（例如商務用 Skype Server Audio/Video 會議）偵測到的中斷，) 具有高優先順序的警示。 相反地，單一機器上的元件失敗不是高優先順序的警示。 商務用 Skype Server 2015 具有這些狀況的內建高可用性功能，例如，在負載平衡器背後的多部前端伺服器。
...
商務用 Skype Server 2015 管理元件可透過綜合交易，提供更高的警示覆蓋範圍。 綜合交易是整合在 Operations Manager 管理元件中的 Windows PowerShell Cmdlet，用來測試端對端使用者案例。 當您指定伺服器以執行綜合交易時，這些指令程式會定期由管理套件觸發。 綜合交易產生的失敗會產生狀態警示。 以下為商務用 Skype Server 2015 的支援綜合交易：
...
|8   <br/> |MCX Peer-to-Peer 立即訊息 (mobile)   <br/> |可在2011年9月發行的 Lync Server 2010 中取得，以供商務用 Skype 2015  <br/> |
...
|共  <br/> |XMPP 立即訊息 (同盟)   <br/> |可在 Lync Server 2013 和商務用 Skype 2015 中使用  <br/> |
| 21  <br/> |視訊互通性伺服器  <br/> |商務用 Skype Server 2015 的新增功能  <br/> |
||||
