...
- **與 XMPP 基礎服務提供者的同盟**
    ...
    商務用 Skype Server 2015 引進在 Edge server 上部署的完整整合式 XMPP proxy () 以及在前端伺服器上部署的 XMPP 閘道。 您可以部署新增及設定 XMPP proxy 和 XMPP 閘道，以允許商務用 Skype Server 2015 使用者從 XMPP 的協力廠商夥伴新增連絡人，以進行立即訊息 (IM) 及顯示狀態。
...
- **Exchange 封存整合**
    ...
    如果您有位於 Exchange 2013 的使用者，且其信箱已放在 In-Place 保留狀態，您可以選擇將商務用 Skype Server 2015 儲存裝置與 Exchange storage 整合的選項。
