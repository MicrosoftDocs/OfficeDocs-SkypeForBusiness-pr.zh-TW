...
若要使用商務用 Skype Server 管理命令介面執行集中式記錄服務功能，您必須是 CsAdministrator 或 CsServerAdministrator 角色型存取控制 (RBAC) 安全性群組的成員，或是包含這兩個群組之一的自訂 RBAC 角色。 若要傳回所有以角色為基礎的存取控制清單 (RBAC) role 此 Cmdlet 已指派給 (包括您) 自行建立的自訂 RBAC 角色，請從商務用 Skype Server 管理命令介面或 Windows PowerShell 提示執行下列命令：
...
本主題的其餘內容將特別針對定義提供者、修改提供者的方式，及提供者定義所包含的內容做討論，以使您的疑難排解作業獲得最佳成效。 有兩種方式可發出集中式記錄服務命令。 根據預設，您可以使用目錄 C:\Program Files\Common Files \ 商務用 Skype Server 2015 \ CLSAgent 中所用的 CLSController.exe。 或者，您可以使用商務用 Skype Server 管理命令介面發出 Windows PowerShell 命令。 您可以使用 Windows PowerShell 來定義新的提供者，以供記錄會話使用，並可完全控制其建立、收集的內容，以及收集資料的層級。
...
- **旗標** OCSLogger 提供的選項可為定義您從追蹤檔案中取得的資訊類型的每個提供者選擇旗標。 您可以依據提供者選擇下列旗標：
  ...
  - **TF_Protocol** 提供通訊協定郵件（如 SIP）和組合的 Community 編解碼器套件郵件。
...
2. 若要檢視現有提供者的設定，請輸入下列項目：
   ...
   ```
    ...
    命令會顯示內含相關旗標、設定和元件的提供者清單。 如果顯示的資訊不夠，或清單太長，以致于預設的 Windows PowerShell 清單格式，您可以透過定義不同的輸出方法來顯示其他資訊。 若要進行這項作業，請輸入：
