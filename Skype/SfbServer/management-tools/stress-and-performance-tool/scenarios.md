...
若要執行商務用 Skype Server 2015 的壓力與效能工具（LyncPerfTool），必須先針對與您相關的案例設定商務用 Skype Server 2015 拓撲。 如果未設定商務用 Skype Server 2015，或設定不正確，您的負載模擬很可能會失敗。 使用商務用 Skype Server 2015 的壓力與效能工具，我們提供的是商務用 Skype Server Management Shell 腳本和基本資源[檔案的範例。](https://www.microsoft.com/download/details.aspx?id=50367) 這些都可以用來做為設定您的商務用 Skype Server 部署的起點。 本文將說明提供的 Windows PowerShell 範例。
