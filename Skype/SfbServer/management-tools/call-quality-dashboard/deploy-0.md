...
QoE 封存的設定套裝程式括建立 QoE 封存資料庫、部署 SQL Server 預存程式，將資料從來源 QoE 計量資料庫移至 QoE 封存資料庫，以及設定 SQL Server 代理程式工作以執行儲存的以固定間隔為過程。 
  
Cube 部署會取得來自 QoE 封存所在位置之使用者的資訊、部署 cube，並設定一般 SQL Server 代理程式工作，以便定期重新整理 cube。
  
入口網站安裝建立儲存 CQD 使用者與每個使用者報告/查詢之對應的存放庫資料庫。 然後，它會設定 IIS web 應用程式，該應用程式是使用者可以查看預先定義的報表集合，以及自訂和建立自己的查詢，以視覺化 cube 中的資料。 入口網站安裝會建立其他兩個 web 應用程式，公開 APIs 使用者可透過程式設計方式存取存放庫及 cube。 （這些 APIs 也是由儀表板內部使用。）
...
1. 將 CallQualityDashboard 複製到要安裝 CQD 之封存資料庫元件的機器（這是已安裝 SQL Server 的電腦）。 
    
2. 執行 MSI （Windows 將會提示以系統管理員許可權執行，請執行此動作）。 
...
6. 在 [QoE 封存設定] 頁面上，提供下列資訊：
   ...
   - **QoE 度量 SQL Server：** QoE 度量 DB 所在位置（這會是資料來源）的 SQL Server 實例名稱。
    
   - **QoE 封存 SQL Server 名稱：** 這是唯讀欄位，且固定為本機電腦的完整功能變數名稱。 封存資料庫只能安裝在本機電腦上。
    
   - **QoE 封存 SQL Server 實例：** 本機 SQL Server 實例名稱，以供建立封存資料庫的位置。 若要使用預設的 SQL Server 實例，請將此欄位保留空白。 若要使用指定的 SQL Server 實例，請指定實例名稱（例如，"）"\"的實例名稱。
    
   - **QoE 封存資料庫：** 根據預設，此選項會設定為「建立新資料庫」。 由於不支援封存資料庫升級，因此如果現有的封存資料庫與要安裝的組建具有相同的架構，則可以使用「使用現有資料庫」選項的唯一情形。
    
   - **資料庫檔案目錄：** 應該放置封存資料庫之資料庫檔案（.mdf 和 .ldf）的路徑。 這應該是與作業系統分開的磁片磁碟機（建議的硬體設定中 HDD2）。 請注意，因為檔案名已在安裝中修復，所以建議您不要使用沒有檔案的空白目錄。
   ...
   - **分割區檔案目錄：** QoE 封存資料庫的磁碟分割應位於何處的路徑。 這應該位於磁片磁碟機（建議的硬體設定中的 HDD3），而不同于操作系統磁碟機和 SQL 資料庫記錄檔磁片磁碟機。 請注意，因為檔案名已在安裝中修復，所以建議您不要使用沒有檔案的空白目錄。
    
   - **SQL 代理程式工作使用者-使用者&amp;名稱密碼：** 網域服務帳戶名稱和密碼（遮罩），用來執行 SQL Server 代理程式工作的「QoE 封存資料」步驟（會執行預存程式，以將資料從 QoE 度量 DB 提取到封存資料庫，所以此帳戶必須具備 QoE 計量 DB 的讀取權，如 [帳戶] 區段中所示）。 此帳戶也需要在 QoE 封存 SQL Server 實例中使用登入。
...
7. 按 [下一步] 時，安裝程式會執行先決條件檢查，並在遇到任何問題時報告。 當所有必要條件檢查都通過時，安裝程式會移至 [Cube 設定] 頁面。 
    ...
    > [!NOTE]
    > 如果安裝程式顯示一則警告訊息，指出 QoE 封存 SQL Server 實例的 SQL Server 代理程式服務目前並未執行，安裝可以繼續，但是安裝後請確定 SQL 代理程式服務正在執行中，並將啟動類型設定為自動，讓排定的工作執行。 
  
8. 在 [Cube 設定] 頁面上，提供下列資訊：
    
   - **QoE 封存 SQL Server 名稱：** 這是唯讀欄位，且固定為本機電腦的完整功能變數名稱。 Cube 只能從具有 QoE 封存資料庫的機器（附注）進行安裝。 Cube 本身可以安裝在遠端電腦上。 請參閱下文）
    
   - **QoE 封存 SQL Server 實例：** QoE 封存 DB 所在位置的 SQL Server 實例名稱。 若要指定預設的 SQL Server 實例，請將此欄位保留空白。 若要指定命名的 SQL Server 實例，請輸入實例名稱（例如，"）"\"的實例名稱。 如果已選取 [QoE 封存元件] 進行安裝，此欄位將預先填入 QoE 封存設定] 頁面上提供的值。
   ...
   - **Cube 使用者-使用者名稱&amp;密碼：** 將觸發 cube 處理的網域服務帳戶名稱和密碼（遮罩）。 如果已選取 [QoE 封存元件] 進行安裝，此欄位將預先填入 SQL 代理工作使用者「封存設定」頁面上的值，但建議您指定不同的網域服務帳戶，以讓安裝程式授與最小必要的許可權。
...
10. 在入口網站設定頁面上，提供下列資訊：
    ...
    - 存放**庫 SQL Server：** 要建立存放庫資料庫的 SQL Server 實例名稱。 如果安裝程式（在其他元件）中已提供 QoE 封存資料庫所在位置的 SQL Server 實例名稱，此欄位將預先填入 QoE 封存 DB SQL Server 實例名稱。 這可以是任何 SQL Server 實例。
    
    - 存放**庫資料庫：** 根據預設，此選項會設定為「建立新資料庫」。 由於不支援存放庫 DB 升級，因此如果現有的存放庫 DB 與要安裝的組建具有相同的架構，則唯一可以使用「使用現有資料庫」選項的情況。
    
    - **IIS 應用程式集區使用者- &amp;使用者名稱密碼：** IIS 應用程式集區應該在其下執行的帳戶。 [使用者名稱] 和 [密碼] 欄位會在選取內建系統帳戶時呈現灰色。 只有從下拉式方塊中選取 "Other"，這樣使用者才能輸入網域服務帳戶資訊時，才會啟用這些欄位。
...
主要入口網站頁面可透過進行`http://<machinename>/CQD`存取。 
...
設定詳細資料儲存在入口網站之實體目錄的 web.config 中。
...
## <a name="configuring-file-access-for-the-cqd-dashboard"></a>設定 CQD 的檔存取（儀表板）
...
針對 HTTP 和 HTTPS 埠系結，安裝程式會在預設埠號碼（埠80針對 HTTP，埠443為 HTTPS）上建立埠系結。 如果電腦上有其他網站使用這些系結，將會發生衝突，且無法預測 IIS 行為。 避免這種問題的最佳方法，就是在安裝 CQD 之前，請先確定沒有其他網站對應至埠80和443。 
...
- 從 CqdPartitionedStreamView 中選取 MIN （StartTime）
    
- 從 CqdPartitionedStreamView 中選取 MAX （StartTime）
    
- 從 CqdPartitionedStreamView 中選取 MIN （EndTime）
    
- 從 CqdPartitionedStreamView 中選取 MAX （EndTime）
...
網路類型是用來描述組織內的不同類型的網路。 這可讓您篩選出（或篩選出）特定的網路類型。
...
匯入建築物可讓您取得特定的洞察力（WiFi/有線等各座大樓的低通話）。 
  
> [!NOTE]
> 這是選用的步驟，但建議使用。 
  
在匯入新的大樓之前，您應該已經識別出預先定義的 BuildingKey。 若要這麼做，請發出 "SELECT MAX （BuildingKey） FROM CqdBuilding" SQL 命令以識別目前的值，並將1新增至結果。
...
|BBssid  <br/> |Bss  <br/> |00-00-00-00-00-00 （您必須使用分隔的 fformat）  <br/> |
...
將您的第一筆記錄（使用 [Select MIN （StartTime）從 CqdPartitionedStreamView SQL 命令）的日期，識別為明天的 EndDate，最後兩個值為 Null。
