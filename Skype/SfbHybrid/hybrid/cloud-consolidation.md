...
description: 本文說明如何針對具有內部部署 () 商務用 Skype (或 Lync) 的組織進行整合，以尋求將其 UC 工作負載移至小組和/或商務用 Skype Online 的組織。
ms.openlocfilehash: 5533b1780edd2ab8a997d0f04665876c2f85f149
ms.sourcegitcommit: 01087be29daa3abce7d3b03a55ba5ef8db4ca161
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 03/23/2021
ms.locfileid: "51119022"
---
# <a name="cloud-consolidation-for-teams-and-skype-for-business"></a>Teams 和商務用 Skype 的雲端整合

許多大型企業擁有多個內部部署 AD 樹系，在某些情況下，客戶會有多個 Exchange 和/或商務用 Skype Server (或 Lync Server) 部署。 此外，即使是只有一個內部部署樹系的組織，透過企業併購的過程，也會發現自己處於類似的情況。 當這些客戶移至雲端時，他們想要將特定內部部署工作負載的多個實例合併到單一 Microsoft 365 或 Office 365 組織中。 本文說明如何針對具有多個內部部署商務用 Skype (或 Lync) 的整合，將其 UC 工作負載移至 Microsoft 雲端，例如，Microsoft 團隊和/或商務用 Skype Online 的組織。
...
假設組織有兩個個別的同盟內部部署的商務用 Skype 部署，其要在 Microsoft 團隊或商務用 Skype Online 中將其合併在線上。
...
4.  針對即將成為混合式 (OriginalCompany 的樹系，[啟用 AAD Connect](configure-azure-ad-connect.md) 。 <span>com) 。 
5.  若要將小組引入組織，請將 [TeamsUpgradePolicy](/powershell/module/skype/grant-csteamsupgradepolicy) 的租使用者型原則，設定為 SfBWithTeamsCollab 或其中一個其他 SfB 模式 (SfBOnly 或 SfBWithTeamsCollabAndMeetings) 。 這一點很重要，是要確保從僅限內部部署的使用者移動到團隊的來電和聊天的路由。
6.  建議您在此點 (，但尚未必要，直到) 為其他樹系 (AcquiredCompany [啟用 AAD](cloud-consolidation-aad-connect.md) 連線為止。 <span>com) 。 假設這兩個樹系中皆已啟用 AAD Connect，則該組織看 **[起來像是](#figure-a)** 一些組織 emc 的常見起點。 
7.  針對其他內部部署所主控的任何 SIP 網域 (在此情況下，AcquiredCompany。 <span>com) ，使用 PowerShell 中的 [商務用 Skype Online 停用這些 SIP 網域](/powershell/module/skype/disable-csonlinesipdomain) `Disable-CsOnlineSipDomain` 。  (此為2018年12月為止的新功能。 ) 
8.  [設定 OriginalCompany 的商務用 Skype 混合](configure-federation-with-skype-for-business-online.md) 。 <span>com (仍已啟用線上 SIP 網域) 的一個部署。
9.  在混合部署中 (OriginalCompany。 <span>com) ，開始 [將使用者從商務用 skype 內部部署移至雲端](move-users-between-on-premises-and-cloud.md) (不論是小組還是不) ，讓帳戶位於商務用 Skype Online 中。 現在，組織看起來如 **[圖 B](#figure-b)** 所示。圖 A 的主要變更如下：
...
11. 若尚未完成，請啟用要成為混合式 (AcquiredCompany 之 [下一個樹](cloud-consolidation-aad-connect.md) 系的 AAD 連線。 <span>com) 。 此時，組織看起來如 **[圖 C](#figure-c)** 所示。這可能是部分組織的另一個常見起點。 
...
13. 如果您使用的是封閉式同盟，您必須將 microsoftonline.com) 純線上租使用者以外的任何 SIP (網域新增 \* 為 **相同** Microsoft 365 或 Office 365 中的允許網域。 請注意，您可能需要一段時間，變更才會生效，而且在初期執行這項動作時，我們建議您在移至步驟14之前做好這項工作。
...
- 這兩個組織都透過 AAD Connect 進行同步處理，因此 AAD 現在具有這兩個內部部署的所有使用者。
- 所有位於內部部署的使用者。  
- *尚未設定* 商務用 Skype 混合式。
- 如果兩個部署中的使用者都使用團隊，他們將無法彼此同盟 (或任何組織) ，也不會與任何商務用 Skype 使用者進行交互操作。 在此階段中，Microsoft 建議您僅針對通道使用團隊。<br><br>
    ![圖](../media/cloudconsolidationfiga.png)

##### <a name="figure-b"></a>圖 B：

- AcquiredCompany。 <span>com 是一種 [已停用](/powershell/module/skype/disable-csonlinesipdomain) 的線上 SIP 網域。 所有使用者都是內部部署。 如果他們使用的小組沒有同盟或互通性。 在此階段中，Microsoft 建議您僅針對通道使用團隊。
- 已對其中一個內部部署組織啟用商務用 Skype 混合。
- 混合組織中的某些使用者已移至雲端 (使用者 A，如紫色陰影) 所示。 這些使用者可以是商務用 Skype Online 使用者或小組只有具有完整互通性及同盟支援的使用者。<br><br>
...
- 如果 AcquiredCompany。 <span>com 先前未同步處理至 AAD，若要從這裡繼續，必須立即同步處理。 不過，它尚未混合 (共用 SIP 位址空間) ，直到組織準備好移至混合式，純內部部署組織 (AcquiredCompany.com) 的線上 SIP 網域仍保持停用，讓線上團隊使用者可以與內部部署使用者通訊。<br><br>
...
- Federates 與一或多個其他純內部部署組織的混合式組織，其中任何一個同步處理到 AAD。 這類組織類似于 **圖 E** 中的假想組織，如下所示。
    ...
    - 這些組織應執行下列其中一項操作：
        ...
        - 若要在完成混合式組織之前，將任何其他商務用 Skype 樹系同步處理到 AAD，組織必須執行步驟 7 (停用任何其他內部部署商務用 Skype 部署中的所有線上 SIP 網域，以同步處理至 AAD) ，然後啟用 AAD 連線，然後繼續進行步驟 10 (解除委任原始混合部署) 的授權。       
                **圖 E**<br>
                ![圖 E 圖表](../media/cloudconsolidationfige.png)
- 單純的商務用 Skype Online 組織 (，其可能也可能不會使用) 與個別內部部署商務用 Skype 組織的 federates 團隊。 一旦此組織停用內部部署組織的線上 SIP 網域，並為內部部署商務用 Skype 組織啟用 AAD 連線，它就會類似于 **[圖 C](#figure-c)** 中已完成步驟1-11 的假想組織。

## <a name="limitations"></a>限制

- 最多隻能有一個 Microsoft 365 或 Office 365 組織參與。 不支援在具有多個組織之案例中進行合併。
- 只有一個內部部署商務用 Skype 樹系可以在混合模式中 (共用 SIP 位址空間一次) 。 其他所有的內部部署商務用 Skype 樹系都必須保持在內部部署中，而且應與對方和 Microsoft 365 或 Office 365 組織相互同盟。
...
- 如果您的環境中有一個以上的商務用 Skype Server 2019 部署，則只有其中一個部署可以設定為使用組織自動語音應答，因為該功能需要商務用 Skype Server 混合式設定。 
- 您可以調整某些先前步驟的順序。 必須滿足的主要需求是，如果全部都為 true：
    - 一個以上的內部部署商務用 Skype 樹系同步處理至單一 AAD 租使用者
    - 使用一個內部部署樹系啟用分割網域
    - 混合式組織中至少有一位使用者已遷移至雲端<br>   接著，您 *必須* 停用任何其他內部部署商務用 Skype 樹系的所有其他線上 SIP 網域。 否則，混合式組織中的線上使用者與其他組織中的內部部署使用者之間的同盟將會以單一方向中斷。

## <a name="implications"></a>影響

- 由於上述高級呼叫功能的支援限制，如上述所述， **組織應該將這些非對稱狀態視為遷移的一部分，而不是將其視為穩定狀態**。  
- 具有多個內部部署商務用 Skype 部署的組織，通常會從可以完全遷移至雲端的部署開始，以繼續整合。 在某些情況下，在某些情況下，可能會有一些使用者群組的 holdouts，而這些使用者群組尚未可行，無法移至小組。 當此考慮涉及多個商務用 Skype 樹系的案例時，請從其他沒有這些限制的樹系開始進行遷移（若有的話）。
...
- 當您為使用者指派 TeamsOnly 模式時，任何其他使用者的所有交談及通話都會在該使用者的團隊用戶端中土地。 
- 如果使用商務用 Skype 內部部署的使用者主要使用商務用 Skype 用戶端，而不是小組，請考慮設定 TeamsUpgradePolicy，讓路由傳送至這些內部部署使用者，而不是小組的商務用 Skype。 在內部部署使用者必須使用 SfB 模式，而不是使用預設) 的 (孤島時，才能正確地路由傳送 TeamsOnly 的使用者與已在內部部署商務用 Skype 的使用者之間的交談及通話。 
