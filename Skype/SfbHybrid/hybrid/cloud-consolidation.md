...
- Federates 與一或多個其他純內部部署組織的混合式組織，其中任何一個同步處理到 AAD。 這類組織類似于**圖 E**中的假想組織，如下所示。
    ...
    - 這些組織應執行下列其中一項操作：
        ...
        - 若要在完成混合式組織之前，將任何其他商務用 Skype 樹系同步處理到 AAD，組織必須執行步驟7（停用任何其他內部部署商務用 Skype 部署中的所有線上 SIP 網域，以同步處理至 AAD），然後啟用 AAD 連線，只需繼續步驟10（解除委任原始混合部署）。       
                **圖 E**<br>
                ![圖 E 圖表](../media/cloudconsolidationfige.png)
- Federates 與個別內部部署商務用 Skype 組織搭配使用的單純商務用 Skype Online 組織（可能會使用小組）。 一旦此組織停用內部部署組織的線上 SIP 網域，並為內部部署商務用 Skype 組織啟用 AAD 連線，它就會類似于**[圖 C](#figure-c)** 中已完成步驟1-11 的假想組織。
...
- 已知此非對稱體驗，在內部部署使用者與不在內部部署目錄中的雲端使用者之間的跨部署案例中，對呼叫功能的官方支援，只限于對等。 
    ...
    - 在上**[圖](#figure-d)** 中，使用者 E 是內部部署的，而且使用者 A、B 或 C 的呼叫只會支援對等。 （呼叫使用者 D 不會有支援限制）。 不過，在內部部署使用者 E 移至雲端後，此限制將不再適用。
...
- 如果使用商務用 Skype 內部部署的使用者主要使用商務用 Skype 用戶端，而不是小組，請考慮設定 TeamsUpgradePolicy，讓路由傳送至這些內部部署使用者，而不是小組的商務用 Skype。 在內部部署使用者必須使用 SfB 模式，而不是使用孤島（預設值）時，才能正確地將交談和通話路由傳送至 TeamsOnly 的使用者，以及仍在內部使用商務用 Skype 的使用者。 
