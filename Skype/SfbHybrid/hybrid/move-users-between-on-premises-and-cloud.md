...
- Adm_Skype4B_Online
ms.custom: ''
description: 摘要：在內部部署中為混合式啟用商務用 Skype Server 的內部部署，您可以在內部部署環境與雲端之間移動使用者 (是否為 Microsoft 團隊或商務用 Skype Online) 。
ms.openlocfilehash: b4b354a6a43ab58740a053f86d9b7da1faaeb0da
ms.sourcegitcommit: 01087be29daa3abce7d3b03a55ba5ef8db4ca161
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 03/23/2021
ms.locfileid: "51110669"
---
...
- 其組織的現有會議會遷移至線上：如果使用者直接移至 TeamsOnly (請參閱下列) 、會議會轉換為小組會議，否則會議會維持商務用 Skype，但是會進行遷移，以供線上（而非內部部署）。  會議移轉會同步發生，並在移動使用者之後約 90 分鐘開始。  若要判斷會議移轉狀態，可以使用 [Get-csMeetingMigrationStatus](../../SfbOnline/audio-conferencing-in-office-365/setting-up-the-meeting-migration-service-mms.md#managing-mms)。 請注意，不會移動會議之前上傳的任何內容。

若要在內部部署與雲端之間移動使用者 (是否為小組或商務用 Skype Online) ，請使用 Move-CsUser 指令程式或商務用 Skype 系統管理員控制台，這兩者都是內部部署工具。 這些工具支援三種不同的移動路徑：

- [從商務用 Skype Server (內部部署) 至商務用 Skype Online](move-users-from-on-premises-to-skype-for-business-online.md)。
- [從商務用 Skype Server (內部部署) 直接向小組成員](move-users-from-on-premises-to-teams.md) (也會將其移至商務用 skype Online) 。  在商務用 Skype Server 2019 中可使用 [僅限從內部自行移至小組] 選項，以及商務用 Skype Server 2015 的累計更新8。 使用舊版商務用 Skype Server 的組織可以一開始將使用者移至商務用 Skype Online，然後在使用者上線時為他們套用 TeamsOnly 模式，便能將使用者移至 TeamsOnly。
- [從線上 (團隊是否僅限) ，或內部部署](move-users-from-the-cloud-to-on-premises.md)。
