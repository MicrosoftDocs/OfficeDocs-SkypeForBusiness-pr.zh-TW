...
在共存狀態中部署商務用 Skype Server 2019 之前，您必須確認已設定並啟動舊版服務。 在部署商務用 Skype Server 2019 試驗集區之前，請務必先識別舊環境中存在的重要服務和功能。 在使用舊版 XMPP 部署的共存狀態中部署 Microsoft 商務用 Skype Server 2019 XMPP 之前，您必須確認舊版 XMPP 服務已設定並啟動，並識別舊版的 XMPP 設定所支援的同盟合作夥伴。 驗證舊版部署需要下列各項：
...
6. 選取 [Standard Edition Server] 或 [Enterprise Edition 前端集區]。 決定是否已為下列的媒體設定 Edge 集區的**關聯**性。 
