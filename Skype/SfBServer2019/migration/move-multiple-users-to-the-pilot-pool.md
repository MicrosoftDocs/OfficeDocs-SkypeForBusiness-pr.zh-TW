...
2. 在命令列中, 輸入下列內容, 並將 [ **** **User1** ] 和 [使用者 2] 替換為您想要移動的特定使用者名稱, 然後以目標池的名稱取代**pool_FQDN** 。 在這個範例中, 我們會將使用者 Hao 唐和她約旦。 
...
4. **註冊機構池**標識現在應該指向您在上一個步驟中指定為**pool_FQDN**的池。 此身分識別的狀態會確認使用者已順利移動。 重複步驟, 確認已**** 移動到您的操作者。 
