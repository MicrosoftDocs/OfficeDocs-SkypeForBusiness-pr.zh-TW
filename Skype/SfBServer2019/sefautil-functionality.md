...
SEFAUtil （次要分機功能啟用）可讓商務用 Skype 伺服器管理員和支援人員的代理商代表商務用 Skype 伺服器使用者設定代理人震鈴、來電轉接和群組呼叫收取設定。 這個工具也可讓系統管理員查詢針對特定使用者所發佈的呼叫路由設定。 在您安裝商務用 Skype Server 2019 年7月累計更新後，下列目前只能透過 SEFAUtil 進行管理的功能也會透過 PowerShell: 進行管理。
...
此 Cmdlet 會停用使用者的來電轉接設定（這裡會顯示兩個不同的參數範例）。
...
此 Cmdlet 會修改 SimulRing 設定（此外，有兩個參數範例，一個用於無人接聽語音信箱，第二個則是不應答）。
