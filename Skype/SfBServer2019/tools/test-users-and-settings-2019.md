...
目標 Fqdn： atl-cs-001.litwareinc.com 結果：失敗延遲：00:00:00 錯誤訊息：此電腦沒有任何指派的憑證。 診斷：您可以針對此失敗，存取更詳細的資訊，而不只是此處顯示的錯誤訊息。 若要以 HTML 格式存取此資訊，請使用類似這個的命令，將儲存在可變 RegistrationTest 中的資訊儲存為 HTML 檔案：
