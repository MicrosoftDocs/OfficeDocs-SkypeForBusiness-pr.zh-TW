...
客戶鼓勵客戶設定暫時的變通方法，以滿足將自動語音應答來電轉接至外部 PSTN 號碼或 RGS 實例的需求。

在品質保證期間，發現「轉接至 PSTN 號碼」功能的問題，將不會及時修復，讓客戶開始從 Exchange UMO 服務開始遷移，在2月28日2020。 作為解決方法，系統管理員可以使用作用中的呼叫轉寄設定，將自動語音應答來電者轉接至內部部署虛擬使用者，以進行所需的 PSTN 電話號碼或 RGS 電話號碼。 預期的經驗如下：

- 系統管理員不需要授權虛擬使用者，因為這是一種替代解決方案。
- 管理員可以將所需的號碼指派給虛擬使用者或使用 SBC 數位操作功能，以操控 PSTN 接收器所看到的來電者識別碼。
- PSTN 來電者在來電轉接期間不會經歷任何延遲，而且會在傳輸成功之後繼續看到自動語音應答的來電者識別碼。

#### <a name="shared-mailbox-is-still-accessible"></a>仍然可以存取共用信箱

在遷移至 CVM 後，使用 Exchange UM Online 設定的共用信箱繼續接收郵件，而且可透過 Outlook 存取使用者。 不過，當您將這些信箱的存取權轉移至 CVM 後，就無法使用此存取權變更這些信箱的問候語郵件。 具有用來捕獲自動語音應答來電者的共用信箱的客戶應該會利用自動語音應答及通話佇列共用信箱功能（ETA 10 月2019）。
...
在遷移至 CVM 後，按一下用戶端的 [**設定語音信箱**] 將繼續進行商務用 Skype Server 2015/2013 客戶前往 Office Web ACCESS （OWA）入口網站頁面。 已從 OWA 的 [語音信箱] 索引標籤中移除所有設定，而且橫幅會以重新導向連結顯示，以將使用者帶至 CVM 使用者設定入口網站。
