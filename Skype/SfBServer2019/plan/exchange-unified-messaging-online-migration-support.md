...
使用 Exchange 整合通訊線上服務之下列任何功能的客戶都會受到影響：
...
|需要系統管理員參與 & 客戶投資的客戶<br><br>要遷移的功能：<ul><li>語音 信箱<li>自動語音應答<li>通話佇列<li>傳真整合</ul>| 2020年2月  | 範例: <br><ul><li>ExchUMO 服務是由協力廠商 PBX 所使用<li>具有 PSTN 訂閱者存取需求的客戶<li>SFB 2010 上的客戶（不支援）<li>傳真整合</ul> |
...
2.  **建立商務用 Skype 混合式拓撲**
    ...
    > [!Note]
    > 您不需要將使用者遷移至線上語音信箱服務遷移。 不過，若要讓內部部署使用者利用電話語音語音信箱服務，必須建立混合式拓撲。

3. **規劃您的自動語音應答遷移**
    
    系統管理員可以隨時開始將其自動語音應答從 ExchUMO 遷移至雲端自動語音應答。 如需詳細資訊，請參閱[設定雲端自動](https://docs.microsoft.com/microsoftteams/create-a-phone-system-auto-attendant)語音應答。 Microsoft 繼續提供額外的自動語音應答功能，讓客戶可能會考慮其遷移所需的功能，管理員應該據此評估功能集和遷移其自動語音應答實例。 如需功能性清單比較，請參閱[ExchUMO 和 Azure 雲端架構服務功能矩陣](#exchumo-and-azure-cloud-based-services-feature-matrix)。
...
6. **考慮在規劃的遷移中使用的退出**
    ...
    - 慣用日期（星期二）：每週二執行遷移聲波。 請選取不超過12/3/2019 的星期二的日期。
...
6. 測試新的自動語音應答。 若要測試，請內部撥打或指派測試電話號碼。  
...
| Vm | 使用者功能 | 錄製個人問候語（已驗證的裝置） |  | Y | Y    |
| Vm | 使用者功能 | 記錄個人問候語（PSTN）-在電話上播放 |  | N | Y    |
...
| Vm | 使用者功能 | 在電話上播放（驗證）-播放郵件 | 在我的已驗證裝置上呼叫我  | Y | Y    |
...
| 機 管 局 | 服務功能 | 內部轉接至 PSTN 號碼（已 RNL）  |  | Y | Y    |
...
**感謝您使用寶貴！如果您沒有看到上述的記錄，這是因為音訊品質不夠清晰，無法 transcribe。**

當您已遷移使用者之後測試語音信箱功能時，請務必考慮下列案例：

- 驗證組織中所有端點類型的語音信箱存取：應用程式和 IP 電話。 
...
|轉錄 | 增強<br>CVM 方式的精確度率和支援的語言遠遠高於 ExchUMO。 |
|使用者設定 | 新的經驗<br>使用者可以從使用者設定入口網站（USP）變更其喜好設定。 使用者可以從其語音信箱電子郵件中的超連結，或其 SFB 用戶端上的使用者設定按鈕，存取其 USP;https://aka.ms/vmsettings.   
 |功能| 如需詳細資訊，請參閱功能設定比較。 |
...
**在遷移後停用訂閱者存取，以避免問候語不一致**訂閱者存取可能持續用於您的租使用者，直到服務完全淘汰為止，即使所有的使用者都已遷移至雲端語音信箱也是一樣。 若要避免使用者混淆和不一致的體驗，請停用訂閱者存取，因為在遷移後問候語會變更。 若要這麼做，請使用 Get-CsExUmContact | 移除每個訂戶存取行的 EXUM 連絡人。{$_.IsSubscriberAccess-eq $true} |Remove-New-csexumcontact 

**自動語音應答來電轉接至 PSTN**客戶鼓勵客戶設定暫時的解決方法，以滿足將自動語音應答來電轉接至外部 PSTN 號碼或 RGS 實例的需求。 
 
在品質保證期間，已使用轉接至 PSTN 號碼功能識別出問題，當客戶開始從 Exchange UMO 2020 服務開始遷移時，將不會及時修復此問題。 作為解決方法，系統管理員可以使用作用中的呼叫轉寄設定，將自動語音應答來電者轉接至內部部署虛擬使用者，以進行所需的 PSTN 電話號碼或 RGS 電話號碼。 
 
預期經驗
- 管理員不需要授權虛擬使用者，因為這是一種解決方法。 
- 管理員可以操作來電者識別碼，PSTN 接收器會將所需的號碼指派給虛擬使用者，或使用 SBC 數位操作功能來進行查看。
- PSTN 來電者在來電轉接期間未經歷任何延遲，而且在傳輸成功後，系統會繼續看到自動語音應答的來電者識別碼  

**共用信箱：** 在遷移至 CVM 後，使用 Exchange UM Online 設定的共用信箱會繼續接收郵件，並可透過 Outlook 繼續供使用者存取。 不過，當您將這些信箱的存取權轉移至 CVM 後，就無法使用此存取權變更這些信箱的問候語郵件。 具有用來捕獲自動語音應答來電者的共用信箱的客戶應該會利用自動語音應答及通話佇列共用信箱功能（ETA 10 月2019）。
  
**升級至 SFB 用戶端上的小組橫幅：** CVM 服務是以 Microsoft 團隊基礎結構為基礎;來自商務用 Skype 用戶端的呼叫可能會導致用戶端上顯示資訊橫幅，但讀取：「不是使用商務用 Skype。 若要取得更豐富的體驗，請切換至 [小組] 或 [啟動 Skype 會議]。
請務必將使用者的商務用 Skype 用戶端更新為最新的 C2R 用戶端更新，以避免出現這種旗標。
  
**設定語音信箱會帶您前往 OWA：** 在遷移至 CVM 後，按一下用戶端上的「設定語音信箱」會繼續取得商務用 Skype Server 2015/2013 客戶進入 Office Web Access （OWA）入口網站頁面。 已從 OWA 的 [語音信箱] 索引標籤中移除所有設定，並會顯示橫幅，以將使用者帶至 CVM 使用者設定入口網站。
 
**變更問候語行動存取：** CVM 中不支援 PSTN 訂戶存取。 針對需要以遠端方式變更其問候語的使用者，會將「變更您的問候語」功能表選項新增至行動用戶端的語音信箱 IVR 服務。 使用者可以在行動用戶端的撥號墊上按下並按住 "1" 鍵來呼叫此服務。
