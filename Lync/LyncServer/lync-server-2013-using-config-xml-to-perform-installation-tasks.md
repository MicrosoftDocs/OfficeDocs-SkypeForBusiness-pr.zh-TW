...
# <a name="using-configxml-to-perform-installation-tasks-in-lync-server-2013"></a>使用 Config.xml 執行 Lync Server 2013 中安裝工作
...
我們建議您使用 Config.xml 檔案來設定 Lync 2013 無訊息安裝。

根據預設，會儲存在核心產品資料夾中的 Config.xml 檔案 (例如，\\產品。WW)，會指示安裝程式以安裝該產品。 例如，下列資料夾中的 Config.xml 檔案會安裝 Lync 2013:

  - \\\\伺服器\\共用\\Lync15\\Lync.WW \\Config.xml

最常使用 Lync 2013 安裝的 Config.xml 元素詳列於下表。
...
<td><p>指定在安裝期間如何處理特定的產品功能。 若要防止安裝 Business Connectivity Services，其中包含干擾 Outlook 2010 的共用的元件使用下列屬性：</p>
<ul>
...
<ul>
<li><p>設定 Id =&quot;名稱&quot;（Windows Installer 屬性的名稱）</p></li>
<li><p>值 =&quot;值&quot;（若要指定給屬性的值）</p></li>
</ul></td>
</tr>
<tr class="even">
...
下列範例會顯示 Lync 2013 的一般無訊息安裝的 Config.xml 檔案。
...
使用 Config.xml 檔案來執行 Office 安裝及維護工作的詳細的資訊位於<https://go.microsoft.com/fwlink/p/?linkid=267514>。
...
3.  以您要使用的無訊息選項來修改元素項目。 請確定您移除註解會讓分隔字元，"\<\!-」 和 「-\>」。 例如，使用下列語法：
