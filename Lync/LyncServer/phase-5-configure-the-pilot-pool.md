---
title: 階段5：設定試生產池
ms.reviewer: ''
ms.author: kenwith
author: kenwith
TOCTitle: 'Phase 5: Configure the pilot pool'
ms:assetid: 190faee2-e2d6-45cb-8315-f39e477df418
ms:mtpsurl: https://technet.microsoft.com/en-us/library/JJ204717(v=OCS.15)
ms:contentKeyID: 48183532
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: e5ffa799a07cdb5b1583ba96708dc79599d52df5
ms.sourcegitcommit: bb53f131fabb03a66f0d000f8ba668fbad190778
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/11/2019
ms.locfileid: "40975940"
---
<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="http://msdn.microsoft.com/en-us/">

<div data-asp="http://msdn2.microsoft.com/asp">

# <a name="phase-5-configure-the-pilot-pool"></a>階段5：設定試生產池

</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**主題上次修改日期：** 2012-10-19_

現在已建立試驗池，且傳統的部署資訊已經與 Lync Server 2013 合併，本節將說明必須對試驗池進行的幾項設定。

<div>

## <a name="in-this-section"></a>本節內容

  - [將試驗集區連線到舊版 Edge Server](connect-pilot-pool-to-legacy-edge-servers_1.md)

  - [授權連線到 Office 通訊伺服器 2007 R2 Edge 伺服器](authorize-connection-to-office-communications-server-2007-r2-edge-server.md)

  - [驗證組態設定](verify-configuration-settings_1.md)

  - [設定 XMPP 閘道的存取原則和憑證](configure-xmpp-gateway-access-policies-and-certificates_1.md)

</div>

</div>

<span> </span>

</div>

</div>

</div>

