---
title: Lync Server 2013：通話許可控制的基礎結構需求
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
TOCTitle: Infrastructure requirements for call admission control
ms:assetid: 52be8826-5796-407b-92fc-32f29a6a933c
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Gg398346(v=OCS.15)
ms:contentKeyID: 48184157
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: 6798d6e6ba10fefe761ab830dc8f173032dce805
ms.sourcegitcommit: bb53f131fabb03a66f0d000f8ba668fbad190778
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/11/2019
ms.locfileid: "40982742"
---
<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="http://msdn.microsoft.com/en-us/">

<div data-asp="http://msdn2.microsoft.com/asp">

# <a name="infrastructure-requirements-for-call-admission-control-in-lync-server-2013"></a>Lync Server 2013 中的通話許可控制的基礎結構需求

</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**主題上次修改日期：** 2012-08-21_

不需要額外的基礎結構需求（例如特殊網路路由器）。 部署 Lync Server 2013 會自動安裝用於強制執行呼叫許可控制（CAC）原則的頻寬原則服務。 [通話許可控制] 只適用于執行 Lync 的用戶端。

</div>

<span> </span>

</div>

</div>

</div>

