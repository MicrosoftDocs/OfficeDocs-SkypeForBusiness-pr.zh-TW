...
8.  若要指定來電者等候專員接聽通話之前的最長保留期間，請選取 **[啟用佇列逾時]** 核取方塊，然後執行下列動作：
    ...
    <!-- end list -->
      ...
      - 若要將通話轉寄給另一個電話號碼，請按一下 [**轉寄給電話號碼**]，然後在 [ **sip 位址**] 欄位中輸入電話號碼，格式為 [sip： \< 號碼 \> @ \< domainname \> (例如，sip:+14255550121@contoso.com) 。
    
      - 若要將呼叫轉寄給另一位使用者，請按一下 [**轉寄給 SIP 位址**]，然後在 [ **sip 位址**] 欄位中，輸入使用者的 URI，格式為 SIP： \< username domainname 的格式 \> @ \< \> 。
