...
會議診斷報告提供所有會議會話成功和失敗的相關資訊。 請注意，Microsoft Lync Server 會區分不同類型的失敗：
...
您可從 [監控報告] 首頁存取會議診斷報告。 您可以按一下下列其中一個度量，[以存取 Lync Server 2013 中的失敗發佈報告](lync-server-2013-failure-distribution-report.md)：
...
會議診斷報告包含一系列的圖形。 圖表中顯示的每一欄實際上都是超連結。 如果您按一下某個欄，就會向下切入[Lync Server 2013 的 [失敗發佈] 報告](lync-server-2013-failure-distribution-report.md)，以瞭解該時段以及該會議類型。
...
### <a name="conference-diagnostic-report-filters"></a>會議診斷報表篩選
...
### <a name="conference-diagnostic-report-metrics"></a>會議診斷報告度量單位
...
<td><p>否</p></td>
<td><p>已完成且重要問題的會議百分比。 將成功率除以總會話數來計算。</p></td>
</tr>
<tr class="odd">
