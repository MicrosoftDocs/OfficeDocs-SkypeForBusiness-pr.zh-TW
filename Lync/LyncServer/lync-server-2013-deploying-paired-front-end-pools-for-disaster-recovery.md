...
---
# <a name="deploying-paired-front-end-pools-for-disaster-recovery-in-lync-server-2013"></a>在 Lync Server 2013 中為嚴重損壞修復部署成對的前端集區
...
9.  從 Lync Server 管理命令介面命令提示字元中，執行下列命令：
    ```powershell
...
> [!NOTE]  
> [！注意事項] 的 <STRONG>自動容錯移轉和容錯回復</STRONG> ] 選項和拓撲產生器中相關聯的時間間隔，只適用于 Lync Server 2010 中引進的語音恢復功能。 選取此選項不表示會自動使用本文件中討論的集區容錯移轉。 集區容錯移轉和容錯回復一律需要管理員以手動方式分別呼叫容錯移轉和容錯回復 Cmdlet。
