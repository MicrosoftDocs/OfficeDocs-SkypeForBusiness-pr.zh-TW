---
title: 為 Lync Server 2013 準備 Active Directory
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
TOCTitle: Prepare Active Directory for Lync Server 2013
ms:assetid: d0978eb6-d842-40e9-b475-73197cc34e08
ms:mtpsurl: https://technet.microsoft.com/en-us/library/JJ205265(v=OCS.15)
ms:contentKeyID: 48185413
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: e36b5260c71c339ae22854b50b33311111aa09d6
ms.sourcegitcommit: bb53f131fabb03a66f0d000f8ba668fbad190778
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/11/2019
ms.locfileid: "40977563"
---
<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="http://msdn.microsoft.com/en-us/">

<div data-asp="http://msdn2.microsoft.com/asp">

# <a name="prepare-active-directory-for-lync-server-2013"></a>為 Lync Server 2013 準備 Active Directory

</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**主題上次修改日期：** 2012-10-01_

在使用 Office 通訊伺服器 2007 R2 的共存狀態中部署 Lync Server 2013 之前，您必須先執行一些其他 Active Directory 工作，才能設定 Lync Server 2013 的架構、林及網域。 架構延伸會新增 Lync Server 所需的 Active Directory 類別和屬性。 如需其他資訊，請參閱[準備 Lync Server 2013 的 Active Directory 網域服務](lync-server-2013-preparing-active-directory-domain-services.md)主題。

**為 Lync Server 2013 準備 Active Directory**

1.  在 Lync Server 2013 前端伺服器上，執行 Lync Server 2013 設定。

2.  選取 [**準備 Active Directory** ]
    
    ![Lync server 2013 部署嚮導，歡迎頁面](images/JJ205265.5f88ae18-9c3c-42ea-a91a-836ecf5d515f(OCS.15).jpg "Lync Server 2013 部署嚮導，歡迎頁面")

3.  完成步驟1到5。
    
    ![部署嚮導、Active Directory Prearation](images/JJ205265.eddd9e94-fa70-453f-8810-b99a2bf0844a(OCS.15).jpg "部署嚮導、active directory Prearation")

</div>

<span> </span>

</div>

</div>

</div>

