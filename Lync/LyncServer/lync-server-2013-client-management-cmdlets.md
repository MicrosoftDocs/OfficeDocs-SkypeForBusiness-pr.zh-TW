...
用戶端管理主要是決定允許哪些用戶端應用程式（例如 Microsoft Lync 2013）登入 Lync Server 2013，並決定這些用戶端應用程式在登入後可使用的功能。
...
大多數適用于用戶端管理的管理工作，都可以從 Lync Server 2013 [控制台] 執行。 您可以使用 Lync Server 管理命令介面或腳本中的 Cmdlet 來執行這些相同的工作。 您可以使用腳本來自動執行特定工作。 以下是直接與用戶端管理相關的 Cmdlet 清單：
