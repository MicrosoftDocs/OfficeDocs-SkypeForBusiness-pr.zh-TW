...
*頻寬原則* 定義即時音訊和影片形式的頻寬使用量限制。 頻寬原則會套用至 *頻寬原則設定檔*，可將其套用至多個網站以取得通話許可控制。

如需在您的 CAC 部署中應該設定什麼頻寬限制的指導方針，請參閱規劃檔中的 [定義您的 Lync Server 2013 中的通話許可控制需求](lync-server-2013-defining-your-requirements-for-call-admission-control.md) 。

如需使用頻寬原則及原則設定檔的詳細資訊，請參閱 Lync Server 管理命令介面檔中的下列 Cmdlet：
...
下列程式中建立的範例原則，會設定整體音訊流量、個別音訊會話、整體影片流量和個別影片的限制。 例如，5Mb \_ 連結頻寬原則設定檔會設定下列限制：
...
## <a name="to-create-bandwidth-policy-profiles-by-using-lync-server-control-panel"></a>使用 Lync Server 控制台建立頻寬原則設定檔
