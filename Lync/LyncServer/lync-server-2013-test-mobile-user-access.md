...
<p>使用 Windows PowerShell 的遠端實例執行時，必須為使用者指派具有執行 Test-CsMcxConference Cmdlet 許可權的 RBAC 角色。 若要查看可使用此 Cmdlet 的所有 RBAC 角色清單，請從 Windows PowerShell prompt 中執行下列命令：</p>
...
3.  利用 Lync Server 功能（例如透過工作和撥出會議進行通話）。 Test-CsMcxConference Cmdlet 提供一種快速快捷的方式，可驗證使用者是否可以使用行動裝置加入和參加 Lync Server 會議。
...
若要執行這項檢查，您必須建立三個 Windows PowerShell 認證物件 (包含每個帳戶之帳戶名稱和密碼) 的物件。 接下來，您必須在呼叫 Test-CsMcxConference 時，包含這兩個帳戶的認證物件和 SIP 位址，如下列範例所示：
...
如果檢查成功，Test-CsMcxConference 將會報告成功的測試結果：
...
如果檢查失敗 Test-CsMcxConference 會報告失敗的測試結果。 此測試結果通常會附帶詳細的錯誤訊息和診斷。 例如：
...
如果 Test-CsMcxConference 失敗，您應該先驗證行動性服務是否正在執行且可以存取。 可以使用網頁瀏覽器來驗證 Lync Server 集區的行動服務 URL 是否可以存取。 例如，下列命令會驗證集區 atl-cs-001.litwareinc.com 的 URL：

`https://atl-cs-001.litwareinc.com/mcx/mcxservice.svc`

若可以存取行動服務，您應該確認測試使用者具備有效的 Lync 伺服器帳戶。 您可以使用類似下列的命令來取得帳戶資訊：

    Get-CsUser -Identity "sip:kenmyer@litwareinc.com" | Select-Object Enabled

如果 Enabled 屬性不等於 True 或如果命令失敗，則表示使用者沒有有效的 Lync 伺服器帳戶。您也應該確認每個使用者帳戶都已啟用行動性。 若要這麼做，請先決定指派給該帳戶的行動原則：
