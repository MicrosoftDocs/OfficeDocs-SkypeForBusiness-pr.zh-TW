...
# <a name="call-park-application-cmdlets-in-lync-server-2013"></a>在 Lync Server 2013 中通話駐留應用程式 Cmdlet
...
通話寄存應用程式可讓使用者保留通話，然後從其他電話取回該通話。 使用這些 Cmdlet 來設定通話駐留軌道式和通話駐留應用程式的設定。

<div>

## <a name="call-park-application-cmdlets"></a>通話駐留應用程式 Cmdlet

下列 Cmdlet 可用來管理通話駐留應用程式。
