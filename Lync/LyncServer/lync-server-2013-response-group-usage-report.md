...
# <a name="response-group-usage-report-in-lync-server-2013"></a>Lync Server 2013 中的回應群組使用量報告
...
您可以從 [監視報告] 首頁存取回應群組使用方式報告。 您可以按一下向下切入[回應群組通話清單報告在 Lync Server 2013 中](lync-server-2013-response-group-call-list-report.md)任何下列計量：
...
> [!WARNING]  
> 回應群組工作流程基本上是一組的指示，以決定 Lync Server 用途當使用者撥特定的電話號碼。 為此，每個工作流程是唯一相關聯的電話號碼。 如果某人撥打該號碼，工作流程會決定如何處理通話。 例如，工作流程可能會造成呼叫路由傳送至一系列的互動語音回應 (IVR) 問題的提示來電者輸入其他資訊 (「 按下 1 硬體支援。 軟體支援請按 2。 或者，工作流程可能會導致可放置在佇列中，與來電者直到代理可接聽來電，將放入保留通話。 代理是否可接聽電話的狀態，也是由工作流程指定的：工作流程可用來設定營業時間 (一星期的哪幾天和哪些時段會有代理接聽電話) 與假日 (代理無法接聽電話的日子)。 撥打的電話號碼所屬的回應群組應用程式，任何時候您基本上呼叫回應群組工作流程。
...
若要檢閱特定工作流程的使用方式統計資料，請在 [工作流程 URI] 方塊中輸入工作流程 URI。 當然，如上所述，工作流程 URI (與工作流程相關聯的 SIP 位址) 不會出現在報告上。 這表示您需要尋找其他方法來決定工作流程的 URI。 若要這麼做的一種方式是使用 Windows PowerShell 和 Lync Server 管理命令介面。 例如，此命令會傳回您所有回應群組工作流程的 URI：
