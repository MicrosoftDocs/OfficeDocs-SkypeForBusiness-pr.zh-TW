...
必須登入和登出群組的代理（與 Lync 伺服器登入或登出不同）稱為「 *正式代理程式*」。 正式代理必須先登入此群組，才可以接聽路由傳送到此群組的電話。 對於以兼職身分接聽群組來電的代理而言，這可能相當實用。 正式代理程式會按一下 Lync 2013 中的功能表項目，以開啟 Windows Internet Explorer Internet browser 及顯示網頁主控台，以登入和登出其群組。
