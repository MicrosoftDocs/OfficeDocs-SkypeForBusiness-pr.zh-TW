...
> [!IMPORTANT]  
> 所有的使用者管理（包括使用者在內部部署和 UNRESOLVED_TOKEN_VAL （skypeforbusiness）之間移動）都必須使用最新安裝的系統管理工具版本執行。 系統管理工具必須安裝在另一部伺服器上，且該伺服器具有現有內部部署部署和網際網路的 connect 存取權。 若要將內部部署的使用者移至 UNRESOLVED_TOKEN_VAL （skype16_online）的<A href="https://docs.microsoft.com/powershell/module/skype/Move-CsUser">Move-CsUser</A>指令程式，必須從連接至內部部署的系統管理工具執行。
...
允許的網域清單包含已設定協力廠商完全限定功能變數名稱（FQDN）的網域。 這兩種情況有時候稱為「*允許的夥伴伺服器*」或「*直接同盟夥伴*」。 您應熟悉開放式同盟和封閉式同盟之間的差異，在內部部署中分別稱為「協力廠商*探索*」和「允許的協力廠商」*網域清單*」。
...
在建立混合部署的 DNS 記錄時，所有 Lync 外部 DNS 記錄都應該指向內部部署基礎結構。 如需有關必要 DNS 記錄的詳細資訊，請參閱[Lync Server 2013 的網域名稱系統（DNS）需求](lync-server-2013-domain-name-system-dns-requirements.md)。
...
<tr class="odd">
<td><p>DNS A Edge Web 會議服務 FQDN 的記錄，例如，webcon.contoso.com 解析為 Web 會議 Edge 外部 IP （s）</p></td>
...
<li><p>Lync 用戶端（來自內部部署 Lync Server 的 Lync Online 通訊）</p></li>
</ul></td>
</tr>
<tr class="even">
