---
title: Lync Server 2013：支援通話駐留的用戶端
ms.reviewer: ''
ms.author: v-lanac
author: lanachin
TOCTitle: Clients supported for Call Park
ms:assetid: c236d2ba-9d83-418c-9cbc-92541f115fb0
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Gg412958(v=OCS.15)
ms:contentKeyID: 48185320
ms.date: 07/23/2014
manager: serdars
mtps_version: v=OCS.15
ms.openlocfilehash: 46b9ac77a82cf8d833c3f06a8fe3c738e2501937
ms.sourcegitcommit: bb53f131fabb03a66f0d000f8ba668fbad190778
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/11/2019
ms.locfileid: "40980425"
---
<div data-xmlns="http://www.w3.org/1999/xhtml">

<div class="topic" data-xmlns="http://www.w3.org/1999/xhtml" data-msxsl="urn:schemas-microsoft-com:xslt" data-cs="http://msdn.microsoft.com/en-us/">

<div data-asp="http://msdn2.microsoft.com/asp">

# <a name="clients-supported-for-call-park-in-lync-server-2013"></a>Lync Server 2013 中支援通話駐留的用戶端

</div>

<div id="mainSection">

<div id="mainBody">

<span> </span>

_**主題上次修改日期：** 2012-09-13_

本節會識別可以用來寄存通話的用戶端，以及可用於檢索暫停通話的用戶端。

<div>

## <a name="clients-supported-for-parking-calls"></a>支援停用通話的用戶端

來自任何 IP、私人分支 exchange （PBX）、公用交換電話網絡（PSTN）或行動電話的呼叫都可以停用。

<div>


> [!NOTE]  
> 只有音訊通話可以停用。 無法停用立即訊息和會議。



</div>

下列用戶端可以使用通話駐留撥出電話：

  - Lync 2013

  - Lync 2010

  - Lync 2010 助理

  - Lync Phone Edition

<div>


> [!NOTE]  
> 行動電話無法使用通話駐留撥出電話。



</div>

</div>

<div>

## <a name="clients-supported-for-retrieving-calls"></a>支援的用戶端檢索通話

軌道範圍是設定為虛擬延伸的區塊（不含指派使用者或電話的延伸）。 當您將 [軌道式] 設定為 [虛擬延伸] 時，行動電話和 PSTN 手機無法取得停用的通話。

同盟使用者無法取得停用的通話。

下列用戶端可以檢索寄存在通話寄存上的通話：

  - Lync 2013

  - Lync 2010

  - Lync 2010 助理

  - Lync Phone Edition

  - IP 通用區域電話

  - 連線至 Lync Server 2013 基礎結構的非 IP 電話，包括通用的區域電話和私人分支 exchange （PBX）電話

</div>

</div>

<span> </span>

</div>

</div>

</div>

