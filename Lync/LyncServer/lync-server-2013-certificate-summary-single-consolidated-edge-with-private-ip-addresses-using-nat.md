...
從公用憑證授權單位（CA）要求指派給 Edge 伺服器外部介面的憑證。 下列文章所列的公用 Ca 在為整合通訊提供證書時已成功示範： [http://go.microsoft.com/fwlink/p/?linkid=3052\&kbid=929395](http://go.microsoft.com/fwlink/p/?linkid=3052%26kbid=929395)。 要求證書時，您可以使用 Lync Server 部署嚮導所產生的憑證申請，或是使用 Lync Server 管理命令介面 Cmdlet 或由公用 CA 提供的程式手動建立要求。 如需有關認證管理的 Lync Server Management 命令介面 Cmdlet 的詳細資料，請參閱在指派憑證時， [Lync server 2013 中的憑證和驗證 Cmdlet](https://docs.microsoft.com/powershell/module/skype/) ，該憑證會指派給存取邊緣服務介面、網路會議 Edge 服務介面，以及音訊/視頻驗證服務。 音訊/視頻驗證服務不應與 A/V 邊緣服務混淆，因為它不會使用憑證來加密音訊與視頻資料流程。 內部邊緣伺服器介面可以使用來自內部（到您的組織） CA 或來自公用 CA 的憑證的憑證。 內部介面憑證只使用 SN，而且不需要或使用 SAN 專案。
...
## <a name="certificates-required-for-single-consolidated-edge-with-private-ip-addresses-using-nat"></a>使用 NAT 將單一合併邊緣與私人 IP 位址一起所需的憑證
