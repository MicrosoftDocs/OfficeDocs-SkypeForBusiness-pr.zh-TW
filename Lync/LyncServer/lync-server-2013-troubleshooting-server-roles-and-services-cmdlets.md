...
疑難排解 Cmdlet 提供不同的方式來驗證 Microsoft Lync Server 2013 是否如預期運作。 例如，CsHealthMonitoringConfiguration Cmdlet 可讓您設定登錄器和 Director 集區的測試帳戶。 然後，您可以接著使用這些測試帳戶來確認使用者可以順利完成一般工作，例如登入系統、交換立即訊息，或打給位於公用交換電話網路 (PSTN) 的電話。
