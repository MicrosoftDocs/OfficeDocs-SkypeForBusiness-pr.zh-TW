...
# <a name="certificate-summary---scaled-consolidated-edge-dns-load-balancing-with-public-ip-addresses-in-lync-server-2013"></a>憑證摘要-調整式合併 edge、 DNS 負載平衡與 Lync Server 2013 中的公用 IP 位址
...
## <a name="scaled-consolidated-edge-using-dns-load-balancing-with-public-ip-addresses"></a>調整式合併 Edge，使用 DNS 負載平衡與公用 IP 位址
...
<td><p>若要部署與 AOL 的公用 IM 連線，憑證必須來自公用 CA，且必須具有伺服器 EKU 和用戶端 EKU。 此外，調整式 Edge server，憑證的私密金鑰必須可匯出私密金鑰及憑證及私密金鑰複製到每部 Edge Server。 系統會針對下列項目將憑證指派給外部 Edge 介面：</p>
<ul>
