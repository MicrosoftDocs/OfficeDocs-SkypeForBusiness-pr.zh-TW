...
使用 QoS，您可以排定延遲敏感性網路流量的優先順序 (例如語音或視音訊流) ，讓流量在較不敏感的流量前面 (例如下載新的應用程式 ，而額外下載第二秒並不) 。 QoS 會使用 Windows 群組原則物件和稱為埠型存取控制清單的路由功能，來識別並標記即時資料流中的所有封包，這項功能會指示您的網路提供語音、視像和螢幕共用自己的私人網路絡頻寬。

在理想情況下，您將在內部網路上執行 QoS，同時準備推出Teams，但您隨時可以執行。 如果您夠小，您可能不需要 QoS。

準備好時，請參閱在 (中) [QoS](QoS-in-Teams.md)Microsoft Teams。
