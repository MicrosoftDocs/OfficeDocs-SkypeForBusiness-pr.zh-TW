...
|離島|兩側|兩側|是|預設設定。 允許單一使用者並排評估兩個用戶端。 聊天和通話可在用戶端中土地，因此使用者必須一直執行這兩個用戶端。 若要避免令人混淆或 regressed 的商務用 Skype 體驗、外部（同盟）通訊、PSTN 語音服務及語音應用程式、Office 整合及其他幾個整合，都繼續由商務用 Skype 來處理。|
