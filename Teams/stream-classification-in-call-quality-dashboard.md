...
ms.locfileid: "61767196"
---
# <a name="stream-classification-in-call-quality-dashboard-cqd"></a>通話品質儀表板和 CQD (串流分類) 

 (和 Microsoft Teams 商務用 Skype Online (CQD) CQD Microsoft Teams 和 商務用 Skype Online 可讓您深入瞭解使用 Microsoft Teams 和 商務用 Skype 服務撥打的通話品質。 本主題提供有關媒體流品質分類的詳細資訊。 若要深入瞭解 CQD 以及如何設定，請參閱設定 [通話品質儀表板](turning-on-and-using-call-quality-dashboard.md)。
...
|步# |度量 |條件 |如果條件為 True，分類 |條件為 False 的分類 |如果公制無法使用，分類 |解釋 |
|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
|1|影片本地框架遺失百分比平均值|編解碼器不是 H264S</br>和</br>StreamDirection 為內入</br></br>如果 FrameLoss > 50%|_可憐_|_好_|_未分類_|顯示給使用者之影片畫面遺失的平均百分比。 平均值包括從網路損失中復原的畫面。 FrameLoss 僅適用于將輸入的非 H264S 資料流程分類。|
|2|視音訊畫面播放速率平均值|< 1|_可憐_|_好_|_未分類_|視音訊串流每秒收到的平均幀數，這是在會話期間所計算。 適用于所有外發資料流程，以及 H264S 的 StreamDirection。|
| |  | | | |  ||
