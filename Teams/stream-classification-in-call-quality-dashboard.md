...
CQD 中的資料流程會根據可用的主要品質度量單位的值，分類為 _良好_、 _較差_ 或未 _分類_ 。 在後續的資料表中，會顯示用於分類資料流程的度量值與條件。 CQD 的 "到期之處" 維度可供您瞭解哪個指標負責分類不 _佳_ 。 如需這些尺寸的詳細資訊，請參閱 [通話品質儀表板中提供的維度與量值](dimensions-and-measures-available-in-call-quality-dashboard.md)。
...
|衡量|案例|條件|簡要|
|:-----|:-----|:-----|:-----|
|音訊品質平均|[負載描述] 不是光澤的。|> 1。0|資料流程的平均網路平均觀點。 網路遺失和抖動的多少會影響收到的音訊品質。|
|往返行程|同時|> 500|平均往返行程網路傳播時間，以毫秒計算。 [RFC3550](https://tools.ietf.org/html/rfc3550)中提供的詳細資料。|
|資料包遺失率|同時|> 0。1|資料流程的平均資料包遺失率。|
|抖動|同時|> 30|資料流程的平均抖動（以毫秒為單位）。|
|隱藏平均樣本的比率|[負載描述] 不是光澤的。|> 0.07|音訊幀數的平均比率，由資料包遺失修複產生的隱藏範例，到音訊畫面總數目。|
||||
