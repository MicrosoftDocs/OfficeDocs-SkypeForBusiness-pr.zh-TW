...
在 CQD 中，資料流程在互動式連接建立 (ICE) 連線失敗，或未報告計算資料流程分類所需的所有度量時，都會標示為未_分類_。
...
- **低資料包利用率**（例如「短通話」案例）需要足夠的資料包利用率，才能計算重要資料流程度量單位。 若沒有這些指標，CQD 就無法將資料流程分類。
  - 常見的低資料包利用率情況是在出席者加入會議來收聽簡報者時，但永遠不會 (麥克風在大多數通話) 中靜音。 在這裡，入站至用戶端的音訊資料流程具有高資料包利用率，而從用戶端輸出的音訊資料流程幾乎沒有資料包利用率。 資料流程的持續時間可能是一個小時或更長，但從用戶端到伺服器的資料流程中的資料包利用率較低，因為麥克風已靜音，且未_分類_的資料流程結果。
