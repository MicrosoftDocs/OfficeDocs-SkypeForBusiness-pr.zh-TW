...
電話系統 TeamsOnly Teams，則支援使用 Teams。  (如果使用者位於群島模式，電話系統僅支援 商務用 Skype.)  
...
- 您必須確保您的使用者正確同步處理至Azure AD正確的商務用 Skype屬性。 這些屬性都是"msRTCSIP-"的首碼。 如果使用者未正確同步處理Azure AD，Teams中的管理工具將無法管理這些使用者。  (例如，除非您正確地同步處理這些屬性，否則無法指派 Teams 策略給內部部署使用者。) 若要詳細資訊，請參閱為 Teams 和 商務用 Skype 設定[Azure AD 連線。](/SkypeForBusiness/hybrid/configure-azure-ad-connect)
