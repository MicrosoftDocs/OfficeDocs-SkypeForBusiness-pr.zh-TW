...
本文說明如何使用 Real-Time 分析 (RTA) 疑難排解個別Microsoft Teams會議品質不佳的問題。 如果您有下列Real-Time，您可以存取分析：

- Teams 系統管理員
- Teams通訊支援專員
- Teams 通訊支援工程師

若要進一Teams系統管理員角色，請參閱使用Microsoft Teams[系統管理員角色來管理Teams](/MicrosoftTeams/using-admin-roles)。
...
今天，個別會議疑難排解可在[Teams結束後透過](use-call-analytics-to-troubleshoot-poor-call-quality.md)通話分析提供給系統管理員使用。 Real-Time分析功能可讓系統管理員在排程會議進行時進行疑難排解。
...
若要查看使用者的所有會議資訊和資料，請前往 Teams[管理中心](https://admin.teams.microsoft.com)。 在 **UserManage** ****  >  使用者下，選取使用者，然後開啟&**設定檔** 頁面上的會議與通話清單。 在 **最近的會議** 下，會看到使用者在過去 24 小時內參與的會議清單，其中即時遙測功能可供使用，包括任何進行中的會議。 如果會議沒有進行中，或沒有即時遙測資料，則會顯示在過去的 **會議中**。
...
若要查看進行中會議之特定使用者的遙測資料，包括有關裝置、網路、音訊、視像和內容共用詳細資料的資訊，請選取會議 **識別碼**。
...
|位元速率 |每秒千位 (Kbps)  |大於 24 Kbps |以千位/碼錶示的音訊串流輸送量。 |
...
|:---|:---|:---|:---|
|往返時間 |毫秒 |小於 500 ms |往返時間是單一封包從用戶端前往遠端端點並返回用戶端所花的時間。 高往返時間可能會導致串流播放延遲。 例如，當會議有兩個人因為延遲而不小心彼此說話時。 |
|位元速率 |每秒百萬位元 (Mbps)  | 僅資訊 |以 mbs/碼錶示的視音訊流的輸送量。 |
...
|編 解碼 器 |String | 僅資訊 |顯示外發視流的視音訊編解碼器與呈現模式。  (範例：H264 SW HW 會同時使用軟體與硬體轉出來表示 H264 視訊串流。) |
|解決方案 |圖元 | 僅資訊 |要送出之影片的解析度。 根據會議端點的最高需求，外發視音訊解析度是動態的。 如果沒有任何用戶端在大於 640 x 360 的畫面中顯示該使用者的視像，則能夠播放 1920 x 1080 影片的用戶端只會傳送 640 x 360 影片 |


### <a name="application-sharing-vbss"></a>在 VBSS (應用程式共用) 
|量值名稱 |單位 |良好閾值 |描述 |
|:---|:---|:---|:---|
|封包遺失 |百分比 |小於 5% |當封包無法到達目的地時，會發生封包遺失。 封包遺失百分比是根據所送封包總數。 |
|往返時間 |毫秒 |小於 500 ms |往返時間是單一封包從用戶端前往遠端端點並返回用戶端所花的時間。 高往返時間可能會導致串流播放延遲。 例如，當會議有兩個人因為延遲而不小心彼此說話時。 |
|位元速率 |每秒百萬位元 (Mbps)  | 僅資訊 |以 mbs/碼錶示的 VBSS 資料流程輸送量。 |
|畫面播放速率 |FPS (每秒)  | 僅資訊 |針對 VBSS，框架速率是內容感知型，可確保盡可能傳送所需的框架，以確保獲得良好的體驗，同時避免在不需要時傳送框架。 例如，在畫面上共用文字檔只需要每秒 1 個框架，才能產生良好的體驗，而以更多活動共用影片或內容時，每秒的框架數會增加至最多 30 個 FPS，以產生更流暢的體驗。 |
|編 解碼 器 |String | 僅資訊 |顯示 VBSS 串流的編解碼器與呈現模式。  (範例：H264 SW HW 會同時使用軟體與硬體呈現來表示 H264 VBSS 資料流程。) |
|解決方案 |圖元 | 僅資訊 |要接收的 VBSS 串流解析度。 |
...
> [!NOTE]
> 使用雲端視像交互操作 (CVI) 加入會議Real-Time分析中不支援。
...
- 即時遙測資料可在會議結束後的 24 小時內用於最近會議下的會議。 24 小時後，您即無法存取資料，而會議會移至 **過去的會議**。 如果會議超過 3 小時，則即時遙測只能使用過去 *3 小時*。
...
[使用Microsoft Teams系統管理員角色來管理Teams](/MicrosoftTeams/using-admin-roles)。
