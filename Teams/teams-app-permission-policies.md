...
您可以在 Microsoft 團隊系統管理中心管理 app 許可權原則。 您可以使用全域 (組織範圍的預設) 原則，或是建立並指派自訂原則。 除非您建立並指派自訂原則，否則貴組織中的使用者將會自動取得全域原則。 在您編輯或指派原則之後，可能需要幾個小時的時間，變更才會生效。
...
> [!NOTE]
> 如果您在 Microsoft 365 政府社區雲端 (GCC) 環境中部署團隊，請參閱 [管理 Microsoft 365 政府的整個組織結構設定](#manage-org-wide-app-settings-for-microsoft-365-government) ，以深入瞭解適用于 GCC 的協力廠商應用程式設定。
...
5. 如果您已選取 [ **允許特定應用程式並封鎖其他 app**]，請新增您想要允許的應用程式：
    ...
    1. 選取 [ **允許應用程式**]。
    1. 搜尋您要允許的應用程式，然後按一下 [ **新增**]。 搜尋結果會篩選到 app 發行者 (**Microsoft** app、 **協力廠商應用程式**，或) 的 **自訂應用程式** 。
...
如前文所述，若要允許協力廠商應用程式，您可以編輯及使用全域 (組織範圍預設) 原則，或是建立及指派自訂原則。
...
#### <a name="can-i-control-line-of-business-lob-apps"></a>我可以 (LOB) 應用程式控制業務線嗎？
是的，您可以使用應用程式許可權原則來控制自訂 (LOB) app 的推出與發佈。 您可以建立自訂原則或編輯全域原則，以根據貴組織的需求來允許或封鎖自訂應用程式。
