...
如果在語音路由策略中定義多個 PSTN 使用方式，將會按照定義的順序評估。 建議您根據與 PSTN 使用狀況相關聯的語音路由數模式，以較一般之最特定的順序定義 PSTN 使用量。 例如，如果 PSTN 使用量定義為將通話路由到美國，而另一個 PSTN 使用量已定義為將通話路由到全球任何其他位置，則 PSTN 通話到美國的 PSTN 使用量應列在 PSTN 使用量之前，先于 PSTN 使用方式將通話路由到全球任何其他位置。
...
### <a name="step-5-assign-audio-conferencing-with-direct-routing-for-gcc-high-or-dod-licenses-to-your-users"></a>步驟 5：指派具有直接路由的音訊會議GCC高或 DoD 授權給使用者
...
- 使用名稱錄製進入和離開通知。 針對使用直接路由的音訊會議，進入和離開通知會以鈴聲在會議中播放。
