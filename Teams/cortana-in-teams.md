...
ms.locfileid: "58726142"
---
# <a name="cortana-voice-assistance-in-teams"></a>Cortana語音協助Teams

> [!Note]
> Cortana iOS 和 Android Microsoft Teams 行動應用程式中支援語音協助，Microsoft Teams適用于美國、英國、加拿大、印度和澳洲的使用者。 Microsoft Teams 會議室Windows僅適用于美國的使用者。 Cortana目前不適用於 GCC、GCC高、DoD 和非美國 EDU 租使用者。 Cortana行動應用程式中Teams語音協助，現在適用于美國 EDU 客戶。 擴充至其他語言和地區，將在未來發行時進行。

> [!Note]
> Cortana語音協助Microsoft Teams 會議室預覽下發行。 在預覽版中，Cortana在連接了拉力麥克風的裝置上，僅支援美國的語言 EN-US。

Cortana Teams行動裝置 App、Windows Microsoft Teams 會議室 和 Microsoft Teams 顯示裝置上的語音協助，可讓 Microsoft 365 企業版 使用者使用口語自然語言簡化通訊、共同作業和會議相關工作。 使用者可以在 Cortana 行動應用程式中選取位於 Teams 行動應用程式右上角的麥克風按鈕，或在 Microsoft Teams 會議室中&#8220;Cortana&#8221;或使用 Microsoft Teams 顯示Microsoft Teams說話。 若要不用手快速與小組聯繫，以及進行中，使用者可以說出查詢，例如 &#8220;打電話給 Megan&#8221; 或 &#8220;將訊息傳送至下一個會議&#8221;。 使用者也可以加入會議，&#8220;加入我的下一&#8221;，並使用語音協助來共用檔案、檢查他們的日曆等。 這些語音協助體驗是使用[Cortana](/microsoft-365/admin/misc/cortana-integration?view=o365-worldwide)企業級服務提供，完全符合 Office 365 的隱私權、安全性和合規性承諾，如線上服務條款[ (OST ](https://www.microsoft.com/licensing/product-licensing/products?rtc=1)) 所反映。
...
租使用者系統管理員可以使用 TeamsCortanaPolicy Cortana使用Teams，控制租使用者中的 (語音) 。 此策略設定為使用者帳戶層級或租使用者層級。 系統管理員可以使用此策略控制項中的 CortanaVoiceInvocationMode 欄位，判斷 Cortana 是否停用、是否僅以按鈕式啟用啟用，或是否同時啟用喚醒字詞 (也適用于支援此功能的裝置 ，例如 Microsoft Teams 顯示) 。
...
此範例顯示使用名稱 &#8220;EmployeeCortanaPolicy&#8221; 更新現有策略，並僅在 Cortana 中啟用 Microsoft Teams 語音協助，並僅使用按鈕調用。 使用者可以在 Cortana 中選取 Cortana 麥克風按鈕Teams。 喚醒 word (&#8220;嗨Cortana&#8221;或&#8220;Cortana&#8221;) 將會停用。  
...
若要在Cortana啟用語音協助，請移動開關 **開啟或****關閉**。 會議結束後，Cortana回到裝置層級設定集。
