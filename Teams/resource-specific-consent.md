...
除了 Azure AD 中的設定之外，在管理[](manage-apps.md#manage-org-wide-app-settings)應用程式頁面上的全組織應用程式設定[](manage-apps.md)、應用程式在管理應用程式頁面上是否受到封鎖或允許，以及指派[](manage-apps.md#allow-and-block-apps)給團隊擁有者的應用程式許可權政策，都決定團隊擁有者是否可以給予同意。 [](teams-app-permission-policies.md)
