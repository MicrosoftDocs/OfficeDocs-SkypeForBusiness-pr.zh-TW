...
這些變更是我們持續努力為每位員工提供 Microsoft 365 的一部分。 有了 Teams 中的前線工作人員功能，貴組織的每一位員工都能使用 Teams 簡化工作日、與同事共同作業，以及存取訊號和專業知識，協助他們完成最佳工作。
...
所有使用者自 2020 年 6 月 30 日起皆無法再使用 StaffHub。 任何嘗試開啟應用程式的人，都會顯示一則訊息，指示他們下載 Teams。
...
#### <a name="why-cant-users-in-my-organization-access-their-staffhub-team-from-shifts-in-teams"></a>為什麼我的組織使用者無法從 Teams 中的 Shifts 存取其 StaffHub 小組？

若要從 Teams 中的 Shifts 存取 StaffHub 團隊，您必須在 2020 年 6 月 30 日之前將 StaffHub 小組移至 Teams，這是 StaffHub 已停用時。

#### <a name="what-happens-if-i-didnt-move-a-staffhub-team-to-shifts-in-teams"></a>如果我沒有將 StaffHub 小組移至 Teams 中的 Shifts，會發生什麼情況？

從 2020 年 6 月 30 日 StaffHub 停用後，無法再查看或移動使用 StaffHub 建立且未移至 Teams 的團隊。 不過，您隨時都可以開始使用 Teams，並可在 Shifts 應用程式中建立班次排程。

如果您有一個與 Office 365 群組相關聯的 StaffHub 小組，但您並未移至 Teams，而現在想要建立與該 Office 365 群組相關聯的 Teams 班次排程，請連連支援人員。

### <a name="about-shifts"></a>關於 Shifts

#### <a name="what-is-shifts"></a>什麼是 Shifts？

Teams 中的 Shifts 應用程式是一種排程管理工具，可保持前線員工與員工之間的連接與同步。主管建立、更新及管理團隊的班排程，小組成員可以從他們的行動裝置來查看他們的排程、查看當天還有誰排程、要求調班或提供班，以及要求請假。  
...
如果您已經在包含 Shifts 的計畫，您可以立即開始使用 Shifts 做為 Teams 的一部分。 請查看 開始使用 [Shifts](https://support.office.com/article/get-started-in-shifts-5f3e30d8-1821-4904-be26-c3cd25a497d6)。

#### <a name="which-devices-or-platforms-is-shifts-available-on"></a>Shifts 提供哪些裝置或平臺？

班次可在 Teams Web 用戶端、Teams 桌面用戶端和 Teams 行動用戶端 (iOS 和 Android) 。
...
核心功能在 Shifts iOS 和 Android App 之間是相同的。 這兩個 App 之間 UI 差異很小，可配合其建議的設計模式。
...
Shifts 資料會儲存在 Azure 中，位於北美和西歐的資料中心。 SharePoint 內容利用 Microsoft 365 群組，並使用相同的儲存空間和支援。
...
有關系統管理指南，請參閱在 Teams 中管理 [貴組織的班次應用程式](manage-the-shifts-app-for-your-organization-in-teams.md)

若要獲得使用者協助，請前往 [前線工作者的 Shifts 協助](https://support.office.com/article/apps-and-services-cc1fba57-9900-4634-8306-2360a40c665b)。

#### <a name="what-licenses-do-i-need-for-teams"></a>Teams 需要哪些授權？

每個使用者都必須有合格的方案提供有效的 Microsoft 365 或 Office 365 授權，而且必須獲派 Teams 授權。 

#### <a name="how-do-i-enable-teams-in-my-organization"></a>如何在組織中啟用 Teams？

根據預設，所有組織都啟用 Teams。 您將 Teams 授權指派給使用者，以控制 Teams 的個別存取權。 若要取得詳細資訊，請參閱 [在貴組織中啟用 Teams](../../Office-365-set-up.md) 及 [管理 Teams 的使用者存取權](../../user-access.md)。

#### <a name="what-if-i-currently-have-skype-for-business-enabled-in-my-organization-but-not-teams"></a>如果我的組織目前已啟用商務用 Skype，但 Teams 卻未啟用，該做什麼？

Teams 支援與商務用 Skype 共存。 詳細資訊請參閱瞭解 Teams 和商務用 [Skype](../../teams-and-skypeforbusiness-coexistence-and-interoperability.md) 共存與互通性，以及使用 Teams 與商務用 Skype 的組織移移及互通性 [指南](../../migration-interop-guidance-for-teams-with-skype.md)。

#### <a name="does-each-user-need-to-have-a-microsoft-365-or-office-365-account-or-azure-active-directory-azure-ad-account"></a>每個使用者是否需要使用 Microsoft 365 或 Office 365 帳戶或 Azure Active Directory (Azure AD) 帳戶？

是，在 Teams 中使用 Teams 和 Shift 的每個主管和團隊成員都必須擁有 Microsoft 365 或 Office 365 帳戶 (Azure AD) 且必須獲得有效的授權。
...
#### <a name="is-there-integration-between-shifts-and-the-outlook-calendar-can-i-export-a-schedule-to-a-calendar"></a>Shifts 與 Outlook 日曆之間是否有整合？ 我可以將排程匯出至日曆嗎？
 
前線工作人員可以在 Shifts 中一目了然地查看其整個工作排程。 我們將支援與 Outlook 日曆的原生整合，做為藍圖的一部分。 同時，使用 Microsoft Flow 設定與 Teams 和 Outlook 和 Google 日曆中的 Shifts 整合。
...
#### <a name="what-powershell-controls-are-available-for-teams"></a>Teams 可以使用哪些 PowerShell 控制項？

您可以透過 Microsoft Teams PowerShell 模組和商務用 Skype PowerShell 模組來管理 Teams。 詳細資訊，請參閱 [Teams PowerShell 概觀](../../teams-powershell-overview.md)。 目前，沒有任何特定的 PowerShell Cmdlet 可用於管理 Shifts。  
