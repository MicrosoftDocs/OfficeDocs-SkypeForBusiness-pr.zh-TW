...
description: 尋找有關 Microsoft StaffHub 的停用，以及移轉至 Microsoft Teams 中的 Shifts 應用程式的常見問題解答。
f1.keywords:
- NOCSH
...
這些變更是我們持續努力的一部分，讓每位員工Microsoft 365。 使用 Teams 中的前線員工功能，貴組織的每一位員工都能使用 Teams 簡化工作日、與同事共同作業，以及存取訊號和專業知識，協助他們完成最佳工作。
...
若要從 Teams 中的 Shifts 存取 StaffHub 小組，您必須在 2020 年 6 月 30 日之前將 StaffHub 小組移至 Teams，這是 StaffHub 已停用時。

#### <a name="what-happens-if-i-didnt-move-a-staffhub-team-to-shifts-in-teams"></a>如果我沒有將 StaffHub 小組移至公司中的 Shifts，會發生什麼Teams？

Teams使用 StaffHub 建立且未移至 Teams 的已停用自 2020 年 6 月 30 日 StaffHub 停用後，無法再查看或移動。 不過，您隨時都可以Teams，在 Shifts 應用程式中建立班排程。
...
如果您已經在包含 Shifts 的計畫中，您可以立即開始使用 Shifts 做為Teams。 請查看 在 [Shifts 中開始使用](https://support.office.com/article/get-started-in-shifts-5f3e30d8-1821-4904-be26-c3cd25a497d6)。

#### <a name="which-devices-or-platforms-is-shifts-available-on"></a>Shifts 提供哪些裝置或平臺？

在 iOS 和 Android Teams 上，Teams桌面用戶端Teams行動用戶端 (班次) 。
...
> [!Note]
> Microsoft 365應用程式和服務不支援 Internet Explorer 11，自 2021 年 8 月 17 日 (Microsoft Teams自 2020 年 11 月 3) 0 日起 (Microsoft Teams將不支援 Internet Explorer 11。 [深入了解](https://aka.ms/AA97tsw)。 請注意，Internet Explorer 11 仍然是受支援的瀏覽器。 Internet Explorer 11 是作業系統Windows元件，並遵循安裝[](/lifecycle/faq/internet-explorer-microsoft-edge)該作業系統之產品的生命週期政策。 
...
有關系統管理指南，[請參閱在](manage-the-shifts-app-for-your-organization-in-teams.md)Teams
...
根據預設，Teams所有組織都啟用此功能。 您指派Teams授權給使用者，以控制個別存取Teams。 若要取得詳細資訊，請參閱在[Teams中啟用許可權](../../Office-365-set-up.md)，以及管理使用者對[Teams。](../../user-access.md)
...
Teams支援與商務用 Skype。 詳細資訊，請參閱瞭解Teams[](../../teams-and-skypeforbusiness-coexistence-and-interoperability.md)及商務用 Skype，以及使用 Teams 與 商務用 Skype 的組織移移及互通性[指南](../../migration-interop-guidance-for-teams-with-skype.md)。

#### <a name="does-each-user-need-to-have-a-microsoft-365-or-office-365-account-or-azure-active-directory-azure-ad-account"></a>每個使用者是否需要在 Azure AD Microsoft 365帳戶Office 365帳戶Office 365帳戶Azure Active Directory (帳戶) 帳戶？
...
您可以將班次排程資訊從 Shifts 桌面Excel Web App 匯出至新的檔案。 然後，您可以將這項資訊導入貴組織的系統與工具。
...
若要深入瞭解，請參閱如何SharePoint [Online 商務用 OneDrive與 Teams。](../../SharePoint-OneDrive-interact.md)
...
您可以透過 Teams PowerShell 模組Microsoft Teams PowerShell 模組和 PowerShell 模組商務用 Skype管理。 詳細資訊，請參閱[powerShell Teams概觀](../../teams-powershell-overview.md)。 目前，沒有特定的 PowerShell Cmdlet 可管理 Shift。  
...
由於 Shifts 是應用程式中Teams，因此您可以透過報表Teams使用方式。 詳細資訊，請參閱 Teams[](../../teams-analytics-and-reports/teams-reporting-reference.md)系統管理中心Microsoft Teams報告Teams [Microsoft 365 系統管理中心活動](../../teams-activity-reports.md)報告。

#### <a name="where-can-i-provide-feedback"></a>我可以在哪裡提供意見回饋？
  
在行動應用程式中 **，點一** 下畫面左上角的更多內容，然後設定  >  **傳送** 意見或 **報告問題**。 同樣地，在桌面和 Web 應用程式中，在畫面左下角，前往協助  >  **分享** 想法或 **報告問題**。