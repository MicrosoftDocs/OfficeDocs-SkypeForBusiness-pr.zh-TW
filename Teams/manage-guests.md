...
**來賓** 是 microsoft 團隊中的使用者類型，包括所有 Microsoft 365 商務標準版、Office 365 企業版、Microsoft 365 Business Basic 及 Office 365 教育版訂閱。 您不需要額外的 Microsoft 365 或 Office 365 授權。 請參閱以下有關 [來賓存取授權](#guest-access-licensing-limits) 的詳細資訊。
...
IT 管理員可以在租使用者層級新增來賓、設定及管理來賓使用者原則和許可權，以及拉入來賓使用者活動的報告。 這些控制項可在 [團隊管理中心] 中取得。 來賓使用者的內容和活動低於 Microsoft 365 或 Office 365 的其他相容性和審核保護。
...
您可以使用 Azure Active Directory (Azure AD) 入口網站來管理來賓及其對 Microsoft 365 或 Office 365 與團隊資源的存取權。 團隊訪客存取可使用 Azure AD 企業對企業 (B2B) 共同作業功能作為基礎基礎結構，以儲存安全原則資訊，例如身分識別屬性、成員資格和多重要素驗證設定。 若要深入瞭解 Azure AD B2B，請參閱 [什麼是 AZURE AD b2b 共同作業？](https://go.microsoft.com/fwlink/p/?linkid=853011) 以及 [AZURE Active Directory b2b 合作常見問題](https://go.microsoft.com/fwlink/p/?linkid=853020)。
