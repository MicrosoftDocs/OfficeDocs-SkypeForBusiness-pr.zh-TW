...
下表說明此範例 SkypeSettings (檔案名所需的檔案名) 設定檔中所示的元素。
...
如果 XML 檔案的格式不正確 (變數值的類型不正確、元素沒有順序、元素未閉合等等), 則會在處理期間忽略指向所找到的錯誤所在位置所需的設定。 XML 中的任何未知元素都會被忽略。 如果省略某個參數, 則會在裝置上保持不變。 如果參數值無效, 則其前一個值會保持不變。
...
|\<ContentCameraEnhancement>  | 布林值 &#x2777; | |當設定為 true (預設值) 時, 將會以數位方式增強內容相機影像: 已偵測到白板邊緣, 且已選取適當的縮放比例, 手寫線會得到增強, 且在白板上書寫的人員則變成透明。  <br><br> 如果您想要將原始影片摘要傳送給會議參與者, 以取得不使用手寫筆繪製白板的空間, 而是使用相機來顯示粘滯便箋、海報或其他媒體, 請將此選項設為 false。  |
...
&#x2777; 布林值旗可以是下列任何一項: true、false、0或1。 如果將布林值或數值保留空白, 可能會轉譯 XML 格式不正確, 並防止變更設定。
...
自訂主題圖像檔案必須放在**C:\Users\Skype\AppData\Local\Packages\Microsoft.SkypeRoomSystem_8wekyb3d8bbwe\LocalState**中, 在\<CustomThemeImageUrl\>變數中輸入檔案名和副檔名。
