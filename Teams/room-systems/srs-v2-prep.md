...
1. 為每個 Microsoft 團隊聊天室主控台準備一個裝置帳戶。 如需詳細資訊, 請參閱[部署 Microsoft 團隊會議室](room-systems-v2.md)。
...
3. 為了改善您的體驗, Microsoft 會收集資料。 若要收集資料, 我們需要下列網站白名單:
...
*裝置帳戶*是 Microsoft 團隊聊天室用戶端用來存取 Exchange 功能 (例如行事曆), 以及啟用商務用 Skype 的帳戶。 如需詳細資訊, 請參閱[部署 Microsoft 團隊會議室](room-systems-v2.md)。
...
8. 開啟 Skype 金鑰並流覽至 [HKEY_USERS\Skype\SOFTWARE\Microsoft\Windows\CurrentVersion\Internet 設定], 然後確保已輸入這些設定: 
    ...
    [HKEY_USERS\Skype\SOFTWARE\Microsoft\Windows\CurrentVersion\Internet 設定]
    
    "MigrateProxy" = dword: 00000001
    
    "ProxyEnable" = dword: 00000001
    
    "ProxyServer" = "xx. xx: 8080"
    
    如果 ProxyServer 不存在, 您可能需要將此金鑰新增為字串, 將 xx. xx: 8080 變更為您 Proxy 伺服器的 ip/主機和埠。
...
您將會使用 [預配套件] 來驗證 Exchange Server 或 Office 365。
