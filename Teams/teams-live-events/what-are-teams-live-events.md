...
ms.locfileid: "51119122"
---
# <a name="what-are-microsoft-teams-live-events"></a>什麼是 Microsoft Teams 即時活動

## <a name="overview"></a>概觀

有了 Teams 即時活動，貴組織中使用者可以廣播視像和會議內容給大型線上觀眾。

Microsoft 365 即時活動將即時視音訊串流新高。 即時活動可在整個參與週期內鼓勵在即時活動之前、期間和之後與出席者建立連線。 您可以使用 Microsoft Stream、Teams 或 Yammer，在觀眾、小組或社群所在的任何位置建立即時活動。  

Teams 提供聊天式共同合作、通話、會議和即時活動，因此您可以擴大會議物件。 Teams 即時活動是 Teams 會議的延伸功能，可讓使用者將視像和會議內容廣播給大量的線上觀眾。 即時活動適用于由活動主持人主導互動的一對多通訊，而觀眾參與主要是為了查看主機共用的內容。 出席者可以在 Yammer、Teams 和/或 Stream 中觀看即時或錄製的活動，並可以使用經管理 Q & A 或 Yammer 交談與簡報者互動。

Teams 即時活動會視為下一版的 Skype 會議廣播，且最終會取代 Skype 會議廣播中提供的功能。 此時，Microsoft 會繼續支援在組織中使用商務用 Skype 的使用者使用 Skype 會議廣播，不會中斷新活動或未來活動的服務。 不過，我們鼓勵您試用 Teams 即時活動，以運用所有全新且令人振奮的功能，包括螢幕分享，以及支援外部硬體/軟體編碼器。
...
- **Teams：** 此製作方法可讓使用者使用網路相機或 Teams 會議室系統的 A/V 輸入，在 Teams 中製作即時活動。 如果您想要使用連接到電腦的音訊和視像裝置，或邀請遠端簡報者參與活動，則此選項是最佳且最快的選項。 此選項可讓使用者輕鬆使用網路攝像機，並分享螢幕作為活動中的輸入。

- **外部應用程式或裝置**：外部編碼器可讓使用者直接從外部硬體或軟體型編碼器使用 Stream 來產生 [即時事件](https://stream.microsoft.com)。 如果您已經有工作室品質設備 (例如媒體混合器) ，支援串流至即時傳訊通訊協定 (RTMP) 服務時，這個選項是最佳選擇。 這類製作通常用於大型活動，例如高官鎮大廳，其中媒體混合器中的單一串流會廣播給觀眾。
...
- **Azure Media**  [Services：Azure Media Services](/azure/media-services/previous/) 提供廣播品質的視訊串流服務，讓您在現今最熱門的行動裝置上，接觸更多觀眾。 媒體服務可增強協助工具、發佈和擴充性，並輕鬆且具有成本效益地將內容串流到您當地或全球的觀眾，這一切都可保護您的內容。
...
租使用者系統管理員可以在 Microsoft Teams 系統管理中心查看即時事件即時使用方式分析。  即時 [活動使用方式報告](../teams-analytics-and-reports/teams-live-event-usage-report.md) 會顯示組織中舉辦的即時活動的活動概觀。  管理員可以查看事件使用方式資訊，包括事件狀態、開始時間、視圖及生產類型。  
