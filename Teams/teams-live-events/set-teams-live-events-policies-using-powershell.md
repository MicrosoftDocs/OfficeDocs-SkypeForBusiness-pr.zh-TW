...
> [!NOTE]
> 這些範例適用于 Teams 中產生的活動。 對於使用外部應用程式或裝置產生的事件，您必須執行其他步驟。 如需詳細資訊，請參閱 [讓使用者排程使用外部應用程式或裝置所產生的事件](set-up-for-teams-live-events.md#enable-users-to-schedule-events-that-were-produced-with-an-external-app-or-device)。
...
如果使用者獲派全域原則，請執行並確認 *AllowBroadcastScheduling* *設定為 *True*：
```PowerShell
Get-CsTeamsMeetingBroadcastPolicy -identity Global
```
...
**您希望有大量使用者能夠排程即時活動，並防止一組使用者排程活動**
