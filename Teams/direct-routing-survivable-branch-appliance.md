...
---
# <a name="survivable-branch-appliance-sba-for-direct-routing"></a>適用于直接路由的可 (分支裝置) SBA 裝置


有時候，使用直接路由連接到 Microsoft Phone System 的客戶網站可能會遇到網際網路中斷。

假設客戶網站 ，稱為分支，暫時無法透過直接路由連接到 Microsoft 雲端。 不過，分支內的內部網路仍然功能完整，使用者可以連接到提供 PSTN (SBC) 會話邊界控制器。

本文說明如何使用可維護分支裝置 (SBA) ，讓 Microsoft Phone System 在中斷時繼續撥打和接聽公用交換電話網路 (PSTN) 通話。
...
- SBC 必須針對媒體旁路進行配置，以確保分支網站中的 Microsoft Teams 用戶端能夠讓媒體直接與 SBC 一起流動。 
...
- Microsoft Teams Windows 桌上出版 

- Microsoft Teams macOS 電腦版 

## <a name="how-it-works"></a>運作方式

在網際網路中斷期間，Teams 用戶端應該會自動切換到 SBA，而持續通話應該不會中斷。 使用者不需要執行任何動作。 當 Teams 用戶端偵測到網際網路已上線且任何撥出通話完成時，用戶端就會回到正常作業模式，並連接到其他 Teams 服務。 SBA 會將收集的通話資料記錄上傳至雲端，通話記錄將會更新，以便租使用者系統管理員能夠查看此資訊。 

當 Microsoft Teams 用戶端處於離線模式時，可以使用下列與通話相關的功能： 
...
若要讓 SBA 功能能夠使用，Teams 用戶端必須知道每個分支網站中哪些 SBA 可用，以及哪些 SBA 會指派給該網站的使用者。 組組步驟如下：

1. 建立 SBA。
2. 建立 Teams 分支的可生存性政策。
3. 將策略指派給使用者。
4. 使用 Azure Active Directory 註冊 SBA 應用程式。

所有配置都是使用商務用 Skype Online PowerShell Cmdlet 完成。  (Teams 系統管理中心尚未支援直接路由 SBA 功能。)  
...
### <a name="create-the-teams-branch-survivability-policy"></a>建立 Teams 分支生存能力政策 
...
若要將策略指派給個別使用者，請使用 Grant-CsTeamsSurvivableBranchAppliancePolicy Cmdlet。 此 Cmdlet 具有下列參數：
...
若要允許租使用者內使用的不同 SBA 讀取 Microsoft 365 所需的資料，您必須在 Azure Active Directory 中註冊 SBA 應用程式。 

有關應用程式註冊的資訊，請參閱下列專案：

- [為 Azure Active Directory 開發商務用應用程式](/azure/active-directory/manage-apps/developer-guidance-for-integrating-applications)

- [使用 Microsoft 身分識別平臺註冊應用程式](/azure/active-directory/develop/quickstart-register-app)。  
...
- API 許可權 = Skype 和 Teams 租使用者系統管理員 Access ->應用程式許可權 -> application_access_custom_sba_appliance。
- 用戶端金鑰：您可以使用任何描述和到期。 
- 請記得在建立用戶端密碼後立即複製。 
...
- [音訊代碼](https://www.audiocodes.com/solutions-products/products/products-for-microsoft-365/direct-routing-survivable-branch-appliances)
...
- 不會針對 Azure AD 連絡人執行反向數位尋找。 

- SBA 不支援呼叫轉呼叫設定。 

- 不支援撥打 E911 (緊急) 緊急號碼。

- 結果會顯示 teamsBranchSurvivabilityPolicy Get-CsOnlineUser TeamsBranchSurvivabilityPolicy。