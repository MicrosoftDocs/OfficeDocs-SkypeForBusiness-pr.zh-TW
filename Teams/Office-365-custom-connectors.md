...
連接器可直接將常用的內容和服務更新傳送至頻道，讓您的小組保持在最新狀態。 有了連接器，Microsoft Teams使用者就可以在其小組的聊天串流中接收來自熱門服務的更新，例如 Trello、Wunderlist、GitHub 和 Azure DevOps Services。
...
> [!NOTE]
> 連接器預設會停用GCC環境。 如果您需要啟用，請設定 ConnectorsEnabled 或 ConnectorsEnabledForTeams 參數，$true [Set-OrganizationConfig](/powershell/module/exchange/set-organizationconfig) Cmdlet 來設定。 您先前需要[連接到 powerShell Exchange Online。](/powershell/exchange/connect-to-exchange-online-powershell)
