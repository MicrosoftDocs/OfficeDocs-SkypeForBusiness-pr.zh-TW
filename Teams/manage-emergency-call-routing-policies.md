...
ms.locfileid: "60605199"
---
# <a name="manage-emergency-voice-routing-policies-for-direct-routing"></a>管理直接路由的緊急語音路由策略

如果您在組織中部署直接路由，[](direct-routing-landing-page.md)您可以使用 Microsoft Teams 中的緊急語音路由策略來設定緊急號碼，並指定緊急電話的路由方式。 緊急語音路由策略會決定為指派該政策的使用者啟用增強型緊急服務、撥打緊急服務 (例如美國) 的 911 號碼，以及撥打緊急服務電話的路由方式。 
...
5. 定義一或多個緊急號碼。 若要執行此操作，請在 [ **緊急號碼**> 下，按一下 [ **新增**，然後執行下列操作：
    ...
    3. **PSTN 使用記錄**：選取 PSTN (公用電話) 電話網絡。 PSTN 使用量記錄是用來判斷使用哪一個路由路由從有權使用緊急電話的使用者路由緊急電話。 與此使用關聯的路由應指向專用於緊急電話的會話初始通訊協定 (SIP) 主幹或緊急位置識別號碼 (ELIN) 閘道，將緊急電話路由到最近的公用安全應答點 (PSAP) 。
