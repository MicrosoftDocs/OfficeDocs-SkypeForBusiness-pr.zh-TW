...
description: 瞭解如何在 Microsoft Teams中使用和管理緊急語音路由Microsoft Teams設定緊急號碼，以及指定緊急電話的路由方式。
ms.custom:
- seo-marvel-apr2020
- ms.teamsadmincenter.voice.emergencycallroutingpolicies.overview
...
5. 定義一或多個緊急號碼。 若要執行此操作，請按一下 [ **緊急** 號碼> 下的 [ **新增**，然後執行下列操作：
    ...
    3. **PSTN 使用記錄**：選取 PSTN (的公用電話) 電話網絡。 PSTN 使用量記錄是用來判斷使用哪一個路由路由從有權使用緊急電話的使用者路由緊急電話。 與此使用關聯的路由應指向會話初始通訊協定 (SIP) 主幹專用緊急電話或緊急位置識別號碼 (ELIN) 閘道，將緊急電話路由到最近的公用安全應答點 (PSAP) 。
