...
**停用商務用 Skype Online 會如何影響組織使用混合式商務用 Skype 設定，以進行內部部署公開交換的電話網絡 (PSTN) 連線與線上電話系統功能？**<br>
有些組織已將使用者分割在駐留內部部署的 PSTN 連線，並在手機系統功能中託管。 只要存取商務用 Skype Online 結束，組織就必須透過 [直接路由](https://docs.microsoft.com/MicrosoftTeams/direct-routing-landing-page)將其內部部署電話網絡轉移至小組，並將其內部部署的電話網絡連線至團隊。
