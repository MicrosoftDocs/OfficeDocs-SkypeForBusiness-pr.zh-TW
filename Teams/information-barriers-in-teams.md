...
>[!NOTE]
>- 無法跨 (建立) 群組的資訊障礙。
>- 使用 bot，Azure Active Directory (Azure AD) 應用程式，而版本 1 不支援某些 API 來新增使用者。
>- 私人頻道符合您設定之IB 政策。
...
不過，自推出 IB 以來，許多其他領域發現它們很有用。 其他常見案例包括：
...
- **系統要求** 新的聊天 - 每次使用者要求與一或多個其他使用者進行新聊天時，會評估該聊天，以確保該聊天未違反任何IB 原則。 如果交談違反 IBM 原則，則交談尚未開始。
...
當IB 原則系統管理員變更原則，或因為使用者設定檔 (變更而啟用原則變更時 ，例如工作變更) ，資訊障礙原則評估服務會自動搜尋成員，以確保他們在團隊中的成員資格不會違反任何原則。
...
- **租使用者允許的區** 段數上限：每個租使用者在設定IB 政策時最多可以設定 100 個區段。 可配置的策略數量沒有限制。
