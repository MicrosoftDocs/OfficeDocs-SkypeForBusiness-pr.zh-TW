...
>[!NOTE]
>- 無法跨 (建立) 群組的資訊障礙。
>- 版本 1 不支援Azure Active Directory (Azure AD) App、API 來傳送活動提要通知，以及一些新增使用者的 API。
>- 私人頻道符合您設定之IB 政策。
...
建立團隊時，系統會SharePoint網站，並針對檔案體驗Microsoft Teams與網站建立關聯。 根據預設，此網站和檔案SharePoint資訊障礙政策。 若要在SharePoint中OneDrive資訊障礙，請遵循使用資訊障礙與SharePoint中的[指引和](/sharepoint/information-barriers#enable-sharepoint-and-onedrive-information-barriers-in-your-organization)步驟。
...
- 團隊 **擁有者不會** 移除：如果已採用新的 IB 原則，導致 Teams 頻道中出現兩個或多個衝突區段，則具有團隊擁有者的區段會獲得較高的喜好設定，而會移除其他區段使用者。 此外，目前不會移除團隊擁有者，即使他們與其他擁有者/使用者發生衝突。 租使用者系統管理員和其他頻道擁有者必須手動移除衝突擁有者。
