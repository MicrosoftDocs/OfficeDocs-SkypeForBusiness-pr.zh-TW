---
title: 適用于 IT 系統管理員的 Microsoft 教育版常見問題
author: SerdarSoysal
ms.author: serdars
manager: serdars
ms.topic: article
...
---
# <a name="microsoft-education-faq-for-it-admins"></a>適用于 IT 系統管理員的 Microsoft 教育版常見問題

> [!Tip]
> 請觀看下列會話以深入瞭解 Microsoft Teams 中的管理： [Microsoft Teams 中的管理、管理和生命週期](https://aka.ms/teams-governance)

## <a name="how-do-i-control-team-creation-im-worried-students-are-going-to-create-inappropriate-teams"></a>如何?控制團隊建立？ 我擔心學生會建立不適當的團隊嗎？
...
一般建議您不要讓任何人建立團隊。 如果每個人都能建立團隊，Teams 就更可能受到廣泛採用。 教職員、教師或學生可以使用 Teams 來建立研究群組或特殊興趣群組。 這項功能可協助在教室內外接受 Teams。
...
在每一個學期或每季的開頭，您需要新的團隊。 採用自動化方法自動建立這些團隊、向正確的使用者填入團隊，以及設定正確的許可權可能是合理的：

- 學校資料同步處理可以針對 Exchange Online 和 SharePoint Online、Microsoft Teams 和 OneNote 課程筆記本的班級團隊、教育用 Intune 的學校群組，以及針對許多其他協力廠商應用程式的名冊和單一Sign-On (SSO) 整合建立Microsoft 365 群組。 若要深入瞭解 [，請參閱學校資料同步處理概觀](/schooldatasync/overview-of-school-data-sync)。
- 使用 PowerShell，您可以建立團隊和頻道，並自動設定設定。 如需詳細資訊，請參閱 [Microsoft Teams PowerShell](/powershell/module/teams/?view=teams-ps)。
- 您可以使用目前處於預覽版中的 Microsoft 圖形 API () 來建立、設定、複製及封存團隊。 如需詳細資訊，請參閱[使用 Microsoft 圖形 API 來搭配 Microsoft Teams 使用](/graph/api/resources/teams-api-overview)。
...
- **保留原則：** 使用此原則刪除所有超過您指定年齡的資料，以確保已移除聊天 (中所有或部分使用者) 和頻道的舊資料。 您也可以將 Teams 設定為保留內容，使其無法刪除。 如需詳細資訊，請參閱 [Microsoft Teams 的保留原則](https://techcommunity.microsoft.com/t5/Microsoft-Teams-Blog/Retention-policies-for-Microsoft-Teams/ba-p/178011)。
- **到期原則：** 將團隊設定為在設定日期之後到期。 在到期前 30 天，團隊的所有擁有者都會收到續約通知，否則系統會將其刪除。 雖然系統管理員可以再復原刪除的團隊 30 天。 此設定對於確保已停用未使用的團隊很有用。 如需詳細資訊，請參閱 [Microsoft 365 群組到期原則](https://support.office.com/article/office-365-group-expiration-policy-8d253fe5-0e09-4b3c-8b5e-f48def064733)。

- **封存小組：** 此設定會讓團隊進入唯讀模式。 他們仍然可以流覽和搜尋，但沒有人可以新增任何文章。 [封存或還原團隊](https://support.office.com/article/archive-or-restore-a-team-dc161cfd-b328-440f-974b-5da5bd98b5a7)說明團隊擁有者如何封存團隊;團隊擁有者也可以使用[圖形 API (預覽) ](/graph/api/resources/teams-api-overview)來封存或還原團隊。
...
是的。 建立新團隊時，使用者可以 **從現有範本** 選取 [建立團隊]，而 Teams 擁有者也可以使用 [圖形 API (預覽)](/graph/api/resources/teams-api-overview)從可用的範本建立新團隊。

## <a name="what-tasks-can-i-automate-via-powershell-or-graph"></a>我可以透過 PowerShell 或 Graph 自動化哪些工作？

[Microsoft 圖形 API (預覽版) ](/graph/api/resources/teams-api-overview)可以執行下列工作：
...
[PowerShell](/powershell/module/teams/?view=teams-ps) 可以執行下列工作：
...
> [!TIP]
> 圖形 API和 PowerShell Cmdlet 會不斷新增功能。 請務必經常查看[Microsoft 圖形 API (預覽) ](/graph/api/resources/teams-api-overview)和[PowerShell](/powershell/module/teams/?view=teams-ps)文章，以取得功能增強功能。  
