...
-   **自訂封鎖的字詞** 您可以上傳一組文字，禁止特定組織的使用者使用他們建立的團隊名稱。 例如，您可以封鎖 CEO、**薪資** 和 **人力資源** 這兩個字詞，禁止用於他們不適用之群組的團隊名稱中。
-   **分類** 您可以建立組織中使用者在建立群組時可設定Microsoft 365分類。 

> [!IMPORTANT]
> 使用 Microsoft 365 群組命名政策時Azure Active Directory Premium P1一或多個群組成員之每個唯一使用者Azure AD或 Azure AD 基本 ED Microsoft 365 U 授權。
...
> [!Note]
> 如果團隊是使用另一個系統 (例如 學校資料同步處理) 的輸入自動建立，請確認輸入資料符合您配置的命名規則;如果沒有，團隊建立將會失敗。

## <a name="can-i-see-who-created-a-team"></a>我可以看到誰建立了團隊嗎？

若要找出誰建立了特定小組[，請參閱搜尋](audit-log-events.md)稽核記錄，以尋找 Microsoft Teams。

## <a name="can-i-control-who-can-create-teams"></a>我可以控制誰可以建立團隊嗎？

一般而言，建議您禁止任何人建立團隊。 如果每個人都可以建立團隊，Teams更容易被廣泛採用。 教職員、教師或學生可以使用Teams建立學習群組或特殊興趣群組。 這有助於Teams教室內外接受。
...
-   學校資料同步處理建立Microsoft 365群組Exchange Online SharePoint線上課程團隊、Microsoft Teams OneNote 課程筆記本、Intune 教育用學校群組，以及 SSO (單一登入) 整合許多其他協力廠商應用程式。 如需深入瞭解，[請流覽學校資料同步處理](/schooldatasync/overview-of-school-data-sync)。
...
-   **保留政策：** 您可以使用此功能刪除您指定的年齡之前的所有資料，以確保所有或部分使用者與頻道 (從聊天中移除舊資料) 資料。 您也可以設定Teams保留內容，以便無法刪除內容。 詳細資訊，請參閱[保留Microsoft Teams](https://techcommunity.microsoft.com/t5/Microsoft-Teams-Blog/Retention-policies-for-Microsoft-Teams/ba-p/178011)。
...
有關組織Teams的詳細資訊，請參閱[管理Microsoft Teams的設定](enable-features-office-365.md)。
...
詳細資訊請參閱群組[中的來賓Microsoft 365存取](https://support.office.com/article/Guest-access-in-Office-365-Groups-bfc7a840-868f-4fd6-a390-f347bf51aff6#PickTab=Manage)。
...
詳細資訊，請參閱在 Microsoft Teams 中搜尋[稽核記錄](audit-log-events.md)。
