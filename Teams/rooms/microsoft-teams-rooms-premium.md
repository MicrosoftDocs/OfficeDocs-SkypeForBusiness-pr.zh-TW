---
title: Microsoft Teams 會議室受管理的服務
author: dstrome
ms.author: dstrome
manager: serdars
ms.reviewer: ''
ms.topic: article
ms.tgt.pltfrm: cloud
ms.service: msteams
audience: Admin
ms.collection:
- M365-collaboration
- m365initiative-meetings
appliesto:
- Microsoft Teams
ms.localizationpriority: medium
search.appverid: MET150
description: 瞭解Microsoft Teams 會議室受管理的服務。
f1keywords: ''
ms.openlocfilehash: 432e09204ae157ece03b2dd0c1f4ac3336adec52
ms.sourcegitcommit: de0233e51364b7730c4b307fd41a2166eda3c3fc
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/26/2022
ms.locfileid: "65741875"
---
# <a name="microsoft-teams-rooms-managed-service"></a>Microsoft Teams 會議室受管理的服務

Microsoft Teams 會議室管理的服務是一種雲端式 IT 管理和監控服務，可讓Microsoft Teams 會議室裝置及其周邊裝置保持在最新狀態，並主動受到監視，支援針對最佳使用者體驗優化的環境。

Microsoft Teams 會議室管理的服務適用于想要改善其會議室體驗的組織，並提供Microsoft Teams 會議室裝置的主動管理，以快速擴展其業務量。 這項服務可讓組織節省時間、金錢和專業知識，否則會花在「自己動手」的聊天室管理上。  

Microsoft Teams 會議室管理服務是以 Microsoft 在支援軟體管理端點、機器學習和智慧自動化方面的專業知識為基礎而建置。 Microsoft Teams 會議室管理的服務可協助組織跨多個遠端網站提供高品質的體驗，並可適應組織可能需要的任何成長速度。

以下是服務的三大層面：  

- 智慧型作業  

   可自動化更新、問題偵測、Microsoft Teams 會議室問題解決方式的軟體和機器學習。  



- 專屬專家  

   提供 24x7 服務作業、層級支援及事件解決協助的專家小組。  



- 增強的深入解析  

   針對許多客戶提供豐富的分析、報告和經驗證的學習。  


## <a name="service-capabilities"></a>服務功能

以下是Microsoft Teams 會議室受管理服務功能的概觀。

|功能  |描述  |
|---------|---------|
|**會議室規劃與推行**   |會議室規劃功能可讓您瞭解會議室設備庫存，並決定新會議室部署的設備標準。         |
|**監控與疑難排解**  |會議室健康情況監控，可偵測會議室系統和周邊設備的問題，並提供由 Microsoft 驅動的主動事件管理回應。         |
|**更新管理**    |管理及傳遞由 Microsoft 所執行之房間系統及其周邊裝置的應用程式和韌體更新。         |
|**Insights與建議**     |Insights客戶健康情況、使用方式、使用率及設備可靠性。         |
|**合規性與認證**   |符合 Microsoft Online Services 的所有適用標準。         |
|**客戶支援**   |24x7x365 票證支援，透過 SOC) 專用Microsoft Teams 會議室受管理服務服務運算中心 (。 適用時，代表您向其他主要廠商上報。 若要深入瞭解，請參閱 [客戶支援](#customer-support)。|
|**支援的裝置**    |已認證Microsoft Teams 會議室系統及其周邊設備。<br>*2021 下半年Surface Hub支援*        |
|**支援的地區**    |請參閱 [支援的地區](#supported-regions)。        |
|**集成**    |立即與客戶託管的服務整合。         |

## <a name="service-features"></a>服務功能

以下是Microsoft Teams 會議室受管理服務提供支援上述功能的重要功能摘要。

|功能  |描述  |
|---------|---------|
|**庫存管理**   |含有大量匯出選項的會議室庫存追蹤，其中列出會議室裝置、周邊裝置、製作及型號。         |
|**會議室註冊**   |透過自動註冊會議室裝置到Microsoft Teams 會議室受管理的服務，輕鬆上線。          |
|**會議室管理**     |受管理服務功能、深入解析、系統管理設定和會議室群組管理的客戶入口網站。         |
|**監視**    |應用程式、作業系統、周邊和韌體健康情況監控。 因周邊健康情況等問題而影響會議體驗問題的自動化事件偵測。 透過Microsoft Teams 會議室管理的服務入口網站，整個房間的房地產和事件的健康情況概觀。|
|**疑難排解與補救**     |適用時自動診斷和自動補救。 Microsoft Teams 會議室受管理的服務 SOC 協助非自動化案例的事件補救。 事件協調的 [立即服務] 整合。         |
|**更新管理**   |根據客戶可設定的部署頻道，自動化會議應用程式和韌體更新協調         |
|**Insights與建議**   |使用狀況、事件、健康情況和整體報告 (歷程記錄檢視)          |

## <a name="service-prerequisites"></a>服務先決條件

若要讓Microsoft Teams 會議室管理的服務維持服務品質，註冊的會議室必須[Microsoft Teams 會議室支援的裝置](requirements.md#hardware-requirements)，而且您必須使用會議室裝置完全允許和存取的所有[必要Office 365 URL 和 IP 位址範圍](/office365/enterprise/urls-and-ip-address-ranges)來準備您的環境。 若要深入瞭解，請參閱Microsoft Teams 會議室檔準備[您的環境](rooms-prep.md)。

做為受管理服務的提供者，Microsoft 需要與組織中負責的專案關係人合作，讓會議室保持健康並從問題中復原。 您必須負責會議室中所使用的未經授權裝置、您選擇在會議室裝置中部署的協力廠商或自訂軟體，並遵守 Microsoft 建議的變更管理做法。

貴組織或貴組織所簽訂之合作夥伴必須準備好以補救服務偵測到的最後一個里程問題。

## <a name="customer-support"></a>客戶支援

Microsoft Teams 會議室管理的服務是透過專用Microsoft Teams 會議室受管理的服務 SOC 提供支援。 從Microsoft Teams 會議室受管理的服務 SOC 到客戶的主要通訊模式是透過Microsoft Teams 會議室管理的服務入口網站。 Microsoft Teams 會議室管理的服務 SOC 會針對系統或貴組織提出的票證提供第一個回應並適當上傳。

|  | 描述 |
|---------|---------|
|**範圍**   |與會議室應用程式、會議室裝置及周邊設備相關的事件會包含在Microsoft Teams 會議室受管理的服務支援中。|
|**觀眾**  |僅限 IT 作業         |
|**支援的語言**    |英語         |
|**透過入口網站提供支援**   | 24X7      |
|**Microsoft Teams 會議室受管理服務 SOC 的初始回應時間**   |  4 小時       |
|**在客戶閒置的情況下自動關閉票證**    |客戶發起的門票：3 個工作天<br>系統啟動票證：15 個工作天</li>   </ul>      |
|**重新導向至更廣泛的Microsoft 支援服務**   |與Microsoft Teams 會議室管理服務範圍之外的另一個 Microsoft 產品相關問題，會導向至適當的支援小組。         |
|**重新導向至原始設備製造商 (OEM) 支援**    |Microsoft Teams 會議室受管理的服務 SOC 可協助您瞭解問題，並在適用時協助協調 OEM 的支援。 在某些情況下，您可能必須直接與 OEM 互動。         |

此外，請記住，Microsoft Teams 會議室管理的服務 SOC 將協助貴組織中的 IT 營運，而不會取代使用者的技術支援中心服務。

如需Microsoft Teams 會議室受管理服務支援涵蓋範圍的詳細資訊，請參閱[支援包含和排除](#support-inclusions-and-exclusions)。

## <a name="supported-regions"></a>支援的地區

Microsoft Teams 會議室進階版授權可在下列國家/地區使用音訊會議：

- 澳大利亞、奧地利、比利時、巴西、保加利亞、加拿大、克羅埃西亞、賽普勒斯、捷克共和國、丹麥、愛沙尼亞、芬蘭、法國、德國、希臘、香港特別行政區、匈牙利、愛爾蘭、義大利、日本、拉脫維亞、立陶宛、盧森堡、馬爾他、墨西哥、摩納哥、荷蘭、紐西蘭、挪威、波蘭、葡萄牙、羅馬尼亞、俄羅斯、新加坡、斯洛伐克、斯洛維尼亞、南非、西班牙、瑞典、瑞士、臺灣、英國、美國。

在下列國家/地區中，Microsoft Teams 會議室進階版授權不含音訊會議：

- Andorra，布威島，聖誕島，科克斯 (基靈) 群島， 庫奇群島、法羅群島、斐濟、吉布拉托、格陵蘭、聽島和麥庫奇群島、冰島、印度、吉裡巴斯提、賴索托、列支敦斯登、澳門特別行政區、外延群島、密克羅尼西亞、紐魯、紐威島、諾裡安島、北馬來納群島、帕勞、巴布亞新邊界、皮索林群島、聖皮埃隆、薩摩亞、聖馬利諾、索羅群島、斯瓦濟蘭、托基羅 東加、吐瓦魯、萬那杜、教他市、瓦利斯及富圖那。

## <a name="support-inclusions-and-exclusions"></a>支援包含與排除


|包含的內容 |排除的內容 |排除專案的支援通道  |
|---------|---------|---------|
|**服務設定**   |         |         |
|Microsoft Teams 會議室 Windows設定的調查與指導方針，包括Microsoft Teams 會議室應用程式設定、第一方網路設定、第一方軟體設定，以及建議的電源設定和設定。     |任何不屬於「Microsoft Teams會議室」應用程式的協力廠商軟體設定和調查。 此外，任何需要在第一方視窗以外的設定，或Microsoft Teams會議室應用程式設定的 OEM 特定設定。         |協力廠商標準支援通道         |
|**會議室帳戶設定**    |         |         |
|會議室帳戶設定的可用檔指示。 針對登入或帳戶設定問題，會執行初始調查。     |執行建議的補救或執行建議補救的問題、協力廠商驗證提供者的設定，以及個人帳戶功能的設定。        |Microsoft 主要支援和協力廠商驗證提供者支援         |
|**周邊支援**    |         |         |
|初始調查會針對相機、麥克風和喇叭等通過認證的周邊設備執行。    |未透過套用標準設定或涉及協力廠商應用程式來解決的問題。         |OEM 支援         |
|**RMA 和裝置更換**   |         |         |
|初始調查會針對相機、麥克風和喇叭等通過認證的周邊設備執行。 系統會為 OEM 執行向 OEM 的 Handoff to OEM 支援，以履行更換或保固要求。    |採購、運送或安裝替換品。         |OEM 支援         |
|**根本原因分析**     |         |         |
|會議室體驗中所發生問題的所有根本原因，都會在可用時提供，但會以可以的方式執行。     |不包含協力廠商軟體問題的根本原因分析。         |協力廠商標準支援通道         |
|**會議體驗**   |         |         |
|我們正在調查Microsoft Teams 會議室應用程式會議設定中的問題，並視需要提供在裝置上正確設定服務品質的指導方針。      |此範圍不包含通話品質問題或網路相關問題的根本原因分析或調查。         |Microsoft 主要支援         |
|**補救、上線和進行中**     |         |         |
|客戶會收到指引，使其會議室符合上線時Microsoft Teams 會議室受管理服務建議的標準    |內部自訂軟體映射或不符合Microsoft Teams 會議室受管理服務之軟體版本建議指導方針的相關調查不在範圍中。   | |       

## <a name="data-collection"></a>資料收集

Microsoft Teams 會議室管理的服務會監視裝置、收集裝置資料，並允許 Microsoft 遠端存取和管理會議室裝置。 Microsoft Teams 會議室管理的服務無法存取會議的即時內容。

收集的資料是專為聊天室帳戶而非個別使用者。 個別使用者的附隨性參照可能會在使用裝置期間出現在活動記錄檔中。 收集的資料是由 Microsoft 依照 [Microsoft 隱私權聲明](https://aka.ms/privacy)進行處理。  

## <a name="related-topics"></a>相關主題

- [Microsoft Teams 會議室受管理的服務](https://rooms.microsoft.com)
