...
貴組織或貴組織所簽約的合作夥伴必須準備修正服務檢測到的最後一個英里問題。
...
|包括的內容 |已排除的專案 |已排除的專案支援通道  |
|---------|---------|---------|
|**服務設定**   |         |         |
|Microsoft 團隊聊天室 Windows 設定的調查與指導方針，包括 Microsoft 團隊聊天室 app 設定、第一方網路設定、第一方軟體設定，以及建議的電源設定和設定。     |未直接納入 Microsoft 團隊聊天室 app 的任何協力廠商軟體設定與調查。 此外，也就是需要第一方 windows 以外的任何 OEM 專用設定，或 Microsoft 團隊聊天室 app 設定。         |協力廠商標準支援通道         |
|**房間帳戶設定**    |         |         |
|在要求時，提供給房間帳戶設定的可用檔的方向。 針對登入或帳戶設定問題，會執行初始調查。     |執行建議的修正，或執行建議修正的問題、協力廠商驗證提供者的設定，以及個別帳戶功能的配置。        |Microsoft 主流支援與協力廠商驗證提供者支援         |
|**週邊支援**    |         |         |
|針對相機、麥克風和喇叭等已驗證的外設執行初始調查。    |無法套用標準配置或涉及協力廠商應用程式的問題。         |OEM 支援         |
|**RMA 與裝置替換**   |         |         |
|針對相機、麥克風和喇叭等已驗證的外設執行初始調查。 已針對 OEM 執行到 OEM 支援的移交，以履行更換或保修要求。    |更換的採購、裝運或安裝。         |OEM 支援         |
|**根本原因分析**     |         |         |
