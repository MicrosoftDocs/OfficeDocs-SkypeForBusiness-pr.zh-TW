...
本文將討論如何使用設定及部署裝置的整合端對端管理 :::no-loc text="Microsoft Teams Rooms"::: :::no-loc text="Azure Monitor"::: 。

您可以 :::no-loc text="Log Analytics"::: 在內部 :::no-loc text="Azure Monitor"::: 進行設定，以提供可協助您管理會議室裝置的基本遙測及警示 :::no-loc text="Microsoft Teams Rooms"::: 。 隨著您的管理解決方案逐漸成熟，您可能會決定要部署其他資料和管理功能來建立更詳細的裝置可用性與效能的視圖。
...
2. [針對管理設定設定測試裝置 :::no-loc text="Log Analytics":::](azure-monitor-deploy.md#configure_test_devices)
...
> [!IMPORTANT]
> 雖然使用最低的設定，但 :::no-loc text="Azure Monitor"::: :::no-loc text="Log Analytics"::: 可以監視執行作業系統的電腦，但在 :::no-loc text="Windows"::: :::no-loc text="Microsoft Teams Rooms"::: 開始部署代理程式到所有裝置之前，仍需要採取一些特定步驟 :::no-loc text="Microsoft Teams Rooms"::: 。
...
> [!NOTE]
> 若要搭配使用 :::no-loc text="Log Analytics"::: :::no-loc text="Azure Monitor"::: ，您必須擁有有效的 :::no-loc text="Azure"::: 訂閱。 如果您沒有 :::no-loc text="Azure"::: 訂閱，您可以建立 [免費試用訂閱](https://azure.microsoft.com/free) 作為起始點。
...
:::no-loc text="Log Analytics"::: 僅收集 :::no-loc text="Windows"::: 設定中指定的事件記錄中的事件。 針對每個記錄，只會收集已選取嚴重性的事件。
...
若要設定 :::no-loc text="Log Analytics"::: 收集 :::no-loc text="Microsoft Teams Rooms"::: 事件，請參閱[ :::no-loc text="Windows"::: 中的 :::no-loc text="Azure Monitor"::: 事件記錄資料來源](https://docs.microsoft.com/azure/azure-monitor/platform/data-sources-windows-events)

![事件記錄檔設定的螢幕擷取畫面](../media/Deploy-Azure-Monitor-2.png "事件記錄檔設定")

> [!IMPORTANT]
> 設定 :::no-loc text="Windows"::: 事件記錄的設定，並 **:::no-loc text="Skype Room System":::** 以事件記錄名稱的形式輸入，然後選取 [ **錯誤**]、[ **警告**] 及 [ **資訊** ] 核取方塊。

## <a name="configure-test-devices-for-azure-monitoring"></a>針對 Azure 監視設定測試裝置
<a name="configure_test_devices"> </a>

您必須準備好 :::no-loc text="Log Analytics"::: 監視 :::no-loc text="Microsoft Teams Rooms"::: 相關事件。 首先，您必須將 :::no-loc text="Microsoft Monitoring"::: 代理部署到一個或兩個 :::no-loc text="Microsoft Teams Rooms"::: 您有實際存取權的裝置，並讓這些測試裝置產生一些資料並將它推入 :::no-loc text="Log Analytics"::: 工作區。

### <a name="install-no-loc-textmicrosoft-monitoring-agents-to-test-devices"></a>安裝 :::no-loc text="Microsoft Monitoring"::: 代理程式以測試裝置

:::no-loc text="Microsoft Monitoring":::使用[[將 :::no-loc text="Windows"::: 電腦連線至 :::no-loc text="Log Analytics"::: :::no-loc text="Azure"::: 服務]](https://docs.microsoft.com/azure/azure-monitor/platform/agent-windows)中提供的指示，將代理部署到測試裝置。 本文提供有關部署代理程式之步驟的詳細資訊，以及如何 :::no-loc text="Microsoft Monitoring"::: :::no-loc text="Windows"::: 取得 :::no-loc text="Log Analytics"::: ***工作區識別碼*** 和 ***主鍵*** 以取得 :::no-loc text="Microsoft Teams Rooms"::: 連接到您部署的裝置， :::no-loc text="Azure Monitor"::: 以及驗證代理程式連線至實例的相關步驟 :::no-loc text="Log Analytics"::: 。
...
> [!NOTE]
> 安裝完代理程式後重新開機裝置 :::no-loc text="Microsoft Monitoring"::: ，並確定 :::no-loc text="Microsoft Teams Rooms"::: 會議 app 已啟動，讓它能夠在事件記錄中產生新的事件。

1.  登入[ :::no-loc text="Microsoft Azure"::: 入口網站](https://portal.azure.com)，然後前往 :::no-loc text="Log Analytics"::: 並選取您的工作區。

2.  列出裝置產生的心跳事件 :::no-loc text="Microsoft Teams Rooms"::: ：
    1.  選取您的工作區並移至 [ **記錄** ]，然後使用查詢來檢索將有自訂欄位的心跳記錄 :::no-loc text="Microsoft Teams Rooms"::: 。
    2.  範例查詢： `Event | where Source == "SRS-App" and EventID == 2000`

3.  確認查詢傳回包含會議 app 所產生之事件的記錄記錄 :::no-loc text="Microsoft Teams Rooms"::: 。

4.  產生硬體問題，並驗證所需的事件已登入 :::no-loc text="Azure Log Analytics"::: 。
    1.  在測試系統上拔掉其中一個週邊裝置 :::no-loc text="Microsoft Teams Rooms"::: 。 這可能是相機、免提、麥克風或前置房間顯示器
...
5.  產生應用程式問題，並驗證所需的事件已記錄。
    1.  [修改 :::no-loc text="Microsoft Teams Rooms"::: 應用程式設定]，然後輸入不正確的會話初始通訊協定 (SIP) 位址/密碼對。
    2.  等待10分鐘，以填入事件記錄檔 :::no-loc text="Azure Log Analytics"::: 。
...
2. 列出裝置產生的事件 :::no-loc text="Microsoft Teams Rooms"::: ：
...
5. 使用 *資料表 1*中所示的對應。 :::no-loc text="Log Analytics":::當您定義新欄位時，會自動附加** \_ CF**字串。

> [!IMPORTANT]
> 請記住，所有 JSON 和 :::no-loc text="Log Analytics"::: 欄位都區分大小寫。
> 
...
收集資料並對應自訂欄位之後，您可以使用 [視圖設計工具] 來開發包含各種磚的儀表板，以監視 :::no-loc text="Microsoft Teams Rooms"::: 事件。 使用 [視圖設計工具] 建立下列磚。 如需詳細資訊，請參閱[使用視圖設計 :::no-loc text="Log Analytics"::: 工具建立自訂視圖](https://docs.microsoft.com/azure/azure-monitor/platform/view-designer)
...
:::no-loc text="Azure Monitor"::: 當主控台遇到問題時，可以提出通知來通知系統管理員 :::no-loc text="Microsoft Teams Rooms"::: 。
...
-   在服務中啟動 runbook :::no-loc text="Azure Automation":::

請參閱[中 :::no-loc text="Azure Monitor"::: 的記錄警示](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-unified-log)，以深入瞭解中的警示 :::no-loc text="Azure Monitor"::: 。

> [!NOTE]
> 下列範例會在 :::no-loc text="Microsoft Teams Rooms"::: 裝置產生硬體或應用程式錯誤時，傳送電子郵件警示。

### <a name="configure-an-email-alert-for-no-loc-textmicrosoft-teams-rooms-hardware-issues"></a>針對硬體問題設定電子郵件通知 :::no-loc text="Microsoft Teams Rooms":::

設定警報規則，檢查 :::no-loc text="Microsoft Teams Rooms"::: 在過去一個小時內遇到過硬體問題的裝置。
1.  登入[ :::no-loc text="Microsoft Azure"::: 入口網站](https://portal.azure.com)，然後前往 :::no-loc text="Log Analytics"::: 並選取您的工作區。

2. 流覽至您的 :::no-loc text="Log Analytics"::: 工作區並選取 [**通知**]，然後選取 [**新增通知規則**]。
...
### <a name="configure-an-email-alert-for-no-loc-textmicrosoft-teams-rooms-application-issues"></a>針對應用程式問題設定電子郵件通知 :::no-loc text="Microsoft Teams Rooms":::
...
![示範 :::no-loc text="Azure Monitor"::: 通知電子郵件] (。/media/Deploy-Azure-Monitor-6.png 「 :::no-loc text="Azure Monitor"::: 通知電子郵件的範例」 ) 

## <a name="configure-all-devices-for-no-loc-textazure-monitoring"></a>設定所有裝置 :::no-loc text="Azure Monitoring":::
<a name="configure_all_devices"> </a>在儀表板和通知設定之後，您可以 :::no-loc text="Microsoft Monitoring"::: 在所有裝置上設定和設定代理程式， :::no-loc text="Microsoft Teams Rooms"::: 以完成您的監視部署。

雖然您可以 :::no-loc text="Microsoft Monitoring"::: 在每個裝置上手動安裝及設定代理，但我們強烈建議您利用現有的軟體部署工具和方法。

如果您是 :::no-loc text="Microsoft Teams Rooms"::: 第一次建立裝置，您可能會想要在 :::no-loc text="Microsoft Monitoring"::: 組建程式中包含代理設定和設定步驟。 如需詳細資訊，請參閱 [使用命令列安裝代理程式](https://docs.microsoft.com/azure/azure-monitor/platform/agent-windows#install-the-agent-using-the-command-line)。

### <a name="deploying-no-loc-textmicrosoft-monitoring-agent-by-using-a-group-policy-object-gpo"></a>:::no-loc text="Microsoft Monitoring":::使用群組原則物件 (GPO) 部署代理程式

如果您在 :::no-loc text="Microsoft Teams Rooms"::: 實施前已部署裝置 :::no-loc text="Azure Monitoring"::: ，您可以使用提供的腳本來設定和設定代理程式，方法是使用 :::no-loc text="Active Directory"::: 群組原則物件。

1.  建立共用的網路路徑，並授予 [ **網域電腦** ] 群組的 [讀取存取權]。

2.  下載64位版本的 :::no-loc text="Microsoft Monitoring"::: 代理程式， :::no-loc text="Windows"::: 從 <https://go.microsoft.com/fwlink/?LinkID=517476>
...
5.  設定 PowerShell 執行原則：
    1.  編輯新建立的 [群組原則] 物件，然後流覽至 [電腦設定 \\ 原則] \\ 管理範本 \\ :::no-loc text="Windows"::: 元件 \\:::no-loc text="Windows PowerShell":::
    2.  啟用 [ **開啟腳本執行** ]，並將 **執行原則** 設定為 [ **允許本機腳本**]。

6.  設定啟動腳本：
    1.  複製下列腳本並將其儲存為 Install-MMAgent.ps1。
    2.  修改 WorkspaceId、WorkspaceKey 和 SetupPath 參數，以符合您的配置。
    3.  編輯相同的群組原則物件，然後流覽至 [電腦 \\ 設定原則] \\ :::no-loc text="Windows"::: 設定 \\ 腳本 (啟動/關閉) 
    4.  按兩下以選取 [ **啟動**]，然後選取 [ **PowerShell 腳本**]。
    5.  選取 [ **顯示**檔案]，然後將 **Install-MMAgent.ps1** 檔案複製到該資料夾。
    6.  選取 [ **新增**]，然後按一下 **[流覽]**。
    7.  選取您剛才複製的 ps1 腳本。

7.  :::no-loc text="Microsoft Teams Rooms"::: 裝置應該 :::no-loc text="Microsoft Monitoring"::: 在第二次重新開機時安裝並設定代理程式。
...
> [!NOTE]
> Agent 健康方案可協助您找出您的環境中過時或損毀的 :::no-loc text="Microsoft Monitoring"::: 代理程式，且通知管理解決方案會提供在指定期間內所產生之警報的詳細資料。
