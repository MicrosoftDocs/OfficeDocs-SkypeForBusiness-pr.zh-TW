...
- 在會議Teams 會議室中設定裝置並連接您需要的 (請參閱您一組裝置之 OEM 檔) 
...
**Pro提示**- 如果您必須使用 Proxy 伺服器提供Teams，請首先 [閱讀本文](../proxy-servers-for-skype-for-business-online.md)。 當要處理Microsoft Teams伺服器即時媒體流量時，建議您完全忽略 Proxy 伺服器。 Microsoft Teams流量已加密，因此 Proxy 伺服器不會使其更安全。 在更廣泛的部署中，建議您遵循準備網路Teams頻寬規劃及評估網路[](../prepare-network.md)適合即時流量的指南。
...
-   如果加入 Teams會議室Azure Active Directory，請Azure AD具有動態成員資格的群組，以保留所有 Teams 會議室 資源帳戶。 這將會簡化未來的管理，例如適用條件式 Access 原則。
-   如果加入 Teams 會議室至 Active Directory，請準備組織單位和 Active Directory 群組以保留您的 Microsoft Teams 會議室 電腦和服務帳戶，以及準備群組原則物件 (GPO) 以啟用 PowerShell 重算。
...
定義內部部署 Active Directory 或 Azure Active Directory群組，以新增Microsoft Teams 會議室使用者 (的所有) 服務。 如果您使用 Azure Active Directory，請考慮使用動態群組來自動新增和移除群組中的資源帳戶。
...
|  &nbsp;  |   &nbsp;  |
|-----------|------------|
| ![管理網站部署。](../media/audio_conferencing_image7.png) <br/>決策點|<ul><li>決定誰將管理網站部署。</li><li> 找出將安裝網站Microsoft Teams 會議室的資源，並承擔組組和測試。</li></ul>| 
...
每個 Microsoft Teams 會議室 系統已實際部署並連接支援的周邊裝置之後，您必須設定 Microsoft Teams 會議室 應用程式，以指派 Microsoft Teams 會議室 資源帳戶和密碼，Teams 會議室 才能登錄Microsoft Teams或商務用 Skype，然後Exchange。

您可以手動設定每個Microsoft Teams 會議室系統。 或者，您可以使用集中儲存的 xml 設定檔Microsoft Teams 會議室管理應用程式設定。
...
在部署中，您想要使用會議室名稱、Microsoft Teams 會議室、已登錄 Microsoft Teams 會議室 資源帳戶，以及指派的周邊裝置來更新資產註冊。 
