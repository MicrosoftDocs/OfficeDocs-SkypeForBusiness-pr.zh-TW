...
-   必須安裝適用于 Windows 10 (ADK) 的 Windows 評估版和部署套件支援版本。 請參閱您可以搭配不同版本 Configuration Manager 使用的 [Windows 10 ADK](https://docs.microsoft.com/configmgr/core/plan-design/configs/support-for-windows-10#windows-10-adk) 版本，並確保您的部署包含正確的版本。

-   網站系統伺服器必須已獲指派發佈點角色，且應啟用啟動影像以進行預先啟動 [執行環境 (PXE) 支援](https://docs.microsoft.com/configmgr/osd/deploy-use/use-pxe-to-deploy-windows-over-the-network) ，以啟用網路啟動的部署。 如果未啟用 PXE 支援，您可以針對您的部署使用 [可引導媒體](https://docs.microsoft.com/configmgr/osd/deploy-use/use-bootable-media-to-deploy-windows-over-the-network) 。

-   網路存取帳戶必須設定為支援 (裸機) 部署案例的新電腦。 若要深入瞭解網路存取帳戶的設定，請參閱 [Configuration Manager 中使用的帳戶](https://docs.microsoft.com/configmgr/core/plan-design/hierarchy/manage-accounts-to-access-content#bkmk_NAA)。
...
-   您的網路應該擁有動態主機設定通訊協定 (DHCP) server，並將自動 IP 位址發佈設定為要部署 Microsoft 團隊會議室單元的子網。
...
-   您的網路（包括交換器和虛擬 Lan (Vlan) ）應該設定為支援 PXE。 如需 IP 協助程式和 PXE 設定的詳細資訊，請參閱您的網路廠商。 或者，如果沒有啟用 PXE 支援，您也可以將 [可引導媒體](https://docs.microsoft.com/configmgr/osd/deploy-use/use-bootable-media-to-deploy-windows-over-the-network) 用於部署。
    ...
    > [!NOTE]
    > 在 Surface Pro 裝置上，只有當您使用乙太網路介面卡或來自 Microsoft 的插接站時，才會支援從網路啟動 (PXE 啟動) 。 協力廠商乙太網路介面卡不支援使用 Surface Pro 進行 PXE 啟動。 如需詳細資訊，請參閱 [乙太網路介面卡與 Surface 部署](https://docs.microsoft.com/surface/ethernet-adapters-and-surface-device-deployment) 。
...
使用下列指示來確認作業系統部署 (的 OSD) 功能是否已正確設定。
...
4.  選取 [ **選用元件** ] 索引標籤來安裝必要的元件：
    ...
    1.  選取星形圖示，然後搜尋 **HTML (WinPE-HTA)**

    2.  選取 **[確定]** ，將 HTML 應用程式支援新增至啟動影像。

5.  *選用：* 若要自訂部署體驗，請選取 [ **自訂** ] 索引標籤。
    -   啟用 **命令支援 (僅測試)** 如果您想要在部署期間存取命令提示字元，請使用此選項。 啟用此功能時，您可以在部署期間的任何時間選取 **F8** 來啟動命令提示字元。
    -   您也可以指定要在部署期間顯示的自訂背景圖像。 若要設定影像，請啟用 **[ (UNC 路徑指定自訂背景圖像** 檔案]，然後選取您的背景。
...
| SRS v2-Sysprep 套件             | 軟體套件       | [自訂 Unattended.xml 的封裝] 來設定 Microsoft 團隊會議室單位            |
| SRS v2-Set-SRSComputerName 套件 | 軟體套件       | HTML 應用程式 (HTA 的封裝) 在部署期間指派電腦名稱稱    |
...
| Windows 10 企業版                | 作業系統影像 |  (安裝 .wim) 中的作業系統安裝檔套件                          |
...
1. 從下載監視代理程式 <https://go.microsoft.com/fwlink/?LinkId=828603> 。

2. 在命令提示字元中開啟 [命令提示字元] 視窗，然後輸入 **MMASetup-AMD64.exe/c：** ，將套件解壓縮至 [ **SRS V2-Microsoft Monitoring Agent 套件**] 資料夾。
...
1. 在 [ **SRS v2-OS 更新套件** ] 資料夾中，建立名為 **Install-SRSv2-OS-Updates.ps1** 的新 PowerShell 腳本。

2. 將下列腳本複製到 **Install-SRSv2-OS-Updates.ps1** 腳本中。 或者，您也可以從 [這裡](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Skype/SfbOnline/downloads/Skype-Room-Systems-v2/SRS-v2-Configuration-Manager-Files.zip?raw=true)下載 Install-SRSv2-OS-Updates.ps1 腳本。
   ```
...
1.  從下載最新版本的 **Microsoft 團隊聊天室部署套件** <https://go.microsoft.com/fwlink/?linkid=851168> ，然後將其安裝到工作站。

2.  從 C： Program 檔案將內容複寫到 [ **SRS v2-srs] 應用程式** 套件資料夾， **\\ (X86) \\ Skype 室系統部署套件** 中。
...
4.  輸入下列資訊來建立套件：
    ...
    -   選取 [ **此套件包含來源** 檔案] 核取方塊，輸入 **SRS V2-Set-SRSComputerName 套件** 資料夾的路徑，然後選取 **[下一步]**。
...
2. 將下列文字複製到 **Unattend.xml** 檔案中。 或者，您也可以從 [這裡](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Skype/SfbOnline/downloads/Skype-Room-Systems-v2/SRS-v2-Configuration-Manager-Files.zip?raw=true)下載 Unattend.xml 檔案。
   ```XML
...
4.  選取 **[匯入下列網路路徑中的所有驅動程式] (UNC)**]，選取 [來源] 資料夾 (例如，C： \\ _Sources \\ 驅動程式 \\ Surface Pro) ，然後選取 **[下一步]**。
...
11. 移至 **軟體庫** \> **作業系統** \> **驅動程式**，選取 **資料夾的 [ \> 建立資料夾**]，然後輸入與您剛匯入驅動程式的 Surface Pro 模型相符的資料夾名稱。
...
1.  發佈軟體套件。
    ...
    3.  根據您的 Configuration Manager 階層) 到清單，然後選取 **[下一步]**，以新增所有發佈點伺服器 (或發佈點群組。
...
3.  選取 **[流覽]**，移至您在步驟1中使用的共用資料夾位置，選取 [ **Microsoft 團隊聊天室] 部署 (en-us) .zip** 檔案，然後選取 **[下一步]**。
...
2. 逐步執行每個步驟，並完成建議的更新：
   ...
   4. 套用 **作業系統**：此步驟指定要部署的作業系統映射，以及要使用的無人參與 Sysprep 答案檔案。
      ...
      -  確認已啟用 [ **針對自訂安裝使用無人參與或 Sysprep 答案** 檔案]，並且已選取 [ **SRS V2-Sysprep 套件** ]。 另請確認 **[檔案名] 已設定** 為 [ **unattend.xml**]。

   5. 套用 **Windows 設定**：此步驟會收集 Windows 安裝的相關資訊。
      -  視您的) 需求而定，提供授權及註冊資訊，包括產品金鑰、當地系統管理員帳戶密碼和時區 (。
...
-   在 Configuration Manager 管理點 (MP) 記錄目錄中的 [Smspxe] （**記錄**）
