...
- 3到6英尺 (0.9 – 1.8 m) 寬-支援
-  (1.8 – 2.7 m) 寬（建議使用）
-  (2.7 –12英尺) 寬-支援
- 超過12英尺。 (3.6 m) 寬-相機涵蓋9–12英尺 (2.7-3.6 m) 並裁掉其餘部分。

## <a name="camera-location"></a>相機位置

內容相機的理想位置是在白板上垂直或水準居中。 本機組建程式碼可能會有高度限制，必須比白板頂端更高的相機升高。

您最多可以將相機安裝到6。  (152 mm) 高於白板頂端，並以白色面板為中心（如圖所示）。 請確定相機影像包含至少6個。 在水準方向 (152 mm) 框線。 您可以在 Microsoft 團隊聊天室 app 中使用相機預覽來判斷相機的最終位置。
...
| 相機 HFoV |3英尺 0.91 (m)      | 6英尺 (1.8 m)     |  (2.74 m)         |12英尺。 (3.65 m)          | 從白板最大距離  |
|:---         |:---               |:---                |:---                 |:---             | :--- |
...
[內容相機] 與 [白板] 上所安裝的牆之間的距離，取決於該模型的 HFoV，也就是不同的。 以較大的 HFoV 安裝攝影機 (120 度（例如) 靠近牆的位置），以及 HFoV 距離牆遠較窄的攝影機。 在您開始安裝選取的相機之前，請先檢查 HFoV。

如果您的白板大於12英尺. (3.65 m) 或沒有角落 (例如完整的牆白板) ，您可以將相機放在中間的任何位置。 如果找不到 [白板角落]，增強軟體會選取中間的區域。
...
3.  (選用) 如果相機是從天花板上掛載，請將其倒置安裝，請核取 [ **旋轉內容相機 180** ] 選項。
4. 選取 [ **儲存並**結束]。
