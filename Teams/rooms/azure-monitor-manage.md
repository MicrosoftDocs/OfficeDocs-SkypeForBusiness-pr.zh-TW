...
您可以設定 Azure 監視器以提供基本遙測，協助管理Microsoft Teams裝置。 請參閱[規劃Microsoft Teams 會議室 Azure 監視器](azure-monitor-plan.md)進行管理，以及使用 Azure 監視器[Microsoft Teams 會議室部署管理](azure-monitor-deploy.md)，以瞭解詳細資料。 隨著您的管理解決方案成熟，您可以使用額外的資料和管理功能來建立更詳細的裝置績效視圖。

## <a name="understand-the-log-entries"></a>瞭解記錄專案

下列事件描述會每隔五分鐘插入事件記錄描述欄位，當 Microsoft Teams 會議室 應用程式在 Windows 記錄中記錄Windows資訊。 此Microsoft Monitoring Agent會將這些記錄傳遞至 Azure 監視器中的記錄分析，將記錄剖析到您使用 Azure 監視器部署Microsoft Teams 會議室[管理中建立儀表板](azure-monitor-deploy.md)。 有了儀表板，您可以查看個別的記錄專案，以判斷動作路線 。如果需要的話。

事件 ID 2000 和 3000 表示該裝置如預期一樣工作。 事件 2001 和 3001 表示找到問題。 如果事件識別碼 4000 比您設定比較基準或貴組織中其他已部署裝置看到的事件 ID 4000 比您預期多，可能需要採取動作。
