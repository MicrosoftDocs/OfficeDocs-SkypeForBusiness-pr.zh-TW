...
您可以將 Azure 監視器設定為提供基本遙測，以Microsoft Teams會議室裝置。 請參閱[使用 azure 監視器Microsoft Teams 會議室](azure-monitor-plan.md)管理，以及使用 Azure 監視器[Microsoft Teams 會議室部署管理，以瞭解](azure-monitor-deploy.md)詳細資料。 隨著監控解決方案的成熟，您可以使用額外的資料和監控功能，建立更詳細的裝置績效視圖。

## <a name="understand-the-log-entries"></a>瞭解記錄專案

下列事件描述會每隔五分鐘插入事件記錄描述欄位，當 Microsoft Teams 會議室應用程式在 Windows記錄中記錄對應資訊時。 此Microsoft Monitoring Agent會將這些記錄傳遞至 Azure 監視器中的記錄分析，將記錄剖析到您使用 Azure 監視器部署Microsoft Teams 會議室[監控所建立儀表板](azure-monitor-deploy.md)。 有了儀表板，您可以查看個別的記錄專案，以判斷動作路線 。如果需要的話。
