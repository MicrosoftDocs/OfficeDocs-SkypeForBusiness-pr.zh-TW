...
- 在會議室系統裝置SKYPE電腦加入網域，可讓您授予網域使用者和群組管理許可權。 如此一來，您就不需要記住本機電腦層級系統管理員帳戶密碼。
- 當您將 Skype 會議室系統裝置電腦加入網域時，您必須建立個別的組織單位 (OU) ，這樣您才能將群組原則物件 (GPO) 排除功能提供給所有 Skype 會議室系統電腦物件所在的 OU。 當您這麼做時，在 OU 中建立電腦物件，Skype Room System 裝置電腦加入網域。
...
- 或者，您可能會決定將裝置電腦留在工作組中。 與桌上型電腦Microsoft Teams或商務用 Skype用戶端一樣，這要求您在會議室系統裝置 PC 上手動Skype根憑證鏈。 如果您的部署是使用公用憑證鏈， (例如委託、VeriSign 等等，則不需要將根憑證鏈) 。 
