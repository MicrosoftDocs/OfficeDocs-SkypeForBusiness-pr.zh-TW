...
您可以在貴組織的一或多個裝置上變更設定。 若要變更設定，請選取您想要管理的裝置，然後 **選取編輯** 設定。 新的窗格會開啟，並包含您可以在裝置上變更的所有設定。 下表列出您可以使用系統管理中心Teams設定。 某些設定只有在您選取單一裝置時才能使用。
...
- **排定的重新開機** 如果您選擇這個選項，您可以重新開機您進行變更的裝置，同時對貴組織的影響較小。
...
當您 **移除** 裝置時，裝置會從貴組織移除，而且不會再出現在系統管理中心Teams 會議室裝置Teams清單中。 

當您 **封鎖** 裝置時，Teams不再與裝置通訊。 封鎖的裝置即使包含在大量編輯的一組裝置中，也不會送出命令。 它仍然會列在狀態為已封鎖Teams 會議室的裝置 **清單中。**

無論裝置是否受到封鎖或移除，如果裝置仍以有效的使用者名稱和密碼進行配置，如果裝置已連接到 Teams 會議室，系統就會自動重新新增到 Teams 會議室 裝置清單中Microsoft 365。
...
1. 前往 **裝置**  >  **Teams 會議室，** 然後選取要封鎖的裝置名稱。
1. 在裝置詳細資料頁面上 **，選取頁面** 右上角的動作。
1. 選取 **封鎖**。
