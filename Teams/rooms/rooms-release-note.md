...
- 聊天室使用者介面的主題和正面改善
- TLS 1.2 用戶端支援。 針對內部部署客戶，在 Microsoft 團隊聊天室啟用經由 TLS 1.2 的通訊需要商務用 Skype Server 2015 累加更新9（CU9）或商務用 Skype Server 2019 累加更新1（CU1）。
...
- 修正當地語系化 EULAs （特別是挪威文）的格式設定問題，避免在 EULA 中繼續進行
...
- 透過 Intune 與 Configuration Manager （MDM），以 Windows 10 企業版裝置進行監控與管理
- 企業級的可靠性
