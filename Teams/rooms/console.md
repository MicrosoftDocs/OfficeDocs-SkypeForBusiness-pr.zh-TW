...
您必須執行這些步驟，Microsoft Teams或商務用 Skype帳戶Exchange已建立和測試 ，如部署帳戶[Microsoft Teams 會議室。](rooms-deploy.md) 您需要以下說明的硬體和軟體Microsoft Teams 會議室[需求](requirements.md)。 本主題包含下列各節：
...
系統關閉之後，可以安全地移除 USB 設定磁片。 此時，如果您使用固定式產品 (，您可以將目標裝置放在其固定位置) 附加會議室所需的周邊設備，然後連接至網路。 請參閱製造商指示。
...
1. 系統會顯示使用者帳戶畫面。 輸入 Microsoft Exchange資源帳戶的登錄位址 (，user@domain) 會議室帳戶與主機一起使用。
...
<a name="Certs"> </a>
> [!NOTE]
> 只有在將使用者連接到Teams 會議室，商務用 Skype。

Microsoft Teams 會議室必須信任所連接之伺服器所使用的憑證。 如果憑證頒發機構是私人的，例如使用 Active Directory 和 Windows 憑證頒發機構進行內部部署，您可以用幾種方法將憑證新Microsoft Teams 會議室新到 Microsoft Teams 會議室：
...
例如，如果您的完全限定網域 redmond.corp.microsoft.com 且您想要您的 Microsoft Teams 會議室 主機位於 「Microsoft Teams 會議室」 OU 中，且該 OU 為 「資源」OU 的子級，則命令為：
