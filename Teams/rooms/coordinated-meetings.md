...
當您在會議室中設定協調會議體驗時，您必須告知Teams 會議室會議室中的裝置和 Surface Hub，以與哪些裝置進行協調。 這是在 UPN 中新增使用者主體名稱 (UPN) 其應該協調的裝置名稱來完成。 如果您不知道要設定為協調會議之每個裝置使用的 UPNs，您可以使用 Microsoft 365 系統管理中心。 
...
| **啟用視像**      | 決定會議參與者是否可以開啟或關閉相機。 您可以在活動參與者想要共用不同視 (，例如參與者使用白板Surface Hub裝置 `true`) 。 如果您不希望參與者在裝置上開啟或關閉相機，請將其設定為 `false` 。<p> 如果 **視音訊預設值** 設定為 `true` ，此設定會被忽略，參與者可以開啟或關閉相機。         |
...
| **信任的帳戶**   | 這是每個會議室裝置或 Teams 裝置應接受會議加入邀請Surface Hub或應寄到哪些會議加入邀請的 UPN 逗號分隔清單。 |

在後續的欄，新增每個Teams 會議室裝置和 Surface Hub。 在每一欄中，填寫對應到您想要的會議室體驗的值。 以下是一個裝置和Teams 會議室裝置Surface Hub：
...
5. 將檔案放在Teams 會議室資料夾中 `C:\Users\Skype\AppData\Local\Packages\Microsoft.SkypeRoomSystem_8wekyb3d8bbwe\LocalState` 。 您可以用幾種方法執行此工作：
    ...
    - **將檔案複製到您的** Teams 會議室您必須啟用檔案共用並建立網路共用，才能將檔案複製到您的裝置。 完成之後，您可以連接到網路共用，然後將檔案複製到裝置。 詳細資訊，請參閱Microsoft Teams 會議室[及作業。](../rooms/rooms-operations.md)
    - **使用群組原則** 建立群組原則以將檔案複製到裝置。 詳細資訊，請參閱 [群組原則概觀](/previous-versions/windows/it-pro/windows-server-2012-r2-and-2012/hh831791(v=ws.11))。
    - **在裝置上** 下載Teams 會議室 您可以使用系統管理模式登入裝置，然後將檔案從網路共用或 USB 磁碟機複製到裝置。 詳細資訊，請參閱 [切換到系統管理模式](../rooms/rooms-operations.md#switching-to-admin-mode-and-back-when-the-microsoft-teams-rooms-app-is-running)。
...
4. Windows組組設計工具，XML 必須位於單一行。 移除每一行之間的所有分行符號，讓 XML 看起來像這樣：
...
建立 XML 組設定檔之後，請使用 Microsoft Teams 上的管理Surface Hub設定[中的步驟，以](surface-hub-manage-config.md)將檔案Surface Hub Surface Hubs。