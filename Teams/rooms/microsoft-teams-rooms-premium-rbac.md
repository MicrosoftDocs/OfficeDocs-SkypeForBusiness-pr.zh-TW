...
description: 瞭解使用受管理之服務Microsoft Teams 會議室存取控制。
f1keywords: ''
...
- 全域系統管理員Azure Active Directory (Azure AD) 
- 透過受管理服務入口網站Microsoft Teams 會議室服務管理員
...
如果內建角色不適合您的組織需求，您可以建立角色並設定其許可權。 若要建立角色，您必須是全域系統管理員或 Managed Service 系統管理員。 

1. 在受管理服務入口網站左側Microsoft Teams 會議室，**請前往** 設定  >  **角色**。
2. 選取 **建立角色**。
3. 在一 **般設定** 頁面的角色 **屬性下**，輸入此角色的名稱。 在 **描述** 下，輸入此角色的詳細資訊。 選擇下 **一個**。
4. 在許可權 **頁面上** 的角色 **許可權** 下，選取適當的核取方塊以選擇此角色的許可權。 選擇 **下** 一步以為此角色建立第一個作業。
5. 在作業 **頁面上** 的作業 **屬性** 下，輸入此作業的名稱。 描述為選擇性。 如果 **此** 角色的使用者應該收到來自此作業範圍之聊天室服務的電子郵件通知，請在通知設定下選取電子郵件通知核取方塊。  選擇下 **一個**。
6. 在成員 **頁面上** 的搜尋使用者或安全性群組方塊中，輸入您租使用者中要提供許可權的使用者或安全性群組名稱，然後完成選取專案。 選擇下 **一個**。 
7. 在範圍 **頁面上** 的搜尋會議室或會議室群組方塊中，輸入允許使用者管理的會議室或會議室群組名稱。 選擇下 **一個**。
8. 在完成 **頁面上** ，查看角色和作業的詳細資訊。 如果您對組組感到滿意，請選擇 新增 **角色**。 如果您想要編輯節，請使用上一 **個按鈕，** 或選取左側導圖中的步驟。  

## <a name="assign-a-role"></a>指派角色

若要指派角色，您必須是全域系統管理員或 Managed Service 系統管理員，或具有角色管理許可權的角色。
...
4. 在一 **般設定** 頁面的作業 **屬性** 下，輸入此作業的名稱。 描述為選擇性。 在 **通知設定** 下，選取此角色的使用者應收到來自此作業範圍之聊天室之服務的電子郵件通知，請選取電子郵件通知核取方塊。  選擇下 **一個**。 
