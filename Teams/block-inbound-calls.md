...
電話系統直接路由和通話方案支援封鎖從公用交換電話網絡 (PSTN) 。 此功能可定義租使用者全域號碼模式清單，以便針對相符專案清單檢查每個傳入 PSTN 呼叫的來電者識別碼。 如果相符，來電會遭到拒絕。
...
您可以使用新增、取得、設定、**移除**     - **CsInboundBlockedNumberPattern** Cmdlet 來管理數位模式。 您可以使用這些 Cmdlet 來管理一個給定的模式，包括切換啟用給定模式的能力。
...
檢視及啟用整個通話封鎖功能是由取得、設定 ****  - **CsTenantBlockingCallingNumbers** Cmdlet 管理。
...
如果身分識別未知，請使用 **Get-CsInboundBlockedNumberPattern** Cmdlet 來先找出正確的模式，並記下身分識別。 接著，執行 **Remove-CsTenantBlockedNumberPattern** Cmdlet，並傳遞適當的身分識別值。
...
您可以使用新增、取得、設定、移除 **** **** ****  - **CsTenantBlockNumberExceptionPattern** Cmdlet，為封鎖的編號模式新增例外。
...
在此範例中，會建立新的數位例外模式，且預設會新增為已啟用的模式。 啟用 **和****描述** 參數為選擇性。
...
在此範例中， **身分識別** 參數為必填專案。 **Set-CsTenantBlockedNumberExceptionPattern** Cmdlet 可讓您修改一或多個指定數位模式身分識別的參數。
...
在此範例中， **需要身分** 識別參數。 此 Cmdlet 會從租使用者清單中移除給定的數位模式。  如果身分識別未知，請使用 **Get-CsInboundBlockedNumberPattern** Cmdlet 來先找出正確的模式，並記下身分識別。 接著，執行 **Remove-CsTenantBlockedNumberExceptionPattern** Cmdlet，並傳遞適當的身分識別值。在測試和驗證之前，請允許複製時間。  
...
在此範例中， **需要 PhoneNumber** 和 **租使用者** 參數。 **PhoneNumber 參數** 應該是不含任何其他字元的數值字串，例如 + 或 -。 在 TRPS 中， **租使用者參數** 為選擇性。 如果租使用者中的數位被封鎖，則產生的 **isNumberBlocked 參數** 會返回 True 的值，如果尚未封鎖，則為 False。
