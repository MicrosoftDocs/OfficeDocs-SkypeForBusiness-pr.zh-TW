...
| | 商務用 Skype-小組通話 | 可用 |
| | 通話方案 | 可用 |
| | 音訊會議（透過允許會議參與者透過 PSTN 加入） | 可用 |
| | Microsoft Phone 系統直向路由 | 可用 |
| | PSTN 呼叫者的大廳 | 可用 |
| | 通話佇列 | 可用 |
| | 老闆及代理人支援 | 可用 |
| | 顧問式與安全轉接 | 可用 |
| | 請勿打擾突破 | 可用 |
| | 區別性鈴聲 | 可用 |
| | 1:1 至團隊、商務用 Skype 和 PSTN 參與者的群組通話升級 | 可用 |
| | [轉寄至] 群組 | 可用 |
| | 轉接至 PSTN 通話 | 可用 |
| | 緊急電話撥打電話方案 | 可用 |
| | 現有的認證 SIP 手機支援 | 可用 |
| | USB HID | 可用 |
| | 通話和會議的 eDiscovery | 可用 |
| | 組織自動語音應答 | 可用 |
| | Skype 消費者-小組通話支援 | 可用 |
| 檔案 | 近 | 可用 |
| | Microsoft Teams | 可用 |
