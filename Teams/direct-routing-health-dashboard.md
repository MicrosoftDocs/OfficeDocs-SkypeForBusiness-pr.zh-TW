...
- **SIP 選項狀態** – 根據預設，SBC 每分鐘都會傳送選項訊息。 此組配置可能會因不同的 SBC 廠商而異。 直接路由會警告未傳送或未進行配置的 SIP 選項。 有關 SIP 選項監控的詳細資訊，以及 SBC 可標示為無法運作的情況，請參閱監控和疑難排解 [直接路由](direct-routing-monitor-and-troubleshoot.md)。
...
- **網路參數** - 所有網路參數都是從直接路由介面到會話邊界控制器測量。 有關建議值的資訊，請參閱準備貴組織的網路[Microsoft Teams，並](./prepare-network.md)查看 Customer Edge 以Microsoft Edge建議的值。
   ...
   - 抖動 – 是使用 RTCP 或 RTP 控制通訊協定 (計算兩個端點之間網路傳播延遲時間變化的毫秒) 。
