...
Microsoft 團隊擁有適用于桌面 (Windows 和 Mac)、web 和 mobile (Android 和 iOS) 的用戶端。 這些用戶端都需要使用中的網際網路連線, 而且不支援離線模式。
...
Microsoft 團隊桌面用戶端是獨立的應用程式, 也[可在 Office 365 專業增強版中](https://docs.microsoft.com/en-us/deployoffice/teams-install)取得。 團隊在兩個視窗中都有提供 (7 +)、32位與64位版本, 以及 macOS (10.10 +)。 在 Windows 上, 小組需要 .NET Framework 4.5 或更新版本;如果您沒有, 團隊安裝程式將會提供將它安裝給您的人員。 
...
IT 管理員可以選擇其慣用的方法, 將安裝檔案分發給組織中的電腦, 例如 System Center Configuration Manager (Windows) 或 Jamf Pro (macOS)。 若要取得適用于 Windows 發佈的 MSI 套件, 請參閱[使用 MSI 安裝 Microsoft 團隊](msi-deployment.md)。  
...
> [!NOTE]
> Microsoft 團隊的架構 (32 位與64位) 不可知是已安裝的 Windows 與 Office 體系結構。
