...
本文將說明團隊如何在各種不同拓撲中使用 Microsoft 365 或 Office 365 通話流程。 此外，它還描述用於對等媒體通訊的獨特團隊流程。 檔會說明這些流程、其用途，以及其在網路上的來源和終止。 針對本文的用途，請假設下列事項：

- 內部部署用戶端使用流程 X 在雲端中與 Microsoft 365 或 Office 365 服務進行通訊。 它源于客戶網路，並以 Microsoft 365 或 Office 365 中的端點結束。

- 內部部署用戶端使用流程 Y 來與 Microsoft 365 或 Office 365 所依賴的網際網路上的服務進行通訊。 它源于客戶網路，並終止為網際網路上的端點。

本文涵蓋下列資訊：

- [**背景**]。 提供背景資訊，例如流程可能流經的網路、流量類型、從客戶網路到 Microsoft 365 或 Office 365 服務端點的連線指示、與協力廠商元件的互通性，以及由團隊用來選取媒體流程的原則。
...
通常，客戶網路有數個網路週邊，且有多個防火牆和/或 proxy 伺服器，會強制執行您組織的安全性原則，而且只允許您已設定和設定的特定網路流量。 因為您要管理此網路，所以您可以直接控制網路的效能，我們建議您完成網路評量，以驗證您網路中的網站和網路與 Microsoft 365 或 Office 365 網路之間的效能。

**網際網路**。 這是整個網路的一部分，該網路區段將由從客戶網路外部連線至 Microsoft 365 或 Office 365 的使用者所使用。 從客戶網路到 Microsoft 365 或 Office 365 的部分流量也會使用它。

已被**造訪或來賓私人網路**。 這是客戶網路外部的網路區段，但不是在公用網際網路中，您的使用者和其來賓可能會造訪（例如，家用私人網路或企業私人網路，不會部署小組，您的使用者及其客戶與團隊服務的互動可能會駐留）。

> [!NOTE]
> Microsoft 365 或 Office 365 的連線也適用于這些網路。

**Microsoft 365 或 Office 365**。 這是支援 Microsoft 365 或 Office 365 服務的網路區段。 它在全球各地分佈，且最接近客戶網路在大部分位置的邊緣。 函數包括傳輸中繼、會議服務器及媒體處理器。

**快速路線（選用）**。 這個網段是整個網路的一部分，可為您提供 Microsoft 365 或 Office 365 網路的專用、私人連線連線。

### <a name="types-of-traffic"></a>流量類型

**即時媒體**。 在即時傳輸通訊協定（RTP）中封裝的資料，支援音訊、影片和螢幕共用工作負載。 一般來說，媒體流量會有很高的延遲，因此您可能會想要使用 UDP 與 TCP 做為傳輸層通訊協定，這是從品質觀點開始互動式即時媒體的最佳傳輸。 （請注意，在最後一個手段中，媒體可以使用 TCP/IP，也可在 HTTP 通訊協定內進行隧道，但由於品質不正確，因此不建議您這麼做。）RTP 流程是使用 SRTP 加以保護，在這種情況下，只會將負載加密。

**通知**。 用戶端與伺服器之間的通訊連結，或用來控制活動的其他用戶端（例如，啟動通話時），並傳送立即訊息。 大多數的信號流量都使用 HTTPS 式的 REST 介面，但在某些情況下（例如，Microsoft 365 或 Office 365 與會話邊界控制器之間的連線），它會使用 SIP 通訊協定。 請務必瞭解，這項流量對延遲而言不太敏感，但如果端點之間的延遲時間超過數秒，可能會導致服務中斷或呼叫超時。

### <a name="connectivity-to-microsoft-365-or-office-365"></a>連線至 Microsoft 365 或 Office 365
...
- Microsoft 團隊會議是由 Microsoft 365 或 Office 365 在第一個參與者加入的同一個區域中託管。 （請注意，如果這項規則在某些拓撲中有例外狀況，就會在這份檔中說明，並以適當的通話流程加以說明。）

- Microsoft 365 或 Office 365 中的小組媒體端點會根據媒體處理需求而使用，而不是以通話類型為基礎。 （例如，點對點通話可能會使用雲端中的媒體端點來處理媒體以進行操作或錄製，而有兩個參與者的會議可能不會使用雲端中的任何媒體端點）。不過，大部分的會議將會使用媒體端點來進行混合和路由用途，並在託管會議的位置進行分派。 從用戶端傳送至媒體端點的媒體流量可能會直接路由，或在 Microsoft 365 或 Office 365 中使用傳輸中繼（如果由於客戶網路防火牆限制而需要）。
...
此拓朴是由客戶使用來自雲端的團隊服務，而不需要任何內部部署（例如商務用 Skype Server 或電話系統直式路由）。 此外，Microsoft 365 或 Office 365 的介面是透過沒有 Azure Express 路由的網際網路完成。
...
- **流程 3** –代表由遠端行動小組使用者從 Microsoft 365 或 Office 365/團隊端點啟動的流程。
- [**流程 4** ]-代表使用者在客戶網路上由 Microsoft 365 或 Office 365/團隊端點所啟動的流程。
...
一對一通話使用常見的模型，在此模式中，呼叫者會透過 IP 位址/埠來取得一組候選物件，包括本機、中繼及反身（用戶端的公用 IP 位址，如繼電器所示）候選。 來電者會將這些候選人傳送給被叫方;呼叫的參與方也會獲得類似的候選人集合，並將其傳送給來電者。 STUN 連線檢查訊息是用來找出哪些來電者/叫方媒體路徑正常運作，且選取了最佳的工作路徑。 媒體（也就是使用 SRTP 安全保護的 RTP/RTCP 資料包），然後使用選取的候選人對傳送。 傳輸中繼會部署為 Microsoft 365 和 Office 365 的一部分。
...
如果客戶網路上有一個用戶端，且有一個用戶端是外部用戶端（例如，行動雲端使用者），則無法直接連線到本機或反身候選作業。 在此情況下，選項是使用其中一個用戶端的傳輸中繼站候選人（例如，內部用戶端從 Microsoft 365 或 Office 365 中的傳輸中繼取得中繼候選人）; 外部用戶端必須能夠將 STUN/RTP/RTCP 資料包傳送到傳輸中繼。 另一個選項是內部用戶端傳送至行動雲端用戶端取得的中繼候選人。 請注意，雖然強烈建議媒體的 UDP 連線，但支援 TCP。

**高層次步驟**：

1. 小組使用者 A 使用流程2解析 URL 網功能變數名稱稱（DNS）。
1. 小組使用者 A 使用流程4，在小組傳輸中繼上分配媒體轉送埠。
1. 團隊使用者 A 使用流程4至 Microsoft 365 或 Office 365，以 ICE 候選人傳送「邀請」給您。
1. Microsoft 365 或 Office 365 使用流程4將通知傳送給團隊使用者 B。
...
1. 團隊使用者使用流程4傳送遙測至 Microsoft 365 或 Office 365。
...
在步驟7（流程4）中，從客戶網路到 Microsoft 365 或 Office 365，以及流程3（從遠端行動小組使用者到 Microsoft 365 或 Office 365）已選取。 這些流程是透過 Microsoft 365 或 Office 365 中的小組傳輸中繼進行中繼。
...
- 使用遠端行動使用者的直接媒體（未透過 Microsoft 365 或 Office 365 中繼）是選擇性的。 換句話說，客戶可能會封鎖此路徑，透過 Microsoft 365 或 Office 365 中的傳輸中繼來強制執行媒體路徑。
...
在 VPN 與客戶網路之間的傳送信號是使用流程2」。 客戶網路與 Microsoft 365 或 Office 365 之間的傳送信號是使用流程4。 不過，媒體會繞過 VPN，而且是透過 Microsoft 365 或 Office 365 中的 [流程 3] 和 [4] 到 [小組] 媒體轉送來路由。
...
在 VPN 與客戶網路之間的傳送信號是使用流程2」。 客戶網路與 Microsoft 365 或 Office 365 之間的傳送信號是使用流程4。 不過，媒體會繞過 VPN，並從客戶網路使用流程2路由至網際網路。
...
VPN 使用者與客戶網路之間的信號，是使用流程2，並使用流程4至 Microsoft 365 或 Office 365。 不過，媒體略過 VPN，且是使用流程6進行路由。

媒體是雙向的。 [流量 6] 到 [遠端行動使用者] 的方向代表，一個端會從連接的角度啟動通訊。

#### <a name="use-case-teams-to-pstn-through-microsoft-365-or-office-365-trunk"></a>使用案例：透過 Microsoft 365 或 Office 365 主幹的團隊至 PSTN

Microsoft 365 和 Office 365 有一個手機系統，可讓您撥打及接聽公用交換電話網絡（PSTN）的通話。 如果 PSTN 幹線是使用電話系統通話方案來連接，則此使用案例沒有特殊的連線需求。 （如果您想要將您自己的內部部署 PSTN 幹線連接至 Microsoft 365 或 Office 365，您可以使用 [電話系統直式路由]。）
...
音訊/視頻/螢幕共用（VBSS）會議服務器是 Microsoft 365 和 Office 365 的一部分。 它的公用 IP 位址必須可從客戶網路取得，而且必須是從 Nomadic 雲端用戶端取得的。 每個用戶端/端點都需要能夠連接到會議服務器。
...
**在 Microsoft 365 或 Office 365 中，依小組傳輸中繼的方式傳送媒體**
...
- 同盟是依定義，在兩個租使用者之間的通訊。 在此案例中，使用團隊的租使用者 federates 使用商務用 Skype 內部部署。 如果承租人 B 也是使用 Microsoft 365 或 Office 365，則商務用 Skype 用戶端將使用流程3與 Microsoft 365 或 Office 365 連線。

- 從同盟商務用 Skype 用戶端到內部部署商務用 skype 伺服器的信號和媒體不在本檔的範圍內。 不過，這裡有清楚的說明。

- 團隊與商務用 Skype 之間的通知是由閘道橋接。

- 在此案例中，媒體是透過團隊傳輸中繼傳送至客戶網路和使用流程4的遠端商務用 Skype 用戶端。
...
團隊與商務用 Skype 之間的通知是由閘道橋接。 不過，媒體是使用流程5在客戶網路內以直接對等方式路由。

**含外部商務用 Skype 使用者的混合式客戶網路（由 Microsoft 365 或 Office 365 進行中繼）**
...
[直接路由] 可讓您使用協力廠商公用交換電話網絡（PSTN）服務提供者，方法是將支援的內部部署客戶擁有的會話邊界控制器（SBC）硬體裝置與 Microsoft 365 或 Office 365 進行配對，然後將電話幹線連接到該裝置。

為了支援此案例，客戶必須部署一個認證的 SBC，以便直接從 Microsoft 認證合作夥伴中傳送。 SBC 必須按照廠商的建議進行設定，並可透過 Microsoft 365 或 Office 365 進行直接的 UDP 流量路由。 媒體可能直接從團隊和/或商務用 Skype 用戶端傳送至 SBC （繞過團隊閘道）或遍歷團隊閘道。 當主幹設定為略過團隊閘道時，與 SBC 的連線是依據 ICE，而 SBC 支援 ICE，而商務用 Skype 媒體端點支援 ICE 完整形式。
...
*圖 18-透過直接路由（依團隊傳輸轉接傳送的媒體）的內部使用者*

請注意：

- SBC 必須有可從 Microsoft 365 或 Office 365 路由的公用 IP 位址。

- 從 SBC 到 Microsoft 365 或 Office 365 的信號及媒體，反之亦然，使用流程4和/或流程4。

- 從客戶網路中用戶端到 Microsoft 365 或 Office 365 的傳送信號及媒體使用流程4。

**含直接路由的遠端使用者（媒體是透過媒體伺服器（MP）路由）**
...
*圖 19-有直接路由的遠端使用者（媒體是透過媒體伺服器（MP）路由）*
...
- 從網際網路上的用戶端傳送信號和媒體至 Microsoft 365 或 Office 365 使用流程3。
...
- 從 SBC 到 Microsoft 365 或 Office 365 的信號，反之亦然，使用流程4和/或流程4。

- 從客戶網路中的用戶端傳送至 Microsoft 365 或 Office 365 使用流程4。

- 從客戶網路中的用戶端傳送到客戶網路中的 SBC 的媒體使用流程5B。

**具有直接路由的遠端使用者（依團隊傳輸轉接傳送的媒體旁路）**
...
*圖 21-具有直接路由的遠端使用者（依團隊傳輸中繼的媒體旁路中繼）*

請注意：

- SBC 必須有可從 Microsoft 365 或 Office 365 和網際網路路由的公用 IP 位址。

- 從 SBC 傳送至 Microsoft 365 或 Office 365，反之亦然，使用流程4和/或流程4。

- 從網際網路上的用戶端傳送到 Microsoft 365 或 Office 365 的信號使用流程3。

- 從網際網路上的用戶端傳送到客戶網路中的 SBC 使用流程3和4（依小組傳輸中繼進行中繼）。
...
**直接路由（透過 Microsoft 365 或 Office 365 的媒體）-跨兩個租使用者的 PSTN hairpin 通話**
...
*圖 24-直接路由（透過 Microsoft 365 或 Office 365 的媒體）-跨兩個租使用者的 PSTN hairpin 通話*
...
- 從客戶網路 X 到 SBC 實例 B 內的 SBC 實例 A 的媒體，必須透過 Microsoft 365 或 Office 365 媒體伺服器中繼，且無法使用旁路模式。
...
在直通路線已左右對齊和部署的情況下，小組流程可能會從流程4重新路由至流程1，並從流程4」重新路由至 [流程 1]。 不過，小組應用程式會對其他 Microsoft 365 或 Office 365 透過網際網路使用流程4和4來進行硬性依賴性;因此，不一定會封鎖這些流程。
...
1. 客戶網路中的小組使用者會使用流程1，將「邀請」傳送至 Microsoft 365 或 Office 365 的 ICE 候選人。
1. Microsoft 365 或 Office 365 會將通知傳送給使用流程3的外部團隊使用者。
1. 團隊外部使用者使用流程3在團隊傳輸中繼上分配媒體轉送埠。
1. 團隊外部使用者使用流程3傳送「答案」給 ICE 候選人，而流程3則會使用流程1轉寄回到小組使用者。
1. 小組使用者 A 和團隊使用者 B 呼叫 ICE 連線測試，並選取由小組傳輸中繼進行中繼的流程1和3。
1. 團隊使用者使用流程1和3，將遙測傳送至 Microsoft 365 或 Office 365。
