...
ms.locfileid: "63711277"
---
# <a name="auto-attendant-and-call-queue-dialing-and-voice-recognition-reference"></a>自動語音留言和通話佇列撥號和語音辨識參照

使用名稱或分機撥號是一種自動語音回應功能，可讓來電者Teams您組織的使用者。 使用語音或電話鍵台，來電者可以說出或輸入要聯繫之人員的完整或部分名稱或分機。 自動語音機會搜尋公司目錄、尋找該人員，然後將來電者轉接給他們。  當您在自動電話機中設定通話流程設定時，您可以設定使用名稱撥號或分機 [撥號選項](create-a-phone-system-auto-attendant.md#call-flow)。


## <a name="searching-for-users"></a>搜尋使用者

Teams使用撥號名稱來聯繫的使用者，不需要擁有電話號碼，或已指派通話方案給他們，但他們必須企業語音使用者商務用 Skype Server **。** 針對多國組織，按名稱撥號會尋找來電者，Microsoft Teams國家/地區的使用者。

Teams使用分機撥號可聯繫的使用者，不需要擁有電話號碼，或已指派通話方案給他們，但他們必須企業語音使用者商務用 Skype Server **。** 您也需要為使用者提供適當的撥號方案。 針對多國組織，撥號分機會尋找來電者，Microsoft Teams國家/地區的使用者。 

根據相關先決條件，在配置自動話務員時，必須明確啟用名稱撥號或分機號碼。
...
> [!NOTE]
> 如果您使用具有語音辨識的撥號名稱，但貴組織的 Active Directory 大於 80，000 個使用者，而且您尚未使用撥號範圍功能限制按名稱撥號的範圍，則使用電話鍵台撥號的來電者[](create-a-phone-system-auto-attendant.md#dial-scope)仍可使用名稱撥號，而語音輸入則適用于所有其他案例。 您可以使用撥號範圍功能，變更特定自動總機的撥號範圍，縮小可到達的名稱。
 
### <a name="search-considerations"></a>搜尋考慮 
在適用任何已配置的撥號範圍包含或排除清單之前，先按名稱撥號會先搜尋整個組織的目錄。 如果針對整個目錄進行的初始搜尋會返回超過 100 個使用者，則不會應用撥號範圍清單、搜尋失敗，而且會告知來電者找到太多名稱。
 
 
## <a name="dial-by-name---keypad-dtmf-entry"></a>按名稱撥號 - 鍵盤 (DTMF) 輸入
撥入的使用者可以使用撥號名稱來聯繫使用者，方法可以是指定他們嘗試聯繫之人員的完整名稱或部分名稱。 輸入名稱時，可以使用各種格式。

搜尋貴組織的目錄時，人員可以使用 "0" () 鍵來表示名字與姓氏或姓氏與名字之間的空格。 當他們輸入名稱時，會要求他們以 # 鍵終止鍵盤輸入。 例如，「輸入您嘗試聯繫之人員的名稱之後，請按 #」。 如果找到多個名稱，系統就會為通話者提供要選取的名稱清單。

> [!NOTE]
> 如果超過 5 個名稱在已應用任何撥號範圍包含或排除清單之後仍然保留，搜尋將會失敗，且來電者會聽到找到太多名稱。 
...
|FirstName 或 LastName |部分 |三月# |按 1 for Mary Marble  <br/> 按 2 for MaryJojo  <br/> 按 3 for Amos |
...
在搜尋使用電話鍵台搜尋人員時，會使用幾個特殊字元。 例如，系統將會要求該人員使用井號鍵 (#) ，而零 (0) 鍵則用於名稱之間的空格。 按星號鍵 (*) 會重複該人員符合的名稱清單。
