...
您用名稱撥號來尋找及聯繫的使用者不需要擁有電話號碼，或已指派通話方案給他們，但他們必須企業語音使用者商務用 Skype Server **啟用**。 使用名稱撥號甚至能尋找並轉接Microsoft Teams國家/地區託管于不同國家/地區之多個國家/地區的使用者。 根據相關先決條件，您可以在自動話務員中明確啟用按名稱撥號。

擴充撥號功能是一項自動總機功能，也是目錄搜尋的一部分。 它可讓撥打您的自動語音應答的使用者使用語音 (語音辨識) 或電話鍵台 (DTMF) 回應，以輸入他們嘗試聯繫之使用者的電話分機，然後將電話轉接給他們。 您用分機撥號來尋找和聯繫的使用者不需要擁有電話號碼，或已指派通話方案給他們，但他們必須企業語音使用者商務用 Skype Server **啟用**。 您也需要為使用者提供適當的撥號方案。 撥打分機甚至能尋找並轉接電話給Microsoft Teams組織在不同國家/地區託管的使用者。 根據相關先決條件，您可以在自動話務員中明確啟用分機撥號。

### <a name="maximum-directory-size"></a>目錄大小上限

當來電者搜尋特定人員時，Active Directory 使用者按名稱撥號和分機撥號可支援的數量沒有限制。 來電者可以在 FirstName + 姓氏 (輸入部分或全名，也可以輸入姓氏 + 名字) ，但需要完整的分機號碼。 單一自動語音機可以使用語音辨識支援的名稱清單大小上限為 80，000 個使用者。
...
搜尋貴組織的目錄時，人員可以使用 "0" () 鍵來表示名字與姓氏或姓氏與名字之間的空格。 當他們輸入名稱時，會要求他們以 # 鍵終止鍵盤專案。 例如，「輸入您嘗試聯繫之人員的名稱之後，請按 #」。 如果找到多個名稱，通話者會獲得一份名稱清單，以選取名稱。
