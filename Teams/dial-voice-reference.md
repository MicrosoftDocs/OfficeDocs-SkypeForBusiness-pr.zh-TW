...
Teams使用撥號名稱來聯繫的使用者，不需要擁有電話號碼或已指派通話方案給他們，但他們必須企業語音使用者商務用 Skype Server **啟用**。 針對多國組織，按名稱撥號會尋找來電者，Microsoft Teams國家/地區的使用者。

Teams分機撥號可聯繫的使用者不需要擁有電話號碼，或已指派通話方案給他們，但他們必須企業語音使用者 **商務用 Skype Server。** 您也需要為使用者提供適當的撥號方案。 針對多國組織，撥號分機會尋找來電者，Microsoft Teams國家/地區的使用者。 
...
> [!NOTE]
> 如果您使用具有語音辨識的撥號名稱，但貴組織的 Active Directory 大於 80，000 個使用者，而且您尚未使用撥號範圍功能限制按名稱撥號的範圍，則使用[](create-a-phone-system-auto-attendant.md#dial-scope)電話鍵台撥號的來電者仍可使用名稱撥號，而語音輸入則適用于所有其他案例。 您可以使用撥號範圍功能，變更特定自動總機的撥號範圍，縮小可到達的名稱。
 
### <a name="search-considerations"></a>搜尋考慮 
按名稱撥號會搜尋組織的目錄，然後針對任何已配置的撥號範圍包含或排除清單篩選結果。 如果初始搜尋會回復超過 100 個使用者，則不會應用撥號範圍清單、搜尋失敗，而且會告知來電者找到太多名稱。
...
您可以為自動語音機啟用語音辨識，但 DTMF (鍵盤輸入) 不會停用。 電話自動語音機已啟用語音辨識，您隨時都可以使用鍵盤輸入。
