...
ms.locfileid: "60741519"
---
# <a name="manage-user-access-to-teams"></a>管理使用者對 Teams 的存取權

您可以指派或移除Teams授權，以管理使用者層級Microsoft Teams存取權。 除了以匿名Teams加入會議之外，貴組織的每個使用者都必須擁有Teams授權，才能使用Teams。 您可以在建立Teams使用者帳戶時指派新使用者或現有帳戶的使用者。
...
使用從系統管理中心管理Teams訊息<a href="https://go.microsoft.com/fwlink/p/?linkid=2024339" target="_blank"></a>策略，控制哪些聊天和頻道傳訊功能可供 Teams。 您可以使用預設策略，或為貴組織人員建立一或多個自訂訊息策略。 若要深入瞭解，請參閱管理 Teams 中的[Teams。](messaging-policies-in-teams.md)
...
如果您需要管理大量Teams的使用者 ，例如數百或數千名使用者，請使用[PowerShell](#using-powershell)或群組[授權在](/azure/active-directory/users-groups-roles/licensing-groups-assign)Azure Active Directory (Azure AD) 。 
...
### <a name="remove-a-teams-license"></a>移除授權Teams授權

> [!IMPORTANT]
> 停用 SKU 需要大約 24 小時Teams SKU 才能生效。

當您從使用者Teams授權時，Teams使用者就會停用，而且他們不會再在應用程式啟動器或首頁Teams看到任何授權。 有關詳細步驟，請參閱 [取消使用者授權](/microsoft-365/admin/manage/remove-licenses-from-users)。

|&nbsp;|&nbsp;|
|---------|---------|
|![使用者已停用Teams之授權之 1 的螢幕擷取畫面。](media/remove-teams-licenses-1.png)    | ![使用者已停用Teams之授權之螢幕擷取畫面 2](media/remove-teams-licenses-2.png)        |
...
以下是如何使用[New-MsolLicenseOptions](/powershell/module/msonline/new-msollicenseoptions)和[Set-MsolUserLicense](/powershell/module/msonline/set-msoluserlicense) Cmdlet 來停用 Teams 的使用者的範例。 例如，請遵循下列步驟，針對Teams特定授權方案的使用者，先停用授權。 然後針對Teams使用者啟用Teams。
