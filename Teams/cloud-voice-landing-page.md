...
- 搭配通話方案使用Teams 電話組合解決方案Teams 電話運算子連線，以及/或Teams 電話直接路由

>[!NOTE]
>如果您是中小型企業， (300) ，Microsoft 現在會電話系統國內通話方案。 若要詳細資訊，請參閱[Teams 電話](/microsoftteams/business-voice/whats-business-voice)中小型企業的指南，以説明您規劃、設定及管理語音解決方案。

## <a name="what-do-you-need-to-read"></a>您需要閱讀哪些資訊？

**全部為必填專案。** 本文中的部分章節與所有組織有關。 例如，每個人都應該閱讀Teams 電話並瞭解連接到公用交換電話網絡或 PSTN (的選項) 。
...
| [**直接路由的基於位置的路由**](pstn-connectivity.md#location-based-routing-for-direct-routing) |如何使用 LBR Location-Based路由 (，) 使用者根據地理位置Microsoft Teams免付費。 如果貴組織在不允許免付費路的位置使用直接路由，請閱讀本節。
...
> [!Important]
> 本文著重于語音解決方案與Microsoft Teams。 由於 商務用 Skype Online 于 2021 年 7 月 31 日停用，因此不再支援內部部署環境之間的 PSTN 連線，無論是透過 商務用 Skype Server 或 &mdash; 雲端連接器版本和 &mdash; 商務用 Skype Online。 本文將介紹Teams語音解決方案，以及如何在必要時使用運算子或直接路由Teams內部部署電話網絡連線電話網絡。
...
Teams 電話為貴組織提供完整的 PBX 功能。 不過，若要讓使用者在組織外撥打電話，您必須將電話Teams 電話到 PSTN (電話) 。 若要Teams 電話 PSTN，您可以選擇下列其中一個選項：

- [**Teams 電話通話方案 。**](pstn-connectivity.md#teams-phone-with-calling-plan) 以 Microsoft 做為 PSTN 電信公司的全雲端解決方案。

- [**Teams 電話使用運算子與 PSTN 電信連線。**](operator-connect-plan.md) 使用運算子連線，如果您現有的接線員是 Microsoft Operator 連線 計畫的參與者，他們可以管理將 PSTN 通話帶到 Teams。 有關運算子的權益和需求連線，請參閱[規劃運算子連線。](operator-connect-plan.md)
...
無論您Teams 電話 PSTN 連接選項，大部分的功能都是相同的。 不過，功能有一些差異，會影響您設定特定 Teams 電話功能 ，例如通話路由和緊急電話。 有關 PSTN 連接選項和這些組組考慮的詳細資訊，請參閱 [PSTN 連接選項](pstn-connectivity.md)。
...
對於升級至 Teams 的組織，最終的目標是將所有使用者移至 TeamsOnly 模式。 使用者Teams 電話 TeamsOnly 模式時，才支援使用應用程式。 如果您需要升級至 Teams 的基本資訊，請從這裡開始：
...
- 使用 商務用 Skype 內部部署的使用者企業語音 ，該使用者將會移往線上並保持 **[](upgrade-to-teams-on-prem-pstn-considerations.md#from-skype-for-business-server-on-premises-with-enterprise-voice-to-direct-routing)內部部署 PSTN 連線**。 將此使用者移Teams需要將使用者的內部部署 商務用 Skype帳戶移至雲端，並協調使用者移轉至直接路由。 
