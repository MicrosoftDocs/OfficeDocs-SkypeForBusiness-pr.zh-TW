...
- Teams 電話直接路由使用您自己的 PSTN 電信公司
- 搭配通話方案使用Teams 電話、Teams 電話運算子連線，以及/或Teams 電話直接路由的組合解決方案


## <a name="what-do-you-need-to-read"></a>您需要閱讀哪些資訊？

**全部為必填專案。** 本文中的部分章節與所有組織有關。 例如，每個人都應該閱讀有關Teams 電話並瞭解連接到 PSTN 公用交換電話網絡 (選項) 。 


| 全部為必填專案 | 描述 |
| :------------|:-------|
| [**Teams 電話**](#teams-phone) | Microsoft 在 PBX 中啟用通話控制和私人分支的技術Exchange (PBX) 雲端Microsoft 365功能Microsoft Teams。 |
...
> [!Important]
> 本文著重于語音解決方案與Microsoft Teams。 由於 商務用 Skype Online 于 2021 年 7 月 31 日停用，因此不再支援內部部署環境之間的 PSTN 連線，無論是透過 商務用 Skype Server 或 &mdash; 雲端連接器版本和 &mdash; 商務用 Skype Online。 本文將介紹Teams語音解決方案，以及如何在必要時使用運算子或直接路由Teams內部部署電話連線網路。
...
Teams 電話為貴組織提供完整的 PBX 功能。 不過，若要讓使用者在組織外撥打電話，您必須將電話Teams 電話到 PSTN (電話) 。 若要將Teams 電話 PSTN，您可以選擇下列其中一個選項：

- [**Teams 電話通話方案 。**](pstn-connectivity.md#teams-phone-with-calling-plan) 以 Microsoft 做為 PSTN 電信公司的全雲端解決方案。

- [**Teams 電話運算子與自己的 PSTN 電信連線。**](operator-connect-plan.md) 使用運算子連線，如果您現有的接線員是 Microsoft Operator 連線 計畫的參與者，他們可以管理將 PSTN 通話帶到 Teams。 有關運算子的權益和需求連線，請參閱[規劃運算子連線。](operator-connect-plan.md)
...
無論您選擇Teams 電話 PSTN 連接選項，大部分的功能都是相同的。 不過，功能有一些差異會影響您設定特定Teams 電話，例如通話路由和緊急電話。 有關 PSTN 連接選項和這些組組考慮的詳細資訊，請參閱 [PSTN 連接選項](pstn-connectivity.md)。
...
- 使用 商務用 Skype 內部部署的使用者企業語音 ，該使用者將會移往線上並保持 **[](upgrade-to-teams-on-prem-pstn-considerations.md#from-skype-for-business-server-on-premises-with-enterprise-voice-to-direct-routing)內部部署 PSTN 連線**。 將此使用者移轉Teams需要將使用者的內部部署 商務用 Skype 帳戶移至雲端，並協調使用者移轉至直接路由。 

- **[使用 商務用 Skype 內部部署的使用者企業語音，](upgrade-to-teams-on-prem-pstn-considerations.md#from-skype-for-business-server-on-premises-with-enterprise-voice-to-microsoft-calling-plan)** 該使用者將會移往線上，並且使用 Microsoft 通話方案。  將該使用者移轉至 Teams 需要將使用者的內部部署 商務用 Skype 帳戶移至雲端，並協調該移動與 A) 將該使用者電話號碼的埠移至 Microsoft 通話方案，或 B) 從可用區域指派新的訂閱者號碼。
