...
**會議模式** 會大幅減少來電者接聽來電者接聽電話之後，與代理人連線所需的時間。 若要讓會議模式正常運作，通話佇列中的代理程式必須使用下列其中一個用戶端：
  ...
  - Microsoft Teams桌面用戶端、Android 應用程式或 iOS 應用程式的最新版本
...
> 
> 使用 **最長閒置** 時間時，有時專員可能會在無法使用後很快收到來自佇列的來電，或在有閒置時間之後收到來自佇列的來電的短暫延遲。
> 
...
|功能                          |Teams桌面<sup>1</sup> |Teams <sup>Mobile2</sup> |Lync |IP Phone | 標準通話佇列 |頻道型通話佇列 | 評論 |
|:--------------------------------|:------------------------:|:-----------------------:|:---:|:--------:|:--------------------:|:------------------------:|:-------------|
|**代理程式路由方法**        |                          |                         |     |          |                      |                          |              |
...
|目前狀態型路由<sup>3</sup>|Y                        |Y                        |N    |Y         |Y                     |Y                         |*建議* |
|專員可以退出宣告               |Y                         |Y                        |Y7<sup> </sup>|Y7<sup> </sup>|Y          |Y                         |*預設*     |
|頻道型佇列             |Y                         |N                        |N    |N         |n/a                   |Y8<sup> </sup>             |              |
|顯示資源帳戶名稱的通話快顯通知 |Y9<sup> </sup>       |Y                        |Y    |          |Y                     |Y                         |              |
|**動態來電者識別碼**            |                          |                         |     |          |                      |                          |              |
...
2. Microsoft Teams iPhone應用程式，Microsoft Teams Android 應用程式。
