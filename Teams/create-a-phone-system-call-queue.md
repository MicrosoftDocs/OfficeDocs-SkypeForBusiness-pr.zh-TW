...
**請參閱下方的 [通話佇列功能相容性](#call-queue-feature-compatibility) 矩陣，以瞭解更多資訊。**
...
**適用于Teams/共同通話桌面使用者Teams標準通話佇列的移動用戶端使用者**

您可以使用電話號碼指定一或多個資源帳戶，為代理人指派外發本機號碼號碼。 代理人可以選取要用於每個外發通話的外發來電號碼。

按一下 **[新增**，搜尋您想要允許代理人在撥打外接電話時用於本機號碼目的的資源帳戶;按一下 [新增，然後按一下 [**新增**> 。****

**標準通話佇列**

針對Teams桌面使用者和標準通話佇列考慮直接將通話佇列成員的本機號碼設定為通話佇列的服務號碼或適當的自動語音回應。 詳細資訊，請參閱在 Microsoft Teams[中管理來電Microsoft Teams](caller-id-policies.md)。
...
如果您想要使用頻道 [Teams](https://support.microsoft.com/office/9f07dabe-91c6-4a9b-a545-8ffdddd2504e)佇列，請選取 [選擇團隊選項，然後按一下 [**新增頻道**。**** 搜尋您想要使用的團隊，選取該團隊，然後按一下 [ **新增**。 選取您只支援 (頻道的頻道，然後按一下 [) **[ Apply**」 。 
...
- **連續路由** 會以呼叫代理程式清單中指定的順序，一個接一個地撥打所有 **呼叫** 代理程式。 如果代理人關閉或不接電話，通話就會撥打給下一個代理人。 這將會重複，直到接電話或打出電話。

- **輪循** 機制會平衡來電的路由，讓每個通話代理程式從佇列取得相同數量的通話。 此路由方法在傳入銷售環境中可能相當理想，以確保所有通話代理人之間享有同等的機會。
...
> 
> 呼叫代理人的通話佇列通話簡報可能會與位置路由限制衝突。 在這種情況下，代理人會收到電話，但無法接聽電話。 此條件會繼續，直到有其他代理人接聽來電、來電者掛斷或發生通話佇列超時條件。  
...
您可以選擇中斷通話，或重新導向到其中一個通話路由目的地。 例如，您可能讓來電者為佇列中的代理人留下語音信箱。 針對外部傳輸，請參閱先決條件和[](plan-auto-attendant-call-queue.md#prerequisites)[外部電話號碼傳輸 - 數位](create-a-phone-system-auto-attendant.md#external-phone-number-transfers---technical-details)格式的技術詳細資料。
...
|功能                          |Teams桌面<sup>1</sup> |Teams <sup>Mobile2</sup> |Lync |IP 電話 | 標準通話佇列 |通道型通話佇列 | 評論 |
|:--------------------------------|:------------------------:|:-----------------------:|:---:|:--------:|:--------------------:|:------------------------:|:-------------|
|**代理路由方法**        |                          |                         |     |          |                      |                          |              |
...
|**傳輸模式**               |                          |                         |     |          |                      |                          |              |
|`Conference Mode`<sup>5</sup>    |Y                         |Y                        |N    |Y6<sup> </sup>|Y                  |Y                         |*建議* |
|`Transfer Mode`                  |Y                         |Y                        |Y    |Y         |Y                     |Y                         |              |
|目前狀態路由<sup>3</sup>|Y                        |Y                        |N    |Y         |Y                     |Y                         |*建議* |
|代理人可以退出宣告               |Y                         |Y                        |Y7<sup> </sup>|Y7<sup> </sup>|Y          |Y                         |*預設*     |
|通道型佇列             |Y                         |N                        |N    |N         |n/a                   |Y8<sup> </sup>             |              |
|Call toast 顯示資源帳戶名稱 |Y9<sup> </sup>       |Y                        |Y    |          |Y                     |Y                         |              |
|**動態本機號碼**            |                          |                         |     |          |                      |                          |              |
...
附註：
1. Microsoft Teams Windows用戶端、Microsoft Teams Mac 用戶端、Microsoft Teams虛擬桌面基礎結構、Microsoft Teams用戶端。
2. Microsoft Teams iPhone Android 應用程式Microsoft Teams應用程式。
3. 選取代理程式路由方法的最長閒置時間會自動啟用目前狀態路由。
4. 只有在將個別使用者新增為標準通話佇列時，才能設定順序。 使用通訊群組清單或Teams頻道的順序會按字母順序排列。
5. 如果電話從已啟用位置路由的直接路由閘道路由至佇列，則不支援會議模式。
6. Microsoft Teams手機。
7. 透過使用者設定入口網站頁面，https://aka.ms/vmsettings
8. 僅支援公用頻道。
9. 不包括Teams用戶端。
10. 自動電話機和通話佇列無法在 PSTN 連接方法之間轉移通話。
...
通話佇列中的通話代理程式支援下列用戶端：
  ...
  - Microsoft Teams Android 應用程式
    ...
    > [!NOTE]
    > 指派直接路由號碼的通話佇列不支援商務用 Skype、Lync 用戶端或 IP 電話商務用 Skype代理。 Teams用戶端僅支援只有兩者共存[Teams模式](/microsoftteams/setting-your-coexistence-and-upgrade-settings)。
