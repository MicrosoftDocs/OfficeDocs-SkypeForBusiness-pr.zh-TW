...
使用者的預設電話號碼是排程會議時包含在會議邀請中的號碼。 詳細資訊，包括新使用者的預設電話號碼指派方式，請參閱在 Microsoft Teams 中設定邀請中包含的電話號碼，或在[商務用 Skype](set-the-phone-numbers-included-on-invites-in-teams.md) Online 中設定邀請[中包含的電話號碼](/SkypeForBusiness/audio-conferencing-in-office-365/set-the-phone-numbers-included-on-invites)。
...
有了Windows PowerShell，您可以管理使用者及其允許或不允許執行哪些操作。 Windows PowerShell單一系統管理Microsoft 365或 Office 365 和 商務用 Skype Online，可簡化您的日常工作，尤其是當您有多個工作需要執行時。 若要開始使用Windows PowerShell，請參閱以下主題：
