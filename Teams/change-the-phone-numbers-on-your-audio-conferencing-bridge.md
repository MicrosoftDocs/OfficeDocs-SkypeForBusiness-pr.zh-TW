...
> [!NOTE]
> 除非另有要求，否則所有這些步驟都必須在系統管理中心Microsoft Teams執行。
...
- 針對在步驟 2 中 (使用者) MMS 帳戶執行會議移移服務。 若要這麼做，請執行下列命令：
...
```
 ...
 有關其他資訊，請參閱在 Microsoft Teams 中設定邀請中包含的電話號碼，[或在](set-the-phone-numbers-included-on-invites-in-teams.md)商務用 Skype Online 中設定[邀請商務用 Skype電話號碼](/SkypeForBusiness/audio-conferencing-in-office-365/set-the-phone-numbers-included-on-invites)。
...
有關會議移移服務的資訊，請參閱在 [MMS ](/SkypeForBusiness/audio-conferencing-in-office-365/setting-up-the-meeting-migration-service-mms) (中) 。
...
- 使用 **Set-CsOnlineDialInConferencingUserDefaultNumber** Cmdlet，根據使用者的原始預設號碼或位置來變更預設的付費或免付費使用者數目。
  ...
  - 若要將位於美國的所有使用者的預設免付費號碼設為 8005551234並自動重新排8005551234，請執行：
...
### <a name="the-unassign-button-isnt-available"></a>無法使用取消分配按鈕
...
有了Windows PowerShell，您可以管理使用者及其允許或不允許執行哪些操作。 Windows PowerShell管理 Microsoft 365或 Office 365 和 商務用 Skype Online 時，使用單一管理點可簡化您的日常工作，尤其是當您有多個工作需要執行時。 若要開始使用Windows PowerShell，請參閱以下主題：
