...
Teams強制執行全小組和全組織的雙因素驗證、透過 Active Directory 單一登入，以及傳輸中和靜態資料加密。 檔案會儲存在SharePoint中，並受到加密SharePoint備份。 筆記會儲存在OneNote中，並受到加密OneNote備份。 資料OneNote儲存在小組或SharePoint中。 Wiki 定位停駐點也可以用來記錄筆記，其內容也會儲存在小組SharePoint網站。
