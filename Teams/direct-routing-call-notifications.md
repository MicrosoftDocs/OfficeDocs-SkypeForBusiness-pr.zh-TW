...
請注意，通話不會在 Microsoft Phone 系統的通話詳細資料記錄中登入，因為這是成功的呼叫。 呼叫將會註冊為「嘗試」，其最終 SIP 程式碼為「487」、最終的 Microsoft 子代碼 "540200"，以及最後一個 SIP 程式碼片語「在別處完成通話」。   （若要查看通話明細記錄，請移至 [團隊管理員] 入口網站、[分析] 和 [報表] 的使用方式報告，然後選取 [PSNT 使用量]。）
