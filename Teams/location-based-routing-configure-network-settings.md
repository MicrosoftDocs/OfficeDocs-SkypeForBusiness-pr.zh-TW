...
使用 [路由Location-Based最佳做法是為每個具有唯一 PSTN 連接的位置建立個別的網站。 您可以建立為路由Location-Based啟用的網站，或尚未針對路由Location-Based網站。 例如，您可能會想要建立未啟用 Location-Based 路由的網站，讓已啟用 Location-Based 路由的使用者在漫遊到該網站時撥打 PSTN 通話。
...
若要Location-Based路由，必須定義可Teams端點可連至網路的 IP 子網，並與定義的網路建立關聯，才能強制執行免付費。 此子閘道聯Location-Based路由，以地理方式找出端點，以判斷是否應該允許特定 PSTN 通話。 支援 IPv6 和 IPv4 子網。 當判斷Teams端點是否位於網站時，Location-Based路由會先檢查符合的 IPv6 位址。 如果沒有 IPv6 位址，請Location-Based路由檢查 IPv4 位址。
