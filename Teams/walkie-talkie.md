...
audience: admin
ms.reviewer: ''
description: 如何從 ITAdmin 的觀點Microsoft Teams在應用程式中設定 Walkie Talkie 應用程式。
...
- **延遲** - 這是從 A 點到網路 B 點取得 IP 封包所花的時間。 網路傳播延遲基本上與兩點之間的實際距離和光速有關，包括介於兩者之間的各種路由器所增加的負荷。 延遲是以 RTT 或 RTT 的往返 (時間) 。
