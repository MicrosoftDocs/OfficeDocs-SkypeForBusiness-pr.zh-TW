...
使用直接路由，您可以將自己的會話邊界控制器 (SBC) 直接連到 [電話系統]。  如需已驗證的 SBCs 清單，請參閱 [支援的會話框線控制器](direct-routing-border-controllers.md)。
