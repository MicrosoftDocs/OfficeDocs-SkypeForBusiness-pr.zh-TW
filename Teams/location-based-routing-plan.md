...
description: 瞭解如何規劃直接路由Location-Based路由Teams 電話路由。
...
在一些國家和地區，忽略 PSTN (交換電話網絡) 降低長途電話費用是違法的行為。 本文將說明如何使用 Location-Based路由，根據使用者Microsoft Teams限制付費旁路。 本文僅適用于直接路由。
...
當使用者Teams路由時Location-Based，則適用下列專案：
...
- 若要接收傳入 PSTN 通話，使用者的接聽端點必須位於呼叫透過已啟用 [路由Location-Based進入的同一個網路網站。 在任何其他情況下 ，例如使用者漫遊時，不允許通話，而且會路由至使用者的呼叫轉 (語音信箱) 。

- 若要將 PSTN Teams另一位使用者，目標使用者的端點必須位於啟動傳輸的使用者所在的同一個網路網站中。 其他任何案例都不允許傳輸。 

- 若要將另Teams使用者轉接到 PSTN，呼叫必須透過與Location-Based呼叫者位於同一個網路網站的啟用路由的閘道進行轉接。 其他任何案例都不允許傳輸。
...
Location-Based路由會依據使用者端點的 IP 位址決定使用者的目前位置，Teams並據此適用規則。 啟用路由的使用者位置Location-Based下列方式分類： 
...
- **使用者所在的內部網站未啟用 Location-Based路由。** <br>在此情境中，使用者位於未針對路由啟用的已知內部網路Location-Based網站。 
...
必須啟用網路網站，Location-Based路由，協助決定漫遊時要路由Location-Based路由的使用者。 如果已啟用 Location-Based 路由的使用者漫遊至已啟用 Location-Based 路由的網站，則只有已啟用該網站 Location-Based 路由的 PSTN 閘道可用於外發通話。 如果已啟用 Location-Based 路由的使用者漫遊至未啟用 Location-Based 路由的網站，則任何未針對 Location-Based 路由啟用的閘道都可以用於外發通話。  

### <a name="apply-location-based-routing-at-the-pstn-gateway"></a>在 PSTN Location-Based上申請路由 

閘道會與網站相關聯，以決定啟用 [路由Location-Based使用者撥打或接收 PSTN 通話時的位置。 必須針對 Location-Based啟用閘道，以確保它受免付費限制限制，且無法由未啟用 Location-Based 路由的使用者使用。 同一個閘道可能與多個網站相關聯，而且可以將它配置為啟用 Location-Based 路由，或視網站Location-Based路由啟用。
...
|與指派使用者 DID 的網站不同，網站已啟用 Location-Based路由 (網站)     |根據使用者的語音路由策略，Location-Based在漫遊網站 GW2 (GW2) 閘道路由的通話        |
...
未啟用 Location-Based路由的使用者，可以從未為其指派 DID 號碼輸入的 Location-Based 路由啟用的閘道接收輸入通話。 如果使用者漫遊到未啟用 [路由Location-Based，他們仍然可以透過一般 PSTN 閘道接聽來電。
...
未啟用 [路由Location-Based可以將 PSTN 通話轉接或轉接給未啟用 [路由Location-Based使用者。 使用者通常無法將 PSTN 通話轉接或轉接給已啟用 Location-Based 路由的使用者，因為啟用 Location-Based 路由的使用者一般只能在 Location-Based 路由啟用的閘道進行 PSTN 通話。 例外是啟用路由Location-Based使用者漫遊至未啟用路由Location-Based網站。 在此情境中，允許轉接的通話。  
...
- 將傳入 VOIP 通話轉接及轉接到 PSTN 時，必須遵守來電者的付費旁路限制。 
    ...
    - 如果來電者已啟用路由Location-Based，則他們只能轉移到位於相同網路Location-Based路由啟用閘道。 

#### <a name="user-enabled-for-location-based-routing"></a>使用者已啟用 Location-Based路由

- 將 PSTN 通話的內送轉接和轉Location-Based PSTN 號碼必須路由到與來電抵達的 [路由Location-Based閘道相同。
...
下表顯示Location-Based使用者 1 的傳入 VoIP 通話路由行為，此路由行為位於不同位置，同時將鈴聲設定為 PSTN 號碼的使用者。 
...
- 針對傳入 PSTN 呼叫委派者，Location-Based呼叫轉譯或同時撥打給其他使用者的相同路由規則也適用于代理人。 若要詳細資訊，請參閱Teams[](#teams-user-transfers-or-forwards-call-to-another-teams-user)轉接或轉接呼叫給另一位 Teams使用者、Teams轉接或轉接呼叫至[PSTN](#teams-user-transfers-or-forwards-call-to-pstn-endpoint)端點，以及同時[撥打。](#simultaneous-ringing) 當代理人將 PSTN 端點設定為同時響鈴目標時，代理人的語音路由策略會用來將通話路由至 PSTN。 

- 針對委派，Microsoft 建議委派者和相關代理人位於同一個網路網站。 
