...
在某些國家和地區，不一定要略過公開交換的電話網絡 (PSTN) 提供者來減少長途通話成本。 本文說明如何使用以位置為基礎的路由，根據其地理位置限制 Microsoft 團隊使用者的免付費略過。 本文只適用于電話系統直接路由。
...
- 若要接收入站 PSTN 呼叫，使用者的應答端點必須位於您的網路網站中，且呼叫 ingresses 是透過支援位置路由的閘道。 在任何其他情況下（例如，如果使用者是漫遊），不允許呼叫且路由至使用者的來電轉接設定， (通常) 語音信箱。
...
- **使用者位於與已指派其所在之 PSTN 閘道的同一個以位置為基礎的路由啟用的網站上。**<br>在這種情況下，使用者位於已啟用位置路由的已知網路網站，而且使用者的直接向內撥號 () 號碼會在同一網路網站的 PSTN 閘道上結束。 例如，使用者是他們的辦公室。 
...
|---------|---------|
|在已指派使用者的情況下，已針對位置路由 (Site1 的網站啟用網站)       |呼叫是透過已啟用位置路由的閘道路由 (GW1 根據使用者的語音路由策略，在 Site1)          |
|已指派使用者的位置不同的網站，對於以位置為基礎的路由的網站， (Site2)     |呼叫是透過可在位置進行路由的閘道路由 () GW2 在漫遊 Site2 中根據使用者的語音路由策略來傳送        |
|不同的網站是指派給使用者的位置，不啟用網站的位置路由 (Site3)   |呼叫是透過未啟用位置路由的閘道傳送，而不是針對以位置為基礎的路由 (GW3) （根據使用者的語音路由原則）       |
...
|---------|---------|
|與使用者所指派的位置相同的網站，針對位置路由 (的網站啟用 Site1)    | 在 Site1 中傳送給 User1's 端點的通話        |
|已指派使用者的位置不同的網站，對於以位置為基礎的路由的網站， (Site2)     | 通話未路由至 Site2 中的端點        |
|不同的網站是指派給使用者的位置，不啟用網站的位置路由 (Site3)     | 通話未路由至 Site3 中的端點        |
...
|---------|---------|---------|
|與發起者 (的同一個網路網站) |允許|允許|
|不同的網路網站、針對位置路由的網站 (User3) |不允許|不允許|
|不同的網路網站，沒有為以位置為基礎的路由啟用網站 (User4) |不允許|不允許|
...
|---------|---------|---------|
|相同的網路網站，針對位置路由)  (   |您只能透過 User2's 語音路由策略，在 Site1 上啟用以位置為基礎的路由 Gateway1 傳送來電轉接         |根據 User2's 語音路由策略，在 Site1 中，呼叫轉寄只能透過以位置為基礎的路由 Gateway1 進行路由         |
|不同的網路網站、針對位置路由的網站 (User3)     |您只能透過 User3's 語音路由策略，在 Site1 上啟用以位置為基礎的路由 Gateway1 傳送來電轉接         |根據 User3's 語音路由策略，在 Site1 上使用以位置為基礎的路由 Gateway1 傳送呼叫轉寄功能         |
|不同的網路網站，沒有為以位置為基礎的路由啟用網站 (User4)     |您只能透過 User4's 語音路由策略，在 Site1 上啟用以位置為基礎的路由 Gateway1 傳送來電轉接         |根據 User4's 語音路由策略，在 Site1 上使用以位置為基礎的路由 Gateway1 傳送呼叫轉寄功能         |
...
|未知的外部網路 (User6)    |您只能透過 User6's 語音路由策略，在 Site1 上啟用以位置為基礎的路由 Gateway1 傳送來電轉接        |根據 User6's 語音路由策略，在 Site1 上使用以位置為基礎的路由 Gateway1 傳送呼叫轉寄功能         |
...
|---------|---------|
|相同的網路網站，針對位置路由)  (    |只能透過 User2's 語音路由策略，在 Site1 上以位置為基礎的路由 Gateway1 傳送通話       |
|針對位置路由 (User3) 啟用不同的網路網站    |呼叫只能透過以位置為基礎的路由 Gateway1 在 Site1 上根據 User3's 語音路由策略來路由        |
|針對位置路由 (的不同網路網站不支援 User4)     |呼叫只能透過以位置為基礎的路由 Gateway1 在 Site1 上根據 User4's 語音路由策略來路由         |
|未知的內部網路 (User5)     |呼叫只能透過以位置為基礎的路由 Gateway1 在 Site1 上根據 User5's 語音路由策略來路由         |
|未知的外部網路 (User6)    |呼叫只能透過以位置為基礎的路由 Gateway1 在 Site1 上根據 User6's 語音路由策略來路由         |
