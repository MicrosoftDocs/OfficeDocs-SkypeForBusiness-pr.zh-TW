...
在一些國家和地區，忽略 PSTN (交換電話網絡) 降低長途電話費用是違法的行為。 本文將說明如何使用 Location-Based路由，根據使用者Microsoft Teams限制付費旁路。 本文僅適用于直接電話系統路由。
...
當使用者Teams路由時，Location-Based會適用下列專案：
...
- 若要接收傳入 PSTN 通話，使用者的接聽端點必須位於呼叫透過啟用 [路由Location-Based的同一個網路網站。 在任何其他情況下 ，例如使用者漫遊時，通話是不允許的，而且會路由至使用者的呼叫轉 (通常是語音信箱) 。
- 若要將 PSTN Teams另一位使用者，目標使用者的端點必須位於啟動傳輸的使用者所在的同一個網路網站中。 其他任何案例都不允許傳輸。 
- 若要將另一Teams使用者轉接到 PSTN，呼叫必須透過與初始來電者位於相同網路Location-Based路由啟用閘道進行轉接。 其他任何案例都不允許傳輸。
...
|與指派使用者 DID 的網站不同，網站已啟用 Location-Based網站 (網站)     |根據使用者的語音路由策略，Location-Based路由 (GW2) 漫遊網站 GW2 的閘道路由通話        |
...
|與指派使用者 DID 的網站不同，網站已啟用 Location-Based 路由 (網站)     | 未路由至 Site2 中的端點的通話        |
|與指派使用者 DID 的網站不同，網站未啟用 Location-Based 路由 (網站)     | 未路由至 Site3 端點的通話        |
...
|User6 (外部網路)    |根據 User6 的語音路由Location-Based，只能透過網站 1 的啟用路由的閘道1 路由來電轉接        |根據 User6 的語音路由Location-Based，只能透過網站 1 的啟用路由的閘道1 路由呼叫轉呼叫         |
...
- 對於來自授權Location-Based路由功能代理人的外發通話，則適用相同的規則。 通話路由是根據代理人的通話授權策略、語音路由策略和位置。 若要詳細資訊，請參閱Teams[將外線通話撥打到 PSTN。](#teams-user-places-an-outbound-call-to-the-pstn) 
