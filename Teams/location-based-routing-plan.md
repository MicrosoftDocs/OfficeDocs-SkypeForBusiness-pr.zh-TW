...
|---------|---------|---------|
|相同的網路網站, 已針對位置路由 (選擇 2) 啟用網站   |您只能透過 User2's 語音路由策略, 在 Site1 上啟用以位置為基礎的路由 Gateway1 傳送來電轉接         |根據 User2's 語音路由策略, 在 Site1 中, 呼叫轉寄只能透過以位置為基礎的路由 Gateway1 進行路由         |
...
|---------|---------|
|相同的網路網站, 已針對位置路由 (選擇 2) 啟用網站    |只能透過 User2's 語音路由策略, 在 Site1 上以位置為基礎的路由 Gateway1 傳送通話       |
