...
在一些國家和地區，若要降低長途電話費用， (PSTN) 的公用交換電話網路絡。 本文將說明如何使用 Location-Based路由，根據使用者Microsoft Teams限制付費旁路。 本文僅適用于直接電話系統路由。
...
- 若要撥打外發 PSTN 通話，下列其中一項必須為 True：
    - 使用者的端點位於啟用 Location-Based 路由的網路網站中，並且會透過已啟用 Location-Based 路由的對應閘道出口。 
...
Location-Based路由使用相同的網路區域、網站和子網定義，商務用 Skype Server使用。 當位置限制免付費時，系統管理員會將該位置的每個 IP 子網和每個 PSTN 閘道關聯至網路網站。 使用者的位置是由 IP 子網決定，使用者Teams PSTN 通話時已連接端點。 使用者可能會有多個位於Teams用戶端，在這種情況下，Location-Based路由會依據其端點位置個別強制執行每個用戶端的路由。 

若要熟悉本文中使用的一些網路術語，請參閱本文中的雲端語音功能網路[Teams。](cloud-voice-network-settings.md)
...
如先前所述，Location-Based路由僅適用于設定為直接路由的使用者。 Location-Based路由不適用於已設定通話方案的使用者。 如果使用者受到免付費Location-Based，則必須啟用 [路由路由>，此限制會控制他們可以撥打和接聽 PSTN 通話的條件，以及可以使用的 PSTN 閘道。 啟用 Location-Based 路由的使用者位於已啟用 Location-Based 路由的網站時，使用者必須透過已連接到網站的 Location-Based 路由啟用閘道撥打電話。 
...
- **使用者位於另一個啟用路由的網站Location-Based與 PSTN 閘道沒有關聯的位置，而 PSTN 閘道是指派其 DID 的位置。**<br>在此情境中，使用者位於已啟用 Location-Based 路由的已知網路網站中，而且該網站不會與指派使用者的 DID 號碼的 PSTN 閘道相關聯。 例如，使用者會前往另一個辦公室。  
- **使用者所在的內部網站並未啟用 Location-Based路由。** <br>在此情境中，使用者位於未針對路由啟用的已知內部網路Location-Based網站。 
- **使用者位於未知的網站。** 
    - 使用者位於未定義為網路網站的內部網路內。 
    - 使用者位於內部網路外部。 例如，使用者位於家中或咖啡店的網際網路上。 

### <a name="apply-location-based-routing-at-the-network-site"></a>在Location-Based上使用路由 
必須啟用網路網站，Location-Based路由，協助決定在漫遊時，Location-Based路由啟用使用者的閘道。 如果已啟用 Location-Based 路由的使用者漫遊至已啟用 Location-Based 路由的網站，則只有已啟用該網站 Location-Based 路由的 PSTN 閘道才能用於外線通話。 如果已啟用 Location-Based 路由的使用者漫遊至未啟用 Location-Based 路由的網站，則任何未針對 Location-Based 路由啟用的閘道都可以用於外發通話。  

### <a name="apply-location-based-routing-at-the-pstn-gateway"></a>在 PSTN Location-Based上申請路由 

閘道會與網站相關聯，以決定啟用 [路由Location-Based使用者撥打或接收 PSTN 通話時，其位置。 必須針對 Location-Based啟用閘道，以確保它受免付費限制限制，且無法由未啟用 Location-Based 的使用者使用。 同一個閘道可能與多個網站相關聯，而且可以將它配置為啟用 Location-Based 路由，或視網站Location-Based路由啟用。
...
未啟用 Location-Based 路由的使用者，可以在未透過指派的語音路由Location-Based路由的任何網站，使用任何閘道撥打外接電話。 不過，如果閘道已啟用 Location-Based路由，即使已指派給語音路由策略，使用者也無法透過閘道撥打外接電話。 如果使用者漫遊至已啟用 Location-Based 路由的網站，則他們只能透過未針對 Location-Based 路由啟用的一般路由閘道撥打電話。
...
|---------|---------|
|在指派使用者的 DID 的相同網站中，啟用網站Location-Based路由 (網站 1)       |根據使用者的語音路由Location-Based， (GW1) 閘道路由的通話         |
|與指派使用者 DID 的網站不同，網站已啟用 Location-Based路由 (網站)     |根據使用者的語音路由策略，Location-Based路由 (GW2) 漫遊網站 GW2 啟用的閘道路由通話        |
...
未啟用 Location-Based 路由的使用者，可以從未為其指派 DID 號碼輸入的 Location-Based 路由啟用的閘道接收輸入通話。 如果使用者漫遊到未啟用 [路由Location-Based的網站，他們仍然可以透過一般 PSTN 閘道接聽來電。
...
|與指派使用者 DID 的網站不同，網站會啟用 Location-Based路由 (網站)     | 未路由至 Site2 端點的通話        |
|網站與指派使用者 DID 的網站不同，未啟用網站路由Location-Based網站 (網站)     | 未路由至 Site3 端點的通話        |
...
來電轉接要求啟動使用者接聽來電，而來電轉接則不需要接聽初始通話。 這表示即使 User1 不在接收輸入通話的位置 (請參閱 Teams 使用者從[PSTN](#teams-user-receives-an-inbound-call-from-the-pstn)區段) 接收輸入通話的資料表，且如果使用者無法接收輸入通話，也無法轉接通話。 

#### <a name="user-not-enabled-for-location-based-routing"></a>未啟用使用者路由Location-Based使用者

未啟用 [路由Location-Based可以將 PSTN 通話轉接或轉接給未啟用 [路由Location-Based使用者。 通常不允許使用者將 PSTN 通話轉接或轉接給已啟用 Location-Based 路由的使用者，因為啟用 Location-Based 路由的使用者通常只能在啟用 Location-Based 路由的閘道進行 PSTN 通話。 例外是啟用路由Location-Based使用者漫遊到未啟用路由Location-Based網站。 在此情境中，允許轉接的通話。  
...
下表顯示是否允許來電轉接和來電轉接，視目標使用者的位置而不同。 在此資料表中，位於 Site1 中的 User1 會啟動移轉或轉Teams移轉給其他已啟用 Location-Based 路由及位於不同位置的使用者。  
...
|User6 (未知外部)    |根據 User6 的語音路由Location-Based，只能透過網站 1 的啟用路由的閘道1 路由來電轉接        |根據 User6 的語音路由Location-Based，只能透過網站 1 的啟用路由的閘道1 路由呼叫。         |
...
語音應用程式傳輸允許來電轉接和同時撥打給使用者和 PSTN。 完成目標通話時，受先前所列的Location-Based路由規則所規範。  
...
- 對於來自授權Location-Based路由啟用代理人的外發通話，則適用相同的規則。 通話路由是根據代理人的通話授權策略、語音路由策略和位置。 若要詳細資訊，請參閱Teams將外線通話撥打[到 PSTN。](#teams-user-places-an-outbound-call-to-the-pstn) 
- 針對傳入 PSTN 呼叫委派者，Location-Based呼叫轉譯或同時撥打給其他使用者的相同路由規則也適用于代理人。 若要詳細資訊，請參閱Teams轉接或轉接呼叫給另一位[Teams](#teams-user-transfers-or-forwards-call-to-another-teams-user)使用者、Teams使用者轉接或轉接呼叫至[PSTN 端點](#teams-user-transfers-or-forwards-call-to-pstn-endpoint)，以及同時響[鈴](#simultaneous-ringing)。 當代理人將 PSTN 端點設定為同時響鈴目標時，代理人的語音路由策略會用來將通話路由至 PSTN。 
