...
在一些國家和地區，若要降低長途電話費用， (PSTN) 公用交換電話網絡。 本文將說明如何使用 Location-Based路由，根據使用者Microsoft Teams限制付費旁路。 本文僅適用于直接電話系統路由。
...
- 若要撥打外發 PSTN 通話，下列其中一項必須為 True：
    ...
    - 使用者的端點位於未針對 Location-Based 路由啟用的網路網站中，並且透過未啟用 Location-Based 路由的閘道Location-Based出口。
...
Location-Based路由使用相同的網路區域、網站和子網定義，商務用 Skype Server使用。 當位置限制免付費時，系統管理員會將該位置的每個 IP 子網和每個 PSTN 閘道關聯至網路網站。 使用者的位置是由 IP 子網決定，Teams PSTN 通話時，使用者與端點連接。 使用者可能會有多個位於Teams用戶端，在這種情況下，Location-Based路由會依據其端點位置個別強制執行每個用戶端的路由。 
...
如先前所述，Location-Based路由僅適用于設定為直接路由的使用者。 Location-Based路由不適用於已設定通話方案的使用者。 如果使用者受到免付費限制Location-Based，則必須啟用 [路由路由>，此限制會控制他們可以撥打和接聽 PSTN 通話的條件，以及可以使用的 PSTN 閘道。 啟用 Location-Based 路由的使用者位於已啟用 Location-Based 路由的網站時，使用者必須透過連接到網站的 Location-Based 路由啟用閘道撥打電話。 

Location-Based路由會依據使用者端點的 IP 位址決定使用者的目前位置，Teams並適用規則。 啟用路由的使用者位置Location-Based下列方式分類： 
- **使用者位於指派其 DID 的 PSTN Location-Based啟用路由的網站。**<br>在此情境中，使用者位於已啟用 Location-Based 路由的已知網路網站中，使用者的 [直接向內撥號 (DID) 號碼會終止于同一個網路網站的 PSTN 閘道上。 例如，使用者位於他們的辦公室。 
- **使用者位於另一個啟用路由Location-Based與 PSTN 閘道沒有關聯的位置，而 PSTN 閘道已指派其 DID。**<br>在此情境中，使用者位於已啟用 Location-Based 路由的已知網路網站中，而且該網站不會與指派使用者的 DID 號碼的 PSTN 閘道相關聯。 例如，使用者會前往另一個辦公室。  
- **使用者位於內部網站，此網站未針對 Location-Based啟用。** <br>在此情境中，使用者位於未針對路由啟用的已知內部網路Location-Based網站。 
- **使用者位於未知的網站。** 
    - 使用者位於未定義為網路網站的內部網路內。 
...
閘道會與網站相關聯，以決定啟用 [路由Location-Based使用者撥打或接收 PSTN 通話時的位置。 必須針對 Location-Based啟用閘道，以確保它受免付費限制限制，且無法由未啟用 Location-Based 路由的使用者使用。 同一個閘道可能與多個網站相關聯，而且可以將它配置為啟用 Location-Based路由，或視網站Location-Based路由啟用。
...
未啟用 Location-Based 路由的使用者，可以在未透過指派的語音路由策略進行 Location-Based 路由的任何網站上，使用任何閘道撥打外接電話。 不過，如果閘道已啟用 Location-Based路由，即使已指派給語音路由策略，使用者也無法透過閘道撥打外接電話。 如果使用者漫遊至已啟用 Location-Based 路由的網站，則他們只能透過未針對 Location-Based 路由啟用的一般路由閘道撥打電話。
...
|---------|---------|
|已指派使用者的 DID 的同一個網站，Location-Based網站 1 (網站)       |根據使用者的語音路由策略，Location-Based在 (GW1) 閘道路由的通話         |
|與指派使用者 DID 的網站不同，網站已啟用 Location-Based路由 (網站)     |根據使用者的語音路由策略，Location-Based路由 (GW2) 路由至漫遊網站 2 的閘道路由通話        |
|與指派使用者 DID 的網站不同，網站未啟用 Location-Based路由 (網站)   |根據使用者的語音路由策略，未針對 Location-Based 路由 (GW3) 啟用 Location-Based 路由的閘道路由通話       |
...
未啟用 Location-Based路由的使用者，可以從未為其獲派 DID 號碼輸入之 Location-Based 路由啟用的閘道接收輸入通話。 如果使用者漫遊到未啟用 [路由Location-Based，他們仍然可以透過一般 PSTN 閘道接聽來電。
  
#### <a name="user-enabled-for-location-based-routing"></a>使用者已啟用 Location-Based路由

相較之下，啟用 Location-Based路由的使用者只能從 PSTN 閘道接收呼叫，當他們位於同一個網站時，其 DID 會指派給該閘道。 下表顯示當使用者 1 移至不同的網路位置時，使用者1 如何接收來電。 如果通話未路由至使用者的端點，則呼叫會傳送到使用者的呼叫轉轉設定 ，如果設定已設定。 一般來說，這是語音信箱。  

|User1 端點位置  |將輸入通話路由至 User1  |
|---------|---------|
|與指派使用者 DID 的網站相同，網站會啟用 Location-Based路由 (網站 1)    | 在 Site1 中路由至 User1 端點的通話        |
|與指派使用者 DID 的網站不同，網站已啟用 Location-Based路由 (網站)     | 未路由至 Site2 中的端點的通話        |
|與指派使用者 DID 的網站不同，網站未啟用 Location-Based路由 (網站)     | 未路由至 Site3 端點的通話        |
...
來電轉接要求啟動使用者接聽來電，而來電轉接則不需要接聽初始通話。 這表示即使 User1 不在接收輸入通話的位置 (請參閱 Teams 使用者從[PSTN](#teams-user-receives-an-inbound-call-from-the-pstn)區段) 接收輸入通話的資料表，且如果使用者無法接聽輸入通話，也無法轉接通話。 

#### <a name="user-not-enabled-for-location-based-routing"></a>未啟用使用者路由Location-Based使用者

未啟用 [路由Location-Based可以將 PSTN 通話轉接或轉接給未啟用 [路由Location-Based使用者。 一般而言，使用者無法將 PSTN 通話轉接或轉接給已啟用 Location-Based 路由的使用者，因為啟用 Location-Based 路由的使用者通常只能在啟用 Location-Based 路由的閘道進行 PSTN 通話。 例外是啟用路由Location-Based使用者漫遊到未啟用路由Location-Based網站。 在此情境中，允許轉接的通話。  
...
- 將傳入 VOIP 通話轉接及轉接到 PSTN 時，必須遵守來電者的付費旁路限制。 
    - 如果來電者未針對 [Location-Based路由啟用，可以將來電者轉接到任何未針對 [路由Location-Based閘道。
    - 如果來電者已啟用路由Location-Based，則他們只能移轉到位於相同網路Location-Based路由啟用閘道。 
...
|User6 (未知)    |根據 User6 的語音路由Location-Based，只能透過網站 1 的啟用路由的閘道1 路由來電轉接        |根據 User6 的語音路由Location-Based，只能透過網站 1 的啟用路由的閘道1 路由呼叫轉呼叫         |

### <a name="simultaneous-ringing"></a>同時響鈴

當已啟用 Location-Based 路由的使用者收到來電且同時啟用響鈴時，Location-Based 路由會分析通話方的位置和被叫方端點，以決定是否應該路由通話。 同時撥打會遵循與Location-Based和轉接相同的規則。 
...
|---------|---------|
|與 User2 (啟動)    |允許         |
|不同的漫遊網路網站Location-Based User3 (路由)    |不允許         |
...
#### <a name="inbound-calls-through-voice-app-auto-attendant-or-call-queue"></a>透過語音應用程式撥打來電 (自動語音留言或通話佇列) 
...
- 針對傳入 PSTN 呼叫委派者，Location-Based呼叫轉譯或同時撥打給其他使用者的相同路由規則也適用于代理人。 若要詳細資訊，請參閱Teams[](#teams-user-transfers-or-forwards-call-to-another-teams-user)轉接或轉接呼叫給另一位 Teams 使用者、Teams轉接或轉接呼叫至[PSTN](#teams-user-transfers-or-forwards-call-to-pstn-endpoint)端點，以及同時[撥打。](#simultaneous-ringing) 當代理人將 PSTN 端點設定為同時響鈴目標時，代理人的語音路由策略會用來將通話路由至 PSTN。 
...
Location-Based路由不適用於下列互動類型。 Location-Based下列情況下，Teams端點與 PSTN 端點互動時，不會強制執行路由： 
- 透過 [呼叫停駐點撥打或取回 PSTN 通話> 
- 內部部署使用者商務用 Skype線上使用者商務用 Skype使用者Teams使用者  
...
在沒有會議授權的使用者啟動的電話會議中，如果電話會議中至少有一位已啟用 Location-Based 路由的使用者，則不允許新增 PSTN 參與者。 如果至少有一個 PSTN 參與者參與或參與這類電話會議，則啟用 Location-Based 路由的參與者想要加入通話，則這類參與者無法新加入通話。

### <a name="media-bypass-requirement-for-location-based-routing"></a>路由的媒體Location-Based要求

如果您要在印度部署 Location-Based路由，則還必須設定媒體旁路。 若要深入瞭解 [，請參閱使用](direct-routing-plan-media-bypass.md) 直接路由和直接路由的當地媒體優化規劃媒體 [旁路](direct-routing-media-optimization.md)。
...
前往設定 [路由的網路Location-Based設定](location-based-routing-configure-network-settings.md)。
