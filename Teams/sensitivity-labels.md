...
您可以建立和設定敏感度標籤，在建立小組時，允許使用者建立具有特定隱私權的小組， (公開或私人) 設定。
...
敏感度標籤也會顯示在小組屬性中，以及位於系統管理中心的管理小組頁面的Microsoft Teams欄。
...
- **API、PowerShell Cmdlet 和範本不支援敏感度Teams Graph標籤**
    ...
    使用者無法透過 Teams Graph API、PowerShell Cmdlet 和 Teams建立團隊時指定 Teams敏感度標籤。 不過，新式Graph API 和 PowerShell Cmdlet 允許使用標籤建立群組。 因此，使用者可以先使用 Api 或 PowerShell Cmdlet Graph使用標籤建立群組，然後將這些群組轉換成 Teams。
...
使用檔Microsoft 365來建立及設定敏感度標籤Teams： 
