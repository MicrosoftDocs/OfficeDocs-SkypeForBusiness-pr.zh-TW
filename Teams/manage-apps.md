...
* [Teams 系統管理員角色](./using-admin-roles.md)。
...
Teams 系統管理中心提供可完成大部分應用程式管理使用案例的選項。 此外，其他入口網站也提供一些選項。
...
若要在應用程式上傳後更新，請在 [管理應用程式] 頁面上的應用程式清單中，選取應用程式名稱，然後選取 [ **更新]**。 執行更新會取代現有的應用程式，而更新的應用程式仍會強制執行所有應用程式許可權原則和應用程式設定原則。
...
當 ISV 將應用程式發佈到全域 App Store 時，可能需要系統管理員來設定或自訂應用程式體驗。 系統管理員可以在設定應用程式時提供給使用者使用。
...
您可以使用 [ **新增至團隊** ] 按鈕來安裝應用程式至團隊。 此選項僅適用于可在小組範圍中安裝的應用程式。 此選項不適用於只能在個人範圍中安裝的應用程式。
...
您現在可以根據組織的需求，自訂應用程式以包含特定的外觀與風格。 請參閱 [在 Teams 中自訂應用程式](customize-apps.md)。
...
## <a name="grant-permissions-and-consent-to-apps-to-use-end-user-information"></a>授與許可權與同意給應用程式以使用使用者資訊
