...
description: 瞭解由 Microsoft Teams 產生的 Debug、Media 和桌面記錄、在何處找到記錄，以及這些記錄如何協助監控和疑難排解。
appliesto:
- Microsoft Teams
...
媒體記錄預設為關閉。 若要啟用媒體記錄，使用者必須在用戶端中開啟Teams選項。 請前往 **設定** 一般，然後選取啟用會議診斷記錄， ( >  ******重新開機Teams) 。** 您必須Teams用戶端才能開始記錄 (請以滑鼠右鍵按一下 dock (Mac) 或工作列中的圖示，然後選取 (Windows) 以 **重新開機。** 在您離開之後，只要按一下應用程式圖示，即可再次開啟) 。
