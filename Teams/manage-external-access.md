...
外部存取是一種讓整個外部網域的 Teams 使用者可以在 Teams 中尋找、通話、聊天以及與您進行會議的方式。 您也可以使用外部存取來與其他組織中仍在使用商務用 Skype (線上及內部部署) 與 Skype (在預覽) 中的人員進行通訊。

> [!NOTE]
> [允許] 或 [封鎖的網域] 只會在「關閉」會議的匿名存取權中套用到會議。

如果您想讓其他組織中的人員能夠存取團隊和頻道，可能是來賓存取較好的方式。 如需外部存取和來賓存取之間差異的詳細資訊，請參閱[比較外部和來賓存取](communicate-with-users-from-other-organizations.md#compare-external-and-guest-access)。 
...
Teams 預設會開啟外部存取，這表示您的組織可以與所有外部網域通訊。 如果您新增封鎖網域，將允許所有其他網域；如果您新增允許網域，所有其他網域都會遭到封鎖。 此規則的例外狀況是在會議中允許匿名參與者。 在 Teams 系統管理中心設定外部存取有三種案例 ([全組織設定]  >  [外部存取])：
...
> [!NOTE]
> 如果您關閉組織中的外部存取，外部使用者仍可透過匿名加入加入會議。 若要深入瞭解，請參閱[在 Teams 中管理會議設定](https://docs.microsoft.com/microsoftteams/meeting-settings-in-teams)。
