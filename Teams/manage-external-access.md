...
Teams 系統管理中心會控制組織層級的外部存取權。 如果您想要控制使用者層級的外部存取權，您必須使用 PowerShell。 請參閱下方的 [使用 PowerShell](#using-powershell) 以瞭解更多資訊。
...
|啟用與特定網域的同盟|`-AllowedDomains`|不適用|
|停用與特定網域的同盟|`-Blocked Domains`|不適用|
