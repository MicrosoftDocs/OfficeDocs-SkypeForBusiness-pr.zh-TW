...
3. 在 [ **選取位置和編號類型** ] 頁面上，指定下列專案，然後選取 [ **下一步]**：

    - **國家或地區**：您取得數位的國家或地區。
    - **電話號碼類型**：號碼類型，例如地理或免付費電話號碼。
    - **指派給** 的數位：指派給數位的物件。 例如，使用者、會議或語音功能。

4. 在 [ **新增帳戶資訊]** 頁面上，完成下列操作，然後選取 [ **下一步]**。
...
5. 在 [ **新增號碼** ] 頁面上， **選取 [選取檔案**]，流覽並選取包含您要傳輸之電話號碼的 CSV 檔案，然後選取 [ **下一步]**。  

    > [!NOTE]
    > CSV 檔案必須只有一個名為 **PhoneNumber** 的標題列。 每個電話號碼都必須在另一列，而且只能是數位或 E.164 格式。

6. 在 **[完成您的訂單** ] 頁面上，選取 **[上傳已簽署的授權書** ]，以上傳已簽署之授權書的掃描複本 (LOA) 。

    如果您尚未下載並簽署 LOA，請執行下列動作：
    
    1. 選 **取 [下載範本** ] 以下載您國家或地區的 LOA。 
    2. 列印 LOA。
    3. 讓 LOA 由獲授權變更帳戶的人員簽署。
    4. 掃描簽署的 LOA，然後選取 **[上傳已簽署的授權書** ] 來上傳。
...
當我們收到您的移轉訂單時，您會收到一封驗證您的要求的電子郵件。 系統會每天檢查並更新您的要求，系統會在電子郵件中通知您的要求進度和狀態。 如果電信業者拒絕您的移轉要求，請連絡 [TNS 服務台](../manage-phone-numbers-for-your-organization/contact-tns-service-desk.md) 尋求協助。

若要檢視移轉訂單的狀態，請在 Microsoft Teams 系統管理中心的左側導覽中，移至 **[語音**  >  **埠訂單**]，然後選取 [**訂購記錄]**。 每個移轉訂單狀態都會列在 **[狀態** ] 欄中。 若要深入瞭解，請參閱 [移轉訂單的狀態為何？](port-order-status.md)


## <a name="reporting-telephone-number-issues"></a>報告電話號碼問題？

如果您在埠完成後的前 24-48 小時內發現移轉號碼有任何問題，請連絡 [TNS 服務台](../manage-phone-numbers-for-your-organization/contact-tns-service-desk.md)。 對於超過 48 小時的任何問題，請連絡Microsoft 支援服務小組。
