...
從 Teams 共用Outlook (共用至 Teams) 可讓使用者共用電子郵件，包括附件Outlook到 Teams 中任何聊天或頻道。

## <a name="outlook-add-in-for-share-to-teams"></a>Outlook共用至Teams 

共用至Teams功能需要適用于 Outlook。 每當使用者以 Web 應用程式或桌面用戶端Teams時，系統就會自動Teams此附加元件。

> [!NOTE]
> 請務必在 Outlook 和[Exchange Online](/exchange/clients-and-mobile-in-exchange-online/add-ins-for-outlook/add-ins-for-outlook)中的用戶端存取規則中Exchange Online檢查適用于 Outlook 的附加元件，以確保[](/exchange/clients-and-mobile-in-exchange-online/client-access-rules/client-access-rules)Outlook的附加元件正確運作。 此外，停用連接體驗可能會防止Outlook的附加元件正常運作。 請參閱[在 Office](https://support.microsoft.com/topic/connected-experiences-in-office-8d2c04f7-6428-4e6e-ac58-5828d4da5b7c)中連接體驗以瞭解更多資訊。  

共用至Teams使用與使用者以電子郵件傳送頻道時相同的傳輸機制。 若要共用至聊天， (電子郵件附件) 電子郵件附件會複製到寄件者OneDrive。 若要共用至頻道，電子郵件和附件會複製到 SharePoint 中的電子郵件SharePoint。****

Outlook 共用至 Teams 的附加元件會使用需求集 1.7，詳請參閱[Outlook](/exchange/clients-and-mobile-in-exchange-online/add-ins-for-outlook/add-ins-for-outlook)的附加元件檔，其中包含 Outlook 附加元件的詳細資料、Outlook 附加元件的環境需求，以及需求集 1.7 支援的特定 Outlook 用戶端。
...
若要停用共用至Outlook所使用的Teams，請執行[此處找到的 Cmdlet](/powershell/module/exchange/disable-app?view=exchange-ps)。 
...
在桌面Teams用戶端Outlook 網頁版共用Outlook，仰賴瀏覽器 WebView。 請參閱[應用程式Office](/office/dev/add-ins/concepts/browsers-used-by-office-web-add-ins)所使用的瀏覽器，以瞭解有關哪些用戶端使用特定瀏覽器的詳細資訊。 
