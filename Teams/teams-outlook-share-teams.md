...
ms.locfileid: "60832937"
---
# <a name="share-to-teams-from-outlook"></a>從 Teams 共用Outlook

從 Teams 共用Outlook (共用Teams) 讓使用者共用電子郵件 ，包括附件，從 Outlook 到 Teams 中的任何聊天或頻道。
...
在桌面Teams用戶端Teams共用至Outlook 網頁版Outlook仰賴瀏覽器 WebView。 請參閱[應用程式Office](/office/dev/add-ins/concepts/browsers-used-by-office-web-add-ins)所使用的瀏覽器，以瞭解有關哪些用戶端使用特定瀏覽器的詳細資訊。 

> [!IMPORTANT]
> 共用Teams使用者瀏覽器必須同時啟用協力廠商 Cookie 和本地儲存空間存取。

共用Teams使用單一登入 (SSO) ，這表示使用者在透過共用至 Teams 使用附加元件時不需要提供認證。 SSO Outlook 網頁版 https://outlook.office365.com/owa/extSSO.aspx https://outlook.office.com/owa/extSSO.aspx 預設支援並回復 URL。 對於虛名網域，系統管理員需要新增適當的Azure Active Directory URL。