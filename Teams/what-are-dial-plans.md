...
ms.locfileid: "58594435"
---
# <a name="what-are-dial-plans"></a>什麼是撥號對應表？

撥號方案是一組命名的標準化規則，將個別使用者撥打的電話號碼轉換成替代格式 (通常是 E.164) ，用於通話授權和語音路由。

撥號方案包含一或多個標準化規則，定義以各種格式表示的電話號碼如何轉換成替代格式。 不同的撥號方案可能會以不同方式解譯相同的撥號字串，因此根據指派給特定使用者的撥號方案，相同的撥號號碼可能會以不同方式翻譯和路由。 最多可有 1，000 個租使用者撥號方案。

請參閱 [建立及管理撥號方案](create-and-manage-dial-plans.md) 以建立和管理租使用者撥號方案。
...
在 Teams中，有兩種撥號方案類型：服務範圍和租使用者範圍 (適用于貴組織) 。 服務範圍撥號方案會針對每個提供服務的國家/地區電話系統定義。 系統會自動為每個使用者指派符合指派給使用者的使用位置的服務國家/地區撥號方案。 您無法變更服務國家/地區撥號方案，但您可以建立租使用者範圍撥號方案，增加服務國家/地區撥號方案。 在布備用戶端時，客戶會取得「有效的撥號方案」，這是服務國家/地區撥號方案與適當範圍租使用者撥號方案的組合。 因此，不需要定義租使用者撥號方案的所有標準化規則，因為規則可能已經存在於服務國家/地區撥號計畫中。
...
> [!NOTE]
> 在未將撥號方案正規化規則適用于撥號號碼的情況下，撥號字串仍然會標準化為預先垂直的「+CC」，其中 CC 是撥號使用者使用位置的國家/地區代碼。 這適用于通話方案、直接路由和 PSTN 會議撥出案例。 此外，如果租使用者撥號方案正規化規則導致號碼不是以「+」開始，則通話服務會嘗試根據租使用者撥號方案將從 Teams 用戶端收到的號碼正規化，如果地區撥號方案沒有相符。 為了避免重複正規化，建議直接路由客戶將數位正規化，以包含 +，然後使用主幹翻譯規則移除 +。 
...
- **步驟 3** 識別每個所需撥號方案的有效號碼模式。 只需要服務層級國家/地區撥號方案未定義的號碼模式。
...
|RedmondSitePrefix  <br/> |在 222 (6) 和 Redmond 網站代碼 (222) 。  <br/> |^6222 (\\ d) {4} $  <br/> |+1425555$1  <br/> |62220100翻譯為 +14255550100  <br/> |
...
> [!NOTE]
> 上一個資料表中顯示的標準化規則名稱不包含空格，但這是一種選擇。 例如，表格中的名字可能是「5 位數分機」或「5 位數分機」，但仍然有效。
