...
您可以將裝置標記新增到多個裝置類型。 不過，當您在系統管理中心開啟裝置窗格時，只會退回該類型的裝置。 例如，您可以將「公司」標記指派給手機和Teams 會議室裝置。 如果您在使用 **DevicesPhone**  >  時搜尋「公司」Teams，則只會退回電話。**** 同樣地，如果您在  >  裝置Teams中搜尋Teams **公司Teams 會議室**，只會Teams 會議室裝置。

若要管理裝置標記，您必須是全域系統管理員、Teams系統管理員，或Teams系統管理員。有關系統管理員角色的資訊，請參閱Microsoft Teams[系統管理員角色來管理Teams](../using-admin-roles.md)。
...
2. 流覽至 **Teams，** 然後選擇任何裝置窗格 **，例如電話**。
3. 選取 **頁面** 右上角的動作，然後選取所有 **裝置標記**。
4. 若要建立裝置標記，請選取 **+新增**，為裝置標記提供值，然後 **選取儲存圖示** 。
5. 若要移除裝置標記，請選取要移除之裝置標記旁的省略號 **...**，然後 **選取刪除。**
    > [!NOTE]
...
1. 請流覽 以Microsoft Teams系統管理中心。 https://admin.teams.microsoft.com
2. 流覽 **至Teams，** 然後選擇包含要新增或移除標記之裝置之裝置窗格。
