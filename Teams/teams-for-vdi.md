...
audience: admin
description: 瞭解如何在虛擬桌面Microsoft Teams虛擬桌面基礎結構或 VDI (中) 執行。
...
桌面Teams應用程式已經過與領先虛擬化解決方案提供者的驗證。 對於多個市場提供者，我們建議您諮詢您的虛擬化解決方案提供者，以確保您符合最低需求。
...
從快取檔案夾 Teams %appdata%/Microsoft/Teams 排除下列Teams。 排除這些專案有助於減少使用者緩存大小，進一步優化非持續性設定。
...
Microsoft 365 Apps 企業版不支援每部電腦安裝 Teams。 若要使用每部電腦安裝，您必須將Teams排除Microsoft 365 Apps 企業版。 請參閱[將桌面Teams應用程式部署到 VM，](#deploy-the-teams-desktop-app-to-the-vm)以及如何透過 Teams[排除Microsoft 365 Apps 企業版](#how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise)部署。

#### <a name="how-to-exclude-teams-deployment-through-microsoft-365-apps-for-enterprise"></a>如何透過Teams排除Microsoft 365 Apps 企業版

若要深入瞭解Teams Microsoft 365 Apps 企業版，請參閱如何將 Teams 排除在 Microsoft 365 Apps 企業版[的新安裝](/DeployOffice/teams-install#how-to-exclude-microsoft-teams-from-new-installations-of-office-365-proplus)，以及使用群組原則來控制[Teams。](/DeployOffice/teams-install#use-group-policy-to-control-the-installation-of-microsoft-teams)
...
2. 執行下列其中一個命令，將 MSI 安裝到 VDI VM：
    ...
    - 每部電腦安裝
        ...
        此程式會Teams 64 位作業系統上的程式檔案 (x86) 資料夾，以及 32 位作業系統上的程式檔案資料夾。 此時，金色影像設定已完成。 非Teams需要每部電腦安裝一次。
...
3. 從 VDI VM 卸載 MSI。 有兩種方法可以卸載Teams。
    ...
    - 命令列：執行下列命令。
      ...
      此程式會Teams程式檔案 (x86) 資料夾或程式檔案資料夾 ，視作業系統環境不同。
...
如果您要從 VDI 上的 商務用 Skype 移至 VDI 上的 Teams，除了兩個應用程式之間的差異之外，在 VDI 也實現時也會有一些差異。 VDI 中目前不支援的Teams VDI 商務用 Skype如下：
...
[**會議政策**](meeting-policies-in-teams.md)：Teams包含內建的 AllOff 會議政策，其中會關閉所有會議功能。 將 AllOff 政策指派給組織中在虛擬化環境中使用Teams所有使用者。
...
1. 在系統管理中心的左側Microsoft Teams，前往您想要指派的政策。 例如：
...
您可以使用系統管理中心Microsoft Teams PowerShell 來設定和指派通話和會議政策給使用者。 可能需要幾個小時 (，) 策略變更才能傳播。 如果您沒立即看到給定帳戶的變更，請在幾個小時後再試一次。
...
- 每部電腦安裝Teams VDI 上的更新不會以非 VDI 用戶端Teams的方式自動更新。 您必須安裝新的 MSI 以更新 VM 映射，如在[VDI](#install-or-update-the-teams-desktop-app-on-vdi)上安裝或更新Teams桌面應用程式中所述。 您必須解除安裝目前的版本，以更新至較新的版本。
- 在 Citrix 環境中，如果使用者在 Teams 執行時與虛擬機器中斷連接，Teams 更新可能會導致使用者重新連接時，AV 狀態未優化。 我們建議您在使用者從Teams中斷連接前先退出此帳戶，以避免發生此情況。
- Teams使用者或每部電腦部署。 不支援Teams使用者和每部電腦同時部署用戶端。 若要從每部電腦或每個使用者遷移到其中一種模式，請遵循卸載程式，然後重新調配至任一模式。
- Azure 虛擬桌面目前不支援 macOS 和 Linux 型用戶端。
- 快速租使用者切換可能會導致 VDI 上的通話相關問題。 重新開機用戶端將會減輕這些問題。
...
- [在 azure Microsoft Teams桌面使用帳戶](/azure/virtual-desktop/teams-on-wvd)
