...
ms.service: msteams
audience: admin
description: 瞭解在組織中管理大型團隊Microsoft Teams，以滿足貴組織的需求。
f1.keywords:
- NOCSH
...
ms.locfileid: "58619669"
---
# <a name="manage-large-teams-in-microsoft-teams---best-practices"></a>管理大型團隊Microsoft Teams - 最佳做法

Microsoft Teams能促進與數十個成員和擁有數千個成員的大型群組之間的溝通。 針對[小組大小更新](limits-specifications-teams.md)Teams限制和規格。 團隊規模增加會導致獨特的管理和營運挑戰。 本文將說明建立及管理由數千個成員組成的大型團隊的最佳作法。
...
- **員工資源群組中的共同** 作業：組織通常有一大群共同興趣的人，他們屬於不同的部門或工作組。 舉個例說，可以有一群對個人財務與投資充滿熱忱的人。 在大型組織中通常難以聯繫。 若要為這類群組開發社群，租使用者系統管理員可以建立大型團隊，做為全公司的公用資源群組，任何人都可以加入並利用。 最後，這些社群會收集新成員和現有成員都可以享有的資訊。

- **內部和外部成員** 之間的共同合作：熱門產品通常會開發一個早期採用者社群，他們急迫嘗試新產品發行，並提供意見回饋。 早期採用者會與產品群組建立關係，協助塑造產品。 在這種情況下，租使用者系統管理員可以設定一個包含內部產品群組和外部產品評估員的大型小組，以促進豐富的產品開發程式。 這些團隊也可以為一組選取的客戶提供客戶支援。

## <a name="create-teams-from-existing-groups"></a>從現有的群組建立團隊

使用連絡人群組、安全性群組或Office群組來快速啟動您的小組。 您可以輸入群組來建立團隊，或從群組建立Office團隊。

將 **群組** 匯出成團隊：當您將最多 3，500 個成員的群組導入 Teams 時，Teams會自動計算群組中的成員總數。 此為一次導入，且群組中未來的變更將不會在 Teams 中自動更新。

**從大型群組Microsoft 365** 團隊：當您從大型群組Microsoft 365團隊時，成員會自動成為群組 **Microsoft 365團隊的** 一部分。 日後，當小組成員加入或離開Microsoft 365群組時，系統會自動新增或移除他們。
...
若要加快新的使用者互動和內容探索，您可以選取使用者預設可用的最愛頻道。 在 **系統管理中心的** 頻道窗格中，檢查顯示成員欄 **下的頻道** 。
...
為了避免新增令人分心的應用程式或 Bot，團隊擁有者可以停用、新增、移除及上傳小組成員的應用程式和連接器。 在系統管理 **中心設定 >**，取消勾選三個允許成員新增 App 或連接器的選項。
...
小組和頻道提及可以用來吸引整個團隊對特定頻道文章的注意。 在文章使用提及後，通知會發送給數千名小組成員。 如果通知過於頻繁，則小組成員可能會超載，並可能會向團隊擁有者抱怨。 若要避免團隊或頻道提及，請取消勾選團隊或頻道窗格中的框，以關閉成員 **設定 > @mentions提及。**
