...
根據租使用者管理員定義的網路拓撲，小組用戶端會在 (.LIS) 的位置資訊服務要求中提供網路連線資訊。 如果有相符的專案，則 .LIS 會傳回用戶端的位置。 這個位置資料會傳回用戶端。  

團隊用戶端包含位置資料做為緊急通話的一部分。 然後，緊急服務提供者會使用這個資料來判斷適當的公用安全回應點 (PSAP) ，並將呼叫路由到該 PSAP，這可讓 PSAP dispatcher 取得來電者的位置。  
...
若要將自動路由至適當的公用安全應答點， (PSAP) 會根據小組使用者使用的國家/地區而有所不同。

如需緊急通話的詳細資訊，包括緊急位址及緊急通話路由的相關資訊、國家/地區的相關資訊，以及網路設定和網路拓朴的相關資訊，請參閱下列內容：
...
[網路設定] 包含包含子網集合的網站，而這些網站則專門用於動態原則指派給使用者。 例如，緊急通話原則與緊急呼叫路由策略可能會指派給「雷蒙德網站」，讓從家裡或其他 Microsoft 位置進行漫遊的任何使用者都是以緊急電話號碼、路由及安全服務台進行設定。  
...
- 如果需要對 security 辦公桌通知進行動態設定，您必須設定信任的 IP 位址和網路網站。

- 如果只需要動態位置，您必須只設定信任的 IP 位址。
...
團隊用戶端從與不同網路識別碼相關聯的位置取得緊急位址。 子網和無線存取點都支援 (WAPs) 。 在 Windows 8.1 和更新版本上支援乙太網路交換器/埠。

若要讓用戶端取得位置，您必須使用網路識別碼來填入 IIS， (子網、WAPs、交換器、埠) 與緊急位置。 您可以在 Microsoft 團隊系統管理中心或使用 PowerShell 執行此動作。
...
您可以將緊急通話路線原則與緊急通話原則指派給使用者和網站。 請記住，緊急呼叫路由策略只適用于直接路由。  (雖然您可以將此原則指派給通話方案使用者，但原則將沒有任何效果。 ) 

您可以在 Microsoft [團隊管理中心] 或 [使用 PowerShell] 中指派原則。 若要深入瞭解，請參閱：
