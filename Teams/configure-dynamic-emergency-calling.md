...
[網路設定] 包含包含子網集合的網站：這些都是專門針對動態原則指派給使用者。  例如，TeamsEmergencyCalling 原則和 TeamsEmergencyCallRouting 原則可能會指派給「雷蒙德網站」，讓從家裡或其他 Microsoft 位置進行漫遊的任何使用者都可以使用緊急號碼、路由及安全性服務台進行設定。針對雷德蒙者。  
