...
## <a name="prerequisites-for-enabling-the-calls-tab-in-teams"></a>在 Teams 
...
若要確保傳入的 VOIP 和 PSTN 通話一直登入使用者的 Teams 用戶端，請更新使用者的並存模式為 TeamsOnly (，也就是說，請指派他們 TeamsUpgradePolicy 的 「UpgradeToTeams」實例。  有關共存模式和 TeamsUpgradePolicy 詳細資訊，請參閱移移和互通性指南，以引導組織Teams[共同](./migration-interop-guidance-for-teams-with-skype.md)商務用 Skype
