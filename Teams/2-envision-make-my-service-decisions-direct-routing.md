...
> [!TIP]
> SBCs 可以*指定為 [* 作用中] 和 [*備份*]。 這表示當被設定為適用于此數值模式的 SBC （或數位模式 + 特定使用者）無法使用時，會將呼叫路由至 backup SBC。
...
如需詳細資訊，請參閱[將小組專用模式指派給使用者，以確保在 Microsoft 團隊中撥打土地](direct-routing-configure.md#assign-teams-only-mode-to-users-to-ensure-calls-land-in-microsoft-teams)。
