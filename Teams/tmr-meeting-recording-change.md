...
> [!NOTE]
> 從傳統Teams到 OneDrive SharePoint (ODSP) 會議錄製的儲存變更，于 2020 年 8 月 30 日完成。 所有錄製現在都儲存在 ODSP 中。 此變更會取代錄製StorageMode政策，而修改 PowerShell 中的設定將不再有任何影響。

|日期&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|事件&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;                                                                                                                                                                                                                                                                                                             |
|:-----------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|2020 年 10 月 5 日<br> *(已完成)* &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;| 您可以啟用 Teams 會議政策，將會議錄製儲存為商務用 OneDrive，SharePoint而非 Microsoft Stream (傳統) |
|2021 年 1 月 7 日開始推出<br> *(已完成)* &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|除非您修改Teams會議Teams，並明確將其設定為 Stream 來延遲這項變更，否則所有新的會議商務用 OneDrive和 SharePoint 都會儲存至 商務用 OneDrive 和 **SharePoint。** 僅看到原則報告為 Stream 是不够的。 您需要明確地將原則值設定為 **Stream**。|
...
|從 2021 年 8 月 16 日開始逐步推出<br> *(已完成)* &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|**所有客戶 (企業、教育和 GCC)**<br>無法將新的會議錄製儲存到 Microsoft Stream (傳統) ;所有客戶都會自動將會議錄製儲存商務用 OneDrive SharePoint，即使他們已經將會議Teams變更為 Stream。<br><br> 我們建議客戶，在您對變更感到滿意選擇加入，而非等待變更生效，以便更好地控制組織中的變更。 |

Microsoft Teams 有用於儲存會議錄製的新方法。 這是從傳統 Microsoft Stream 轉換到新 Stream[](/stream/streamnew/new-stream)的第一個階段，此方法會將錄製儲存在 Microsoft OneDrive 商務用和 SharePoint Microsoft 365 中，並提供許多優點。

> [!NOTE]
> 如果Teams會議錄製無法成功上傳到 OneDrive/SharePoint，就會出現「錄製未預期結束」錯誤訊息，而錄製將會暫時儲存至 Azure 媒體服務 (AMS) 。 儲存于 AMS 後，系統不會嘗試嘗試自動將錄製上傳到 OneDrive/SharePoint串流。
...
> [!Note]
> 如果您的部分使用者已指派每個召集人或每個使用者的策略，如果您希望他們同時將會議錄製儲存在 商務用 OneDrive 和 SharePoint，您必須在此策略上設定此設定。 如需詳細資訊，請參閱[在 Teams 中管理會議原則](meeting-policies-in-teams.md)。
...
**當串流 (例如錄製) 儲存在 SharePoint/OneDrive中時，如何決定它們要在哪裡？系統管理員是否能夠變更其位置？**

根據預設，所有錄製檔案都會OneDrive記錄的使用者 **的帳戶。** 對於頻道會議，錄製內容會一SharePoint到頻道的網址。 系統管理員無法變更錄製的儲存位置。

**如何處理來自前員工的錄製？**

由於影片與 商務用 OneDrive 和 SharePoint 中任何其他檔案一樣，因此在員工離職後處理擁有商務用 OneDrive保留SharePoint[程式](/onedrive/retention-and-deletion#the-onedrive-deletion-process)。
...
如果會議錄製從原始位置移動Teams或複製會議錄製，則不支援隱藏式商務用 OneDrive SharePoint。
...
Teams錄製的檔案會商務用 OneDrive SharePoint，並包含在這些服務的配額中。 請參閱[SharePoint配額](/sharepoint/sites/plan-site-maintenance-and-management#quotas)[商務用 OneDrive配額](/onedrive/set-default-storage-space)。

與 Stream 相比，[您可以](/office365/servicedescriptions/sharepoint-online-service-description/sharepoint-online-limits)商務用 OneDrive更多儲存空間，以及使用 SharePoint。
...
**如果使用者沒有儲存空間或商務用 OneDrive，或儲存SharePoint配額已滿，錄製會在哪裡？**
