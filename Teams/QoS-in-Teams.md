...
Microsoft 團隊中的服務品質（QoS）是一種允許即時網路流量對網路延遲（例如，語音或影片資料流程）保密的方式，以不太敏感的通信量（例如下載新的應用程式，需要額外的時間來下載，而不是大筆）。 QoS 會識別並標示即時資料流中的所有資料包（使用 Windows 群組原則物件，以及稱為埠的存取控制清單的路由功能，如以下所示），這可協助您的網路為網路頻寬的專用部分提供語音、影片和螢幕共用。
...
- 延遲的往返時間（RTT）-媒體資料包需要很長的時間才能到達其目的地，這會在交談中的兩個參與方之間產生明顯的延遲，導致其他人互相交談
...
網路上的流量堵塞會大大影響媒體質量。 缺乏頻寬會導致效能下降，以及不佳的使用者體驗。 隨著團隊採用和使用量的增加，您可以使用 [報告]、[[每使用者通話分析](use-call-analytics-to-troubleshoot-poor-call-quality.md)] 和 [[通話品質儀表板] （CQD）](turning-on-and-using-call-quality-dashboard.md)來找出問題，然後使用 QoS 及選擇性頻寬新增功能進行調整。
...
當您實現 QoS 時，您會使用數個擁塞管理功能（例如 Cisco 的優先順序佇列和[類別式加權公平排列（CBWFQ）](https://www.cisco.com/en/US/docs/ios/12_0t/12_0t5/feature/guide/cbwfq.html#wp17641)）和擁塞規避功能（例如[加權隨機的預先檢測（WRED）](https://en.wikipedia.org/wiki/Weighted_random_early_detection)）來定義多個佇列。
...
您也可以使用群組原則物件（GPO）來實現 QoS，以引導用戶端裝置在 IP 包頭中插入 DSCP 標記，將其識別為特定類型的通訊（例如，語音）。 路由器和其他網路裝置可以設定為可辨識此情況，並將通信量放在個別、較高優先順序的佇列中。
...
在團隊中，不同工作負載所使用的 QoS 來源埠應該積極管理，並視需要調整。 參照表格在[每個媒體類型的 [初始埠範圍](#choose-initial-port-ranges-for-each-media-type)] 中，埠範圍都是可調整的，但 DSCP 標記無法進行設定。 一旦您實現這些設定之後，您可能會發現特定媒體類型需要多或較少的埠。 [[每使用者通話分析](use-call-analytics-to-troubleshoot-poor-call-quality.md)] 和[[通話品質儀表板（CQD）](turning-on-and-using-call-quality-dashboard.md) ] 應該用於決定在團隊實施之後調整埠範圍，以及定期視需要變更。
