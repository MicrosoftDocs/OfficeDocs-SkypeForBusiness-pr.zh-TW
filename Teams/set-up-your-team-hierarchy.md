...
ms.locfileid: "58597947"
---
# <a name="set-up-your-team-targeting-hierarchy"></a>設定您的團隊目標階層

設定團隊目標階層會允許貴組織將內容發佈至一組大型團隊。 團隊目標階層會定義階層中所有團隊彼此關聯、哪些使用者可以發佈工作，以及哪些團隊使用者有權發佈工作。 除非為貴組織設定小組目標階層，否則所有使用者的發佈功能都停用。 若要設定小組目標階層，您必須建立定義階層的檔案，然後將它上傳到 Teams以將其套用至您的組織。 架構上傳之後，Teams應用程式就可以使用它。
...
以下範例說明階層在 Teams 中如何Teams。 建立工作清單之後，發佈小組成員就可以選取要傳送的收件者團隊 (發佈) 清單的收件者。 選取團隊時，發佈小組可以按照階層、屬性或兩者的組合來篩選。<br>
...
例如，在下列階層中，回收、通訊和人力資源可以將工作發佈至階層中的每個下層節點 (小組) ，但西北區只能將工作發佈至紐約市和波士頓市商店團隊。 範例階層可讓回收、通訊和人力資源群組發佈適用于整個公司的工作，例如福利資訊或來自 CEO 的郵件。 美國西北地方只能將人員排程、天氣資訊等工作發佈至紐約市和波士頓市商店團隊。
...
定義階層的架構是根據 CSV 檔案中的逗號分隔 () 值。 檔案必須採用 UTF-8 格式。 CSV 檔案中的每個列會對應到團隊階層中的一個節點。 每一列都包含為階層內節點命名的資訊，選擇性地將節點連結至團隊，以及可用於篩選支援該節點之應用程式中團隊的屬性。
...
|如果您需要為屬性指定多個值，且這些值並非互斥，請使用 **屬性名稱：UniqueValue** 格式作為欄名稱。 <br><br>**重要** 請確定使用僅英文冒號 (：) unicode 不支援做為屬性欄分隔符號。 |冒號前的文字字串 (：) 成為屬性的名稱。 在冒號 (：) 前包含相同文字字串的所有欄會組成一個區段，組成篩選功能表中的一個區段。 冒號後的每個字串都會變成該節的值。<br><br>每一列的值為 0 (0) 或 1。 值為 0 表示屬性不適用於節點，而值為 1 表示屬性會適用于該節點。|您希望使用者能夠根據部門篩選儲存區。 商店可以有多個部門，因此此屬性的值並非互斥。<br><br>在此範例中，我們新增部門：服裝、部門：電子、部門：食物、部門：住家和園藝、部門：運動用品做為屬性欄。 部門會變成屬性名稱，使用者可以根據服裝、電子、食物、住家園藝和運動用品部門進行篩選。|

當您新增屬性欄時，請記住下列事項：

* 您指定的欄名稱或在冒號 (：) 之前指定的欄名稱，會變成屬性的名稱。 此值會顯示在使用階層Teams應用程式中。
* 您的階層中最多隻能有 50 個屬性欄。
...
* 欄名稱前面必須有一個雜湊標記 (#) 。 它最多隻能有 100 個字元，只包含 A-Z、a-z 和 0-9 的字元。 例如，#Operations及#Frozen貨物。
...
屬性 `Store layout` 具有包含 、和 `Compact` `Standard` 的值 `Large` 。 屬性 `Departments` 欄可以設定為 0 (`0` 或) 的值 `1` 。 上述 `Store` 影像不會顯示版面配置 `Departments` 和屬性。 它們新增在這裡，可協助顯示如何將屬性新加到節點專案。 這三個數據桶欄也是如此。
...
下列腳本可用來建立團隊，並上傳.csv檔案至您的Microsoft Teams租使用者。 如果您有現有的階層，此腳本會取代它。
