...
| 資料類型 | V1 | V2 和 V2.1 |
|:--- |:--- |:--- |
| **使用者** |僅支援 [授課者] 角色，因此需要手動設定您的教育領導者的組織層級權限|支援多個角色，以便設定角色型權限|
| **組織** | 僅支援 [學校] 的彙總層級。<br><br>因此，不提供多重匯總層級，並提供比較不同類型學校 (例如小學與中學、科學與藝術學校) 的有限能力|支援多層階層圖，包括學區/機構、大學、學院、教職員、校園、地區、方案等。<br><br>允許多個匯總層級，並輕鬆比較每個層級的組織單位、指派權限至特定層級、根據組織層級設定目標等等。|
...
- 確保 **反映出組織的真實完整階層圖**。 在某些情況下，此階層圖不會反映在 SIS 中，在這種情況下，需要手動新增到 CSV 檔案，才能同步處理。
