...
取得服務號碼 (服務號碼是自動電話機使用的特殊電話號碼類型) 這是您希望從組織外部直接撥號來取得自動總機所需的電話號碼。 這可能包括 [從另一個提供者移轉號碼](../phone-number-calling-plans/transfer-phone-numbers-to-teams.md) 或 [要求新的服務號碼](../getting-service-phone-numbers.md)。
...
> [!NOTE]
> 如果您第一次遵循設定商務語音的步驟，而且您正使用步驟 **6：** 設定公司主要電話號碼的自動語音機，表示您已完成此選項卡上的步驟。移至下一個定位停駐點： [自動傳訊一般資訊](?tabs=general-info#steps)。

您建立的每個自動助理都需要資源帳戶。 這類似于使用者帳戶，除了該帳戶與自動通話或通話佇列相關聯，而不是與人員相關聯。 在此步驟中，我們會建立帳戶、指派帳戶Microsoft 365 電話系統 *虛擬* 使用者授權，然後指派服務號碼。
...
如果您需要讓電話號碼可以撥打這個自動電話機，請指派該號碼給資源帳戶。
...
4. 如果您想要使用此自動總機做為公司目錄，請在目錄搜尋 **下，選取****按名稱撥號**。 當您啟用此選項時，來電者可以說出使用者的名稱，或在電話鍵臺上輸入。 任何擁有授權電話系統使用者都是合格的使用者，而且可以使用名稱撥號。 
