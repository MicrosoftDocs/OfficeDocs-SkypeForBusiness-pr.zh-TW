...
透過部署新技術，您的組織可以實現成本節約、安全性合規性、員工滿意度及操作效率等業務價值，但也會影響您的使用者的生產力與組織結構 (您的網路) 。 在您的整個組織啟用新技術之前，請先執行正式的使用者試驗。 就像您在繪製整個聊天室前，在牆上繪製小的色彩片一樣，您可以執行試驗來驗證技術與使用者的準備情況、找出並減少問題，並協助確保整個組織範圍的實施順利完成。
...
- https://aka.ms/SkypeToTeams-SetCoexistence)您想要驗證的 (。
...
完成試驗之後，就可以收集所有意見反應問卷、最終的網路統計資料，以及針對您的目標進行分析的支援入場券，判斷是否要實施您的轉寄方案。 您可能會發現貴組織已準備好進行廣泛的部署，或者您想要將您的試驗延伸到更多使用者，或者您想要在您發現的任何問題遭到緩解之後，日後再重新查看試用版。 請記住，您的試驗是在 _可控_ 環境中預測技術與使用者結果的絕佳方式。小心地提前跳躍了。

如果您的結果指出：

- **您的試驗目標 (例如，已完成使用者滿意度和網路品質)**，您應該準備好繼續進行下一個階段的推出。 根據專案的目標，這可能是下列其中一項：
  - 將試生產延伸至其他參與者
  - [在商務用 Skype (**孤島** 模式下，為部分或所有組織啟用團隊)](https://aka.ms/SkypeToTeams-SetCoexistence)
  - [從商務用 Skype 將使用者升級至團隊 (**只有** 部分或所有組織的模式)](https://aka.ms/SkypeToTeams-SetCoexistence)
- **您的試用版並未達到您想要的結果 (例如，使用者滿意度和網路品質)**，請花一些時間來調整方案的適當調整，並重新討論您的試用版。
