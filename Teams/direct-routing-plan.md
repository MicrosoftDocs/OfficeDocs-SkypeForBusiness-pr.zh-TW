...
Microsoft 電話直接路由可讓您將支援、客戶提供的會話邊界控制器 (SBC) 系統Microsoft 電話系統。  例如，您可以使用這項功能設定內部部署公用交換電話網絡 (PSTN) 與 Microsoft Teams 用戶端的Microsoft Teams，如下圖所示： 

![顯示內部部署 PSTN 連接配置的圖表。](media/PlanDirectRouting1-PSTNwithTeams.png "與用戶端內部部署 PSTN Microsoft Teams組")

  > [!NOTE]
  > 商務用 Skype Online 也可讓您配對客戶提供的 SBC，但這需要在 SBC 和 Microsoft Cloud 之間進行內部部署 商務用 Skype Server 部署或稱為雲端連接器的 商務用 Skype 特殊版本。 此案例稱為混合式語音。 相反地，直接路由允許在支援的 SBC 和 Microsoft Cloud 之間直接連接。
...
- 設定客戶所有電話設備之間的互通性，例如 PBX (PBX) 、類比裝置和 Microsoft 電話 系統。
...
|Microsoft 365或Office 365組織|您Microsoft 365或Office 365使用者，以及 SBC 的組Microsoft Teams或連至 SBC 的組織。|
...
使用者可以Teams PSTN 或 Teams開始一對一Teams通話Teams PSTN 參與者。 此案例稱為臨時會議。 通話路徑取決於升級通話的使用者是否已指派 Microsoft 音訊會議授權：
...
直接路由 SIP 介面只會信任由憑證頒發機構 (註冊的憑證) Microsoft 信任的根憑證計畫之一部分。 請確定您的 SBC 憑證由程式的一部分 CA 簽署，且憑證的擴充金鑰使用量 (EKU) 包括伺服器驗證。
