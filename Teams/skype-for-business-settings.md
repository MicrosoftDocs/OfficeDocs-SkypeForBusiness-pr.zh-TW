...
如果貴組織的共存商務用 Skype未設為只顯示 ，您只會看到 Teams **頁面**。 同樣地，您只會看到使用者的商務用 Skype，如果使用者的共存模式不是只 **Teams。** 若要深入瞭解共存模式，請參閱瞭解Teams及商務用 Skype互通性，[以及](teams-and-skypeforbusiness-coexistence-and-interoperability.md)設定[您的共存和升級設定](setting-your-coexistence-and-upgrade-settings.md)。
...
- 如果您關閉推入通知，使用者下次在行動裝置上啟動推商務用 Skype就會收到所有通知。
...
## <a name="manage-skype-for-business-settings-for-individual-users"></a>管理商務用 Skype使用者的個人設定
