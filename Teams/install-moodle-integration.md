...
description: 瞭解如何在 LMS Learning (LMS) 中安裝及設定 Moodle open-source Learning 系統管理Microsoft Teams。
...
[Moodle](https://moodle.org/)是 LMS Learning中最熱門的 (開放來源管理系統) ，現在已與 Microsoft Teams！ 此整合可協助授課者和教師在 Moodle 課程中共同作業、詢問成績和作業相關問題，以及隨時更新通知 ，Teams！
...
1. 從左側 **Upload** 選取 [檔案選項>、流覽上方下載的檔案，Upload **此檔案**。
...
1. 執行 PowerShell 腳本，方法為：
    ...
    * 輸入應用程式名稱Azure AD Ex (名稱。 Moodle/Moodle 外掛程式) 。
    * 輸入您的 Moodle 伺服器的 URL。
    * 複製 **腳本產生的****應用程式** 識別碼和應用程式金鑰，然後儲存它們。
1. 接下來，您需要在 Moodle 外掛程式中新增 Id 和 Key。 返回外掛程式系統管理頁面 (外掛程式>外掛程式> Microsoft 365整合) 。
...
1. 接下來，您需要在 Moodle 伺服器和伺服器之間同步Azure Active Directory。 視您的環境，您可以在此階段選取不同的選項。 請注意，您在這裡設定的配置會隨著每一個 Moodle cron 執行， (一天執行一次) 讓所有專案保持同步。若要開始使用：
    ...
    * 在同步 **處理使用者Azure AD** 區段，選取適用于您環境的核取方塊。 一般來說，您至少會選取：
        * 在 Moodle 中為 Azure AD
        * 更新 Moodle 中所有使用者的帳戶，Azure AD
    * 在使用者 **建立限制** 區段，您可以設定篩選，Azure AD同步到 Moodle 的使用者。
    * 使用 **使用者欄位映射** 區段，您可以自訂 Azure AD至 Moodle 使用者設定檔欄位的映射。
    * 在 Teams **同步** 處理區段，您可以選擇自動建立群組 (，Teams) 或所有現有 Moodle 課程的群組。
1. 若要驗證 cron 工作 (如果您想要第一次執行，請手動執行) 請按一下 [同步處理使用者與 Azure AD> 區段 **的 [** 排定的工作管理Azure AD連結。**** 這會將您帶至排 **程工作** 頁面。
...
1. 返回網站管理與整合 (外掛程式>管理> Microsoft 365管理) ，然後選取 **Teams 設定。**
1. 按一下 **Microsoft 應用程式註冊入口網站** 連結，然後使用 Microsoft Id 登入。
1. 輸入應用程式的名稱，例如 (名稱。 MoodleBot) 並按一下 [ **建立>** 按鈕。
1. 複製應用程式 **識別碼，** 然後將其貼到小組頁面的 Bot 應用程式 **識別碼****設定** 欄位。
1. 按一下 [ **產生新密碼>** 按鈕。 複製產生的密碼，並貼 **到小組頁面** 的 Bot 應用程式 **密碼設定** 欄位。
1. 卷起到表單底部，然後按一下 [ **儲存變更**。

現在，您產生了應用程式識別碼和密碼，是時候將 Bot 部署到 Azure 了。 按一下 [部署至 **Azure**> 按鈕，並填寫表單中的必要資訊 (Bot 應用程式識別碼、Bot 應用程式密碼和 Moodle 密碼位於小組 **設定** 頁面上，而 Azure 資訊位於 [設定) > 頁面上。**** 填寫好表單之後，請按一下核取方塊以同意條款與條件，然後按一下 [購買 (所有 Azure 資源會部署到免費層級) 。
...
8. 按一下 **[儲存變更**
9. 儲存變更之後，請回到 [小組 **設定**> 選項卡，按一下 [下載清單檔案> 按鈕，然後將清單套件儲存到您的電腦 (您將在) 的下一節中使用它。

## <a name="step-4-deploy-your-microsoft-teams-app"></a>步驟 4：部署您的Microsoft Teams應用程式

> [!VIDEO https://www.youtube.com/embed/2rMb7gtM_ZM]

現在，您的 Bot 已部署至 Azure，且已配置為與 Moodle 伺服器交談，現在該是部署您的 Microsoft Teams 應用程式。 若要這麼做，您將在上一個步驟的 Moodle 外掛程式小組 設定載入您下載的清單檔案。

安裝應用程式之前，您必須確認已啟用外部 App 和側載應用程式。 若要這麼做，您可以遵循 [這些步驟](./admin-settings.md)。 一旦確定已啟用外部應用程式後，您可以遵循下列步驟來部署應用程式。

1. 開啟Microsoft Teams。
2. 按一下 **流覽** 欄左下角的 [市集」 圖示。
3. 按一下Upload **選項** 清單中的自訂應用程式連結。 *注意：* 如果您是以全域管理員登入，您可以選擇將應用程式上傳到組織的 App Store，否則您只能載入 Teams 應用程式 (「側載」) 。
4. 選取您 `manifest.zip` 先前下載的套件，然後按一下 [ **儲存**。 如果您尚未下載清單套件，您可以從 Moodle 中外掛程式設定的小組清單選項卡執行此操作。
...
若要與我們分享任何功能要求或意見回應，請流覽我們的 [意見回應入口網站](https://feedbackportal.microsoft.com)。
