...
description: 瞭解如何在 LMS Learning中安裝及設定 Moodle Learning () 應用程式Microsoft Teams。
...
ms.locfileid: "58592299"
---
# <a name="installing-the-moodle-integration-with-microsoft-teams"></a>安裝 Moodle 與 Microsoft Teams

> [!VIDEO https://www.youtube.com/embed/OHlPt22nKoE]

[Moodle](https://moodle.org/)是 LMS Learning中最熱門的 (開放來源管理系統) ，現在已與 Microsoft Teams 整合！ 此整合可協助授課者和教師圍繞 Moodle 課程共同作業、詢問成績和作業相關問題，以及隨時更新通知 ，Teams！
...
* 自動將 Moodle 定位停駐點和 Moodle 小幫手 Bot 自動安裝在每個同步的團隊中。  (即將推出) 
...
1. 從左側 **Upload** 選取檔案選項，流覽上方下載的檔案，Upload **此檔案**。
...
1. 從 Microsoft 365 或 Office 365整合外掛程式頁面，選取設定 **選項卡**。
...
1. 接下來，您需要在 Moodle 伺服器和伺服器之間同步Azure Active Directory。 視您的環境，您可以在此階段選取不同的選項。 請注意，您在這裡設定的配置會隨著每一個 Moodle cron 執行 (一天執行一次) 讓所有專案保持同步。若要開始使用：
    ...
    * 在 Teams **同步** 處理區段，您可以選擇自動建立群組 (，Teams) 或所有現有 Moodle 課程的群組。
...
1. 返回外掛程式管理頁面 (管理>外掛程式> Microsoft 365整合) 並選取 **Teams 設定頁面。** 您需要設定一些安全性設定，才能啟用Teams整合。
    ...
    * 接下來，您需要編輯已驗證的使用者許可權，才能讓他們建立 Web 服務權杖。 按一下 **[編輯角色'已驗證的使用者'連結** 。 向下卷起並尋找 **建立 Web 服務權杖** 功能，並 **標記允許核取方塊** 。

## <a name="step-3-deploy-the-moodle-assistant-bot-to-azure"></a>步驟 3：將 Moodle 小幫手 Bot 部署到 Azure

> [!VIDEO https://www.youtube.com/embed/gbkJxf8FlfY]

免費的 Moodle Assistant Bot Microsoft Teams可協助教師和學生回答在 Moodle 中課程、作業、成績和其他資訊的問題。 Bot 也會在 Teams 內傳送 Moodle 通知給學生和Teams。 此 Bot 是由 Microsoft 維護的開放原始碼專案，可在[GitHub。](https://github.com/microsoft/Moodle-Teams-Bot)
...
若要安裝 Bot，您首先需要在 [Microsoft 身分](https://identity.microsoft.com/Landing)識別平臺上註冊。 這可讓您的 Bot 針對您的 Microsoft 端點進行驗證。 若要註冊 Bot：

1. 返回外掛程式系統管理頁面 (管理>外掛程式> Microsoft 365整合) ，然後選取 **Teams 設定。**
...
現在，您產生了應用程式識別碼和密碼，是時候將 Bot 部署到 Azure 了。 按一下 [部署至 **Azure>** 按鈕，並填寫表單中的必要資訊 (Bot 應用程式識別碼、Bot 應用程式密碼和 Moodle 機密位於小組 **設定** 頁面上，而 Azure 資訊位於 [設定) > 頁面上。 填寫好表單之後，請按一下核取方塊以同意條款與條件，然後按一下 [購買 (所有 Azure 資源會部署到免費層級) 。
...
5. 從 **概觀區段** 複製訊息 **端點** 。
6. 在 Moodle 中，開啟您 **Moodle 外掛程式設定** 小組清單頁面。
...
現在，您的 Bot 已部署至 Azure，且已配置為與 Moodle 伺服器交談，現在該是部署您的 Microsoft Teams 應用程式。 若要這麼做，您將在上一個步驟的 設定 載入您從 Moodle 外掛程式小組下載的清單檔案。

安裝 App 之前，您必須確認已啟用外部 App 和側載應用程式。 若要這麼做，您可以遵循[下列步驟。](./admin-settings.md) 一旦確定已啟用外部應用程式後，您可以遵循下列步驟來部署您的應用程式。

1. 開啟Microsoft Teams。
2. 按一下 **流覽** 欄左下角的 [市集」 圖示。
3. 按一下Upload **選項清單中的** 自訂應用程式連結。 *注意：* 如果您是以全域管理登入，您可以選擇將應用程式上傳到組織的 App Store，否則您只能載入 Teams 應用程式，而您屬於 ("sideloading") 的一部分。
4. 選取 `manifest.zip` 您先前下載的套件，然後按一下 [ **儲存**。 如果您尚未下載清單套件，您可以從 Moodle 中外掛程式設定的 Team設定中執行此操作。
