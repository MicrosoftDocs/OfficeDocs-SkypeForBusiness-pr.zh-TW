...
本節中的文章適用于連線到公用交換電話網路 (PSTN) 的所有選項：通話方案、電信業者連線、電信業者連線行動 (公開預覽版) ，以及直接路由。 如需所有 PSTN 連線選項的詳細資訊，請參閱 [PSTN 連線選項](pstn-connectivity.md)。

如果您選擇 [通話方案]、[運算子連線] 或 [電信業者連線行動，則大部分的通話路由是由 Microsoft 或您的提供者負責。 不過，直接路由需要額外步驟來設定通話路由。 
