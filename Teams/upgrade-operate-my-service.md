...
服務管理是一個涵蓋服務部署Microsoft Teams使用者啟用後之日常營運的一個廣泛主題。 服務Teams包括Microsoft 365或Office 365部署于內部部署之基礎結構元件，例如 (網路) 。
...
<td><ul><li>使用這份檔來協助操作角色的繪圖作業。</li><li>與各自的支援小組開會，將名稱指派給所需活動清單中的每個專案。</li><li>取得指派角色的接受或簽收。</li><li>請確定對應的團隊擁有適當的訓練、準備狀態和資源，以完成所需的活動。</li></ul></td></table>
...
本指南涵蓋管理服務管理Teams領域。 最有可能的是，您針對所依賴的支援技術，Teams方案。 如果沒有，您也需要為內部部署和線上 (技術元件建立) 方案。 這可協助確保您的使用者享有高品質、可靠的Teams。
...
Microsoft 365 系統管理中心[服務](https://portal.office.com/)健康情況[儀表板](https://portal.office.com/adminportal/home#/servicehealth)，除了從屬服務之外，您還可以在這裡Teams服務目前的健康情況。

### <a name="monitoring-with-the-mobile-app"></a>使用行動應用程式監控

Microsoft 365 系統管理應用程式可在 Apple iOS、Android 和 pc Windows (行動) 。 應用程式會提供系統管理員服務健康情況及近期變更的資訊。 應用程式支援推入通知，可在公告發布後立即提醒您。 這可協助您隨時瞭解服務的狀態、健康情況，以及任何即將進行的變更。 通知支援使它成為建議系統管理員的監控工具。 詳細資訊，請參閱：
...
Microsoft Teams雲端服務。 有了這項功能，您才能快速提供新功能。 持續創新為組織帶來明顯的好處，但組織內部必須妥善管理這些變更，以避免使用者對服務台的抵觸或升級。

系統會自動Teams更新給使用者。 您的使用者一定會擁有最新用戶端和功能，Teams服務。 無法管理向使用者推出Teams更新，因此透過有效的通訊、訓練和採用計畫來管理變更至關重要。 如果您的使用者瞭解這項變更、瞭解其優點，並有權運用新功能，就能更快速地進行調整，並歡迎 &mdash; 這項變更。

### <a name="monitoring-for-change"></a>監控變更

變更管理的第一個步驟是監控計畫變更Teams。 監控這些變更的最佳來源是Microsoft 365藍圖，[](https://www.microsoft.com/microsoft-365/roadmap)其中列出目前正在開發、正在推出給客戶或已完全啟動的功能。 您可以使用提供的Teams來搜尋特定功能，或將藍圖下載到 Excel檔案以進一步分析。 藍圖會針對每項功能提供簡短描述，以及預期的發行日期。

在[Microsoft Teams部落](https://techcommunity.microsoft.com/t5/Microsoft-Teams-Blog/bg-p/MicrosoftTeamsBlog)格中，您可以瞭解產品更新的最佳作法、趨勢Teams新聞。 預期會在這裡Teams主要功能更新。 您也可以透過 RSS 提要訂閱部落格。 接著，您可以將[RSS](https://techcommunity.microsoft.com/gxcuf89792/rss/board?board.id=MicrosoftTeamsBlog)提要直接新Teams到頻道中，因此所有重要消息會直接在 Teams。
...
初始試驗開始之後，建立一般步頻以測量實際使用量Teams至關重要。 這可讓貴組織深入瞭解實際使用量如何與在構想階段期間預測的使用量保持一致。 雖然本節著重于Teams使用量，這應該是衡量及評估整體使用量Microsoft 365或Office 365工作的一部分。
...
來自 Teams 的使用狀況資料可在報表儀表板中。 Teams，您可以在三個不同的報告中找到使用方式資料。 第一份報表提供跨產品視圖，說明使用者使用各種服務在 Microsoft 365 或 Office 365。 此報告可在這裡找到：Microsoft 365[管理中心的報表 - 使用中使用者](https://support.office.com/article/Office-365-Reports-in-the-Admin-Center-Active-Users-FC1CF1D0-CD84-43FD-ADB7-A4C4DFA8112D)
...
| 測量使用量 | 在部署完成Teams之後，測量 (磁碟機值階段中的使用量) 。 按需要解決使用問題。 | Biweekly | |
|  (磁碟機值階段)  | | | |
...
- **專案開始時**：在專案開始時評估使用者情緒，您可以提早瞭解使用者對Teams的體驗。
...
NPS 是一個範圍從 –100 到 100 的索引，可測量客戶建議公司產品或服務的意願。 NPS 是以透過電子郵件或其他電子方式傳送給使用者的匿名問卷為基礎。 NPS 會測量提供者與消費者之間的效度。 它只包含一個問題，要求使用者從 1 到 10 評等其體驗，並提供其他批註的選項。 然後根據下列評等來將使用者分類：
...
雖然基本 NPS 數位很有用，但從分析使用者批註可取得最大值。 它們可協助瞭解使用者為何會 (建議) 建議Teams建議。 這些批註可以提供寶貴的意見，協助專案或服務管理團隊瞭解提供高品質服務所需的調整。
...
| 根據 IP 或 URL Microsoft 365或Office 365更新網路 | 更新適用的網路元件 (防火牆、Proxy 伺服器、VPN、用戶端防火牆等) 以反映 Office 365 URL 和[IP 位址範圍的變更](/microsoft-365/enterprise/urls-and-ip-address-ranges)。 | 需要時 | |
...
我們強烈建議您及早提名品質優等獎得主。 獲得提名後，他們應開始熟悉改善[及](monitor-call-quality-qos.md)監控通話品質中的內容，Teams相關的訓練教材。
...
用戶端的主要優點Teams之一是用戶端會自動保持在最新狀態。 PC 和 Mac 上的用戶端會使用背景程式進行更新，此程式會檢查新建立，並下載應用程式閒置時的新用戶端。 行動Teams應用程式會透過各自的 App Store 保持最新狀態。
...
4. **流覽Microsoft Teams社群：** 這個 [Microsoft Teams社群](https://techcommunity.microsoft.com/t5/Microsoft-Teams/ct-p/MicrosoftTeams)提供專屬空間供Teams。 這個Teams社群會提供討論清單、部落格文章，以及圍繞在 Teams。 您可以張貼問題或搜尋先前的討論，以尋找問題的解決方案。
...
| 活動 | 描述 | 節奏 | 已指派團隊 |
|---|---|---|---|
| 端點需求 | 請Teams端點繼續符合取得用戶端Teams[中](get-clients.md)所列的所有Microsoft Teams。 | 每月 | |
...
若要支援使用者，組織可能需要任何數目的相關工作，而特定工作會因組織而異。 這些工作最終必須由已指派這些營運職責的支援小組管理。 下列工作通常必須執行，才能支援 Teams。
...
根據預設，在 Exchange Online中擁有信箱的所有使用者Microsoft 365群組，因此，Microsoft Teams。 如果您想要更嚴密地控制並限制新團隊[ (](assign-roles-permissions.md#permissions-to-create-teams)的建立，進而建立新的 Microsoft 365 群組) ，您可以將群組的建立和管理許可權委派給一組系統管理員。 如果貴組織想要執行此選項，請參閱本文所述的程式，以允許使用者提交指派的團隊處理的要求。
