...
服務管理是一個廣泛的主題，涵蓋在部署並為使用者啟用 Microsoft 團隊服務之後的日常作業。 團隊服務涵蓋 Microsoft 365 或 Office 365，以及部署在內部部署 (的基礎結構元件（例如，網路) ）。
...
- Azure Active Directory (Azure AD) 提供適用于團隊的驗證與授權服務。
...
本指南涵蓋管理團隊服務的主要重點領域。 您最有可能的是，針對小組所依賴的支援技術，您有適當的服務管理方案。 如果不是，您將需要為這些技術元件建立適當的服務管理方案， (內部部署和線上) 。 這將有助於確保您的使用者能在小組中享受高品質、可靠的體驗。
...
您可以在 Apple iOS、Android 和 Windows (的電腦) 和行動裝置上使用 Microsoft 365 系統管理應用程式。 App 提供服務健康情況與即將進行變更的服務系統管理員資訊。 App 支援推播通知，在公告發布之後幾乎立即就能提醒您。 這可協助您隨時掌握服務的狀態、健康情況及任何即將進行的變更。 通知支援會將它提供給系統管理員的建議監視工具。 如需詳細資訊，請參閱：
...
您可以利用服務通信 API，以程式設計方式存取服務健康情況與變更，以監控服務健康情況與即將進行的變更。 您可以使用這個 API 來建立您自己的監視工具，或將現有的監視工具連線至 Microsoft 365 或 Office 365 服務通訊，這可能會簡化您的環境監視方式。 如需詳細資訊，請參閱適用 [于企業開發人員的 Microsoft 365 或 Office 365](https://docs.microsoft.com/office/developer-program/microsoft-365-developer-program-faq)。

### <a name="dailyweeklymonthlyas-needed-tasks"></a>每日/每週/每月/所需任務

| 活動 | 描述 | 節奏 | 指派的小組 |
|---|---|---|---|
| 監視服務健康情況 | 使用可用的工具，主動監視 Microsoft 團隊服務健康情況、 (和相依服務) 。 相依服務包括： Exchange Online、SharePoint Online、商務用 OneDrive、Azure Active Directory。 | 即時 | |
...
系統管理中心的使用方式報告可由獲指派 **全域管理員** 角色的人員存取，或由特定產品的管理員角色 (**Exchange 管理員**、 **商務用 Skype 系統** 管理員、 **SharePoint 系統管理員**) 。
...
在您使用 [報告儀表板] 來測量使用量之後，請務必將測量的使用方式與您在專案的構想階段中定義的任何重要成功指示器 (KSIs) 。 您可以定義一個可能定義為作用中用法的 KSI，或是一個間接連結至作用中用法的。
...
3. 使用訓練與採納小組收集使用者的直接意見反應 (請參閱本文稍後的 [評估使用者觀點](#assess-user-sentiment)) ，並查看認知及採用活動的有效性。
...
| 測量使用量 | 完成部署之後，在 [磁碟機價值階段] 中測量及評估團隊的使用量 () 。 根據需要解決使用方式問題。 | 快遞 | |
|  (磁片磁碟機值階段)  | | | |
...
雖然基 NPS 編號很有用，但您可以從分析使用者的意見來取得最大的價值。 他們將協助您瞭解使用者為何 (或無法) 建議團隊給其他人。 這些意見反應可提供寶貴的意見反應，以協助專案或服務管理小組瞭解提供品質服務所需的調整。
...
| 根據 Microsoft 365 或 Office 365 IPs 與 Url 的變更來更新網路 | 更新適用的網路元件 (防火牆、proxy 伺服器、Vpn、用戶端防火牆等) ，以反映 [Office 365 url 和 IP 位址範圍](https://aka.ms/o365ips)的變更。 | 視需要 | |
| 提供建築物資料 | 將更新的子網資訊提供給品質擁護者 (或相關的專案關係人) ，以確保 [CQD 中的建築物定義](https://docs.microsoft.com/SkypeForBusiness/using-call-quality-in-your-organization/turning-on-and-using-call-quality-dashboard#upload-building-information) 保持在最新狀態。 | 視需要 | |
| 實施變更 | 在網路上實施變更，以支援變更團隊的業務與技術需求。 網路元素可以包含：<ul><li>道</li><li>疑難排解</li><li>有線及 Wi-Fi 網路</li><li>網際網路連線與 ExpressRoute</li><li>DNS</li></ul> | 視需要 | |
...
| 網路疑難排解與修正 | 協助團隊技術人員、服務擁有者和主要的專案關係人，進行疑難排解並修正與團隊連線、可靠性或品質相關的問題。 網路元素可以包含：<ul><li>道</li><li>疑難排解</li><li>有線及 Wi-Fi 網路</li><li>網際網路連線與 ExpressRoute</li><li>DNS</li></ul> | 視需要 | |
| 災害復原與高可用性測試 | 在網路基礎結構上執行一般的高可用性和災害復原測試，以確保它符合 (Slo) 或服務層級協定的服務層級目標，以確保小組服務)  (Sla。 | 次 | |
...
| 活動 | 描述 | 節奏 | 指派的小組 |
|---|---|---|---|
|  (s) 的提名及訓練品質擁護者 | 提名並訓練品質專家。 | 視需要 | |
|  (QERs) 執行體驗品質檢查 | 執行 QER 以找出品質與可靠性的趨勢、針對已定義的目標進行審查，以及向組織中的主要干係人報告。 | 部署期間每週 (每週)  | |
| 磁片磁碟機修正 | 根據 QER 評估與調查，在整個組織中協調修正活動。 | 視需要 | |
| 在 CQD 中更新組建資料 | 在 CQD 中更新或新增新的建築物定義當您變更網路時，請參閱將 [建築物資訊) 上傳](https://docs.microsoft.com/SkypeForBusiness/using-call-quality-in-your-organization/turning-on-and-using-call-quality-dashboard#upload-building-information) (。 | 視需要 | |
...
Microsoft 團隊端點可以定義為任何電腦、Mac、平板電腦或行動裝置 (，或是執行團隊用戶端的任何其他) 裝置。 這個字詞 *不* 只會包含裝置本身，但使用者如何連線到裝置，例如，使用裝置內建的麥克風或喇叭、earbuds 或優化的耳機。 部署之後，就不一定會忘記端點。 團隊端點需要持續的護理與維護。 下列各節說明焦點所在的特定區域。
...
Wi-Fi 驅動程式可能會有問題。 舉例來說，驅動程式可能會在存取點之間產生非常嚴格的漫遊行為，可能會造成不必要的存取點切換，從而導致通話品質不佳。 執行品質不佳的 Wi-Fi 驅動程式可能會透過經驗檢查 (，請參閱 [改善及監視團隊的通話品質](monitor-call-quality-qos.md) ，以) 更詳細的資訊。 必須執行品質驅動的程式，以監視新的 Wi-Fi 驅動程式，並確保在將其部署到一般使用者群體之前，對其進行測試。

### <a name="endpoint-management"></a>端點管理

支援的端點和介面裝置目錄 (例如耳機) 應該可以使用且已維護。 此目錄將包含已選取並驗證為構想與板載階段一部分的核准裝置清單。 通常會針對貴組織中的每個角色類型選取特定裝置，以符合該角色的屬性需求。 所有端點都有週期，您需要管理與這些裝置相關聯的供應商合約、保修、取代、發佈及修復原則。
...
2. **驗證用戶端連線：** 連線問題導致小組中的功能或登入問題。 我們建議 (特別針對您驗證服務連線的新網站或位置) 。 請確定每個網站都遵循下列 [Office 365 url 和 IP 位址範圍](https://aka.ms/o365ips) 指引。 您可以利用 [Microsoft 網路評估工具](https://www.microsoft.com/download/details.aspx?id=53885) 來執行連線測試，以驗證媒體埠是否已正確開啟以供團隊功能使用。 [網路就緒](prepare-network.md)指示中提供如何執行連線測試的詳細步驟。

3. **檢查已知問題清單：** 請參閱 [小組的疑難排解](https://docs.microsoft.com/MicrosoftTeams/troubleshoot/teams) ，判斷是否有其中一個問題對使用者造成負面影響。 如果有一個) 解決問題，請遵循 (提供的因應措施。
...
5. **聯繫 Microsoft 支援人員：** 您可以與 Microsoft 支援人員聯繫，以解決團隊線上或手機的問題。 如需詳細資訊，請參閱 [商務用客戶支援-系統管理協助](https://docs.microsoft.com/microsoft-365/admin/contact-support-for-business-products)。 針對 Premier 客戶，您可以遵循 [Microsoft 團隊的連絡人支援人員 (卓越客戶) ](https://support.microsoft.com/premier/contacts)，啟動支援要求。
...
| Wi-Fi 驅動程式 | 在電腦上測試並更新 Wi-Fi 驅動程式。 使用 CQD ([改善及監視) 的團隊通話品質，以](monitor-call-quality-qos.md) 驗證結果。 | 視需要 | |
| 端點管理 | 維護支援的端點和介面裝置的目錄， (例如耳機) 。 管理供應商合約、保修、發行、取代及修復原則。 | 次 | |
...
### <a name="team-creation-optional"></a>團隊建立 (選用的) 

根據預設，所有在 Exchange Online 中擁有信箱的使用者，都有權建立 Microsoft 365 群組，因此也有在 Microsoft 團隊中的小組。 如果您想要嚴密控制並 [限制新團隊的建立](assign-roles-permissions.md#permissions-to-create-teams) (，因此建立新的 Microsoft 365 群組) ，您可以將群組建立與管理許可權委派給一組管理員。 如果您的組織想要使用此選項，請參閱本文所述的程式，讓使用者能夠提交由指派的小組處理的要求。
...
| 授權管理 | 透過利用 [pstn 使用量報告](https://docs.microsoft.com/skypeforbusiness/skype-for-business-online-reporting/pstn-usage-report) 和 [pstn 分鐘池](https://docs.microsoft.com/skypeforbusiness/skype-for-business-online-reporting/pstn-minute-pools-report) 報告，規劃使用者與消費版授權 (通話方案和通訊點數) 的目前及未來需求。 | 周更新 | |
| 電話號碼管理 | 管理未來增長所能使用的電話號碼，並調整庫存階層，以符合您的組織需求。 | 周更新 | |
| 團隊建立 (選用的)  | 審查及處理小組建立的要求。 | 視需要 | |
