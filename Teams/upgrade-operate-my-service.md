...
服務管理包含管理端對端管理Teams活動與程式。 如先前所述，服務管理的某些元件 ，Microsoft 365 或 Office 365 服務本身所組成的基礎結構，是由 Microsoft 負責，而客戶則負責管理 Teams、網路和端點的各個層面。
...
本指南中定義的活動和角色在大部分情況下都應該有效，但每個 Teams 部署都是唯一的;因此，您可以使用本指南做為起點來自訂活動和預設角色，以滿足您的需求。
...
- SharePoint Online 提供在頻道中共用檔案的能力，商務用 OneDrive提供在私人聊天中共用檔案的機制。
...
[Microsoft Teams和商務用 Skype共存與互通性](teams-and-skypeforbusiness-coexistence-and-interoperability.md)
...
您必須瞭解服務的整體健康情況Microsoft Teams，以便主動提醒貴組織中其他人任何影響服務的事件。 如先前所述，Teams依賴其他 Microsoft 365 Office 365服務，例如 Azure Active Directory、Exchange Online、SharePoint，以及商務用 OneDrive。 因此，監控從屬服務的健康情況也相當重要。
...
您可以利用服務通訊 API 以程式化方式存取服務健康情況及變更，以監控服務健康情況及即將到來的變更。 使用此 API 建立您自己的監控工具，或將現有的監控工具連接到Microsoft 365或Office 365通訊，可能簡化您監控環境的方法。 有關詳細資訊，請參閱[Microsoft 365開發人員Office 365或Enterprise資訊](/office/developer-program/microsoft-365-developer-program-faq)。
...
系統會自動Teams更新給使用者。 您的使用者一定會擁有最新用戶端和功能，Teams服務。 無法管理向使用者推出Teams更新，因此透過有效的通訊、訓練和採用計畫來管理變更至關重要。 如果您的使用者瞭解 &mdash; 這項變更、瞭解其優點，並有權運用新功能，他們將能夠更快速地進行調整，並歡迎這項變更。

### <a name="monitoring-for-change"></a>監控變更

變更管理的第一個步驟是監控計畫為 Teams。 監控這些變更的最佳來源是Microsoft 365藍圖，[](https://www.microsoft.com/microsoft-365/roadmap)其中列出目前正在開發、正在推出給客戶或已完全啟動的功能。 您可以使用提供的Teams來搜尋特定功能，或將藍圖下載至 Excel檔案以進一步分析。 藍圖會針對每項功能提供簡短描述，以及預期的發行日期。
...
您可以在這裡找到針對桌面、Web 和行動裝置所發行的功能清單。 同一組版本資訊也可在說明的新增功能 **上**[找到](get-help-in-microsoft-teams.md)。
...
來自 Teams 的使用狀況資料可在報表儀表板中。 Teams使用方式資料，可以在三個不同的報告中找到。 第一份報表提供跨產品視圖，說明使用者使用各種服務在 Microsoft 365 或 Office 365。 此報告可在這裡找到：Microsoft 365[管理中心中的報告 - 使用中使用者](https://support.office.com/article/Office-365-Reports-in-the-Admin-Center-Active-Users-FC1CF1D0-CD84-43FD-ADB7-A4C4DFA8112D)
...
系統管理中心中的使用方式報告可由已指派全域系統管理員角色或產品特定系統管理員角色 ( Exchange系統管理員、商務用 Skype系統管理員、SharePoint系統管理員) 。**** **** ****
...
| 測量使用量 | 在部署完成後Teams評估雲端硬碟價值階段 (使用量) 。 按需要解決使用問題。 | Biweekly | |
|  (磁碟機值階段)  | | | |
...
使用者情緒可以透過許多不同的方法進行評估。 這些可能包括電子郵件問卷、個人或電話式面試，或直接在 Teams 或 Yammer。 如要詳細資訊，請參閱在 Microsoft Teams 中[使用者](best-practices-feedback.md)意見Microsoft Teams。
...
NPS 是一個範圍從 –100 到 100 的索引，可測量客戶建議公司產品或服務的意願。 NPS 是以透過電子郵件或其他電子方式傳送給使用者的匿名問卷為基礎。 NPS 會測量提供者與消費者之間的效度。 它只包含一個問題，要求使用者以 1 到 10 為體驗評分，並提供其他批註的選項。 然後根據下列評等來將使用者分類：
...
雖然基本 NPS 數位很有用，但分析使用者批註會獲得最大的值。 它們可協助您瞭解使用者為何會 (建議) Teams建議。 這些批註可以提供寶貴的意見，協助專案或服務管理團隊瞭解提供高品質服務所需的調整。
...
許多核心規劃元素會進入優化、正確調整大小及補救您的網路基礎結構，以確保高品質、有效率地Microsoft Teams服務。 我們的網路準備指南涵蓋規劃工作 [與](prepare-network.md) 需求。 由於升級、擴充或其他商務需求，網路通常會隨著時間而演進。 您必須考慮到網路規劃活動中Teams需求。
...
| 活動 | 描述 | 節奏 | 已指派團隊 |
|---|---|---|---|
| 監控Microsoft 365或Office 365 IP 和 URL | 使用提供的 RSS Office 365監控 URL 和[IP 位址](/microsoft-365/enterprise/urls-and-ip-address-ranges)範圍的任何變更，並針對適用的網路群組提出[](https://go.microsoft.com/fwlink/p/?linkid=236301)變更要求。 | 日常 | |
| 根據 IP 或 URL Microsoft 365或Office 365更新網路 | 更新適用的網路元件 (防火牆、Proxy 伺服器、VPN、用戶端防火牆等) 以反映 Office 365 [URL 和 IP 位址範圍的](/microsoft-365/enterprise/urls-and-ip-address-ranges)變更。 | 需要時 | |
...
| 災害復原和高可用性測試 | 在網路基礎結構上執行一般高可用性和災害復原測試，以確保它符合針對 Teams 服務的服務等級目標 (SLOs) 或服務等級協定 (SLA) 。 | 每月 | |
...
Microsoft Teams端點可定義為執行用戶端的任何 PC、Mac、平板電腦或行動裝置 (或) 裝置Teams裝置。 端點一詞不僅包含裝置本身，也包含使用者如何連接到裝置，例如，使用裝置內建的麥克風或喇叭、耳塞或優化的耳機。 部署端點之後，不得忘記端點。 端點Teams需要持續保養和維護。 下列各節說明要專注的特定區域。

### <a name="endpoint-requirements"></a>端點需求

用戶端的其中一Teams優點就是用戶端會自動保持在最新狀態。 PC 和 Mac 上的用戶端會使用背景程式進行更新，此程式會檢查新建立，並下載應用程式閒置時的新用戶端。 行動Teams應用程式會透過各自的 App Store 保持最新狀態。
...
1. **驗證服務健康狀態：** 使用者可能會遇到的問題可能與事件有關，該事件會對服務Teams服務及其從屬服務造成負面影響。 首先，建議您確認沒有使用中服務問題。 請參閱[如何檢查Microsoft 365健康狀態](/office365/enterprise/view-service-health)。 請記得檢查從屬服務的狀態，例如 (、Exchange、SharePoint商務用 OneDrive) 。 上一節的監控服務健康情況會進一步探討服務 [健康情況監控](#monitor-service-health)。

2. **驗證用戶端連接：** 連接問題會導致功能或登錄問題Teams。 我們建議您 (新網站或位置，) 驗證服務的連接。 確保每個[Office 365都遵循下列 URL 和 IP 位址](/microsoft-365/enterprise/urls-and-ip-address-ranges)範圍指南。 您可以利用[Microsoft 網路評定工具](https://www.microsoft.com/download/details.aspx?id=53885)執行連接測試，驗證媒體埠已正確開啟，Teams功能。 網路準備指南提供如何執行連接測試 [的詳細](prepare-network.md) 步驟。

3. **檢查已知問題清單：** 請參閱 [Teams疑](/MicrosoftTeams/troubleshoot/teams)難解答，判斷使用者是否受到這些問題之一的不良影響。 如果有一個 (，請遵循) 以解決問題。

4. **流覽Microsoft Teams社群：** Microsoft Teams [社群](https://techcommunity.microsoft.com/t5/Microsoft-Teams/ct-p/MicrosoftTeams)提供專屬空間供Teams。 社群Teams討論清單、部落格文章，以及圍繞在 Teams。 您可以張貼問題或搜尋先前的討論，以尋找問題的解決方案。
...
| Wi-Fi驅動程式 | 測試並Wi-Fi PC 上的驅動程式。 使用 CQD ([改善及](monitor-call-quality-qos.md)監控通話品質以驗證Teams) 。 | 需要時 | |
...
根據預設，在 Exchange Online中擁有信箱的所有使用者Microsoft 365群組，因此，Microsoft Teams。 如果您想要更嚴密地控制並限制新[團隊 (的](assign-roles-permissions.md#permissions-to-create-teams)建立，進而建立新的 Microsoft 365 群組) ，您可以將群組的建立和管理許可權委派給一組系統管理員。 如果貴組織想要執行此選項，請參閱本文所述的程式，以允許使用者提交指派的團隊處理的要求。
...
| 授權管理 | 利用 [PSTN](/skypeforbusiness/skype-for-business-online-reporting/pstn-usage-report) 使用方式報表和 [PSTN](/skypeforbusiness/skype-for-business-online-reporting/pstn-minute-pools-report) 分鐘數 (方案與通訊) 方案，規劃使用者和消費型授權目前和未來的需求。 | 每週 | |
