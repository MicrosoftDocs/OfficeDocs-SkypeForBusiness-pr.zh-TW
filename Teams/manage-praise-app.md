...
> [!NOTE]
> 系統管理員必須擁有小組授權，才能存取此功能。 如果您嘗試在沒有團隊授權的情況下存取此功能，您會收到錯誤訊息。

Microsoft 團隊中的稱讚應用程式可協助使用者向組織或教室的成員顯示感謝。 您可以選擇一組徽章以供選擇，以及建立您自己徽章的選項，稱讚旨在協助您辨識小組使用者在團隊使用者所做的大量工作（從教育者到第一線員工員工）所做的工作。 若要深入瞭解，請參閱 [將稱讚傳送給其他人](https://support.microsoft.com/office/send-praise-to-people-50f26b47-565f-40fe-8642-5ca2a5ed261e)。

系統管理員可以從 Microsoft 團隊系統管理中心控制哪些徽章可供其組織使用。 在左側導覽中，移至 [ **團隊 app] > [管理 app** ]。 在應用程式清單中，按一下 [ **稱讚** ]，然後選取 [ **設定** ]。  您可以從這裡選擇啟用預設和內建徽章集，並建立自訂徽章。
...
選取 [ **建立自訂徽章** ]。 您可以從這裡，在側邊面板中設計自訂徽章。 您最多可以建立25個自訂徽章。 
