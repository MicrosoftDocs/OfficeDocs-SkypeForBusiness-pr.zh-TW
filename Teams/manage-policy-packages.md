...
|教育版（使用遠端學習的主要學校教師）    |建立一組原則，適用于主要老師，以在使用遠端學習時最大化學生的安全性與共同作業。      |
|教育版（使用遠端學習的主要學校學生）    |建立一組原則，以套用到主要學生，在使用遠端教學時最大化學生的安全性與共同作業。      |
...
#### <a name="assign-a-policy-package-to-a-large-set-batch-of-users"></a>將原則套件指派給大型使用者組（批次）
