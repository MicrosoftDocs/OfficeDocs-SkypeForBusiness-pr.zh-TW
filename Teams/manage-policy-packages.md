...
Microsoft Teams 中的 原則套件 是預先定義的原則和原則設定的集合，您可以將之指派給組織中具有類似角色的使用者。 我們已建立策略套件，以簡化、簡化及協助在管理貴組織使用者群組時提供一致性。  
...
策略套件Teams您允許或限制貴組織特定人員集的功能。 每個策略套件Teams使用者角色所設計，並包含預先定義的策略和策略設定，可支援該角色常見的共同合作與通訊活動。
...
|中小企業使用者無需商務 (語音)  |建立適用于中小型企業的應用程式設定策略，Teams非商務語音體驗 (使用者) 。
...
1. 在系統管理中心的左側導Microsoft Teams，選取 [政策 **套件**，然後按一下 [**新增**> 。
    ...
    :::image type="content" source="media/policy-packages-add.png" alt-text="系統管理中心中策略套件頁面上的新增按鈕螢幕擷取畫面。":::
