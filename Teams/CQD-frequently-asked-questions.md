...
[為什麼我的自訂報表在我知道需要更多專案時才會傳回最大10000列？](#why-do-my-custom-reports-only-return-a-maximum-of-10000-rows-when-i-know-there-should-be-more-entries)
...
### <a name="why-do-my-custom-reports-only-return-a-maximum-of-10000-rows-when-i-know-there-should-be-more-entries"></a>為什麼我的自訂報表在我知道需要更多專案時才會傳回最大10000列？

CQD 是專為匯總資料查詢設計的，不是針對資料匯出而設計。 建議您盡可能地重組報表，以免超出10000列限制。 首先，使用較廣的基數維度（例如 Month、Year、Date、Region、Country 等）查看您的 Kpi。您可以從該處深化到較高基數的維度。 [技術支援人員] 和 [Location-Enhanced 報表] 都提供此深入分析工作流程的良好範例。
