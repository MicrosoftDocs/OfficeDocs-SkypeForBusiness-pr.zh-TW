...
針對新使用者, 預設的會議付費電話號碼是根據在使用者啟用音訊會議服務時, 在使用者的 Office 365 設定檔中設定的國家/地區指派。 如果在會議橋中有一個與使用者的國家/地區相符的收費號碼, 該號碼會自動指派為使用者的預設付費號碼。 如果沒有其中一個號碼, 則會將定義為會議橋接器預設收費號碼的號碼指派為使用者的預設付費號碼。  
