...
您可以根據名稱或分機號碼，將自動語音機配置為允許來電者搜尋貴組織的目錄。 在自動助理中，您可以選擇要包含或排除的使用者群組，指定哪些人可以搜尋目錄。  (這稱為撥號 *範圍*.) 

來電者可以撥打自動語音機，方法可以是直接電話號碼 ，如果有已配置，也可以從另一個自動語音機或通話佇列重新導向。

## <a name="call-queues"></a>通話佇列

通話佇列與實體建築物中的等候室類似。 來電者等候等候，而來電會路由至佇列中的代理人。 通話佇列通常用於銷售和服務功能。 不過，通話佇列可用於通話數量超過內部容量的任何情況，例如忙碌設施中的接待員。
...
從通話佇列接聽來電的代理人企業語音啟用線上或內部部署使用者。 此外，如果通話佇列使用直接路由號碼，需要電話會議或轉接通話的代理人也需要：
...
將通話轉接至外部電話號碼時，執行轉接 (即與自動電話機或通話佇列關聯的) 的資源帳戶必須擁有 Microsoft 365 電話系統 虛擬使用者授權，且指派下列其中一項：
...
在設定自動電話機和通話佇列之前，您應該先決定如何在公司中使用這些功能。 這些決策會決定您設定自動電話機和通話佇列時所選取的設定。
...
- 您想要在 [貴組織中啟用](call-park-and-retrieve.md) 通話駐位和取回功能，以協助人員或部門之間的呼叫交還嗎？
- 針對語音提示，您想要錄製自己的語音或使用系統產生的語音嗎？  (系統產生的語音很容易更新。) 

## <a name="technical-decisions"></a>技術決策

使用自動語音機和通話佇列將來電者與貴組織人員聯繫時，在開始進行組組之前，有一些技術決策需要做出。
...
如果需要，您可以針對每個佇列使用這些選項的組合。 具有電子郵件地址的群組可用於語音信箱。 使用 Teams 有許多優點，包括共用檔案儲存空間和代理人之間的聊天、可接收語音信箱的一般信箱，以及可包含與商務用應用程式或 Power Apps 整合的可擴展平臺。

我們建議您在開始進行組組之前，選擇將呼叫代理程式新排入佇列的策略。

如果您有現有的自動電話機和通話佇列基礎結構，而且要移轉至 Teams，您需要一個方案，將現有的電話號碼轉移到新的自動電話機和通話佇列。 您可能需要建立移轉訂單 [，](phone-number-calling-plans/port-order-overview.md) 才能將號碼從其他提供者移動。 我們建議您暫時取得一或多個新電話號碼，並測試自動電話機和通話佇列流程，然後再切換至您目前使用中的號碼。
...
> [!NOTE]
> 單一自動電話機只能支援單一的「撥號方式」方法。  若要允許來電者按名稱和號碼撥號，您必須建立自動語音機，該自動語音機具有按名稱撥號的選項，另一個則具有分機撥號的選項。  這些選項會路由到針對這些「撥號方式」案例所配置的個別自動助理。

當自動電話機或通話佇列重新導向通話時，您可以從下列通話路由目的地選擇：

- **組織中可以接聽** 語音通話的人。 這可以是線上使用者，或是使用 商務用 Skype Server 在內部部署商務用 Skype Server。
...
通話佇列會保留來電者，直到指派給佇列的代理人可以進行通話。 有兩種情況可能會將來電者導向佇列：

- **通話溢出** - 如果佇列中的通話數超過您設定的限制，則新的來電者會重新導向到佇列外。
...
- 支援通話佇列有一個透過中間自動電話機直接撥打的電話號碼。 有自動回應應答，支援行可分別提供休假時間與假日通話路由。
...
- 避免經常更新通話路由。 如果您日後變更自動語音機的功能表選項，在前 30 天內，在語音提示中撥打。
