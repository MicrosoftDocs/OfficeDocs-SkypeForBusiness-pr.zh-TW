...
- 每個自動電話機和每個通話佇列的資源帳戶
- 免費Microsoft 電話系統 - 每個資源帳戶的虛擬使用者授權
...
在通話佇列中使用資源帳戶進行電話線路識別碼時，或當自動電話機或通話佇列將通話轉接至外部電話號碼時，資源帳戶必須擁有 電話系統 虛擬使用者授權，且指派下列其中一項：
...
> 
> 在混合式情況下，資源帳戶必須在內部部署中建立。 詳細資訊請參閱規劃 [雲端通話佇列](/skypeforbusiness/hybrid/plan-call-queue)。
...
- Microsoft 電話 1449/1.0.94.2020051601 或更新版本
...
- **立即重新** 導向 - 通話可以重新導向至下列其中一個 (，) 接聽時或初次問候之後。
...
- **運算子** (自動) -為自動話務員定義的運算子。 定義運算子是選擇性的。 運算子可以是此清單的其他目的地。
...
- 0 (0) 會將來電者重新導向至運算子。 該自動助理的運算子已配置為組織中 **的人**。
...
- 首先描述服務，然後按 (選項，例如：針對 Sales 按 1) ，而不是以其他方式按 (。 按 1 表示銷售) 。
...
3. [為您想要建立](manage-resource-accounts.md) 的每個自動話務員和通話佇列建立資源帳戶。 為每個帳戶指派一個電話系統 - 虛擬使用者授權，以及選擇性的服務號碼。
