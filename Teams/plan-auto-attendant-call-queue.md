...
針對每一個通話佇列，您可以指定佇列中的代理人是否可以退出宣告接聽來電，以及通話是否應該根據他們目前狀態在 Teams。
...
> [!NOTE]
> 單一自動助理只能支援單一的「撥號方式」方法。  若要允許來電者按名稱和號碼撥號，您必須建立自動語音機，該自動語音機具有按名稱撥號的選項，另一個則具有分機撥號的選項。  這些選項會路由到針對這些「撥號方式」案例所配置的個別自動助理。
