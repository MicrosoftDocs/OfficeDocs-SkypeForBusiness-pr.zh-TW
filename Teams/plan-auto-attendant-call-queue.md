...
ms.locfileid: "65611909"
---
# <a name="plan-for-teams-auto-attendants-and-call-queues"></a>規劃Teams自動語音應答和通話佇列

自動語音應答可讓您根據來電者輸入來設定功能表選項來路由通話。 自動語音應答的功能表選項，例如「適用于銷售人員，請按 1--For Services 按 2」，可讓組織提供一系列選項，引導來電者快速前往其目的地，而不需要由人力運算子來處理來電。
...
自動語音應答的主要目的是根據來電者對所提供功能表選項的輸入，將來電者導向至適當的人員或部門。 來電者可以導向至組織內的特定人員、撥打佇列等候與下一位可用的代理人通話，或是撥打語音信箱。 您可以針對上班時間、上班時間和假日指定不同的通話路由選項。
...
來電者可以透過直接電話號碼、設定或從其他自動語音應答或通話佇列重新導向來連線自動語音應答。
...
針對每個通話佇列，您可以指定佇列中的代理程式是否可以選擇不接聽電話，以及是否應根據他們的目前狀態指示傳送電話至Teams。

您可以將電話號碼指派給通話佇列，但是通話佇列不會針對上班時間和假日提供個別的通話路由。 除非您的通話佇列是 24/7 的人員，我們建議將電話號碼指派給在上班時間重新導向到通話佇列的自動語音應答。

## <a name="prerequisites"></a>必要條件

若要設定自動語音應答和通話佇列，您需要下列資源：

- 每個自動語音應答和每個通話佇列的資源帳戶
- 免費Microsoft 電話系統 - 每個資源帳戶的虛擬使用者授權，可直接從Teams使用者或外部電話號碼撥號
- 針對您要直接從外部電話號碼撥號的每個資源帳戶，至少有一個[Microsoft 服務號碼](getting-service-phone-numbers.md)、電信業者連線號碼、直接路由號碼或混合式號碼
 - 服務號碼可能是付費或免付費電話號碼
...
- 如果通話佇列使用會議模式，則指派音訊會議授權或線上語音路由原則

如果您的專員使用Microsoft Teams應用程式進行通話佇列通話，則他們必須處於 TeamsOnly 模式。

在通話佇列中使用資源帳戶做為通話行識別碼用途時，資源帳戶必須具備電話系統虛擬使用者授權，以及下列其中一項指派：

- [通話方案](calling-plans-for-office-365.md)授權和指派的電話號碼
- 已指派[電信業者連線](operator-connect-plan.md)電話號碼
- 使用直接路由) 時，選擇性地指派電話號碼 ([線上語音路](manage-voice-routing-policies.md) 由原則

當自動語音應答或通話佇列正在將來電轉接至外部號碼時，如下所述的特定資源帳戶必須具有電話系統虛擬使用者授權，以及下列其中一項指派：
...
授權的資源帳戶：
- 在第一個接聽來電的自動語音應答上授權資源帳戶，當該自動語音應答轉接給其他自動語音應答或呼叫佇列以外部轉接來電時
- 在所有其他通話案例中，授權執行外部轉接的自動語音應答或通話佇列的資源帳戶

> [!NOTE]
> 如果指派給資源帳戶的通話方案已停用或遭到移除， [則通訊點](what-are-communications-credits.md)數若在租使用者 (中提供，但未指派給資源帳戶) ，則會被使用。 如果沒有通話方案或通訊點數，則通話將會失敗。
>
> 自動語音應答的直接路由服務號碼和通話佇列僅支援Microsoft Teams使用者和通話代理程式。
> 
> 不支援通話方案、電信業者連線和直接路由主幹之間的傳輸。
> 
...
- Microsoft 365 群組或Teams

如有需要，您可以針對每一個佇列使用這些選項的組合。 具有電子郵件地址的群組可用於語音信箱。 使用Teams提供許多優點，包括代理程式之間的共用檔案儲存和聊天、可接收語音信箱的常見信箱，以及可包含與企業營運應用程式或Power Apps整合的可延伸平臺。

建議您在開始設定之前，選擇將通話代理程式新增至佇列的策略。

如果您有現有的自動語音應答和通話佇列基礎結構，而您要移轉到Teams，您將需要一個方案，將現有的電話號碼移轉到新的自動語音應答和通話佇列。 您可能需要建立 [移轉訂單](phone-number-calling-plans/port-order-overview.md) ，才能從其他提供者移動您的號碼。 我們建議您暫時取得一或多個新的電話號碼，並測試自動語音應答和通話佇列流量，然後再將它們切換到您目前處於服務中的號碼。

*會議模式* 是通話佇列中的一個選項，可大幅減少連線TEAMS VOIP 通話和 PSTN 通話給專員所需的時間。 若要讓會議模式正常運作，通話佇列中的代理程式必須使用下列其中一個用戶端：

- Microsoft Teams桌面用戶端、Android應用程式或iOS應用程式的最新版本
- Microsoft 電話系統版本 1449/1.0.94.2020051601 或更新版本
  
將專員的Teams帳戶設為僅Teams模式。 不符合需求的代理程式不包含在通話路由清單中。
...
2. 取得電話系統 - 您計畫建立之每個資源帳戶的[虛擬使用者授權](teams-add-on-licensing/virtual-user.md)。 這些授權是免費的，因此我們建議您在日後決定變更資源帳戶時，多多取得一些授權。

3. 為您要建立的每個自動語音應答和通話佇列建立[資源帳戶](manage-resource-accounts.md)。 將每個帳戶指派電話系統 - 虛擬使用者授權，以及服務號碼。
...
7. 如果您打算允許擴充功能撥打，請確定您已將使用者的分機號碼新增至其Azure Active Directory設定檔。

完成上述步驟後，就可以建立自動語音應答和通話佇列。 因為自動語音應答和通話佇列可以將通話重新導向至彼此，請參閱您建立的工作流程圖表，以判斷應該先建立哪一個自動語音應答或通話佇列。 在上述圖表中的範例中，您會在建立 Contoso 主自動語音應答之前建立銷售和支援通話佇列，因為主要自動語音應答需要將來電者導向銷售和支援通話佇列。
...
如果您需要更廣泛的功能，例如與工作流程、機器人和簡訊整合，請考慮[Azure 通訊服務](/azure/communication-services/overview)。
