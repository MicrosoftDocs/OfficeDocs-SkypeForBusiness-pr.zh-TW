...
功能表提示可以建立使用文字到語音 (系統產生的提示) 或上傳錄製的音訊檔案。 語音辨識接受免手流覽的語音命令，但來電者也可以使用電話鍵台流覽功能表。
...
您可以根據名稱或分機號碼，將自動語音機配置為允許來電者搜尋貴組織的目錄。 在自動助理中，您可以選擇要包含或排除的使用者群組，指定哪些人可以搜尋目錄。  (稱為撥號 *範圍*.) 
...
將通話轉接至外部電話號碼時，執行轉接 (即與自動電話機或通話佇列關聯的) 的資源帳戶必須擁有 Microsoft 365 電話系統 虛擬使用者授權，並指派下列其中一項：
...
- 最新版的桌面Microsoft Teams Android App 或 iOS 應用程式
...
- **組織中可以接聽** 語音通話的人。 此使用者可以是線上使用者，或是使用 商務用 Skype Server 在內部部署商務用 Skype Server。
...
- 第 1 (個) 鍵會將來電者重新導向到銷售通話佇列。 此通話佇列會連接到包含指派給該佇列的銷售團隊的團隊。
- 這兩 (2) 鍵會將來電者重新導向到支援通話佇列。 此通話佇列會連接到包含指派給團隊的支援小組的團隊。
- 支援通話佇列有一個透過中間自動電話機直接撥打的電話號碼。 有自動回應應答，支援行可分別提供休假時間與假日通話路由。
- 這三 (3) 會將使用者重新導向至公司目錄的另一個自動總機。 公司目錄自動語音機可讓來電者撥打其名稱或分機來撥打組織中個人的電話。

我們建議您建立一或多個類似上述圖表的圖表，以繪製您的通話路由。 請務必在圖表或隨附檔中包含下列專案：

- 哪些自動電話機會透過電話號碼直接存取？
- 每個自動乘務員的休假和假日路由需求是什麼？
- 每個通話佇列的成員資格。  (您可以個別新增使用者，或將佇列與不同類型的群組進行比對。 將佇列與小組進行繪圖，可提供最多樣化的體驗。) 
...
- 首先描述服務，然後按 (選項：針對 Sales 按 1) ，而不是按其他方式 (例如。 按 1 表示銷售) 。
