...
|**條件**   |您可以監控裝置離線時的健康情況。 [深入瞭解 Teams](../devices/device-management.md) 系統管理中心中的裝置管理。 |
|**範圍**   |您可以提及規則評估頻率，指定您想要監控裝置健康情況的頻率。 根據預設，團隊裝置在離線時，會即時監控。 |
|**裝置使用者**   |您可以根據已登錄的使用者選取裝置，指定哪些裝置需要主動離線監控。 請參閱選取 [裝置以進行配置](#select-devices-for-configuration) ，以進一步瞭解詳細資料。 |
|**動作**  > **頻道通知**   |在動作區段，您可以指定要取得通知的團隊頻道。 目前，系統將會建立名為系統管理通知和通知的預設小組，以及名為 **MonitoringAlerts** 的頻道，其中會傳送通知。 <BR/> <BR/> 您的租使用者中的全域系統管理員和 Teams 系統管理員會自動新加入這個預設團隊。|
...
1. 您可以選取已登錄這些裝置的使用者，以選取您想要監控的 Teams 裝置。 從 **裝置** 使用者區 **段選取** 新增。
