...
description: 瞭解如何根據群組成員資格或直接透過遠端學校 (遠端學校、遠端學校或遠端學校) 指派策略給) 使用者。
f1keywords: ''
...
請記住，Teams，除非您建立並指派自訂) Teams，否則使用者會自動取得全域 (全組織的預設) 策略。 由於學生人口通常是最大的一組使用者，而且他們通常會收到限制最嚴格的設定，建議您執行下列操作：
