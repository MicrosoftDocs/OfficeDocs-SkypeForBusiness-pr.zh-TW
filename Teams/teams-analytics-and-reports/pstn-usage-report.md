...
Teams系統管理中心 (PSTN) 公用交換電話網路) Microsoft Teams使用方式報告，讓您概觀貴組織的通話和音訊會議活動。 如果您使用 Microsoft 做為電話電信業者，您可以查看通話方案的詳細通話活動;如果您使用自己的電話電信業者，也可以查看直接路由的詳細通話活動。
...
按一下 **[匯出Excel**，然後在 [下載> 選項卡上，按一下 [下載以在報表準備就緒時下載報告。 匯出程式可能需要幾秒鐘到幾分鐘才能完成，視資料量而不同。
...
> | 5 | UPN | `nvarchar(128)` | UserPrincipalName (中輸入) 名稱Azure Active Directory。<br/>這通常與使用者的 SIP 位址相同，而且可以與使用者的電子郵件地址相同 |
...
> | 10 | 國內/國際 | `nvarchar(16)` | 無論通話是國內 (，) 或 (國家/地區外) 根據使用者的位置進行撥打 |
...
> | # | 名稱 | [資料類型 (SQL Server) ](/sql/t-sql/data-types/data-types-transact-sql) | 描述 |
> | :-: | :-: | :-: |:------------------- |
> | 0 | CorrelationId | `uniqueidentifier` | 通話識別碼。 同一個通話的多個支段可以共用相同的 CorrelationId |
> | 1 | AAD ObjectId | `uniqueidentifier` | 在 Azure Active Directory 中Azure Active Directory。<br/> 對於 Bot 通話類型，此及其他使用者資訊可能為 Null/empty |
> | 2 | UPN | `nvarchar(128)` | UserPrincipalName (登錄名稱，Azure Active Directory) 或接收來電的使用者或 bot 的名稱。<br/>這通常與使用者的 SIP 位址相同，而且可以與使用者的電子郵件地址相同 |
> | 3 | 顯示名稱 | `nvarchar(128)` | 使用者或通話機器人的名稱 (例如通話佇列或自動) 自動Microsoft 365 系統管理中心 |
> | 4 | 使用者國家/地區 | `nvarchar(2)` | 使用者的國家/地區代碼， [ISO 3166-1 Alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2) |
> | 5 | 邀請時間 | `datetimeoffset` | 當初始邀請從 Teams 使用者或 Bot 通話傳送至 SBC，或從 SBC 直接路由的 SIP Proxy 元件在輸入到 Teams 或 bot 通話時收到 |
...
- [Microsoft Graph](/graph/api/callrecords-callrecord-getpstncalls?view=graph-rest-1.0&tabs=http)
