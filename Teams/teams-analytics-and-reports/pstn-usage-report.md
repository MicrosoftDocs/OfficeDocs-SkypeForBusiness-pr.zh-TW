...
> | 5 | 開始時間 | `datetimeoffset` | SIP Proxy 從外寄 (Teams/Bot 上的 SBC 收到最後答案 (SIP 訊息 "200 確定") 給 PSTN 使用者) 的時間，或 SIP Proxy 在輸入通話 (PSTN 使用者將邀請傳送至 Teams 後端的下一個躍點至 Teams/Bot) 的時間。<br/>如果通話失敗和未接聽，這等於邀請或失敗時間 |
