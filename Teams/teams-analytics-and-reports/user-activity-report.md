...
|**500**   |此表格提供使用者的使用方式細分。   <ul><li>[**使用者名**] 是使用者的顯示名稱。 您可以按一下顯示名稱，移至 Microsoft 團隊系統管理中心的 [使用者設定] 頁面。</li><li>[ **1:1 通話**] 是使用者在指定期間內參與的1:1 通話數目。</li><li>[**頻道訊息**] 是使用者在特定的時段內，在小組聊天中張貼的唯一訊息數目。</li><li>[**回復訊息**] 是使用者在特定期間內于團隊頻道中張貼的唯一回復訊息數。</li> <li>[**張貼訊息**] 是在指定期間內，使用者在團隊頻道中張貼的唯一張貼訊息數目。</li><li>[**組織會議**數] 是使用者在指定期間內組織的排程與點對點會議數目。</li><li>[**井然有序排程的會議**] 是使用者在指定期間內組織的排程會議數。</li><li>[組織中的**會議**] 是使用者在指定的時間期間組織的臨時會議數量。</li><li>[**會議參與**] 是使用者在特定期間內參與的排程和臨時會議數。</li><li>**參與會議的會議**是指使用者在指定時段內參與的排程會議數。</li><li>[參與中的**會議**] 是指使用者在特定的時段內參與的 ad hoc 會議數目。</li><li>[**總會議**數] 是使用者在指定的時段內參與的排程和點對點會議總數。</li><li>[**交談訊息**] 是使用者在特定的時段內，在私人聊天中張貼的唯一訊息數目。</li><li>**緊急郵件**是指使用者在指定的時段內，在聊天中張貼的緊急訊息數目。</li><li>[**音訊時間**] 是使用者在指定的時段內參與的總音訊時間。</li><li>[**視頻時間**] 是使用者在特定的時段內參與的視頻總時間。</li><li>**螢幕共用時間**是指使用者在指定的時段內參與的螢幕共用時間總計。</li>  <li>[**上一個活動**] 是使用者參與團隊活動的最後一個日期（UTC）。</li><li>當使用者被視為有效，但其值為零的情況下，表示聊天訊息、1:1 通話、通道訊息、會議總數及會議的值為零的**其他活動**追蹤。 範例動作是在使用者開啟通道訊息後但不回復，或當使用者收到私人訊息但不回應它時。</li> </ul>請注意，**群組通話**已由**會議**所**取代，這**兩個值的總和等於由**群組通話**衡量的專案。
