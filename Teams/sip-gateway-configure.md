...
您可以在動態主機組組通訊協定或 DHCP 伺服器中設定 SIP 閘道 () URL。 遠端工作的使用者必須手動設定。
...
在 DHCP 伺服器中Teams SIP 閘道置備伺服器 URL，將 SIP 裝置新增到貴組織。 若要深入瞭解 DHCP 伺服器，請參閱 [部署及管理 DHCP](/learn/modules/deploy-manage-dynamic-host-configuration-protocol)。 此外，您也可以使用 DHCP 選項 42 指定網路時間通訊協定 (NTP) 伺服器，以及 DHCP 選項 2，以指定協調通用時間 (UTC) 的位移。 貴組織的裝置會路由至 SIP 閘道置備伺服器。 成功布備的 SIP 電話會顯示Teams標誌和用於登錄的柔和按鈕。
...
4. 在 SIP 裝置上，撥打註冊功能代碼，後面接著驗證碼。 在 SIP 裝置上，撥打 SIP 閘道用於註冊一次驗證驗證) 的註冊功能代碼 55* (，後面接著此特定裝置在 Teams 系統管理中心產生的驗證 \* 碼。 例如，如果驗證碼是123456，請撥打 \* 55 \* 123456註冊裝置。
...
> - 當 **lang_xx設定** 參數未透過設定 URL 設定時，會使用英文做為預設語言。
