...
-   將媒體維持在公司網路子網的邊界內。
-   允許用戶端與 SBC Teams媒體流，即使 SBCs 位於具有私人 IP 的公司防火牆後面，且 Microsoft 無法直接看到。
...
- **在亞太地區，Contoso** 在不同的國家/地區設有多個辦公室。 
  ...
  在大多數國家/地區，公司仍擁有當地分公司 (TDM) 時除多工。 在 APAC 地區中，TDM 主幹集中化不是一個選項，因此無法切換到 SIP。 假設亞太地區有五十多個 Contoso 分公司，有數百個閘道 (SBC) 。 在此情境中，由於缺少公用 IP 位址和/或本地網際網路中斷，因此無法將所有閘道配對至直接路由介面。 此外，部分國家/地區會強制規定沒有當地 PSTN 網路連接就無法符合的法規要求。
...
![圖表顯示流量流量 Local Media 優化。](media/direct-routing-media-op-2.png "如果使用者是外部使用者，則使用已連結的集中式 SIP 主幹進行集中式 SBC 時的流量流程")
...
當使用者位於下游 SBC 所在的本地分支辦公室時，媒體流量會直接在使用者與本地下游 SBC 之間流動。 如果使用者位於公用網際網路 (以外的辦公室) ，媒體會從使用者流向 Proxy SBC 的公用 IP，而 Proxy SBC 會代理到相關的下游 SBC () 。
...
-  Proxy SBC 只會電話系統外部 IP 位址顯示，並路由流程從下游 SBC (在此案例中，本地 SBC 越南) 電話系統。 
...
下圖顯示使用者超出公司網路邊界時的流量流程。 使用者不在內部部署 (不在公司網路邊界) 。 使用者透過直接路由電話Teams撥打至 越南。 
...
- 本地分支辦公室的下游 SBC 無法直接電話系統，而是對應到設定 Local Media 優化時由 Contoso 系統管理員定義的虛擬網路拓撲中。
...
假設新加坡的 SBC 已針對新加坡和印尼的當地下游 SBC 越南 PROXY SBC。 使用者位於越南 SBC 的同一個位置。 線上語音路由策略指定越南 (區碼 +84) 通話應路由至 越南 中的本地 SBC。 所有其他通話 ，如果 越南 中的 SBC 失敗，越南中的通話應該會路由至新加坡的 Proxy SBC。 下表摘要列出範例組組。 
...
在下列圖表中，指派給 越南 中當地分支辦公室的使用者，在內部部署時，透過 Teams 直接路由Teams。 
...
- Proxy SBC 只電話系統外部 IP 位址顯示，並路由流程從下游 SBC (在此案例中，將 越南) 中的本地 SBC 電話系統。 
...
假設新加坡的 SBC 已針對新加坡和印尼的當地下游 SBC 越南 PROXY SBC。 印尼的內部使用者位於當地分公司，正在撥打直接路由越南。 線上語音路由規則指定使用越南 (區碼 +84) 的通話應路由至 越南 中的本地 SBC。 所有其他通話 ，萬一 SBC 越南失敗，越南的通話應該會路由至新加坡的 Proxy SBC。 新加坡的 Proxy SBC 設定為 "Always bypass" 模式，越南中的本地 SBC 設定為 'Only for Local Users' 模式。 下表摘要列出範例組組。 
