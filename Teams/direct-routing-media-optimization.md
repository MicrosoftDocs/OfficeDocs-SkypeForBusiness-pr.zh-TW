...
- 控制Teams用戶端與客戶會話框線控制器之間的媒體流量如何 (SBC) 。
- 將媒體保持在公司網路子網的邊界內。
- 允許Teams用戶端和 SBC 之間的媒體串流，即使 SBC 位於具有私人 IP 的公司防火牆後面，而且 Microsoft 也無法直接看到。

本機媒體優化支援兩種案例：

- 透過連接至主要會話初始通訊協定的集中式 SBC 集中處理所有本機主幹 (SIP) 主幹-- 提供電話語音服務給公司的所有當地分公司。

- 建置 SBC 的虛擬網路拓撲，其中本機分公司中的 SBC 會連線到集中式 Proxy SBC，並透過其外部 IP 位址Microsoft 電話系統通訊。 在虛擬網路拓撲中，下層 SB 會透過內部 IP 通訊，而電話系統無法直接看到。
...
- **在歐洲**，Contoso 在大約 30 個國家/地區設有辦公室。 每個辦公室都有自己的私人分支Exchange (PBX) 。
...
- **在歐洲**，所有主幹都是集中式的，媒體會根據使用者位置在 SBC 中心和使用者之間流動。
  ...
  - 如果使用者已連線至公司網路 (的本機子網，則該使用者為內部) ，媒體會在中央 SBC 的內部 IP 和使用者的Teams用戶端之間流動。

  - 如果使用者超出公司網路的界限，例如，如果使用者使用公用無線網際網路連線，則使用者會被視為外部使用者。 在此情況下，媒體會在中央 SBC 的外部 IP 和Teams用戶端之間流動。

- **在 APAC 地區**，集中式 Proxy SBC 已與 Microsoft Direct Routing 配對，後者會在直接路由介面和當地分公司的下層 SB 之間導向媒體。

  在 APAC 的直接路由中，無法直接看到當地分公司中的下層 SBC，但是它們是使用Set-CSOnlinePSTNGateway Cmdlet 在 Microsoft 電話 System 中建立虛擬網路拓撲來配對。 媒體會盡可能保持本機。 外部使用者的媒體在Teams用戶端和 Proxy SBC 的公用 IP 之間流動。
...
在內部部署期間，使用者會被指派給德國的當地分公司。 使用者透過Teams直接路由來電。

- 使用者的Teams用戶端會直接透過 REST API 與電話系統通訊，但通話期間產生的媒體會流向中央 SBC 的內部 IP 位址。

- SBC 會將流程重新導向至電話系統和連線的 PSTN 網路。

- 只有透過外部 IP 位址電話系統才能看到中央 SBC。
...
下圖顯示當使用者不在內部部署且未連線至公司網路時的流量 (，也就是使用者的裝置是透過行動裝置或公用 Wi-Fi) 連線到網際網路。 使用者透過Teams直接路由撥打電話：

- 使用者的Teams用戶端會直接透過 REST API 與電話系統通訊，但在此情況下，通話期間產生的媒體會流向中央 SBC 的外部 IP 位址。
...
使用者獲指派至越南中的本機分公司，且位於內部部署，可透過Teams進行直接路由通話。

- 使用者的Teams用戶端會直接透過 REST API 與電話系統通訊，但通話期間產生的媒體會流向本機 SBC 的內部 IP 位址。

- 本機 SBC 會將流程重新導向至新加坡的 Proxy SBC 以及連線的本機 PSTN 網路。

-  Proxy SBC 僅可透過外部 IP 位址電話系統，並在此情況下，將流量從下方 SBC (路由至越南) 中的本機 SBC 以電話系統。

- 本機分公司中的下層 SBC 無法直接電話系統，但在設定本機媒體優化時由 Contoso 系統管理員定義的虛擬網路拓撲內對應。
...
![再次顯示流量本機媒體優化的圖表。] (當使用者位於「家庭」網路時，媒體/direct-routing-media-op-3.png「Proxy SBC 與連接的下方 SB 的流量」) 

### <a name="external-user"></a>外部使用者

下圖顯示當使用者超出公司網路界限時的流量。 使用者不在內部部署 (不在公司網路) 的範圍內。 使用者透過Teams直接路由到越南中的電話號碼。

- 使用者的Teams用戶端會直接透過 REST API 與電話系統通訊，但通話期間產生的媒體會先流向新加坡 Proxy SBC 的外部 IP 位址。
...
- Proxy SBC 只能透過外部 IP 位址電話系統。

-  本機分公司中的下層 SBC 無法直接電話系統，但在設定本機媒體優化時由 Contoso 系統管理員定義的虛擬網路拓撲內對應。 在範例中，使用者會被視為外部使用者，因為使用者超出公司網路的界限。
...
| 使用者實體位置 | 使用者撥打號碼 | 線上語音路由原則 | 為 SBC 設定模式 | 媒體Flow |
|:------------|:-------|:-------|:-------|:-------|
| 德國 | +49 1 437 2800 | 優先順序 1：^ \+ 49 (\d {8}) $ -DEsbc.contoso.com<br>優先順序 2：.* - proxysbc.contoso.com| DEsbc.contoso.com – 永遠略過 <br>proxysbc.contoso.com – 永遠略過 | Teams使用者<–> DEsbc.contoso.com |

下圖顯示德國內部使用者透過Teams直接路由電話撥打到德國電話號碼的高層級流量。

- 使用者的Teams用戶端會直接透過 REST API 與電話系統通訊。

- 通話期間產生的媒體會流向本機 SBC 的內部 IP 位址。

- 本機 SBC 會將流程重新導向至阿姆斯特丹的 Proxy SBC 以及連線的本機 PSTN 網路。

- Proxy SBC 僅能透過外部 IP 位址電話系統，並在此情況下，將流量從下方 SBC (路由至德國的本機 SBC) 電話系統。
...
| 使用者實體位置 | 使用者撥打號碼 | 線上語音路由原則 | 為 SBC 設定模式 | 媒體Flow |
|:------------|:-------|:-------|:-------|:-------|
| 法國 | +49 1 437 2800 | 優先順序 1：^ \+ 49 (\d {8}) $ -DEsbc.contoso.com <br>優先順序 2：.* - proxysbc.contoso.com |  DEsbc.contoso.com – 永遠略過 proxysbc.contoso.com – 永遠略過 | Teams使用者<– > DEsbc.contoso.com  |

下圖顯示位於法國的德國內部使用者透過Teams與德國的號碼進行直接路由路由通話時的高層級流量。
...
| 使用者實體位置 | 使用者撥打號碼 | 線上語音路由原則 | 為 SBC 設定模式 | 媒體Flow |
|:------------|:-------|:-------|:-------|:-------|
| 越南 | +84 4 3926 3000 | 優先順序 1：^ \+ 84 (\d {9}) $ -VNsbc.contoso.com <br>優先順序 2：.* - proxysbc.contoso.com | VNsbc.contoso.com – 僅適用于本機使用者 <br> proxysbc.contoso.com – 永遠略過 | Teams User <–> VNsbc.contoso.com |

在下列圖表中，指派給 越南 中本機分公司的使用者，在內部部署時，會透過Teams進行直接路由通話。
...
- 本機分公司中的下層 SBC 無法直接電話系統，但在虛擬網路拓撲中對應。
...
| 使用者實體位置 | 使用者撥打號碼 | 線上語音路由原則 | 為 SBC 設定模式 | 媒體Flow |
|:------------|:-------|:-------|:-------|:-------|
| 印尼 | +84 4 3926 3000 | 優先順序 1：^ \+ 84 (\d {9}) $ -VNsbc.contoso.com <br> 優先順序 2：.* - proxysbc.contoso.com |VNsbc.contoso.com – 僅適用于本機使用者 <br> proxysbc.contoso.com – 永遠略過 | Teams User <–> proxysbc.contoso.com <–> VNsbc.contoso.com |


在下圖中，內部使用者在印尼分公司的內部部署期間，透過Teams直接路由撥打電話至越南中的號碼。
...
- 本機分公司中的下層 SB 無法直接電話系統，但在虛擬網路拓撲中對應。
...
| 問題 | 解決 方案 |
| :--- | :--- |
| 當Teams用戶端公用 IP 與客戶信任的 IP 清單相符時，Teams不會識別為 **內部** 用戶端。 | 本機媒體優化要求Teams用戶端子網符合租使用者設定的[網路子網](/powershell/module/skype/new-cstenantnetworksubnet)|
| 當Teams用戶端被識別為內部時，通話升級會導致通話中斷。| 停用直接路由 SBC 上的本機媒體優化。|
| 內部客戶之間從 1 到 1 通話的通話升級為外部客戶的多方通話/資源導致通話中斷 | 正在處理修正程式。 或者，停用直接路由 SBC 上的本機媒體優化。|
| Teams使用者將通話設為保留。 音樂會在 PSTN 端播放，而本機媒體優化正在運作。 Teams使用者繼續通話。 PSTN 通話會繼續進行，但本機媒體優化無法運作，且通話會透過中央 (Proxy) SBC 繼續進行 | 當使用者停用電話來啟動等候音樂 (MoH) 時，呼叫控制器會將通話從 1：1 升級為多方通話，以叫用媒體控制器和媒體處理器 (做為 AVMCU 混搭) MoH 接獲保留的使用者。 通話繼續之後，降級為 1 對 1 通話，並不會依照設計進行。 停用直接路由 SBC 上的本機媒體優化。|
