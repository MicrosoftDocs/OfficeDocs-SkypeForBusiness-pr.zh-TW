...
ms.locfileid: "58866395"
---
# <a name="plan-for-local-media-optimization-for-direct-routing"></a>規劃直接路由的當地媒體優化

公用交換電話網路 (PSTN) 語音被視為商務關鍵型應用程式，對語音品質有很高的期望。 直接路由可讓您控制媒體流量流量，以配合全球各種企業的各種網路拓撲和市內電話設定。 
...
- 使用者的用戶端Teams直接透過 REST API 電話系統通訊至 電話系統，但在此案例中，通話期間產生的媒體會流向中央 SBC 的外部 IP 位址。 
...
![圖表顯示流量流程 Local Media 優化。](media/direct-routing-media-op-2.png "如果使用者是外部使用者，則使用已連接集中式 SIP 主幹的集中式 SBC 時的流量")

## <a name="proxy-sbc-with-connected-downstream-sbcs"></a>Proxy SBC 與已連接的下游 SBC

若要建立解決方案，讓 PSTN 服務在非選項的 APAC 地區所有當地分支辦公室中提供，而 TDM 主幹的集中化並非選項，Contoso 系統管理員會將一個 SBC (proxysbc.contoso.com) 也稱為 Proxy SBC 配對至直接路由服務。 
...
下圖顯示當使用者位於 APAC 地區的辦公室內時，案例的高層流量流量。 指派給位於 越南 的當地分公司，且位於內部部署中的使用者，會透過 Teams 進行直接路由Teams。 

- 使用者的用戶端Teams直接透過 REST API 電話系統通訊，但通話期間產生的媒體會流向當地 SBC 的內部 IP 位址。

- 本地 SBC 會將流程重新導向至新加坡的 Proxy SBC，並重新導向到已連接的當地 PSTN 網路。

-  Proxy SBC 只會電話系統外部 IP 位址，並路由流程從下游 SBC (在此案例中為 越南) 中的本地 SBC 電話系統。 
...
若要區分本地媒體優化模式，租使用者系統管理員必須使用 Set-CSonlinePSTNGateway Cmdlet，將 -BypassMode 參數設定為每個 SBC 的 'Always' 或 'OnlyForLocalUsers' 。 詳細資訊，請參閱設定 [Local Media 優化](direct-routing-media-optimization-configure.md)。  
...
假設新加坡的 SBC 已針對新加坡和印尼的當地下游 SBC 越南 PROXY SBC。 使用者位於越南 SBC 的同一個位置。 線上語音路由策略指定越南 (使用區碼 +84) 的通話，在 越南 中路由至當地 SBC。 所有其他通話 ，以及如果 越南 中的 SBC 失敗，越南中的通話應該會路由至新加坡的 Proxy SBC。 下表摘要列出範例組組。 
...
在下列圖表中，指派給 越南 中當地分公司的使用者在內部部署時，會透過 Teams 直接路由Teams。 
