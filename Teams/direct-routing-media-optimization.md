...
ms.locfileid: "58725512"
---
# <a name="plan-for-local-media-optimization-for-direct-routing"></a>直接路由的本地媒體優化規劃

公用交換電話網路 (PSTN) 語音是商務關鍵型應用程式，對語音品質有很高的期望。 直接路由可讓您控制媒體流量流量，以配合全球各種企業的各種網路拓撲和市內電話設定。 
...
下圖顯示當使用者位於 APAC 地區的辦公室內時，案例的高層流量流量。 指派給 越南 當地分公司且位於內部部署中的使用者，會透過 Teams 進行直接路由Teams。 

- 使用者的用戶端Teams直接透過 REST API 電話系統通訊，但通話期間產生的媒體會流向當地 SBC 的內部 IP 位址。

- 本地 SBC 會將流程重新導向至新加坡的 Proxy SBC，並重新導向到已連接的當地 PSTN 網路。

-  Proxy SBC 只會電話系統外部 IP 位址顯示，並路由來自下游 SBC (在此案例中，越南) SBC 電話系統。 
...
下圖顯示德國內部使用者透過 Teams 直接路由電話號碼到德國號碼的高層流量流量。 
...
- Proxy SBC 只電話系統外部 IP 位址顯示，並路由流程從下游 SBC (在此案例中，德國的當地 SBC) 電話系統。 
...
位於荷蘭的 SBC 已配置為德國當地下游 SBC 的 Proxy SBC。 Proxy 和 (的 SBC 都) 一直旁路模式。 法國的內部使用者位於當地分公司，正在撥打直接路由到德國。 線上語音路由策略指定使用 (+49) 撥打至德國的當地 SBC。 所有其他通話 ，如果德國的 SBC 失敗，則所有在德國的通話都應該路由至位於荷蘭的 Proxy SBC。 下表摘要列出範例組組。 
...
假設新加坡的 SBC 已針對新加坡和印尼的當地下游 SBC 越南 PROXY SBC。 印尼的內部使用者位於當地分公司，正在撥打直接路由越南。 線上語音路由規則指定，越南 (區碼 +84) 的通話應路由至 越南。 所有其他通話 ，萬一 SBC 越南失敗，越南的通話應該會路由至新加坡的 Proxy SBC。 新加坡的 Proxy SBC 設定為 "Always bypass" 模式，而 越南 中的本地 SBC 則設為 "僅適用于當地使用者」 模式。 下表摘要列出範例組組。 
...
| Teams將通話保留。 PSTN 端播放音樂，而 [Local Media 優化功能」 正在使用中。 使用者Teams繼續通話。 PSTN 的通話繼續，但 [本地媒體優化 (SBC) 通話 | 當使用者將通話暫停以啟動等候音樂 (MoH) 時，呼叫控制器會從 1：1 升級為多方通話，以呼叫媒體控制器和媒體處理器 (做為 AVMCU 混合器) ，MoH 會透過此呼叫到達已保留的使用者。 在通話繼續後取消升級為 1：1 通話，永遠不會像每個設計一樣發生。 停用直接路由 SBC 上的 Local Media 優化。|
