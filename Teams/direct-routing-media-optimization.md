...
![顯示流量流程 Local Media 優化的圖表](media/direct-routing-media-op-2.png "如果使用者是外部使用者，則使用已連接集中式 SIP 主幹的集中式 SBC 時的流量")
...
-  Proxy SBC 只會電話系統外部 IP 位址顯示，並路由來自下游 SBC (在此案例中，越南) 的當地 SBC 電話系統。 
...
在下列圖表中，指派給 越南 中當地分支辦公室的使用者在內部部署時，會透過 Teams 直接路由Teams。 
...
在下列圖表中，內部使用者在印尼分公司的內部部署中，透過 Teams直接路由電話越南。 
...
| 當用戶端被識別為內部Teams呼叫升級會導致通話中斷。| 停用直接路由 SBC 上的 Local Media 優化。|
| 從內部客戶之間的 1 到 1 通話升級為與外部客戶/資源進行多方通話，導致通話中斷 | 正在處理修正程式。 或者，停用直接路由 SBC 上的 Local Media 優化。|
| Teams將通話保留。 PSTN 端播放音樂，而 [Local Media 優化功能」 正在使用中。 使用者Teams繼續通話。 PSTN 通話繼續，但 [本地媒體優化 (SBC) 通話 | 當使用者將通話暫停以啟動等候音樂 (MoH) 時，通話控制器會從 1：1 升級為多方通話，以呼叫媒體控制器和媒體處理器 (做為 AVMCU 混合器) ，MoH 會透過此呼叫到達已保留的使用者。 在通話繼續後取消升級為 1：1 通話，永遠不會像每個設計一樣發生。 停用直接路由 SBC 上的 Local Media 優化。|
