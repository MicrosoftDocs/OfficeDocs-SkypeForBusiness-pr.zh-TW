...
媒體旁路可讓您縮短媒體流量的路徑，並減少傳輸中的躍點數量，以提升績效。 在媒體旁路時，媒體會保留在會話邊界控制器 (SBC) 與用戶端之間，而不是透過 Microsoft Phone 系統傳送。 若要設定媒體旁路，SBC 和用戶端必須位於相同的位置或網路。
...
若沒有媒體旁路，當用戶端撥打或接聽來電時，SBC、Microsoft Phone System 和 Teams 用戶端之間的訊號和媒體流程會如下列圖表所示：
...
- 媒體處理器是 B2BUA，這表示它可以在 MP 和 S) BC (之間變更編解碼器，例如 SILK 從 Teams 用戶端變更為 MP 和 G.711。
...
媒體流量在 SBC Teams用戶端之間流動，如果有直接的Teams，或者如果用戶端無法使用公用 IP 位址到達 SBC，則媒體流量會透過 Teams 傳輸轉場。

### <a name="requirements-for-direct-media-traffic-between-the-teams-client-and-the-sbc"></a>用戶端與 SBC (之間Teams媒體流量的需求)  

用戶端必須能夠存取指定的埠， (請參閱) 公用 IP 位址上的資料表。 
...
對於不支援媒體旁路的所有其他端點，我們會將通話轉換為非旁路，即使它以旁路通話開始。 這會自動發生，而且不需要系統管理員執行任何動作。 這包括在 Microsoft Edge 商務用 Skype、Google Chrome、Mozilla Firefox) 上，支援直接路由通話 (WebRTC 用戶端的 3PIP 電話和 Teams Web 用戶端。 
