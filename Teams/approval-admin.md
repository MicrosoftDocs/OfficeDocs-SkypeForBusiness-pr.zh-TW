...
> [!NOTE]
> 這項功能尚未發行至 政府社群雲端 (GCC) 、政府社群雲端 GC) CH (，以及 (DOD) 使用者。
...
- 當系統管理員第一次為整個組織建立範本時，系統會自動為租使用者的所有系統管理員 ，包括全域和小組的服務系統管理員Teams建立新的 Teams 小組。 這些系統管理員會新增為團隊擁有者，以便共同管理組織範本。 小組建立之後，組織的新管理員必須手動新增為團隊擁有者，以便他們擁有管理全組織範本的相同許可權。

> [!Note]
> 如果系統管理員刪除團隊，您可以在 AAD Azure Active Directory (入口網站) 還原所有資料。 一個月後，或者如果系統管理員在回收站中刪除此小組，您就會失去所有相關的資料。
