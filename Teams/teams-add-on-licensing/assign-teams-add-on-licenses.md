...
> [!NOTE]
> 請參閱[小組附加元件授權](microsoft-teams-add-on-licensing.md)，瞭解適用于附加元件授權的小組功能。 您也可以找到需要購買哪些授權，以及如何購買（視您的方案而定）的相關資訊，讓使用者取得音訊會議、免付費電話號碼，以及撥打貴組織外部電話號碼的功能。 在您決定要為使用者提供哪些功能之後，請將授權指派給他們。
...
- 如果您是針對混合式使用者使用內部部署的公用交換電話網絡（PSTN）連線，則只需指派電話系統授權即可。 請勿指派通話方案授權。
...
如果您需要指派大量使用者（例如幾百或數千位使用者）的授權，請[在 Azure Active Directory （AZURE AD）中使用 Powershell 或群組式授權](https://docs.microsoft.com/azure/active-directory/users-groups-roles/licensing-groups-assign)。  
...
3. 在 Windows PowerShell 命令提示字元中，執行下列腳本來指派授權給您的使用者，其中 \<CompanyName:License> 是您的組織名稱，以及您要指派之授權的識別碼。 例如，litwareinc： MCOMEETADV。
    ...
    若要指派 Microsoft 商務語音（不含通話計畫）授權，請在腳本中使用下列語法：
...
| 產品名稱| SKU 元件名稱 |
|--------------|---------------|
| Microsoft 企業版 E5 （含電話系統） | ENTERPRISEPREMIUM |
| Microsoft 企業版 E5 （不含音訊會議） | ENTERPRISEPREMIUM_NOPSTNCONF |
| Microsoft 企業版 E5 （含音訊會議） | ENTERPRISEPREMIUM |
...
| Microsoft 商務語音（無需通話方案） | BUSINESS_VOICE_DIRECTROUTING  |
| 適用于美國的 Microsoft 商務語音（不含通話方案）| BUSINESS_VOICE_DIRECTROUTING _MED |
| 音訊會議 | MCOMEETADV | 
| 每分鐘付費語音會議（隨您的付費）</br>*需要設定並啟用通訊信用額度。* | MCOMEETACPEA |
| 電話系統 | MCOEV |
| 國內和國際通話方案 | MCOPSTN2 |
| 國內通話方案（每個使用者/每個月的每個使用者/每個月的每個月的3000分鐘1200，歐盟國家/地區的每個使用者/月份） | MCOPSTN1 |
| 國內通話方案（每個國家/地區每個使用者/月的120分鐘） </br>*美國未提供此方案。* | MCOPSTN5 |
| 國內通話方案（每個國家/地區每個使用者/月的240分鐘） </br>*美國未提供此方案。* | MCOPSTN6 |
| 通訊點數 | MCOPSTNPP |
