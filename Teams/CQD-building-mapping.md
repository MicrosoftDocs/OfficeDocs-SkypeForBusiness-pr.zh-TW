...
當您使用 CQD 時，您必須知道端點的位置，以及端點是否連接到您可以管理的網路或無法管理的網路，前提是只能改善您可以管理的網路。 將子網和建築物資訊上傳至 CQD，您可以啟用 CQD 來判斷端點是否已連接到內部 (受管理的) 網路，或已 (未管理的) 網路。 這就是為什麼為貴組織建立建築物地圖並將其上傳到 [CQD](CQD-upload-tenant-building-data.md)非常重要的原因。
