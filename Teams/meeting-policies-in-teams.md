...
此設定可控制使用者會議中是否會允許桌面和/或視窗分享。 沒有指派任何原則 (的會議參與者，例如外部參與者) 繼承會議召集人的原則。
...
這是每一使用者原則。 此設定可控制使用者是否可以在會議中分享白板。 外部參與者，包括匿名、來賓和外部存取使用者，會繼承會議召集人的原則。
...
這是每一使用者原則。 此設定可控制使用者是否可以在會議中建立和共用記事。 外部參與者，包括匿名、來賓和外部存取使用者，會繼承會議召集人的原則。 在有最多 100 個參與者的會議中，支援 [會議記錄 **]** 索引標籤。
...
|設定值  |加入行為 |
|---------|---------|
|**每個人**   |所有會議參與者會直接加入會議，而不需在大廳等候。 這包括已驗證的使用者、外部存取使用者、來賓和匿名使用者。     |
|**組織中的人員、信任的組織和來賓**     |Authenticated users within the organization, including guest users and the users from trusted organizations, join the meeting directly without waiting in the lobby. Anonymous users wait in the lobby.   |
|**組織中的人員和來賓**    |Authenticated users from within the organization, including guest users, join the meeting directly without waiting in the lobby. Users from trusted organizations and anonymous users wait in the lobby. This is the default setting.           |
|**僅限召集人**    |只有會議召集人可直接加入會議，而不需在大廳等候。 其他每個人 (包括組織內已驗證的使用者、來賓使用者、來自信任組織的使用者和匿名使用者) 都必須在大廳等候。           |
|**組織中的人員**  |Authenticated users from within the organization, excluding guest users, join the meeting directly without waiting in the lobby. Guests and users from trusted organizations and anonymous users wait in the lobby.|
