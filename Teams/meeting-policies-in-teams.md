...
::: zone target="docs"
會議原則是針對由您組織中的使用者所排程的會議，控制會議參與者可用於會議的功能。 您可以使用全域 (組織範圍的預設) 原則，此原則會自動建立或建立並指派自訂原則。 您可以在 Microsoft 團隊系統管理中心或使用 [PowerShell](teams-powershell-overview.md)管理會議原則。

> [!NOTE]
> 如需使用角色來管理會議簡報者與出席者權限的相關資訊，請參閱 [小組會議中的角色](https://support.microsoft.com/office/roles-in-a-teams-meeting-c16fa7d0-1666-4dde-8686-0a0bfe16e019?ui=en-us&rs=en-us&ad=us)。
...
這是針對每個使用者的原則，在會議開始之前就會套用。 此設定可控制團隊會議是否可以在 Outlook (Windows、Mac、web 及行動裝置) 中排程。

![螢幕擷取畫面顯示排程新會議的功能](media/meeting-policies-outlook-add-in.png)

如果您關閉此功能，使用者就無法進行排程。 在 Outlook 中建立新會議時，小組會議。 例如，在 Windows 版 Outlook 中，新的 [ **小組會議** ] 選項不會顯示在功能區中。

### <a name="allow-channel-meeting-scheduling"></a>允許頻道會議排程

使用現有的 AllowChannelMeetingScheduling 原則來控制可在小組頻道行事曆上建立的事件種類。 這是針對每個使用者的原則，在會議開始之前就會套用。 此設定控制使用者是否可在團隊頻道中排程會議。 根據預設，此設定為開啟狀態。

如果已關閉此原則，使用者將無法建立新的頻道會議。 不過，您可以透過事件的召集人來編輯現有的頻道會議。
...
- [頻道回復撰寫] 方塊上的 [**排程會議**] 按鈕。
  ![[排程會議] 按鈕的 [回復] 核取方塊](media/schedule-meeting-disabled-in-chat2.png)
  
- 頻道標題上的 [**排程會議**] 按鈕。
  ![頻道標題中的 [排程會議] 按鈕](media/schedule-now-in-header.png)

在頻道行事曆中：

- 頻道行事曆標頭上的 [**新增活動**] 按鈕將會停用。
  ![已停用頻道行事曆標頭上的按鈕](media/add-new-event-disabled.png)

- 使用者無法在頻道行事曆中拖曳並選取時間區塊來建立頻道會議。
...
請注意，如果您關閉 [ **允許排程私人會議** ] 和 [ **允許頻道會議排程**]，就會針對小組中的使用者停用 [ **新增必要的出席** 者] 和 [ **新增頻道** ] 選項。 根據預設，此設定為開啟狀態。

### <a name="allow-meet-now-in-private-meetings"></a>在私人會議中允許立即開會

這是針對每個使用者的原則，在會議開始之前就會套用。 此設定控制使用者是否可以啟動未計畫的私人會議。  根據預設，此設定為開啟狀態。
...
- [[IP 音訊] 模式](#mode-for-ip-audio)
- [[IP 影片] 模式](#mode-for-ip-video)
...
### <a name="mode-for-ip-audio"></a>[IP 音訊] 模式

這是每個使用者的原則。 此設定控制是否可以在會議和群組通話中開啟音訊。 以下是此設定的值。

|設定值 |行為  |
|---------|---------|
|**已啟用輸出和傳入音訊**    |在會議中允許傳出和傳入音訊。 這是預設設定。 |
|**禁止**     |在會議中會關閉傳出和傳入音訊。     |

如果使用者設定為 [ **停用** ]，該使用者仍然可以排程及組織會議，但無法使用音訊。 若要加入會議，他們必須透過公開的交換電話網絡撥入 (PSTN) ，或進行會議通話，然後透過電話加入。 沒有指派任何原則的會議參與者 (例如，匿名參與者) 將此設定為預設 **啟用的傳出和傳入音訊** 。 在團隊行動用戶端上，如果停用此設定，使用者必須透過 PSTN 撥入會議。
...
這是每個使用者的原則。 此設定控制是否可以在會議和群組通話中開啟影片。 以下是此設定的值。

|設定值 |行為  |
|---------|---------|
|**已啟用外寄及內送影片**    | 在會議中允許外寄和內送的影片。 這是預設設定。 |
|**禁止**     | 在會議中，待發和內送的影片會關閉。 在團隊行動用戶端上，使用者無法在會議中共用影片或相片。 <br><br>請注意，如果 [ **ip 音訊] 模式** 已停用，則 [ **Ip video] 模式** 也會保持停用狀態。  |
...
這是每個組織單位和每個使用者原則的組合。 影片是會議的關鍵元件。 在某些組織中，系統管理員可能會想要進一步控制哪些使用者的會議擁有影片。 此設定控制是否可在使用者所主持的會議中，以及由使用者開始的1:1 和群組通話中開啟影片。 在團隊行動用戶端上，此設定可控制使用者是否可以在會議中共用相片和影片。

依啟用此原則設定的使用者所組織的會議，如果參與者也已啟用原則設定，允許會議參與者在會議中共用影片。 沒有指派任何原則的會議參與者 (例如，匿名和聯盟參與者) 繼承會議召集人的原則。

> [!NOTE]
> 請記住，此設定會控制外寄影片，而 **IP 視訊模式** 設定則會控制外寄和內送的影片。 若要深入瞭解，請參閱 [哪些 IP 影片原則設定優先？](#which-ip-video-policy-setting-takes-precedence) 以及 [管理會議參與者的音訊/視頻](#manage-audiovideo-for-meeting-participants)。
...
|---------|---------|---------|
|Daniela   | 全域   | 按       |
|Amanda    | Location1MeetingPolicy        | 出      |
...
#### <a name="which-ip-video-policy-setting-takes-precedence"></a>優先使用哪個 IP 影片原則設定

對於使用者而言，影片的限制性原則設定會優先。 以下是一些範例。

|允許 IP 視訊|[IP 影片] 模式|會議體驗|
|---------|---------|---------|
|召集人： **開啟**<br><br>參與者： **開啟** |參與者： **已停用**        |[ **IP 影片]** 設定的優先順序優先。 獲指派此原則的參與者無法開啟或查看其他人所共用的影片。|
|召集人： **開啟**<br><br>參與者： **開啟** |參與者： **已啟用傳出和傳入的影片**          |獲指派此原則的參與者可以開啟或查看其他人所共用的影片。         |
|召集人： **開啟**<br><br>參與者： **關閉** |參與者： **已啟用傳出和傳入的影片**         |[ **允許 IP 視頻** ] 設定優先。 參與者只能看到傳入的影片，而且無法傳送外寄的影片。         |
|召集人： **開啟**<br><br>參與者： **關閉** |參與者： **已停用**         |[ **IP 影片]** 設定的優先順序優先。 參與者看不到入站或外寄的影片。|
|召集人： **關閉**    |       |[ **允許 IP 視頻** ] 設定的優先順序，是因為召集人已關閉該功能。 任何人都無法在指派此原則的使用者所組織的會議中開啟影片。         |

### <a name="manage-audiovideo-for-meeting-participants"></a>管理會議參與者的音訊/視頻

|如果您想要 .。。  |設定下列原則設定  |
|---------|---------|
|在會議中停用參與者的音訊和影片  |IP 音訊的模式： **已停用**<br> IP 影片的模式： **已停用**<br>允許 IP video：暫缺       |
|在會議中僅針對參與者啟用入站影片和音訊  |IP 音訊的模式： **已啟用傳出和傳入音訊**<br> IP 影片的模式：**啟用外寄及** 內送影片<br>允許 IP 影片： **關閉**       |
|在會議中停用參與者的影片 (參與者只提供音訊) |  IP 音訊的模式： **啟用傳出和傳入音訊**<br> IP 影片的模式： **已停用**<br>允許 IP video：暫缺
|在會議中啟用參與者的音訊和影片    |IP 音訊的模式： (預設) **啟用傳出和傳入音訊**<br> IP 影片的模式： (預設) **啟用外寄及** 內送影片<br>允許 IP **影片： (** 預設)     |

套用會議召集人原則與使用者原則之間的限制性最高的原則。 例如，如果召集人擁有限制影片的原則，且使用者的原則不限制影片，會議參與者會繼承會議召集人的原則，且在會議中不能存取影片。 這表示他們只能使用音訊加入會議。

> [!NOTE]
> 當使用者啟動由手機加入的群組通話時，不會顯示 [ **使用電話語音音訊** ] 畫面。 這是我們正在解決的已知問題。 若要解決此問題，請選取 [**其他連接選項**] 底下的 [**電話音訊**]。  

#### <a name="teams-mobile-clients"></a>團隊行動用戶端

針對小組行動用戶端上的使用者，在會議期間共用相片和影片的功能也是由 [ **允許 IP 視頻** 或 **IP 視訊模式]** 設定所決定。 根據優先的原則設定，將無法使用共用影片與相片的功能。 這不會影響螢幕共用，您可以使用個別的 [螢幕共用模式](#screen-sharing-mode) 設定來設定。 此外，您也可以設定 [團隊行動原則](https://docs.microsoft.com/powershell/module/skype/new-csteamsmobilitypolicy) ，避免行動使用者透過行動裝置連線使用 IP 影片，這表示他們必須使用 WiFi 連線。
...
如果會議需要最高品質的影片體驗（例如 CEO 董事會會議和小組即時事件），建議您將頻寬設定為 10 Mbps。 即使已設定最高的體驗，小組的媒體堆疊也會在偵測到某些網路條件時適應低頻寬情況（視情況而定）。
...
|---------|---------|---------|
|Daniela   | 全域   | 按       |
|Babek    | Location1MeetingPolicy        | 出   |
...
這是每個使用者的原則。 組織是否有此組使用者無論會議召集人已設定的內容，都不會控制外部參與者所能執行的動作。 這個參數會控制外部參與者是否可以受到控制或要求對共用者的螢幕進行控制，這要視共用資源在組織的會議原則中所設定的內容而定。 團隊會議中的外部參與者可以分類如下：  
...
這是允許 leaderless 電話撥入式會議會議的每個召集人原則。 此設定會控制撥入使用者是否無需經過驗證的使用者，就能從出席中的組織加入會議。 根據預設，此設定會關閉，這表示撥入使用者會在大廳等候，直到組織中的經過驗證的使用者加入會議為止。

> [!NOTE]
> 如果已關閉此設定，且撥入使用者會先加入會議，且位於大廳，組織使用者必須加入與團隊用戶端的會議，才能從大廳承認使用者。 沒有可撥打給使用者的大廳控制項。

### <a name="automatically-admit-people"></a>自動承認人員

這是每個召集人原則。 此設定控制使用者是否直接加入會議，或在大廳等候，直到經過驗證的使用者准許。 此設定不會套用至撥入使用者。
...
|設定值  |加入行為 |
|---------|---------|
|**任何人**   |所有會議參與者都能直接加入會議，不需要在大廳等候。 這包括經過驗證的使用者、信任組織中的外部使用者 (聯盟) 、來賓及匿名使用者。     |
...
|**僅限召集人**    |只有會議召集人可以直接加入會議，不需要在大廳等候。 其他人，包括組織中的經過驗證的使用者、來賓使用者、受信任組織的使用者以及匿名使用者都必須在大廳等候。           |

### <a name="allow-dial-in-users-to-bypass-the-lobby"></a>允許撥入使用者略過大廳

這是每個召集人原則。 此設定控制由手機撥入的人員是否直接加入會議，或無論是否已 **自動准許 [人員** ] 設定，也會在大廳中等待。 根據預設，此設定為關閉狀態。 關閉此設定時，撥入使用者會在大廳等候，直到組織使用者與團隊用戶端加入會議，然後允許。 開啟此設定時，當組織使用者加入會議時，撥入使用者會自動加入會議。

> [!NOTE]
> 如果撥入使用者在組織使用者加入會議之前加入會議，則會將它們放在大廳中，直到組織使用者使用團隊用戶端加入會議並允許會議為止。 如果您變更任何使用者的預設設定，就會套用至該使用者所組織的所有新會議，以及使用者未修改會議選項的任何先前會議。
...
|設定值 |行為  |
|---------|---------|
|**已停用，但使用者可以覆寫**     | 在會議期間，使用者不會自動開啟 [即時] 標題。 使用者可以在 [溢出 () **...** ] 功能表中看到 [**開啟即時標題**] 選項，將其開啟。 這是預設設定。 |
