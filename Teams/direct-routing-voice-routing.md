...
ms.locfileid: "54337421"
---
# <a name="configure-voice-routing-for-direct-routing"></a>設定直接路由的語音路由

本文將說明如何設定直接路由電話系統語音路由。  這是下列步驟中的步驟 3，用於配置直接路由：
...
**撥打Flow左側 (1) ：** 如果使用者撥打 +1 425 XXX XX XX 或 +1 206 XXX XX XX，通話會路由至 SBC sbc1.contoso.biz 或 sbc2.contoso.biz。 如果 sbc1.contoso.biz 或 sbc2.contoso.biz，通話會中斷。 
...
對於所有其他通話，如果使用者同時擁有系統 (Microsoft 電話 Microsoft 通話方案) ，則系統會使用自動路由。 如果管理員所建立的線上語音路由中沒有任何符合號碼模式，則通話會透過 Microsoft 通話方案路由。 如果使用者只有系統Microsoft 電話，通話會因為沒有可用的比對規則而中斷。
