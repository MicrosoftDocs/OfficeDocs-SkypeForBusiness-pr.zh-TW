...
在內部部署或混合使用商務用 Skype 伺服器起點的組織, 如果他們想要為使用者提供其**** 通訊的互通性與可預見性, 也應該考慮使用這種模式。提供可預測的時程表供其升級至小組 (而不是以**孤島**模式來依賴採用飽和)。
...
此共存模式對於使用企業語音進行商務用 Skype 內部部署的 organiztions 特別有用, 可能需要一些時間才能升級至小組, 且想要儘快從優秀團隊會議中受益。
