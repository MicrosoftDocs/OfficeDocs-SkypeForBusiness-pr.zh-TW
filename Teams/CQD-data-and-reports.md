...
針對合規性原因， (EUII) 資料的使用者身分識別資訊 (也稱為個人身分識別資訊或 PII) ，只會保留30天。 隨著 NRT 資料超過30日標記，包含 EUII 的欄位會被清除，從而產生 EUII 免費 NRT 資料。 包含 EUII 資料的欄位如下：
...
|可靠性報告     |包括音訊、影片、影片畫面共用 (VBSS) 及應用程式共用報告         |
