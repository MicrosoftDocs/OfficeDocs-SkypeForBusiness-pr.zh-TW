...
在使用 Power BI 查詢範本 (PBIX 檔案) Microsoft Teams 通話品質儀表板 (CQD) 之前，您必須使用 *下載中包含的 MicrosoftCallQuality.pqx* 檔案，安裝 Power BI 的 Microsoft 通話品質連接器。[](https://github.com/MicrosoftDocs/OfficeDocs-SkypeForBusiness/blob/live/Teams/downloads/CQD-Power-BI-query-templates.zip?raw=true)

請參閱[使用 Power BI 分析 CQD 資料Teams](CQD-Power-BI-query-templates.md)瞭解這些範本。
...
1. 檢查您的電腦是否已經有 *\[ \] \\ 檔Power BI Desktop \\ Custom 連接器資料夾*。 如果沒有，請建立此資料夾。<sup>1</sup>
...
如果此設定程式期間的任何步驟都不清楚，可以在快速入門中找到程式的詳細說明[：](/power-bi/desktop-quickstart-connect-to-data)連線資料Power BI Desktop。
...
2. 決定要 (以匯總符號) 名稱表示的維度和度量，然後手動選取這些維度和度量，並將它們拖曳到黑色視覺效果。 或者，將它們拖曳 *到視覺效果選項* 下方的值欄位。
...
3. 接下來，選取您想要篩選的任何維度，並將這些維度拖曳到篩選窗格此視覺欄位 *上的篩選。* Microsoft 通話品質連接器目前支援基本 *篩選 (從* 可能維度值) 、進位 *篩選 (清單中* 選取值，手動指定要篩選的值和運算元，類似通話品質 *儀表板) ，* 以及相對日期篩選 (僅適用于結束時間和開始時間維度) 。**  通話品質儀表板不支援根據 *前 N* 個篩選。
...
雖然使用 Power BI，但 Microsoft 通話品質連接器並不支援所有 Power BI 功能，無論是因為通話品質儀表板資料模型的限制，還是 DirectQuery 連接器的一般限制。 下列清單會說明連接器的一些較值得注意的限制，但這份清單不應視為詳盡無遺：
...
4. **參照緩存資料 –** Power BI目前不支援以任何方式參照 DirectQuery 連接器的緩存資料。 任何參照查詢結果的嘗試都會產生新的查詢。

5. **相對資料篩選 –** Microsoft 通話品質連接器支援，但僅支援 *開始時間和**結束時間* 維度。 雖然 *日期* 維度可能是相對日期篩選的明顯選擇，但 *日期* 不會儲存為日期時間物件，因此不支援 Power BI。

6. **僅度量查詢 -** Microsoft 通話品質連接器目前不支援。 建立具有三個或多個度量且沒有維度的視覺效果時，欄資料會轉置。 若要避免這種情況，請一直將至少一個維度 (例如：月年) 視覺效果中。 這是預定在即將推出的 Microsoft 通話品質連接器中解決Power BI。

7. **政府社群雲端 (GCC) 支援 –** 針對 GCC環境中的客戶，Microsoft 通話品質連接器只會在Power BI Desktop使用。 Microsoft 通話品質連接器目前無法與Power BI服務GCC相容。
...
### <a name="when-i-add-certain-dimensions-to-my-reports-the-visual-immediately-returns-couldnt-load-the-data-for-this-visual-removing-the-dimension-fixes-the-visual----what-is-happening"></a>當我新增特定維度至報表時，視覺效果會立即返回「無法載入此 **視覺效果的資料」**。 移除維度會修正視覺效果 --會發生什麼情況？

這是 Microsoft 通話品質連接器中的已知問題;任何顯示為整體數位的維度都會在 Power BI 中顯示為"匯總」欄，其中 Power BI 會嘗試預設摘要動作 (通常是"Sum") 。 在某些情況下，此行為會成功加總值，即使結果並不實用，因為第二個 WiFi 通道等維度的 "加總"是沒有意義的。 在某些情況下，此摘要動作會失敗，且會造成視覺上的錯誤。

若要解決此問題，首先請從視覺效果移除維度。 從 [欄位」 清單中選取維度，流覽至功能區中的 [欄工具」 索引鍵，按一下 [摘要化> 下拉式功能表，然後選取 [ **不摘要** 資料> 。 維度現在可以再次新加到視覺效果中。
...
- **ErrorType 1 - 查詢結構錯誤：** 查詢結構錯誤通常是由連接器無法建立正確格式的查詢所導致。 使用不支援的功能時，通常會發生此情況，如上述限制所指定。 請仔細檢查您並未針對該查詢使用任何計算結果欄或自訂視覺效果。
  ...
  - **ErrorType 7 - 無度量錯誤：** 通話品質儀表板查詢需要量值才能運作。 請仔細檢查您的查詢是否包含量值。 Microsoft 通話品質連接器中的量值會以其名稱 (加) 符號的匯總表示。

如果您遇到此範圍以外的任何其他錯誤，請通知通話品質儀表板小組，以便我們協助疑難排解問題並在適當時更新檔。

## <a name="footnotes"></a>註腳

**<sup>1</sup>** 特定程式與應用程式 (例如 OneDrive) 可能會導致您的檔根資料夾變更;請確定 *Power BI Desktop \\ Custom Connectors* 目錄位於目前的根資料夾檔資料夾內。
