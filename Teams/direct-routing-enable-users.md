...
## <a name="ensure-that-the-user-is-homed-online-applicable-for-skype-for-business-server-enterprise-voice-enabled-users-being-migrated-to-teams-direct-routing"></a>確保使用者位於線上， (適用于商務用 Skype Server 企業語音使用者移Teams直接路由) 
...
3. 如果線上管理使用者的電話號碼，請發出命令： 
    ...
    建議但不需要將所使用的電話號碼配置為包含國家/地區代碼的完整 E.164 電話號碼。 支援使用擴充功能來設定電話號碼，當針對基本號碼進行查詢時，會用來查詢使用者。 這可讓公司以相同的基本號碼和唯一分機來設定電話號碼。 若要成功進行尋找，邀請必須包含具有分機的完整號碼，如下所示：
    ```PowerShell
