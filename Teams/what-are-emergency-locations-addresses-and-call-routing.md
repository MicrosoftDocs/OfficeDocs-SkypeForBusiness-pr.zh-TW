...
>[!Note]
>您管理緊急電話的一些方式有一些差異，電話系統方案或 PSTN 電話系統直接路由。 本文將說明這些考慮事項。
...
每個緊急位址都可以有一個地理 (與經度) 與它相關聯。 在一些國家/地區，這些地理代碼可用來協助以動態位置路由緊急電話。 

如果您在系統管理中心使用位址地圖搜尋功能來定義緊急位址，Teams地理代碼會自動與緊急位址相關聯。 如果您使用 PowerShell 定義位址，您也可以將地理代碼與位址建立關聯。 不過，Microsoft 建議您使用 Teams 系統管理中心的地圖搜尋功能，為通話方案建立緊急位址，以確保位址已格式化、驗證，並擁有適當的地理位置代碼。  
...
Microsoft 通話方案的動態緊急電話提供根據用戶端目前位置設定及路由緊急Teams功能。 自動路由至適當的公用安全應答點 (PSAP) 或通知安全服務台人員的能力會視使用者使用Teams而異。  

針對通話方案使用者，只有美國支援路由緊急電話的動態位置，如下所示。  (有關動態緊急電話和直接路由的資訊，請參閱 [直接路由的考慮](#considerations-for-direct-routing)。

- 如果美國Teams方案使用者的 Teams 用戶端動態取得美國國內的緊急位址，該位址會用於緊急路由，而非登入位址，且該通話會自動路由至該位址服務區域中的 PSAP。
...
緊急電話路由策略參照線上 PSTN 使用方式，此使用方式必須擁有適當的直接路由配置，以將緊急通話正確路由至適當的 PSTN (閘道) 。 特別是，您必須確保緊急撥號字串有 OnlineVoiceRoute。 詳細資訊，請參閱設定 [直接路由](direct-routing-configure.md)。 
 ...
 (：Teams用戶端在緊急號碼前面預先輸入「+」符號，其方式與商務用 Skype類似;即 +911。 此行為會于未來幾個月內修改，Teams緊急電話不會再在號碼前面傳送「+」;也就是說，911.) 
...
會話邊界控制器 (SBC) ELIN 應用程式包含緊急 (識別) 號碼。 如果 SBC ELIN 應用程式已整合至直接路由部署，您必須在 ELIN 應用程式中設定緊急位址和相關電話號碼，然後將 ELIN 記錄上傳到各自 PSTN 中的緊急通話資料庫。  Teams ELIN 識別碼的緊急位置必須與 ELIN 應用程式中的緊急位置相符。
...
緊急通話政策可以授予Teams使用者帳戶、指派給網路網站，或兩者同時提供。  當用戶端Teams或變更網路連接時，Teams會執行用戶端所在的網路網站的檢查：
