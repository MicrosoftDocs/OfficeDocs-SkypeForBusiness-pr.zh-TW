...
Microsoft Teams 用戶端所發現的問題大多可以追溯到防火牆或 Proxy 連線。 確認防火牆或 Proxy 中已開啟必要的 URL、IP 位址和連接埠，將會讓不必要的疑難排解工作降到最低。 如需 Microsoft Teams 所需的 URL 和 IP 的特定資訊，請參閱 [Microsoft 365 和 Office 365 URL](https://support.office.com/article/Office-365-URLs-and-IP-address-ranges-8548a211-3fe7-47cb-abb1-355ea5aa88a2) 和 IP 位址支援文章。 下列案例需要在防火牆中開啟特定的 URL 和連接埠。
...
## <a name="when-teams-is-offline-or-in-low-bandwidth-conditions"></a>當 Teams 離線或處於低頻寬條件時

好消息是，即使您離線或是在低頻寬情況下執行，Teams 仍持續執行。 Teams 會將您所有未傳送的郵件 (聊天) 24 小時，並一回到線上就傳送。 如果您離線超過 24 小時，Teams 可讓您選擇重新發送或刪除未接收的郵件。 我們正在努力將這項功能新增到新的聊天中，並將于提供這項功能時更新此檔。
