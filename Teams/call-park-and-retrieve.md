...
呼叫代答號碼的範圍預先定義為 10-99，且無法修改。 第一個已停駐的通話會呈現 10 的代答碼，下一個已停駐的通話會呈現 11 的代答碼，等等。 直到 99 呈現為代答程式碼。 之後，呈現的取件代碼會再次從 10 開始。  如果超過 89 個使用中保留的通話，呈現的代答碼會持續遞增到 99 以上，因此第 90 個有效保留的通話會呈現 100 個代答碼，第 91 個使用中保留的通話會呈現 101 的代答碼。
...
為了讓該策略能夠執行，必須將它指派給使用者。 您可以 [個別指派該策略給使用者](assign-policies.md) ，或將使用者指派給群組。

將呼叫停駐策略指派給群組

1. 在 [ **通話停駐點政策」** 頁面上，按一下 [ **群群組原則工作分派** > 選項卡上的 [ **新增群組**。
