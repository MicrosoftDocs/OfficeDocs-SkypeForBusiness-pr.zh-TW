...
繁忙（忙碌選項）是一種新的設定，可讓您設定當使用者已在通話或會議中，或有來電處於保留狀態時，處理來電的方式。 您可以使用占線信號拒絕新的或來電的通話。 您可以在租使用者層級或使用者層級啟用 busy 選項。 不論其忙碌選項的設定方式為何，通話或會議中的使用者，或使用保留通話的使用者，都不會阻止您開始新的通話或會議。 此設定預設為停用。
