...
- **語音信箱** - 與您指定的 Microsoft 365 群組相關聯的語音信箱。 您可以選擇是否要語音信箱轉譯，以及「請在鈴聲後面留下訊息」。 系統提示。
  - 在 Microsoft 365 系統管理中心 中，針對您指定的 Microsoft 365 群組，啟用 [**讓組織外部的人員傳送電子郵件給這個小組**]。
...
## <a name="whats-new-for-auto-attendants-in-the-past-six-months"></a>過去六個月自動語音應答的新功能
...
4. 指定此自動語音應答 [的支援語言](create-a-phone-system-auto-attendant-languages.md) 。 此語言將用於系統產生的語音提示。
...
如果您將撥號鍵指派給目的地，建議您選擇 [目錄 **] 搜尋****的 [無**]。 在執行目錄搜尋之前，會先比對撥號鍵。  如果來電者開始使用指派給特定目的地的撥號鍵輸入名稱或分機，則會在完成輸入名稱或分機之前，路由到該目的地。 建議您為目錄搜尋建立個別的自動語音應答，並使用撥號鍵將主要自動語音應答連結至該自動語音應答。
...
- 上班時間可以設定為一天當中的分隔時間，而所有未設為上班時間的時數，都視為下班後。
- 您可以針對下班後設定不同的來電處理選項和問候語。

根據您設定自動語音應答和通話佇列的設定方式而定，您可能只需要指定具有直接電話號碼之自動語音應答的下班後通話路由。
...
- 如果是直接路由號碼，傳送的號碼是根據 SBC 上的 P-資訊識別 (一) 設定，如下所示：
  - 如果設為 [停用]，則會顯示原始來電者的電話號碼。 停用是預設和建議的設定。
...
3. 這些測試會識別導致自動語音應答無法接聽來電的租使用者、原則或資源帳戶設定，並且也會提供修正已發現之任何問題的步驟。
